...
Motivation: 结直肠癌是全球主要的癌症致死原因之一，胃肠道息肉是其关键前体。早期精准分割息肉对预防癌症进展至关重要，但人工标注耗时且存在观察者差异，现有深度学习方法缺乏可解释性，限制了临床应用。

Method: 提出PolypSeg-GradCAM框架，融合U-Net架构与梯度加权类激活映射（Grad-CAM），在Kvasir-SEG数据集（1000张标注内窥镜图像）上进行训练与评估，实现自动且可解释的息肉分割。

Result: 模型在测试集上达到平均IoU为0.9257，训练和验证集上的Dice系数均高于0.96，Grad-CAM可视化显示预测聚焦于临床相关区域，提升了模型透明度。

Conclusion: PolypSeg-GradCAM在保持高分割精度的同时增强了模型可解释性，推动了可靠、可信的人工智能辅助结肠镜检查，有助于提升结直肠癌的早期预防。
...
<details>
  <summary>Details</summary>
Motivation: 糖尿病视网膜病变是成人视力丧失的主要原因，尤其在医疗资源匮乏地区构成重大公共卫生挑战，亟需高效、可扩展的早期筛查工具。

Method: 采用ResNet-18、EfficientNet-B0和SqueezeNet等多种卷积神经网络对视网膜图像进行训练和评估，选择在准确率与计算效率之间最优的模型，并集成云平台实现可扩展性和安全的数据管理。

Result: 最终模型在分类疾病严重程度方面达到85.4%的准确率，支持临床和远程医疗环境中的实时筛查，系统还提升了医患互动并降低了医疗成本。

Conclusion: PerceptronCARE展示了人工智能驱动的远程医疗解决方案在扩大糖尿病视网膜病变筛查覆盖范围，特别是在偏远和资源受限地区方面的巨大潜力。
...
<details>
  <summary>Details</summary>
Motivation: 传统的正则化方法通常依赖启发式设计，在不同场景下可靠性与效果有限，因此需要一种更通用、内在一致的正则化机制来提升模型泛化能力。

Method: 提出Self Identity Mapping (SIM)，通过重构输入从其变换后的输出实现正则化；引入ρSIM，采用补丁级特征采样和基于投影的方法降低计算复杂度，实现高效、模型与任务无关的正则化模块。

Result: 在图像分类、少样本提示学习、域泛化等任务上均取得一致性能提升；验证了ρSIM在语义分割、图像翻译、音频分类和时间序列异常检测等密集预测与非视觉任务中的有效性；并证明其与其他正则化方法正交，可进一步提升其性能。

Conclusion: SIM是一种通用、高效、即插即用的正则化框架，通过保留语义信息和改善梯度流显著增强表示学习，具有广泛适用性和实用价值。
...
<details>
  <summary>Details</summary>
Motivation: 在单轮平均梯度（SAG）场景中，样本梯度被混合在一个批次均值中，导致梯度反演困难，现有方法难以有效恢复原始数据。

Method: 提出MAGIA，结合闭式组合重缩放以获得更紧的优化界，并利用基于动量的全批次与子集损失混合机制，通过探测随机数据子集感知隐含的单图像信号，实现稳健重建。

Result: 实验表明，MAGIA在大批次场景下显著优于现有先进方法，能够实现高保真多图像重建，且计算成本与标准求解器相当，无需任何辅助信息。

Conclusion: MAGIA为单轮平均梯度下的梯度反演提供了高效、鲁棒的解决方案，推动了无标签推断的隐私攻击研究进展。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于学习的3D重建方法依赖真实世界的多视角数据，而这类数据不易获取；同时，视频扩散模型虽具强大学习能力但局限于2D输出，限制了其在需要3D交互场景中的应用。

Method: 提出一种自蒸馏框架，在RGB解码器基础上引入3DGS解码器，并用RGB解码器的输出作为监督信号来训练3DGS解码器，利用视频扩散模型生成的合成数据进行纯合成训练。

Result: 实验结果表明，该方法在静态和动态3D场景生成任务上均达到最先进的性能，支持从文本提示或单图像生成3D场景，并可扩展至单目视频输入生成动态3D场景。

Conclusion: 所提出的自蒸馏框架有效利用视频扩散模型的隐式3D知识，实现了无需真实多视角数据的高质量3D场景生成，推动了虚拟环境生成在机器人、自动驾驶等领域的应用。
...
<details>
  <summary>Details</summary>
Motivation: 由于阿拉伯语的连笔书写、多样的字体、变音符号以及从右到左的排列方式，其文档OCR任务仍然具有挑战性，而现有MLLM在阿拉伯语上的表现有限。

Method: 采用解码器-only微调策略，在预训练的多模态大语言模型基础上，利用包含合成和真实文档的大规模数据集对模型进行微调，并构建了一个高质量、专家验证的基准数据集Misraj-DocOCR用于评估。

Result: Baseer在阿拉伯语文档OCR任务上显著优于现有方案，实现了0.25的词错误率（WER），并在该领域树立了新的标杆。

Conclusion: 针对特定领域微调通用多模态大模型可有效提升复杂语言OCR性能，为形态丰富的语言提供了高精度OCR的强基线。
...
<details>
  <summary>Details</summary>
Motivation: 由于InSAR数据稀疏且时空变化复杂，传统方法难以准确预测地面位移，因此需要一种能同时捕捉空间模式和时间依赖性的新方法。

Method: 提出一种混合CNN-LSTM模型，先将稀疏InSAR点测量数据构建成密集时空张量，再利用CNN提取空间特征、LSTM建模时间动态，实现未来形变预测。

Result: 在爱尔兰东部Sentinel-1数据上的实验表明，该模型比LightGBM和LASSO等基线模型具有更高的预测精度和空间一致性，且解释性分析显示其能捕捉复杂的非持久性变形动态。

Conclusion: 所提出的时空深度学习框架显著提升了高分辨率地表形变预测性能，验证了其在城市基础设施监测和地质灾害预警中的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 为了系统评估AI模型对基础概念的理解能力，在处理复杂任务前验证其基本认知能力。

Method: 提出Scrapbook框架，通过生成包含大量关于单一概念且具有丰富语言变化的问题的数据集来测试AI模型。

Result: 实验发现当前模型在识别和计数物体方面表现良好，但在理解绝对/相对位置、几何形状及附加约束问题时存在明显不足；部分模型表现出肯定性偏差，答案不一致。

Conclusion: Scrapbook框架为生成多样化、全面的数据集提供了有效工具，有助于系统性评估和改进AI模型在基础概念理解上的性能。
...
<details>
  <summary>Details</summary>
Motivation: 随着多模态AI系统在创意工作流中的广泛应用，理解视觉信息经过文本中介后的信息损失变得至关重要，但这一退化过程尚未被充分量化。

Method: 构建“描述再生成”管道，生成150对图像，并采用LPIPS、SSIM和颜色距离等现有指标，从感知、结构和色彩维度衡量信息保持程度。

Result: 99.3%的样本表现出显著感知退化，91.5%的样本显示出显著结构性信息丢失。

Conclusion: “描述再生成”瓶颈导致的信息损失是当前多模态系统中可测量且普遍存在的限制，需引起重视并加以改进。
...
<details>
  <summary>Details</summary>
Motivation: 小岛屿发展中国家（SIDS）在气候脆弱地区往往缺乏详细的建筑结构数据，限制了灾害风险评估和城市韧性规划，亟需低成本、可扩展的数据获取方法。

Method: 基于高分辨率卫星影像，比较了地理空间基础模型结合浅层分类器与微调深度学习模型在屋顶坡度和材料分类上的性能，并评估了引入邻近SIDS训练数据对模型效果的提升作用。

Result: 最佳模型在屋顶坡度和材料分类上分别达到0.88和0.83的F1分数，结合本地能力建设，显著提升了对EO数据的利用能力。

Conclusion: 该AI驱动方法可有效填补SIDS建筑数据空白，为基于证据的城市治理和灾害风险管理提供可行的技术路径。
...
<details>
  <summary>Details</summary>
Motivation: 由于不同环境中摄像头视角和数量的差异，导致视觉特征不一致，限制了VLA模型的泛化能力，因此需要提升模型对不同视角的适应性。

Method: 提出VLA-LPAF模块，通过单视角图像微调，并在潜在空间中融合多视角观测，增强模型的视角适应能力，且仅使用2D数据。基于RoboFlamingo构建RoboFlamingo-LPAF实例。

Result: 实验表明，RoboFlamingo-LPAF在CALVIN上平均任务成功率提升约8%，LIBERO上提升15%，自定义仿真基准上提升30%，并在真实任务中验证了其视角适应性。

Conclusion: VLA-LPAF能有效缓解视角异质性问题，显著提升VLA模型在多环境下的泛化性能，具备实际应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 事件相机具有高时间分辨率、高动态范围和低延迟的优势，但需要更可靠的深度估计方法。

Method: 设计了局部-全局优化模块，并引入基于KL散度的不确定性建模方法，提升预测可靠性。

Result: 在DSEC数据集上进行了大量实验，URNet在定性和定量评估中均优于当前最先进的方法。

Conclusion: URNet有效提升了事件相机立体深度估计的性能和可靠性，具有显著优势。
...
<details>
  <summary>Details</summary>
Motivation: 子宫内膜异位症常导致慢性盆腔疼痛并可能涉及神经，但对外周神经的成像仍具挑战性。

Method: Visionerves结合深度学习模型自动分割解剖结构，并通过符号空间推理进行纤维追踪和神经识别，利用模糊空间关系编码解剖知识，无需手动选择ROI。
...
<details>
  <summary>Details</summary>
Motivation: 现有驾驶行为数据集主要来自发达国家，难以反映发展中国家复杂混合交通环境下的驾驶行为多样性，且常涉及驾驶员面部视频带来的隐私问题。因此，需要一个在真实本地环境下采集、兼顾隐私保护的多模态数据集以支持道路安全研究和智能交通系统发展。

Method: 通过定制Android应用，使用智能手机的加速度计、陀螺仪和GPS传感器与前向道路视频同步采集数据，确保多源数据时间对齐；在多种道路类型上记录三种驾驶行为（正常、激进、危险），并设计了参与者选择、驾驶场景和环境控制方案。

Result: 构建了包含原始层、处理层和语义层的三层结构数据集V-SenseDrive，实现了高频率传感器数据与视频的精确同步，适用于驾驶员行为分类、交通安全分析和ADAS开发等研究。

Conclusion: V-SenseDrive填补了全球驾驶行为数据集中关于发展中国家异质交通环境的空白，为面向本地情境的智能交通解决方案提供了重要数据基础，并展示了隐私保护与多模态数据采集的可行平衡。
...
<details>
  <summary>Details</summary>
Motivation: 为了提升多模态大模型在特定领域（如OCR和文档理解）的性能，同时保持强大的通用能力，探索高效的训练方法和国产硬件上的大规模训练可行性。

Method: 采用多阶段渐进式训练策略和高精度数据合成流水线，结合领域增强技术，并在百度昆仑P800芯片上实现全栈自主训练。

Result: Qianfan-VL在CCBench、SEEDBench IMG、ScienceQA、MMStar等基准上达到SOTA；在OCRBench得873分，DocVQA达94.75%；Qianfan-VL-8B和70B在MathVista上达78.6%，展现出强推理能力；单任务在5000芯片上训练效率超90%。

Conclusion: 该工作建立了一套有效的领域增强型多模态模型开发方法，适用于多种企业级部署场景，并验证了国产AI基础设施训练顶级多模态模型的能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法因缺乏成对的真实世界训练数据和域间差异，难以在真实场景中泛化；传统基于大气散射模型的方法难以处理复杂的实际雾霾模式。

Method: 将大气散射模型重构为常微分方程（ODE），借鉴Rectified Flow思想，学习从有雾图像到清晰图像的最优ODE轨迹，并提出基于马尔可夫链布朗运动（MCBM）的非均匀雾生成方法以增强训练数据的真实性与多样性。

Result: 实验表明，HazeFlow在多个真实世界去雾基准数据集上达到了最先进的性能。

Conclusion: HazeFlow通过物理引导的ODE建模和更真实的雾生成方法，显著提升了去雾模型在真实场景中的适应性和去雾效果。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习模型在农业边缘设备上的部署面临资源限制的挑战，需要更轻量、高效的模型。

Method: 采用结构化通道剪枝、量化感知训练（QAT）以及NVIDIA TensorRT在Jetson Orin Nano上进行加速，解决了残差连接、注意力机制等复杂结构的剪枝难题。

Result: 模型大小减少68.5%，计算量降低3.2 GFLOPs，FP16下推理速度达184 FPS，比基线快28.7%；在CottonWeedDet12数据集上，39.5%剪枝率的EcoWeedNet取得83.7%精确率、77.5%召回率和85.9% mAP50，优于YOLO11n和YOLO12n。

Conclusion: 压缩后的EcoWeedNet在保持高精度的同时显著提升了效率，适用于精准农业中的边缘部署。
...
<details>
  <summary>Details</summary>
Motivation: 现有模型在处理多模态医学数据时难以应对模态缺失和不平衡问题，缺乏鲁棒性和实用性。

Method: 引入可学习的模态token实现缺失感知的融合，并在单模态对比学习基础上加入融合多模态表示进行增强。

Result: 在大规模临床数据集上验证，涵盖视觉与表格模态，疾病检测与预测任务中表现SOTA，尤其在仅有一个模态可用时仍保持高性能，并成功集成到CT基础模型中。

Conclusion: 该方法高效、通用且可扩展，为实际临床应用提供了一种低成本、鲁棒的多模态学习解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 为了明确在肺栓塞CTPA图像分割任务中，不同深度学习架构（尤其是CNN与Vision Transformer）的性能差异，并评估预训练策略的有效性。

Method: 构建包含490个标注CTPA扫描的高质量数据集，在统一框架下比较九种3D分割模型（CNN和ViT），使用Dice分数等指标评估性能，并在内部及公开数据集上验证泛化能力。

Result: 3D U-Net（ResNet编码器）表现最优；CNN整体优于ViT；分类预训练反而降低分割性能；模型性能具有一致性；中心和大栓塞可准确分割，远端栓塞仍具挑战。最佳模型在60例测试中达到平均Dice分数0.7131，检出181个栓塞，49个假阳性，28个假阴性。

Conclusion: 3D CNN模型（特别是3D U-Net）仍是PE分割的首选；预训练需谨慎，任务特异性特征更重要；未来应聚焦远端栓塞分割与高质量数据构建。
...
<details>
  <summary>Details</summary>
Motivation: 现有计算方法很少显式建模手形，限制了手语识别的准确性和语言学分析。

Method: 结合解剖学指导的图结构和对比学习，设计图神经网络以分离时间动态与静态手形配置。

Result: 在包含37类手形的基准任务上达到46%的准确率，显著优于基线方法的25%。

Conclusion: 该方法有效提升了手形识别性能，为手语的计算建模和语言学研究提供了新工具。
...
<details>
  <summary>Details</summary>
Motivation: 在胎儿超声等医疗场景中，由于图像特征和临床环境的异质性，可靠地进行分布外（OOD）检测对深度学习模型的安全部署至关重要。现有研究多关注不确定性量化方法，而忽略了分类任务本身的影响。

Method: 通过在四种分类任务上实验八种不确定性量化方法，评估不同任务下的OOD检测性能，并分析其在图像特征偏移和解剖结构偏移两种OOD定义下的表现差异。

Result: OOD检测性能显著依赖于所选分类任务，最佳任务取决于ID-OOD的定义方式；图像特征偏移与解剖特征偏移对应的最佳任务不同。此外，OOD检测性能好并不意味着 abstained prediction（拒绝预测）效果最优。

Conclusion: 应根据具体的下游医学应用需求，合理选择分类任务和不确定性量化策略，以实现更安全可靠的模型部署。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉定位系统依赖大数据库或3D模型，在无网络或GPS的资源受限场景不实用；而正射地理数据轻量且易获取，却未被充分用于定位，本文旨在填补这一空白。

Method: 构建包含16,425张无人机图像的OrthoLoC数据集，涵盖多模态、跨域配对结构；设计AdHoP细化方法，可集成至任意特征匹配器以提升性能。

Result: OrthoLoC支持公平基准测试，分离检索与匹配；实验验证了域差异、分辨率和共视性对定位的影响；AdHoP将匹配成功率提升最多95%，平移误差减少最多63%。

Conclusion: OrthoLoC为基于正射地理数据的视觉定位提供了有效基准，AdHoP显著提升了定位精度，推动了无GNSS环境下轻量级高精度定位的发展。
...
<details>
  <summary>Details</summary>
Motivation: 在许多实际场景中，缺乏可用于训练的正常样本或参考数据，传统异常检测方法难以适用，因此需要一种仅依赖测试图像本身的零样本异常检测方法。

Method: 基于Deep Image Prior思想，设计了 patch-based 自重构网络SSDnet，通过掩码、patch打乱和高斯噪声防止恒等映射，并采用基于内积相似性的感知损失来捕捉结构信息。

Result: 在MVTec-AD和fabric数据集上分别达到0.99和0.98的AUROC，以及0.60和0.67的AUPRC，性能优于现有最先进方法。

Conclusion: SSDnet实现了高效的零样本图像异常定位，无需外部训练数据或标签，在有噪声或缺失像素情况下仍保持鲁棒性，具有较强实用性。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言模型在低资源语言中因缺乏配对数据、翻译偏差和英语中心预训练而难以准确对齐图文信息。

Method: 构建基于LaBSE验证的英-孟图文对和11万张双语提示生成图像的数据集，采用冻结的MaxViT提取视觉特征，孟加拉语原生mBART-50解码，并设计轻量级跨模态桥接模块；引入三重损失：Patch对齐损失（PAL）、InfoNCE全局对比损失和基于Sinkhorn的最优传输（OT）细粒度匹配损失。

Result: 在Flickr30k-1k和MSCOCO-1k上显著优于基线模型，BLEU-4、METEOR和BERTScore-F1均有提升，并将真实与合成数据的中心距离缩小41%。

Conclusion: 该三重损失协同机制有效改善了低资源语言下的视觉接地效果，减少了虚假匹配，为非英语语言的视觉-语言对齐提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的高效纯视觉BEV方法缺乏对完整自动驾驶任务的支持，而大型多模态模型难以部署；因此需要一个兼具高性能与实时性的紧凑型全栈模型。

Method: 提出TinyBEV，采用模型无关的多阶段知识蒸馏策略，结合特征级、输出级和自适应区域感知监督，从大模型（UniAD）向轻量级纯摄像头模型迁移多模态知识。

Result: 在nuScenes数据集上，TinyBEV以28M参数实现39.0 mAP检测精度、1.08 minADE运动预测性能和0.32碰撞率，运行速度达11 FPS，比UniAD减少78%参数且快5倍。

Conclusion: TinyBEV证明了在资源受限条件下仍可保留完整的驾驶智能，有效缩小了大规模感知-规划模型与实际实时部署之间的差距。
...
<details>
  <summary>Details</summary>
Motivation: 现有标注方法将球标记在模糊前端，忽略了与速度相关的运动线索，导致检测不对称且不准确。

Method: 提出新的标注策略，将球位置标在模糊轨迹中心，并引入BlurBall模型联合估计球的位置和模糊属性，使用多帧输入和注意力机制（如Squeeze-and-Excitation）提升检测效果。

Result: 新标注方法在多种模型上均提升检测性能，BlurBall模型取得当前最优的检测结果，并能更可靠地预测轨迹。

Conclusion: 利用运动模糊信息进行中心化标注和建模，可显著提升高速运动小目标的检测与轨迹预测性能，适用于实时体育分析。
...
<details>
  <summary>Details</summary>
Motivation: 在视频中运行大规模开放词汇检测器每帧计算成本高昂，因此需要一种高效且准确的方法来减少检测器调用频率，同时保留足够的检测精度。

Method: 仅在固定间隔的关键帧上使用OWLv2进行检测，并利用压缩域中的运动向量（MV）通过简单的3x3网格聚合实现检测框的平移和缩放更新，辅以区域增长检查和可选的单类切换机制，实现无需训练的检测传播。

Result: 在ILSVRC2015-VID验证集上，MVP在mAP@0.5达到0.609，mAP@[0.5:0.95]达到0.316，在低IoU阈值下接近逐帧OWLv2-Large性能，且优于基于跟踪器的传播方法；相比需标注训练的YOLOv12x（mAP@0.5=0.631），MVP无需标签且保持开放词汇能力。

Conclusion: 基于压缩域运动向量的检测传播是一种实用且高效的方法，可在大幅降低计算开销的同时维持较强的零样本检测覆盖能力，适合应用于开放词汇视频检测场景。
...
<details>
  <summary>Details</summary>
Motivation: 实现虚拟物体与真实场景的逼真融合需要准确的光照估计，而颜色准确性是影响视觉真实感的关键因素，但常被现有方法忽略。

Method: 构建包含多样化光照颜色的HDR数据集，系统评估多种适应策略，重点测试基于预训练白平衡网络的输入预处理方法。

Result: 输入图像的白平衡预处理在所有测试场景中均优于其他策略，显著提升了三种主流光照估计方法的颜色鲁棒性。

Conclusion: 简单的白平衡预处理即可有效提升现有HDR光照估计模型的颜色准确性，该方法通用、高效且无需模型重训练。
...
<details>
  <summary>Details</summary>
Motivation: 由于数据私有性和隐私问题，传统依赖大量标注数据的支票欺诈检测方法面临数据稀缺挑战，因此需要一种无需训练且能适应多种支票格式的检测方案。

Method: 提出一种基于视觉语言模型（VLM）和多模态大语言模型（MLLM）的零样本检测框架，利用模型的跨模态理解能力直接定位支票中的关键字段，无需训练即可实现自动化检测。

Result: 在包含110张不同格式支票的手工整理数据集上验证了该方法的有效性，表现出强健的性能和良好的泛化能力，并能生成高质量标注数据用于后续模型训练。

Conclusion: 所提出的训练-free框架为支票关键字段检测提供了一种低门槛、高适应性的解决方案，适用于现实金融场景，并可作为生成标注数据的工具以支持专用检测模型的开发。
...
TL;DR: 本文提出了CHART NOISe数据集，用于评估视觉语言模型在含噪声和遮挡图表上的理解能力，揭示了现有模型在退化输入下的系统性脆弱性，并提出改进鲁棒性的基线策略。


<details>
  <summary>Details</summary>
Motivation: 现实中的图表常包含失真或遮挡，且需要复杂推理，而现有基准测试多基于清晰图表和事实性问题，无法充分评估模型在真实场景下的可靠性与鲁棒性。

Method: 构建了一个结合图表失真、遮挡和选择题形式的新型数据集CHART NOISe，引入‘提示反向不一致性’作为评估指标，并对ChatGPT-4o、Claude Sonnet 4和Gemini 2.5 Pro等先进VLM进行评测，同时提出质量过滤和遮挡检测等缓解策略。

Result: 实验发现当前VLM在输入退化时性能显著下降，幻觉现象（如数值捏造、趋势误判）增多，且模型过度自信；CHART NOISe能有效暴露模型的推理缺陷，并验证了所提缓解方法的初步有效性。

Conclusion: CHART NOISe为图表理解任务提供了更贴近真实场景的评测基准，强调了提升VLM在噪声环境下可靠性和一致性的必要性，并为未来研究提供了数据与方法基础。
...
<details>
  <summary>Details</summary>
Motivation: 传统4D-MRI重建方法依赖相位分 bin 或模板扫描，难以捕捉呼吸时序变异性，流程复杂且计算负担重。

Method: 提出一种神经表示框架，将呼吸运动建模为由1D代理信号引导的连续形变；采用两个协同网络：空间解剖网络（SAN）编码连续3D解剖结构，时间运动网络（TMN）结合Transformer提取的呼吸信号生成时序一致的形变场。

Result: 在19名志愿者自由呼吸数据上验证，新方法能准确捕捉规律与不规则呼吸模式，保持血管和支气管连续性，解剖保真度高；处理时间从约5小时降至15分钟训练，并可在1秒内完成单个体积推断。

Conclusion: 该框架无需模板和相位分bin，可连续重建任意呼吸状态下的3D图像，性能优于传统方法，在4D放疗计划和实时自适应治疗中具有广泛应用前景。
...
<details>
  <summary>Details</summary>
Motivation: 由于快速移动的小物体具有不可预测的运动模式和较小的视觉标记，导致精确跟踪困难，尤其在体育机器人应用中对轻量级且准确的跟踪系统需求迫切，而传统卡尔曼滤波方法在此类场景下性能下降明显。

Method: 研究使用包含10,000帧标注壁球图像的数据集，评估OCSORT、DeepOCSORT、ByteTrack、BoTSORT和StrongSORT五种先进跟踪方法，重点分析推理速度和每图像更新频率对跟踪精度与可靠性的影响。

Result: 实验结果显示DeepOCSORT平均ADE为31.15像素，显著低于ByteTrack的114.3像素，但ByteTrack推理速度最快（26.6ms），DeepOCSORT为26.8ms；所有方法的空间误差在3-11cm之间（ADE 31-114像素），表现出明显的跟踪漂移。

Conclusion: 当前基于卡尔曼滤波的跟踪方法在处理高速小物体时存在根本性局限，误差比标准基准高3-4倍，需开发专门适用于快速移动小物体的新型跟踪方法。
...
<details>
  <summary>Details</summary>
Motivation: 在压缩域视频动作识别中，如何高效利用已有信息（如运动向量）提升模型性能并降低计算开销是一个关键问题。现有方法往往需要额外训练或引入参数，限制了效率和部署灵活性。

Method: 提出MoCrop模块，基于H.264中的运动向量生成帧级裁剪策略；采用轻量级流程（包括去噪与合并、蒙特卡洛采样、基于运动密度子矩阵搜索的自适应裁剪），在推理时对所有I帧应用统一裁剪。

Result: 在UCF101上，结合ResNet-50实现+3.5% Top-1精度（相同FLOPs）或+2.4%精度且减少26.5% FLOPs；应用于CoViAR时，在计算量从11.6降至8.5 GFLOPs时仍保持88.5% Top-1精度；在MobileNet-V3、EfficientNet-B1和Swin-B上均表现良好，显示强通用性。

Conclusion: MoCrop无需训练、不增加参数，可即插即用，显著提升压缩域视频动作识别的效率与精度，具备良好的通用性和实时部署潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在低比特率下表现不佳，保留冗余信息或学习过于集中的符号分布，难以兼顾压缩效率与分析性能。

Method: 通过向量量化（VQ）将连续视觉特征映射到离散索引，利用码本在边缘端进行特征压缩，并选择性传输至云端，结合语义增强提升信息保留能力。

Result: 实验表明，CAFC-SE在低比特率条件下显著优于现有方法，在压缩率和分析准确率方面均取得更好表现。

Conclusion: CAFC-SE通过离散化特征表示和选择性传输，有效缓解了低比特率下的信息损失问题，为边缘-云系统提供了高效可靠的特征压缩方案。
...
<details>
  <summary>Details</summary>
Motivation: 为了在资源受限环境下实现高效、高精度的医学图像分割，需要开发更轻量且性能优越的模型。

Method: 提出MK-UNet，引入多核深度卷积块（MKDC）以捕获多分辨率空间关系，并结合通道、空间和分组门控注意力机制增强特征表达。

Result: MK-UNet仅用0.316M参数和0.314G FLOPs，在六个二值医学图像基准上超越SOTA方法；相比TransUNet减少333倍参数和123倍FLOPs，DICE分数更高；相比UNeXt提升最多6.7% DICE分数且参数少4.7倍。

Conclusion: MK-UNet在显著降低计算成本的同时大幅提升分割精度，是适用于实时、高保真医学诊断（如床旁设备）的理想解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 弥合手术视频与患者体数据之间的差距，提升手术导航的准确性。

Method: 将3D高斯分布绑定到CT网格上，通过光度监督联合优化高斯参数和网格形变，并利用父三角形关系保持对齐。

Result: 在猪内脏手术和模拟人肝的合成数据上验证了方法的有效性，能够在单目RGB数据下合理形变术前CT。

Conclusion: BridgeSplat能够有效融合术中视觉信息与术前CT，实现精确且可变形的手术导航。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在处理源域数据不可用的领域自适应问题时，依赖包含噪声的完整伪标签集进行优化，难以保证标签质量，限制了自训练效果。

Method: 首先通过置信度过滤和超分辨率增强融合生成少量高质量伪标签作为初始种子；然后利用扩散模型的强大去噪能力和对复杂分布的建模能力，将这些不完整、分布不规则的种子伪标签传播为完整的高质量伪标签。

Result: 所提方法有效避免了直接优化完整伪标签集的困难，显著提升了伪标签质量，在多个遥感图像数据集上实现了优于现有方法的分割性能。

Conclusion: DGLE为源域自由的领域自适应语义分割提供了一种有效的伪标签优化范式，验证了基于扩散模型进行标签传播的可行性与优势。
...
<details>
  <summary>Details</summary>
Motivation: 传统欧氏空间在表示层次化数据时存在局限，而双曲空间更适配此类结构；为更好建模C2FSCIL任务中的“粗到细”范式，需探索更适合的几何空间。

Method: 将特征提取器嵌入Poincaré球模型的双曲空间，引入双曲对比损失和双曲全连接层进行优化与分类，并利用双曲空间中的最大熵分布估计细类特征向量的概率分布以生成增强特征。

Result: 在C2FSCIL基准实验中，该方法显著提高了粗类和细类的分类准确率，尤其在少样本条件下表现优越。

Conclusion: 双曲空间能更有效地捕捉C2FSCIL任务中的层次结构，所提方法通过双曲嵌入、损失函数及特征增强策略，实现了优于传统欧氏方法的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于外观的方法在去除对象时无法有效处理未被明确标注的因果视觉伪影（如阴影、反射），且缺乏对对象几何存在与其视觉效应之间因果关系的建模，导致去除不完整或过度擦除问题。

Method: 提出一个几何感知的两阶段框架：第一阶段在几何空间（如深度）中严格按掩码监督去除对象，确保结构感知的编辑；第二阶段基于更新后的几何条件生成逼真的RGB图像，隐式消除由3D几何变化引起的因果视觉效果。引入基于正负样本对的偏好驱动目标来指导几何去除学习。

Result: 在两个主流基准上实验表明，该方法在去除对象及其相关视觉伪影方面优于现有方法，实现了最先进的性能，同时避免了结构误插入和过度擦除问题。

Conclusion: 通过显式建模对象几何与视觉效应之间的因果关系，所提出的两阶段框架有效解决了传统方法在对象移除中遗漏因果伪影或破坏周围结构的问题，为图像编辑提供了更强的可控性和真实性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的NR-IQA对抗攻击在白盒设置下有效，但在更现实的黑盒场景中迁移性差，难以攻击未知模型，因此需要提高攻击的可迁移性。

Method: 提出Signed Ensemble Gaussian黑盒攻击（SEGA），通过对源模型应用高斯平滑并集成其平滑梯度来逼近目标模型梯度，同时设计扰动滤波掩码去除不合适的扰动以保证视觉不可感知。

Result: 在CLIVE数据集上的实验表明，SEGA相比现有方法具有更强的迁移攻击能力，能更有效地对未知NR-IQA模型实现黑盒攻击。

Conclusion: SEGA显著提升了黑盒环境下对NR-IQA模型的攻击可迁移性，为评估和增强此类模型的鲁棒性提供了有效工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于多任务学习的微笑识别方法存在计算效率低、需要复杂损失平衡的问题，且依赖辅助任务监督，限制了其在实际应用中的部署。

Method: 提出HadaSmileNet，采用Hadamard乘积实现Transformer特征与D-Marker特征的参数自由融合，并系统评估了15种融合策略以确定最优方案。

Result: 在UvA-NEMO、MMI、SPOS和BBC四个基准数据集上达到新的最先进结果，准确率分别为88.7%、99.7%、98.5%和100%，相比先前方法提升0.8%至5.0%；模型参数减少26%，训练更简便，可视化显示特征判别能力增强。

Conclusion: HadaSmileNet通过直接融合深度特征与领域知识，在保持高效计算的同时显著提升识别性能，适用于需要实时情感计算的多媒体数据挖掘应用。
...
TL;DR: 提出了一种名为Live-E2T的新框架，用于实时视频流中的威胁行为监测，结合结构化语义表示、在线事件去重更新机制和基于思维链的大语言模型推理，显著提升了检测精度、实时性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在实时性能和决策可解释性之间难以兼顾，无法满足实时威胁监测的需求。

Method: 将视频帧分解为Human-Object-Interaction-Place语义元组，设计在线事件去重与更新机制，并采用思维链策略微调大语言模型以生成可解释的威胁评估报告。

Result: 在XD-Violence和UCF-Crime等基准数据集上，Live-E2T在威胁检测准确率、实时效率和可解释性方面均显著优于现有方法。

Conclusion: Live-E2T有效统一了实时性与可解释性，为实时威胁监控提供了高效且透明的解决方案。
...
TL;DR: 本文提出了一种增强多模态大语言模型美学理解能力的新方法，包括构建专业数据集PhotoCritique、设计具有语言引导多视角视觉融合机制的模型PhotoEye，以及发布专业美学理解基准PhotoBench。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在美学视觉理解方面存在不足，尤其在需要摄影专业知识的实际场景中表现不佳，难以实现从一般视觉理解到审美理解的跨越。

Method: 构建了由专业摄影师和爱好者讨论生成的大规模、多样化的数据集PhotoCritique；提出了语言引导的多视图视觉融合模型PhotoEye，以多角度理解图像美学；并发布了用于评估美学理解能力的专业基准PhotoBench。

Result: 在现有基准和新提出的PhotoBench上，PhotoEye均显著优于现有模型，展现出更强的美学分析与描述能力。

Conclusion: 通过专业化数据集、创新模型结构和高标准评测基准，可有效提升MLLMs在真实场景下的视觉美学理解能力。
...
TL;DR: 本文提出了一种基于XMem模型的实时MRI引导放疗中的肿瘤分割框架，旨在应对TrackRAD2025挑战。该方法利用记忆增强架构对长序列cine-MRI进行肿瘤分割和运动追踪，在标注数据有限的情况下仍表现出合理的性能，并满足临床实时性需求，尽管因实验记录丢失而无法提供定量结果。


<details>
  <summary>Details</summary>
Motivation: 为了提升MRI引导放疗中肿瘤追踪的精度，实现更准确、安全的癌症治疗，尤其是在实时性和数据稀缺的临床条件下。

Method: 采用XMem模型，一种具有记忆机制的增强架构，用于在长序列cine-MRI中进行连续肿瘤分割与运动跟踪，支持实时处理。

Result: 初步开发过程中的定性观察表明，该框架具备合理的分割性能，并能满足临床实时性要求，但由于实验记录丢失，未能提供具体的量化评估结果。

Conclusion: 基于XMem的肿瘤分割框架在无大量标注数据的情况下展现了应用于实时MRI引导放疗的潜力，有助于推动精准放疗的发展。
...
TL;DR: 本文提出了一种用于多对比磁共振成像超分辨率（MC-MRI SR）的Spatial-Semantic Consistent Model（SSCM），通过动态空间变换、语义感知标记聚合和空频融合模块，在减少参数量的同时实现了最先进的性能，有效解决了空间语义一致性与高频细节恢复问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在建模空间-语义一致性和利用频域信息方面不足，导致多对比MRI超分辨率中精细结构对齐差和高频细节恢复不充分。

Method: 提出SSCM模型，包含三个核心组件：动态空间变换模块（Dynamic Spatial Warping Module）实现跨对比度空间对齐，语义感知标记聚合块（Semantic-Aware Token Aggregation Block）保证长距离语义一致性，空频融合块（Spatial-Frequency Fusion Block）恢复精细结构。

Result: 在公开和私有数据集上的实验表明，SSCM在参数更少的情况下达到了最先进的性能，显著提升了空间和语义一致性重建质量。

Conclusion: SSCM有效解决了MC-MRI SR中的空间-语义一致性问题，兼顾结构对齐与高频细节恢复，具有高效性和优越的重建性能。
...
TL;DR: 提出OraPO与FactS方法，通过单阶段强化学习和基于事实的奖励机制，在低资源条件下显著提升放射学报告生成的效率和性能。


<details>
  <summary>Details</summary>
Motivation: 现有放射学报告生成方法依赖大规模数据和计算资源，训练成本高且效率低，难以在资源受限环境下应用。

Method: 提出Oracle-educated GRPO（OraPO）框架，结合FactScore-based奖励（FactS），利用轻量级oracle将失败探索转化为偏好监督，并通过提取临床事实并验证其与真实标签的蕴含关系来提供细粒度、可解释的句子级奖励。

Result: 在CheXpert Plus数据集上达到新的SOTA性能（F1为0.341），仅使用小型基础VLM和少量训练数据（比先前方法少2-3个数量级）并在普通硬件上实现。

Conclusion: OraPO与FactS构成的框架在资源受限条件下显著提升了放射学报告生成的学习效率和临床准确性，具有高实用价值和推广潜力。
...
TL;DR: 提出了一种无需训练的自适应多风格融合（AMSF）框架，可在扩散模型中实现多个参考风格的可控融合，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于参考的方法受限于只能使用单一风格图像且缺乏平衡多种风格影响的机制，难以实现混合美学和扩展到更多风格。

Method: 通过语义标记分解模块将所有风格图像和文本提示编码，并自适应地注入到冻结的扩散模型的每个交叉注意力层；引入相似性感知重加权模块，在每步去噪过程中重新校准各风格成分的注意力，实现平衡且可控制的融合。

Result: 定性和定量评估表明，AMSF在多风格融合效果上 consistently 优于最先进方法，且能无缝扩展到两种或更多风格。

Conclusion: AMSF为扩散模型中的表达性多风格生成提供了实用的解决方案，具有良好的可扩展性和用户可控性。
...
TL;DR: 提出了一种基于多级融合的4D毫米波雷达与相机融合框架MLF-4DRCNet，用于3D目标检测，在点级、场景级和建议级实现多模态信息融合，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有4D雷达-相机融合方法沿用LiDAR设计的鸟瞰图融合范式，忽视了雷达点云稀疏、几何不完整的问题，且融合局限于粗粒度场景级，限制了检测性能。

Method: 提出MLF-4DRCNet，包含三个模块：增强雷达点编码器（ERPE）密度化雷达点云并编码；分层场景融合池化（HSFP）通过可变形注意力融合多尺度特征；建议级融合增强（PLFE）优化区域建议。融合点、场景和建议三级信息。

Result: 在View-of-Delft和TJ4DRadSet数据集上达到SOTA性能，尤其在VoD数据集上性能媲美LiDAR-based模型。

Conclusion: MLF-4DRCNet通过多级融合有效克服了4D雷达点云稀疏和噪声问题，实现了高性能的3D目标检测，验证了其在自动驾驶中替代LiDAR的潜力。
...
TL;DR: 提出了一种无需训练的双潜变量引导框架PDLS，用于在扩散模型中更准确地逆向重建受损图像，在保持结构保真度的同时避免语义漂移。


<details>
  <summary>Details</summary>
Motivation: 现有方法在图像逆向过程中难以兼顾结构保真与语义准确性，常导致细节模糊或属性错误等语义漂移问题。

Method: 基于Rectified Flow模型，将逆向过程分解为结构路径和语义路径，通过提示词引导语义流，并将双路径引导建模为最优控制问题，利用LQR推导出闭式解以动态调节生成轨迹。

Result: 在FFHQ-1K和ImageNet-1K上的实验表明，PDLS在高斯去模糊、运动去模糊、超分辨率和自由形式修复等任务中均优于单潜变量基线方法，重建结果更忠实于原图且语义一致性更高。

Conclusion: PDLS通过双路径控制机制有效解决了扩散模型逆向中的语义漂移问题，无需针对每张图像进行优化，兼具高效性与高质量重建能力。
...
TL;DR: Prima是首个用于神经影像的视觉语言模型，基于超过22万项MRI研究训练，具有分层视觉架构，在真实临床环境中表现出卓越的诊断性能，支持可解释性诊断与工作优先级推荐，并展现出算法公平性。


<details>
  <summary>Details</summary>
Motivation: 应对全球MRI需求增长带来的医疗系统压力、诊断延迟和医生 burnout，特别是在资源匮乏和农村地区，亟需高效、公平的AI辅助诊断工具。

Method: 利用大型学术医疗系统的数据，开发名为Prima的视觉语言模型（VLM），采用分层视觉架构提取通用且可迁移的MRI特征，并在涵盖3万项MRI研究的全系统范围内进行一年期测试，评估其在52种神经系统疾病诊断中的表现。

Result: Prima在52种神经影像诊断任务中平均ROC曲线下面积达到92.0，优于现有最先进的通用和医学AI模型；具备生成可解释性鉴别诊断、为放射科医生排序工作列表、提供转诊建议的能力，并在不同人群和MRI设备上展现算法公平性，有助于缓解医疗系统偏见。

Conclusion: Prima作为首个面向真实临床神经影像的VLM基础模型，展示了大规模健康系统数据训练的AI模型在提升诊断效率、公平性和临床工作流方面的巨大潜力，推动AI驱动医疗的发展。
...
TL;DR: 提出了一种新的统一模型推理框架Understanding-in-Generation（UiG），通过在生成过程中融入强大的理解能力来增强文本到图像生成性能，利用“图像编辑”作为桥梁，在生成后逐步修正和提升图像质量，在长文本提示下显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的基于思维链的统一模型在文本到图像生成中将理解和生成过程分离，限制了对生成缺陷的指导能力。

Method: 提出Understanding-in-Generation（UiG）框架，引入‘图像编辑’机制，在生成图像后利用模型的理解能力生成编辑指令，并迭代优化图像。

Result: 在TIIF基准的长提示设置下，相比现有方法提升了3.92%的性能。

Conclusion: UiG通过在生成过程中融合模型的理解能力，有效弥补了生成能力的不足，显著提升了文本到图像生成的质量。
...
TL;DR: 本文提出了一个针对内窥镜图像的深度估计基准，并发布了一个包含真实和合成数据的新数据集EndoSynth，显著提升了模型在未见真实数据上的性能。


<details>
  <summary>Details</summary>
Motivation: 由于缺乏鲁棒的基准和高质量的内窥镜图像数据集，现有深度估计模型在临床场景中的泛化能力受限。

Method: 构建了一个涵盖最先进的相对和度量深度估计模型的综合基准，并引入了一个带有真实深度和分割掩码的新型合成数据集EndoSynth；使用该合成数据集对基础模型进行微调。

Result: 在真实、未见过的内窥镜图像上，基于EndoSynth微调后的模型显著提高了深度估计精度，验证了合成数据在提升模型泛化能力方面的有效性。

Conclusion: 本文通过提供公开的基准测试和高质量合成数据集，推动了内窥镜图像深度估计领域的发展，为未来研究提供了重要资源。
...
TL;DR: 本文提出了一种名为LEAF-Mamba的新型模型，用于RGB-D显著性目标检测，通过引入局部强调状态空间模块和自适应融合模块，在保持线性复杂度的同时有效捕捉多尺度局部依赖并实现跨模态融合，实验表明其在性能和效率上优于现有方法，并具有良好的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理RGB-D显著性目标检测时，受限于CNN的感受野局限或Vision Transformer的高计算成本，且直接应用状态空间模型会导致局部语义不足和跨模态融合不充分。

Method: 提出LEAF-Mamba模型，包含两个新组件：1）局部强调状态空间模块（LE-SSM），用于捕捉多尺度局部依赖；2）基于SSM的自适应融合模块（AFM），实现互补的跨模态交互与可靠融合。

Result: 在多个数据集上实验表明，LEAF-Mamba在16种先进方法中表现出更优的性能和效率，并在RGB-T SOD任务上展现出强大的泛化能力。

Conclusion: LEAF-Mamba有效解决了现有方法在局部建模和跨模态融合上的不足，为RGB-D显著性检测提供了一种高效且高性能的解决方案。
...
TL;DR: 提出了一种轻量级食品图像分类算法，结合窗口多头注意力机制（WMHAM）和空间注意力机制（SAM），在Food-101和Vireo Food-172数据集上实现了较高的分类精度，同时显著降低了模型参数量和计算量。


<details>
  <summary>Details</summary>
Motivation: 为了解决Vision Transformer模型在食品图像分类中参数多、计算复杂度高的问题，提升模型效率和实用性。

Method: 提出一种集成窗口多头注意力机制（WMHAM）和空间注意力机制（SAM）的轻量级分类算法，通过窗口划分降低计算成本，并利用SAM增强关键空间区域的特征表示。

Result: 在Food-101和Vireo Food-172数据集上分别达到95.24%和94.33%的准确率，显著减少参数量和FLOPs。

Conclusion: 所提方法在保持高分类性能的同时显著提升计算效率，适合资源受限环境下的部署。
...
TL;DR: 提出OSDA框架，用于无标注的开放集土地覆盖发现、分割与描述，结合像素级精度与高层语义理解，支持无需人工标注的遥感图像分析。


<details>
  <summary>Details</summary>
Motivation: 解决遥感中开放集土地覆盖分析需细粒度空间定位与语义开放分类的问题，尤其是对无监督新类别对象的检测、分割与可解释语义标注。

Method: 三阶段框架：1）基于可提示微调SAM模型进行精确发现与掩码提取；2）通过两阶段微调多模态大语言模型（MLLM）实现语义归因与上下文描述；3）使用LLM作为评判器并结合人工评分进行评估。

Result: 实现了在无需人工标注的情况下对多样化卫星影像中的新颖地物进行准确发现、分割和语义描述，具备良好的跨数据集鲁棒性与可解释性。

Conclusion: OSDA为动态土地覆盖监测提供了可扩展、无标签且可解释的解决方案，在自动制图更新和大规模地球观测中具有广泛应用潜力。
...
TL;DR: PlantCLEF 2021挑战旨在利用植物标本馆数据提升数据贫乏地区植物自动识别能力，聚焦南美圭亚那盾区约1000种高多样性植物，采用跨域分类任务评估模型性能。


<details>
  <summary>Details</summary>
Motivation: 解决当前深度学习在植物识别中依赖大量野外照片而忽视生物多样性丰富但数据稀缺的热带地区的问题，探索利用历史积累的数字化标本数据弥补数据不足的潜力。

Method: 构建包含数十万份标本图像和少量野外照片的训练集，引入物种形态与功能性状特征作为辅助信息，设计跨域分类任务，测试集仅使用野外拍摄照片进行评估。

Result: 多个研究团队提交了系统方案，结果显示结合标本数据与功能性状可有效提升模型在数据稀缺区域的识别性能，但跨域差异仍是主要挑战。

Conclusion: 植物标本数据结合功能性状信息有助于提升数据贫乏地区的植物自动识别效果，为未来跨域适应和小样本学习提供了可行方向。
...
TL;DR: 提出了一种名为AGSwap的自适应组交换方法，用于文本到图像生成中的跨类别对象融合，并构建了大规模数据集COF，实验表明AGSwap优于现有最先进方法。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成中跨类别对象融合方法常产生视觉混乱或语义不一致的结果，且缺乏全面的基准数据集。

Method: 提出AGSwap，包括组嵌入交换和自适应组更新两个组件，并构建了基于ImageNet-1K和WordNet的大规模层次化数据集COF。

Result: 在简单和复杂提示下，AGSwap均优于现有最先进的组合式文本到图像生成方法，包括GPT-Image-1。

Conclusion: AGSwap能有效提升跨类别对象融合的语义一致性与视觉质量，COF数据集为该领域提供了重要资源。
...
TL;DR: PlantCLEF 2019挑战赛旨在评估在数据稀缺地区植物自动识别系统的性能，基于主要覆盖圭亚那地盾和北亚马逊雨林的10,000种植物数据集。


<details>
  <summary>Details</summary>
Motivation: 尽管深度学习推动了植物自动识别的发展，但现有数据仅覆盖少数物种，而全球近36.9万种植物中大多数缺乏足够数据，尤其是在生物多样性丰富但数据匮乏的热带地区。

Method: 通过组织PlantCLEF 2019挑战赛，使用包含约10,000个物种的数据集评估参赛系统性能，并与顶尖热带植物专家的表现进行对比。

Result: 挑战赛成功汇集了多种自动化识别方法，评估结果显示部分系统已接近甚至达到专家水平，验证了在高多样性区域进行大规模植物识别的可行性。

Conclusion: PlantCLEF 2019为数据稀缺地区的植物识别提供了重要基准，推动了跨机构方法比较和技术发展，表明结合深度学习与有限标注数据仍可实现高效物种识别。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法受限于闭集词汇，且依赖高质量数据集和耗时微调，难以适用于开放世界场景。

Method: 框架包含三个阶段：1）概览：利用视觉-语言模型获取文本查询与视觉区域间的语义关联；2）聚焦：借助扩散模型的细粒度先验补全结构与形状信息；3）演化：通过注意力演化模块抑制无关激活，生成纯净分割掩码。

Result: 在多个实验中，该方法在弱监督和零样本设置下均优于现有方法，展现出高效性和可扩展性。

Conclusion: RSVG-ZeroOV无需任务特定训练，有效挖掘冻结基础模型在零样本开放词汇RSVG中的潜力，为实际应用提供了新思路。
...
TL;DR: 提出了一种基于文本语义的属性提示组合（APC）框架，用于提升行人重识别（ReID）的判别性和泛化能力，结合属性提示生成器和快慢训练策略，在单域和跨域ReID任务中均表现出优越性能。


<details>
  <summary>Details</summary>
Motivation: 现有ReID模型受限于单域或跨域场景，易过拟合或抑制身份判别特征，缺乏兼顾判别性与泛化能力的统一框架。

Method: 设计属性提示生成器（APG），包含语义属性字典（SAD）和提示组合模块（PCM），利用文本语义生成判别性特征；提出快慢训练策略（FSTS），通过快速更新流学习ReID特定知识，慢速更新流保留预训练视觉语言模型的通用表征。

Result: 在常规和领域泛化ReID数据集上实验表明，该方法在判别性和泛化性方面均优于当前最先进方法。

Conclusion: APC框架有效融合文本语义与视觉特征，通过属性提示和双流训练策略，实现了更强的跨域适应能力和身份判别能力，提升了ReID的实际应用潜力。
...
TL;DR: 提出基于最优传输的OTCCLIP框架，通过细粒度视觉与文本特征集之间的最优传输距离重构图像-文本对，提升对抗数据投毒攻击的鲁棒性，并改善零样本和线性探测性能。


<details>
  <summary>Details</summary>
Motivation: 现有防御方法仅依赖图像和文本的全局表示匹配图文对，忽略细粒度特征，易引入错误配对，影响CLIP预训练效果。

Method: 提出基于最优传输的距离度量方法，重新分配图文对，并通过最优传输目标函数促进模态间和模态内细粒度对齐。

Result: 实验表明，OTCCLIP能有效降低投毒攻击的成功率，并在被污染数据集上显著优于以往方法，在零样本和线性探测任务中表现更优。

Conclusion: OTCCLIP通过细粒度特征对齐和最优传输机制，有效提升了CLIP模型在面对数据投毒攻击时的鲁棒性和下游任务性能。
...
TL;DR: 提出了一种名为Learning from Interactions (LFI) 的认知启发框架，通过显式建模视觉理解为交互过程，利用交互查询和基于交互的监督，实现从视觉语言模型（VLMs）到视觉基础模型（VFMs）的有效知识迁移，在多个任务上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉基础模型（VFMs）主要采用结果导向范式，忽视了视觉理解中的交互过程，导致与擅长跨模态交互的视觉语言模型（VLMs）之间存在表征差异，限制了知识迁移和泛化能力。

Method: 提出Interaction Queries以在网络层间保持持久的结构关系，并设计基于VLMs中跨模态注意力机制的交互式监督策略，从而捕捉动态交互模式，实现更高效的知识迁移。

Result: 在TinyImageNet分类和COCO检测/分割任务上分别取得3.3 mAP和1.6 mAP/2.4 AP的提升；在PACS和VLCS零样本迁移任务中提升2.4和9.3；收敛更快且参数开销小；人类评估显示语义一致性是对比方法的2.7倍。

Conclusion: LFI通过建模视觉理解的交互过程，有效缩小了VLMs与VFMs之间的表征鸿沟，实现了更高效的知识迁移，在多种视觉任务尤其是跨域场景中表现出优越的性能和认知对齐性。
...
TL;DR: 本文提出了一种新的混合提示驱动的Segment Anything模型（HyPSAM），用于RGB-热成像显著目标检测（RGB-T SOD）。该方法结合动态融合网络（DFNet）生成高质量初始显著图作为视觉提示，并设计即插即用的优化网络（P2RNet），利用文本、掩码和框等混合提示引导SAM进行精细化分割，显著提升了跨模态特征融合与边界精度。实验表明，该方法在多个数据集上达到SOTA性能，且具有良好的通用性，可兼容多种现有RGB-T SOD方法。


<details>
  <summary>Details</summary>
Motivation: 由于模态间特征融合不充分以及数据稀缺，现有方法在RGB-T SOD中难以准确学习显著目标的边界和完整性，因此需要一种更有效的跨模态融合与分割机制。

Method: 提出HyPSAM框架，包含两个核心组件：1）动态融合网络（DFNet），采用动态卷积和多分支解码结构自适应地融合RGB与热成像模态信息，生成高质量初始显著图；2）即插即用的优化网络（P2RNet），通过文本、框和掩码三类提示联合引导预训练的Segment Anything Model（SAM）对显著图进行精细化修正，提升定位精度与完整性。

Result: 在三个公开数据集上的大量实验表明，所提方法在RGB-T SOD任务中达到了最先进的性能。同时，HyPSAM展现出良好的通用性，能够无缝集成到不同的RGB-T SOD方法中并带来显著性能提升。

Conclusion: HyPSAM通过引入混合提示机制有效增强了SAM在RGB-T SOD任务中的零样本泛化能力，验证了提示工程在多模态显著性检测中的巨大潜力，为后续研究提供了新思路。
...
TL;DR: 本文提出了一种名为TriFusion-AE的多模态自编码器，通过融合文本先验、单目深度图和LiDAR点云，提升在噪声和对抗性攻击下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: LiDAR点云易受噪声、遮挡和对抗性干扰影响，现有自编码器在复杂真实场景下性能下降，因此需要更鲁棒的重建方法。

Method: 提出TriFusion-AE，采用跨模态交叉注意力机制，融合文本语义信息、图像深度特征和LiDAR空间结构，实现联合表示学习，并可与任意CNN-based点云自编码器结合。

Result: 在nuScenes-mini数据集上验证，该模型在强噪声和对抗攻击下显著优于CNN基线模型，而在轻微扰动下提升有限。

Conclusion: TriFusion-AE通过多模态融合增强了点云重建的鲁棒性，尤其在极端条件下表现优异，且具有模型无关性，适用于低数据现实场景。
...
TL;DR: 本文提出了一种名为COLT的持续工具使用方法，用于增强开源视频大语言模型在不断变化的工具流中自动获取工具使用能力，同时避免灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有的视频大语言模型在固定工具库假设下难以适应现实世界中持续演进的工具数据流，因此需要一种能够持续学习新工具并保留旧知识的方法。

Method: COLT引入了一个可学习的工具代码本作为工具特定的记忆系统，并根据用户指令与代码本中工具特征的相似性动态选择相关工具，从而实现对新工具的持续学习和旧工具记忆的保持。

Result: 在多个视频大语言模型基准测试和专门构建的VideoToolBench数据集上进行了广泛实验，结果表明COLT在工具使用性能方面达到了最先进的水平。

Conclusion: COLT有效提升了开源视频大语言模型在持续工具流中的工具使用能力，解决了灾难性遗忘问题，并展示了卓越的泛化性能。
...
TL;DR: 本文提出了一种无需训练的3D高斯点阵重建增强方法FixingGS，利用扩散模型提取跨视角一致的先验信息，有效去除稀疏视角下的伪影并修复缺失内容。


<details>
  <summary>Details</summary>
Motivation: 稀疏视角下的3D重建因视觉信息不足导致严重伪影，现有基于生成先验的方法难以保证多视角一致性。

Method: 提出FixingGS，通过蒸馏扩散模型获得更准确且跨视角一致的先验，并设计自适应渐进增强策略优化欠约束区域的重建。

Result: 实验表明，FixingGS在视觉质量和重建性能上优于现有最先进方法。

Conclusion: FixingGS能有效利用扩散先验提升稀疏视角3DGS重建质量，兼顾细节真实性和多视角一致性，且无需训练。
...
TL;DR: 本文提出了一种名为Bi-VLM的低比特量化方法，通过基于高斯分位数非均匀分离权重，并结合显著性感知的混合量化算法，在视觉语言模型中实现了高效压缩，在多个基准上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型（VLMs）计算和内存开销大，限制了其在硬件受限环境中的应用，亟需高效的低比特量化方法。

Method: 提出Bi-VLM，将模型权重按高斯分位数非均匀划分为异常值（显著）和多个正常值（非显著）子集，采用显著性感知的混合量化算法对不同子集施加不同的缩放和二值矩阵约束。

Result: 在语言模型部分，Bi-VLM在四个基准和三种模型上的视觉问答任务中比现有最佳方法提升3%-47%；整体VLM提升4%-45%。量化后模型图像token存在90%-99%冗余，可进一步剪枝以提升效率。

Conclusion: Bi-VLM通过非均匀权重划分与显著性感知量化，在极低比特（≤2位）下显著提升了视觉语言模型的压缩效率与性能，同时揭示了量化模型中视觉token的高度冗余性，为后续高效推理提供了方向。
...
TL;DR: 提出DiSSECT框架，通过引入多尺度向量量化构建离散表示瓶颈，提升医学图像自监督学习的泛化性和可迁移性。


<details>
  <summary>Details</summary>
Motivation: 现有自监督学习方法在医学图像中易受捷径学习影响，依赖复杂结构或特定先验，缺乏可扩展性和泛化能力，尤其在胸片等解剖相似性高、病变细微的场景下表现受限。

Method: 提出DiSSECT框架，在自监督学习流程中引入多尺度向量量化，形成离散表示瓶颈，迫使模型学习可重复、结构感知的特征，抑制视图特异性或低效模式。

Result: 在多个公开医学图像数据集上验证，DiSSECT在分类和分割任务中表现出色，无需或仅需极少微调，且在低标签情况下展现出高标签效率。

Conclusion: DiSSECT通过离散化表示学习提升了医学图像表征的鲁棒性、可迁移性和泛化能力，优于现有最先进方法。
...
TL;DR: 本文提出了一种结合热成像、深度学习和车联网通信的实时鹿-车碰撞预警系统，基于自建热成像数据集实现了高精度检测，并通过实地测试验证了其在多种天气条件下的有效性与低延迟性能。


<details>
  <summary>Details</summary>
Motivation: 鹿-车碰撞在美国每年造成大量伤亡和经济损失，且传统可见光摄像头在恶劣环境下的检测效果有限，因此需要更鲁棒的实时检测与预警方案。

Method: 采用热成像技术采集数据，构建包含12000多张热成像图像的数据集，使用深度学习模型进行训练，并集成CV2X通信技术实现车辆间及路侧单元的信息共享。

Result: 系统在实验中达到98.84%的平均精度、95.44%的精确率和95.96%的召回率；实地测试显示在各种天气条件下检测准确率为88%-92%，远超传统可见光摄像头；端到端延迟稳定低于100毫秒。

Conclusion: 该研究验证了基于热成像与车联网的融合方案在减少鹿-车碰撞方面的可行性与优越性能，为智能交通中的野生动物碰撞预防提供了有效技术路径。
...
<details>
  <summary>Details</summary>
Motivation: 现有工具增强型LLM依赖监督模仿或粗粒度强化学习，且反思机制脆弱，常导致错误重复；缺乏对错误诊断与修复的显式学习机制。

Method: 提出“反思-调用-最终”（Reflect-Call-Final）的逐步策略，结合DAPO和GSPO目标及针对工具使用的奖励机制进行训练，使模型生成包含证据诊断和可执行修正调用的简洁反思。

Result: 在BFCL v3和新提出的Tool-Reflection-Bench基准上实验显示，该方法显著提升了多轮工具调用成功率和错误恢复能力，减少了冗余调用。

Conclusion: 将反思过程显式化并直接优化，能有效提升模型在工具使用中的可靠性，并为智能体从失败中学习提供了可复现的路径。
...
TL;DR: 本文提出了一种名为Surgical Application-Aligned Diffusion (SAADi)的新框架，通过将扩散模型与下游任务偏好对齐来生成更有效的手术图像数据，显著提升了分类和分割性能。


<details>
  <summary>Details</summary>
Motivation: 由于标注手术数据稀缺，深度学习在计算机辅助干预中的应用受限；同时现有扩散模型存在数据记忆化问题，导致生成样本不一致或缺乏多样性。

Method: 构建下游模型偏好的合成图像对（偏好与非偏好），并通过轻量级微调使扩散模型显式地与下游目标对齐，支持迭代优化以进一步提升性能。

Result: 在三个手术数据集上实验显示，分类任务提升7%-9%，分割任务提升2%-10%，对稀有类别改善显著，迭代优化额外带来4%-10%的性能增益。

Conclusion: SAADi有效缓解了数据稀缺问题，克服了生成样本退化现象，提出了任务感知对齐原则，为手术视觉应用提供了新的发展方向。
...
TL;DR: 本文提出了VIR-Bench，一个包含200个旅行视频的新基准，用于评估多模态大语言模型在长距离时空轨迹理解方面的能力，实验表明现有模型表现不佳，同时通过构建旅行规划代理验证了该基准的实际应用价值。


<details>
  <summary>Details</summary>
Motivation: 现有的视频理解基准主要关注室内或短距离户外场景，缺乏对长距离时空轨迹理解的评估，限制了多模态大语言模型在导航和具身AI等现实任务中的发展。

Method: 提出VIR-Bench，将行程重建作为核心任务，设计用于评估MLLMs在长时间、大范围地理空间视频中的理解能力，并基于此开发了一个旅行规划代理进行案例研究。

Result: 实验结果显示当前最先进的多模态大语言模型（包括闭源模型）在VIR-Bench上表现较差，说明该任务具有较高难度；同时，基于该基准构建的旅行规划代理展现出更优的行程推荐能力。

Conclusion: VIR-Bench有效揭示了现有MLLMs在长距离时空理解上的局限性，不仅提供了一个新的评估标准，还证明了其在实际应用中的指导价值。
...
TL;DR: 提出一种基于模型的神经网络KMDS-Net，用于动态PET图像去噪，利用帧间空间相关性和帧内结构一致性，显著提升去噪性能。


<details>
  <summary>Details</summary>
Motivation: 动态PET成像中短时间帧的统计信息有限，导致图像质量难以提高，现有方法在时空分辨率和去噪效果上存在不足。

Method: 构建基于核空间多维稀疏（KMDS）模型，结合深度学习，将参数估计过程用神经网络替代，实现端到端的自适应优化网络KMDS-Net。

Result: 在模拟和真实数据上的实验表明，KMDS-Net在去噪性能上优于基线方法，能有效提升动态PET的时空分辨率。

Conclusion: 所提出的KMDS-Net可有效改善动态PET图像质量，具有临床应用潜力。
...
TL;DR: 提出ColorBlindnessEval，一个受石原色盲测试启发的基准，用于评估视觉-语言模型在对抗性视觉场景中的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有VLM在复杂视觉模式下的数字识别能力不足，尤其在模拟色盲的对抗性场景中表现不佳。

Method: 构建包含500张类石原图像的数据集，涵盖0-99的数字和多种颜色组合，使用Yes/No和开放式提示评估9个VLM，并与人类表现对比。

Result: 实验显示VLM在对抗性上下文中识别数字能力有限，普遍存在幻觉问题，性能显著低于人类。

Conclusion: 需要提升VLM在复杂视觉环境中的鲁棒性，ColorBlindnessEval为评估和改进VLM在关键应用场景中的可靠性提供了有效工具。
...
TL;DR: 提出一种结合光流法分割标签插值与多任务学习的新框架，以解决手术场景理解中的时空不平衡问题，提升机器人辅助手术的理解精度和效率。


<details>
  <summary>Details</summary>
Motivation: 现有的单任务方法难以全面理解复杂的手术场景，且多任务学习受限于像素级标注数据的稀缺和时空标注不平衡问题。

Method: 利用光流法从标注的关键帧中估计并传播标签到相邻未标注帧，结合多任务学习框架，增强稀疏空间监督，平衡时空信息。

Result: 该方法有效提升了手术场景理解的准确性和效率，缓解了标注数据稀疏带来的限制。

Conclusion: 所提出的框架通过光流引导的标签插值增强了多任务学习的训练效果，在减少标注成本的同时提高了机器人辅助手术场景理解的性能。
...
TL;DR: Citrus-V 是一个结合图像分析与文本推理的多模态医学基础模型，支持检测、分割和链式思维推理，实现病灶定位、结构化报告生成和临床诊断推断。


<details>
  <summary>Details</summary>
Motivation: 现有医学影像模型泛化能力差，需多个专用网络；临床应用需要精确的视觉定位、多模态融合和推理能力。

Method: 提出 Citrus-V 模型，集成检测、分割和多模态链式思维推理，并采用新型多模态训练方法，发布涵盖多种任务的开源数据集。

Result: 在多个基准上优于现有的开源医学模型和专家级影像系统，实现从视觉定位到临床推理的统一 pipeline。

Conclusion: Citrus-V 支持精确病灶量化、自动报告生成和可靠第二意见，为医学影像分析提供统一且高效的解决方案。
...
TL;DR: 本文提出了一种名为Hyper-Bagel的统一加速框架，用于同时加速多模态理解和生成任务，通过推测解码和多阶段蒸馏实现显著的速度提升。


<details>
  <summary>Details</summary>
Motivation: 随着上下文中交织的多模态标记越来越多，扩散去噪和自回归解码的迭代过程带来了巨大的计算开销，因此需要一种高效的统一加速方法。

Method: 采用分而治之的策略，使用推测解码进行下一个标记预测，并通过多阶段蒸馏过程加速扩散去噪。

Result: 在多模态理解任务中实现了两倍以上的加速；在生成任务中，无损的6-NFE模型在文本到图像生成中实现了16.67倍加速，在图像编辑中实现了22倍加速；进一步开发的1-NFE模型支持近实时交互式编辑与生成。

Conclusion: Hyper-Bagel框架在保持高质量输出的同时，显著提升了多模态任务的效率，结合对抗蒸馏与人类反馈学习，实现了成本效益和响应速度的最优化。
...
TL;DR: 该研究评估了多模态大语言模型（如GPT-4o、Gemini 2.5）和视觉语言模型（如CLIP、SigLIP）在基督教圣像单标签分类任务中的表现，并与微调的ResNet50进行对比，发现部分通用模型优于传统方法，尤其在引入类别描述时零样本性能提升，但少样本效果有限。


<details>
  <summary>Details</summary>
Motivation: 评估通用多模态大模型是否能在无需专门训练的情况下，有效处理基督教圣像这类视觉复杂且依赖文化背景的艺术图像分类任务，并探索其在数字人文元数据整理中的潜力。

Method: 使用ArtDL、ICONCLASS和Wikidata三个支持Iconclass体系的数据集，选取前10个高频类别，在三种条件下测试模型：仅用类别标签、加入Iconclass描述、以及五例少样本学习；并与微调的ResNet50基线比较。

Result: Gemini-2.5 Pro和GPT-4o整体优于ResNet50基线，但在Wikidata上准确率显著下降（SigLIP表现最佳），提示模型对图像尺寸和元数据对齐敏感；添加类别描述普遍提升零样本性能，而少样本学习效果不佳，仅偶有轻微提升。

Conclusion: 通用多模态大语言模型具备处理视觉复杂的文化遗产图像分类的能力，可作为数字人文中元数据整理的工具，未来应研究提示优化及其他分类策略。
...
TL;DR: 提出了一种可学习的、无需超参数的图构建方法LRGC，用于视觉图神经网络（ViG），在ImageNet-1k上优于现有ViG模型。


<details>
  <summary>Details</summary>
Motivation: 现有ViG模型依赖非学习、非参数化的统计方法构建图像图结构，难以选择最优节点邻域，且需手动设置超参数，缺乏灵活性和适应性。

Method: 提出Learnable Reparameterized Graph Construction (LRGC)，通过节点对之间的键-查询注意力机制计算关系，并采用软阈值重参数化进行可微分的边选择，使图结构构建过程可学习且无需预设超参数。

Result: ViG-LRGC在ImageNet-1k数据集上优于同类规模的最先进ViG模型，验证了其有效性。

Conclusion: LRGC为视觉图神经网络提供了更优的可学习图构建方式，消除了传统方法的偏差，实现了每层自适应阈值学习，提升了模型性能。
...
TL;DR: 本文提出了一种基于对抗强化学习的点提示优化框架Point Prompt Defender，通过攻击-防御机制自动优化SAM模型的提示质量，提升分割性能和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖启发式或人工设计的提示，难以扩展且泛化能力有限，因此需要一种自动化、任务无关的提示优化方法。

Method: 构建一个双空间图表示的点提示环境，使用深度Q网络训练攻击者和防御者代理，在对抗过程中学习抑制有害提示并恢复分割精度。

Result: 实验表明该方法显著提升了SAM在多种任务下的鲁棒性和分割性能，且无需重新训练模型。

Conclusion: Point Prompt Defender提供了一个灵活、可解释、即插即用的提示优化框架，有效增强了基于提示的分割模型的通用性。
...
TL;DR: SmartWilds是首个结合无人机影像、相机陷阱和生物声学记录的多模态野生动物监测数据集，支持濒危物种研究与生态保护。


<details>
  <summary>Details</summary>
Motivation: 为解决濒危物种研究、保护生态学和栖息地管理中的关键需求，推动多模态AI在环境监测中的应用。

Method: 在俄亥俄州The Wilds保护区收集了四个连续日的同步多模态数据，包括无人机图像、相机陷阱照片与视频、生物声学记录，并对不同传感器模态的表现进行了比较分析。

Result: 成功实现了三种模态在220英亩牧场上的同步监测，覆盖多种动物物种，展示了各传感器在土地利用、物种检测、行为分析和栖息地监测方面的互补优势。

Conclusion: 本研究建立了可重复的多模态野生动物监测协议，并公开数据集以促进保护性计算机视觉研究，未来将扩展数据类型和时间覆盖范围。
...
TL;DR: 本文提出了一个名为RS3DBench的新基准，包含54,951对遥感图像和像素级对齐的深度图及文本描述，用于推动大规模通用3D视觉模型在遥感领域的应用，并基于稳定扩散提出了一种新的深度估计模型，取得了当前最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有遥感数据集大多缺乏完整的深度信息或未能精确对齐深度数据与图像，限制了3D视觉感知模型的发展，因此需要构建一个高质量、多模态对齐的基准数据集。

Method: 构建了一个包含图像、深度图和文本描述的大型配对数据集RS3DBench，并基于稳定扩散模型设计了一个多模态融合的遥感深度估计模型。

Result: 提出的模型在RS3DBench上实现了最先进的深度估计性能，验证了数据集的有效性和模型的优越性。

Conclusion: RS3DBench为遥感图像的3D理解提供了重要资源，推动了地理人工智能和通用3D视觉模型的发展。
...
TL;DR: 提出了一种无需Structure-from-Motion的去模糊3D高斯点阵方法DeblurSplat，利用事件相机和预训练立体匹配模块实现高质量场景重建。


<details>
  <summary>Details</summary>
Motivation: 解决运动模糊对3D高斯点阵重建质量的影响，避免传统SfM流程中因相机位姿估计不准导致的累积误差。

Method: 采用DUSt3R模块直接从模糊图像生成初始点云，并融合事件流解码潜在清晰图像，提供细粒度监督信号用于优化重建。

Result: 实验表明，该方法在多种场景下均能生成高保真新视角图像，且渲染效率显著优于现有去模糊3D-GS方法。

Conclusion: DeblurSplat通过结合事件相机与预训练立体匹配模型，实现了高效、准确的去模糊3D重建，为动态模糊场景提供了新的解决方案。
...
TL;DR: 提出MoiréNet，一种基于U-Net的卷积神经网络框架，结合频域和空域特征，有效去除显示设备与相机传感器间产生的莫尔条纹，在性能领先的同时参数量减少48%，适用于资源受限场景。


<details>
  <summary>Details</summary>
Motivation: 莫尔条纹由显示像素阵列与相机传感器网格之间的频谱混叠引起，具有各向异性和多尺度特性，传统去莫尔方法难以有效处理这些复杂伪影。

Method: 提出MoiréNet，包含方向性频-空编码器（DFSE）以识别莫尔条纹方向，并通过频-空自适应选择器（FSAS）实现特征自适应抑制；结合U-Net结构融合频域与空间域特征。

Result: 在公开且广泛使用的数据集上达到SOTA性能，参数量仅为5.513M，相比ESDNet-L减少48%，同时保持更高的图像恢复质量。

Conclusion: MoiréNet在去莫尔效果和参数效率之间取得良好平衡，适合智能手机摄影、工业成像和增强现实等资源受限应用。
...
TL;DR: 本文提出了一种新的音频-视觉分割框架FAVS，通过频域分解与重组的视角重新思考AVS任务，引入FDED和SCMC模块以解决音频与视觉模态在频域上的固有矛盾，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有AVS方法忽略了音频和视觉模态在频域上的本质差异，尤其是音频高频中的噪声干扰与视觉高频中丰富结构细节之间的矛盾，导致分割性能受限。

Method: 将AVS任务重构为频域分解与重组问题，设计了频率感知的FAVS框架，包含FDED模块（基于残差的迭代频域分解）和SCMC模块（基于专家混合架构的动态路由），分别用于提取模态特定语义与结构特征，并增强跨模态语义一致性。

Result: 在三个基准数据集上实现了最先进的性能，大量可视化结果验证了FDED和SCMC模块的有效性。

Conclusion: FAVS通过频域建模有效解决了音频与视觉模态间的冲突，显著提升了音频-视觉分割的精度与鲁棒性，具有良好的应用前景。
...
TL;DR: 本文综述了面向视觉感知任务的四种代表性可解释人工智能（xAI）方法：显著图、概念瓶颈模型、基于原型的方法和混合方法，分析了其机制、优缺点及评估指标。


<details>
  <summary>Details</summary>
Motivation: 深度学习在图像分析中表现优异但常被视为“黑箱”，缺乏可解释性限制了其在关键应用中的可靠性，因此需要发展可解释AI以增强人类对模型决策过程的理解。

Method: 对四类主流xAI方法进行系统性综述，包括显著图、概念瓶颈模型、原型方法和混合方法，并分析其工作机制、优势与局限性以及评估方式。

Result: 提供了当前视觉任务中xAI方法的全面比较与总结，明确了各类方法的适用场景与现存挑战。

Conclusion: 该综述为未来xAI的研究方向和实际应用提供了指导，强调了在保持模型性能的同时提升可解释性的重要性。
...
TL;DR: 本文提出了一种改进的去噪扩散概率模型（DDPM），通过新的噪声调度和时间步嵌入技术生成高质量的合成LiDAR点云数据，用于增强自动驾驶车辆感知性能，实验表明该方法在多个指标上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 由于真实世界LiDAR数据采集耗时且易受噪声和稀疏性影响，限制了自动驾驶中3D视觉系统的性能，因此需要有效的方法生成高质量、多样化的合成数据以弥补数据缺陷。

Method: 采用去噪扩散概率模型（DDPM），引入新颖的噪声调度策略和时间步嵌入技术，提升模型对时序信息的感知能力，并基于投影生成更真实的点云数据。

Result: 在IAMCV和KITTI-360数据集上广泛评估，使用四种性能指标与SOTA方法对比，结果表明该方法能有效缓解噪声和稀疏问题，生成具有丰富空间关系和结构细节的多样化点云，在多数基准上表现更优。

Conclusion: 所提出的改进DDPM在生成高质量LiDAR点云方面优于现有方法，能够有效提升自动驾驶感知任务的性能，具备应用于实际场景的潜力。
...
TL;DR: 提出了一种新的异常引导自监督预训练方法（AGSSP），用于金属表面缺陷检测，通过两阶段框架利用异常先验信息提升模型性能，在多个指标上显著优于ImageNet预训练模型。


<details>
  <summary>Details</summary>
Motivation: 由于自然图像预训练存在域差距，而直接在工业数据上进行自监督预训练难以区分细微缺陷与复杂背景噪声，导致缺陷检测性能受限。

Method: 采用两阶段自监督预训练框架：第一阶段通过蒸馏异常图知识来预训练骨干网络，捕捉缺陷显著特征；第二阶段利用伪缺陷框预训练检测器，使其对齐定位任务。同时提出一种知识增强的异常图生成方法，并构建大规模工业图像数据集。

Result: 实验表明，AGSSP在多种设置下均能持续提升性能，相比ImageNet预训练模型，mAP@0.5最高提升10%，mAP@0.5:0.95提升11.4%。

Conclusion: AGSSP有效解决了工业缺陷检测中数据稀缺和域差异问题，通过引入异常先验显著提升了自监督预训练效果，具备良好的实用性和推广性。
...
TL;DR: 提出首个音频驱动的通用逼真虚拟头像合成方法，结合通用语音模型与新型通用头像先验（UHAP），实现高保真唇形同步与丰富的表情细节。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要将音频映射到几何变形，忽略音频相关的外观变化，难以生成兼具几何与外观细节的逼真头像。

Method: 提出通用头像先验（UHAP），在跨身份多视角视频上训练，并利用中性扫描数据监督以捕捉身份特异性细节；设计通用语音模型将原始音频直接映射到UHAP的潜在表达空间（包含几何与外观变化）；采用单目编码器实现对新主体的高效个性化。

Result: 解码音频驱动的表达码可生成高度逼真的虚拟头像，具备精确的唇形同步和细腻的表情细节（如眉毛运动、眼神变化、口腔内外观）；实验表明该方法在唇形同步精度、图像质量与感知真实感等指标上优于现有几何主导的方法。

Conclusion: 该方法是首个可泛化的音频驱动逼真头像合成框架，能同时建模精细的几何与外观变化，显著提升合成头像的真实感与表现力。
...
TL;DR: 提出了一种基于机器学习的模块化管道SynapFlow，用于自动化检测、时间追踪和特征提取双光子显微镜3D+时间数据中的树突棘。


<details>
  <summary>Details</summary>
Motivation: 树突棘的结构动态分析对研究学习和记忆的神经基础至关重要，但大规模分析仍具挑战性且耗时。

Method: 结合基于Transformer的检测模块、深度追踪组件、利用空间一致性的时序追踪模块以及特征提取单元，实现端到端的自动化分析。

Result: 在开源和自建标注数据集上验证了方法的有效性，发布了数据、代码和预训练权重，为树突棘动态分析提供了新基准。

Conclusion: 该方法可显著提升树突棘在3D+时间数据中分析的效率与可扩展性，推动相关神经科学研究的发展。
...
TL;DR: 提出一种结合视觉-语言模型（VLM）和预训练视觉模型的新型零样本图像分类框架，通过自学习循环实现无需标注数据的动态适应，在十个多样化数据集上优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 深度学习通常依赖大量标注数据，这在实际数据稀缺场景中构成障碍，因此需要不依赖标注数据的分类方法。

Method: 结合VLM和预训练视觉模型，采用基于置信度的伪标签策略，在测试数据上训练轻量级分类器，通过迭代优化融合语义与视觉特征。

Result: 在十个不同数据集上的实验表明，该方法优于基线零样本分类方法，实现了更优的分类性能。

Conclusion: 所提框架有效利用VLM和预训练视觉模型的优势，无需微调VLM或使用大语言模型，即可实现高性能零样本图像分类。
...
TL;DR: 本文提出了MirrorScene3D数据集和ReflectiveGS方法，利用镜面反射作为补充视角来提升镜像环境中的3D重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有3D重建方法在镜像环境中性能下降，且常忽视镜面反射所包含的有用信息。

Method: 构建了MirrorScene3D数据集，并提出ReflectiveGS方法，扩展3D高斯点阵以利用镜面反射作为额外视角进行重建。

Result: 在MirrorScene3D上的实验表明，ReflectiveGS在SSIM、PSNR、LPIPS和训练速度上均优于现有方法。

Conclusion: 通过有效利用镜面反射信息，ReflectiveGS显著提升了镜像环境下的3D重建效果，为该领域设定了新基准。
...
TL;DR: 本研究利用深度学习方法，通过生成对抗网络（GAN）增强数据集，从手术中的白光图像中定位胆道系统，以减少腹腔镜胆囊切除术中的胆管损伤风险。


<details>
  <summary>Details</summary>
Motivation: 腹腔镜胆囊切除术虽具有恢复快、美观效果好等优点，但存在较高的胆管损伤风险，影响患者生活质量与生存率，因此需要提高术中胆管的可视化程度以避免损伤。

Method: 构建并标注一个用于训练YOLO检测算法的图像数据库，采用经典数据增强技术，并提出使用生成对抗网络（GAN）生成合成训练数据的一部分。

Result: 实验结果表明，所提出的方法能够有效提升胆道系统的检测性能，同时讨论了相关的伦理问题。

Conclusion: 结合GAN生成的合成数据可有效提升深度学习模型在胆道定位中的表现，有助于降低腹腔镜手术中胆管损伤的风险。
...
<details>
  <summary>Details</summary>
Motivation: 为了在标注数据有限的情况下实现高效的领域自适应分割，受SAM启发，设计一个灵活利用点提示的框架。

Method: 提出Prompt-DAS框架，结合辅助中心点检测任务和提示引导的对比学习，在训练和测试阶段可灵活使用任意数量的点提示。

Result: 在多个具有挑战性的基准上实验表明，该方法在UDA、WDA和基于SAM的方法上均表现出优越性能。

Conclusion: Prompt-DAS通过灵活的提示机制和对比学习，有效提升了域自适应分割的性能，适用于多种标注设置。
...
TL;DR: 本文提出了一种用于视觉-语言模型的细粒度推理步骤链框架，通过步骤级奖励模型实现更精确的推理质量评估，并在多个基准上取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言推理方法多采用粗粒度推理链，难以进行细粒度结构化推理，且中间步骤的质量评估困难，缺乏有效的奖励机制。

Method: 提出步骤级推理数据构建、过程奖励模型（PRM）和基于细粒度奖励的强化学习训练框架，支持推理时扩展。

Result: 在多个具有挑战性的视觉-语言基准上建立了强有力的基线，实现了持续性能提升，并通过消融研究验证了各组件作用及推理时扩展的特性。

Conclusion: 该工作为视觉-语言模型提供了可解释、可评估的细粒度推理框架，有望推动复杂多模态推理的发展。
...
TL;DR: 提出一种基于弱监督学习的食品图像语义分割方法，利用ViT生成的类激活图作为SAM的提示，结合图像预处理和多掩码策略提升分割效果，在FoodSeg103数据集上取得良好结果。


<details>
  <summary>Details</summary>
Motivation: 减少对像素级标注的依赖，利用零样本能力和视觉Transformer的注意力机制提升食品图像分割效果。

Method: 使用Swin Transformer生成类激活图（CAMs）作为Segment Anything Model（SAM）的提示，采用图像预处理和单/多掩码生成策略优化SAM输出，仅使用图像级标签进行训练。

Result: 在FoodSeg103数据集上平均每张图像生成2.4个掩码（不含背景），多掩码情况下mIoU达到0.54。

Conclusion: 该方法可有效用于加速食品图像标注任务，或集成于食物与营养追踪应用中。
...
TL;DR: 提出DyL-UNet，一种基于动态学习的U-Net架构，通过构建回声动力学图（EDG）和引入心脏相位-动力学注意力（CPDA）机制，在保持分割精度的同时显著提升超声心动图序列的时间一致性。


<details>
  <summary>Details</summary>
Motivation: 超声心动图易受形变和斑点噪声影响，导致帧间分割抖动，即使单帧分割准确，时间不稳定性仍会影响功能评估和临床解释。

Method: 设计DyL-UNet模型，采用多分支Swin-Transformer编码器-解码器处理单帧图像，并通过动态学习构建回声动力学图（EDG）提取视频动态信息，在跳跃连接处引入心脏相位-动力学注意力（CPDA）机制以增强时间一致性。

Result: 在CAMUS和EchoNet-Dynamic数据集上的实验表明，DyL-UNet在保持与其他方法相当的分割精度的同时，实现了更优的时间一致性。

Conclusion: DyL-UNet能有效提升超声心动图分割的时间稳定性，为自动化临床分析提供了可靠解决方案。
...
TL;DR: 提出WaveletGaussian框架，通过在小波域中应用扩散模型（仅用于低频子带）并结合轻量网络优化高频子带，显著提升稀疏视角下3D高斯溅射重建效率。


<details>
  <summary>Details</summary>
Motivation: 现有方法在稀疏视角下性能下降，依赖扩散模型修复渲染结果虽有效但计算开销大，需更高效的方法。

Method: 将扩散模型引入小波域，仅对低分辨率LL子带进行扩散修复，高频子带由轻量网络优化；采用在线随机掩码策略生成训练对，替代低效的留一法。

Result: 在Mip-NeRF 360和OmniObject3D数据集上实验表明，该方法在保持竞争力渲染质量的同时大幅减少训练时间。

Conclusion: WaveletGaussian通过小波域分解与高效训练策略，在稀疏视角下实现了快速且高质量的3D高斯重建，优于传统扩散修复方法。
...
<details>
  <summary>Details</summary>
Motivation: 发现 Sa2VA 在指代表达视频对象分割任务中未发挥全部潜力，主要由于训练与推理过程存在不一致。

Method: 提出 Sa2VA-i，修正了原模型在训练和推理流程中的不一致问题，提升了模型性能。

Result: Sa2VA-i 在 MeViS、Ref-YT-VOS、Ref-DAVIS 和 ReVOS 等多个基准上显著提升性能，最高提升达 +11.6 J&F（MeViS），且 Sa2VA-i-1B 模型性能媲美原 Sa2VA-26B 模型。

Conclusion: 实现细节对模型性能有重要影响，改进后的 Sa2VA-i 展现出显著优势，并为指代表达视频分割领域提供了有价值的见解。
...
TL;DR: 提出一种无需训练的方法，将多光谱数据以零样本模式输入仅支持RGB的通用多模态模型（如Gemini2.5），通过适配输入空间和注入领域特定指令，实现对遥感任务的有效支持。


<details>
  <summary>Details</summary>
Motivation: 现有的多光谱图像分析依赖专门训练的模型，成本高且无法与通用大模型兼容，限制了其在遥感中的广泛应用。

Method: 利用通用多模态模型的视觉理解能力，将多光谱输入映射到其训练过的RGB视觉空间，并通过指令注入方式引入领域知识，实现零样本迁移。

Result: 在土地覆盖和土地利用分类等遥感基准上显著提升了Gemini2.5的零样本性能，验证了方法的有效性和易适应性。

Conclusion: 该方法使地理空间专业人员能轻松利用强大的通用多模态模型处理非标准传感器数据，充分发挥其推理和上下文能力。
...
TL;DR: 本研究探讨了多模态大语言模型（MLLMs）在基于基础设施摄像头图像中零样本检测和描述交通事故的能力，并结合YOLO、Deep SORT和SAM等视觉分析技术提升性能。


<details>
  <summary>Details</summary>
Motivation: 由于真实交通事故数据稀缺且标注成本高，研究旨在利用零样本学习减少对大规模标注数据的依赖，提升交通监控系统的自动化与实用性。

Method: 使用CARLA模拟生成的DeepAccident数据集评估Gemini 1.5/2.0、Gemma 3和Pixtral模型的零样本事故识别能力，并结合YOLO、Deep SORT和SAM生成增强提示以提升准确性与可解释性。

Result: Pixtral表现最佳，F1得分为0.71，召回率达83%；Gemini系列在增强提示下精度提升至90%，但F1和召回率下降明显；Gemma 3性能最均衡，指标波动最小。

Conclusion: 将MLLMs与先进视觉分析技术结合，能有效提升零样本交通事故检测的准确性与可解释性，具有在真实交通监控系统中应用的巨大潜力。
...
TL;DR: 本文提出了Track-On2，一种简单高效的基于Transformer的在线长期点跟踪模型，通过架构改进、内存优化和合成训练策略，在多个基准上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决在显著外观变化、运动和遮挡下实现跨视频帧的长期点跟踪问题，并满足实时和流式应用的需求。

Method: 扩展了之前的Track-On模型，提出Track-On2，采用因果处理方式和记忆机制来保持时间一致性；在推理时先进行粗粒度块级分类再细化，并系统研究合成训练设置对长期鲁棒性的影响。

Result: Track-On2在五个合成和真实世界基准上均达到最先进水平，超越了现有在线跟踪器甚至利用双向上下文的强离线方法。

Conclusion: 因果式、基于记忆且纯用合成数据训练的架构是现实世界点跟踪任务中可扩展的有效解决方案。
...
TL;DR: KAMERA是一个用于多摄像头、多光谱同步和实时检测海豹与北极熊的综合系统，显著减少了数据处理时间，并完全开源。


<details>
  <summary>Details</summary>
Motivation: 为了提高在阿拉斯加周边海域进行空中调查时对冰栖海豹的检测效率和准确性。

Method: 通过严格的校准和硬件同步，结合多光谱数据进行目标检测，并将所有图像和检测结果映射到世界平面。

Result: 相比之前的方法，数据处理时间最多减少80%，并实现了准确的调查区域估算和快速结果评估。

Conclusion: KAMERA系统有效提升了野生动物监测的效率和精度，其开源设计有望推动科学界的类似应用。
...
TL;DR: NeuCODEX是一种面向脉冲神经网络的协同推断架构，通过联合优化时空冗余、学习驱动的压缩模块和动态早退机制，显著降低边缘到云的数据传输、能耗和延迟，实现高效能、低功耗的边缘智能。


<details>
  <summary>Details</summary>
Motivation: 由于固定的时间步开销，全SNN在边缘推理面临延迟和能耗挑战；现有边缘-云协同推理系统受限于高延迟和特征传输成本。

Method: 提出NeuCODEX架构，引入学习型脉冲驱动压缩模块以减少数据传输，并采用基于输出置信度的动态早退机制来自适应终止推理。

Result: 在CIFAR10、Caltech、CIFAR10-DVS和N-Caltech数据集上验证，原型部署于ResNet-18和VGG-16，在真实边缘-云测试平台中实现最高2048倍的数据传输减少、90%以上的边缘能耗降低和3倍端到端延迟下降，精度损失小于2%。

Conclusion: NeuCODEX有效解决了SNN在资源受限环境下部署的瓶颈，实现了实用且高性能的脉冲神经网络边缘智能。
...
TL;DR: 本文提出了一种鲁棒的自监督立体匹配方法RoSe，通过引入视觉基础模型的先验知识和场景对应先验，提升恶劣天气条件下的立体匹配性能。


<details>
  <summary>Details</summary>
Motivation: 现有自监督立体匹配方法在夜间、雨雾等恶劣天气下性能显著下降，主要由于天气退化导致特征提取困难和光度一致性假设失效。

Method: 利用视觉基础模型提供鲁棒先验增强CNN特征提取，并构建包含清晰与恶劣图像对的合成数据集，提出包含场景对应学习和恶劣天气蒸馏的自监督训练范式。

Result: 实验表明该方法在恶劣天气条件下优于现有的自监督立体匹配方法，具有良好的有效性和泛化能力。

Conclusion: 通过引入基础模型先验和场景对应监督信号，可显著提升自监督立体匹配模型在恶劣天气下的鲁棒性。
...
TL;DR: 本文提出了一种基于YOLO的结肠息肉检测管道YOLO-LAN，采用M2IoU损失、数据增强和负样本训练，在Kvasir-seg和BKAI-IGH NeoPolyp数据集上表现优于现有方法，具有更高的mAP指标和临床实用性。


<details>
  <summary>Details</summary>
Motivation: 结肠镜检查中人工检测息肉存在不一致和遗漏风险，需要更准确、实时的自动检测方法以提高结直肠癌早期诊断效果。

Method: 提出YOLO-LAN检测框架，基于YOLO模型，使用M2IoU损失函数、多样化数据增强策略及负样本模拟真实临床场景进行训练。

Result: 在Kvasir-seg数据集上，YOLOv12实现mAP$_{50}$=0.9619，mAP$_{50:95}$=0.8599；YOLOv8实现mAP$_{50}$=0.9540，mAP$_{50:95}$=0.8487，显著提升检测精度，尤其在多尺度和定位准确性方面表现优异。

Conclusion: YOLO-LAN在不同模型基础上均优于现有方法，具备良好的鲁棒性和精确定位能力，适用于AI辅助的结直肠癌筛查临床应用。
...
TL;DR: 本文研究了LSVOS挑战赛中MOSEv2赛道的复杂半监督视频对象分割问题，通过改进SeC和SAM-2框架，分析其长期记忆和概念感知记忆的作用，最终在测试集上取得39.89%的JF得分，排名第一。


<details>
  <summary>Details</summary>
Motivation: 解决复杂半监督视频对象分割中的遮挡与干扰物抑制问题，提升模型在对象重新出现时的分割连续性与准确性。

Method: 基于SeC方法改进SAM-2框架，引入并分析长期记忆与概念感知记忆机制，前者维持时间连续性，后者提供语义先验以抑制干扰。

Result: 在MOSEv2测试集上达到39.89%的JF得分，位列榜首。

Conclusion: 长期记忆与概念感知记忆的有效结合显著提升了视频对象分割性能，尤其在处理遮挡、重现和干扰物方面表现出色，验证了所提方法在复杂场景下的优越性。
...
<details>
  <summary>Details</summary>
Motivation: 现有VLM在处理视觉信息时采用序列化方式，与人类视觉的并行机制差异大，且其内部机制不透明，限制了理解和创新。因此需要从人类视觉机制出发，深入解析VLM的内部工作机制。

Method: 将VLM的视觉处理分解为对象识别和空间感知两个路径；通过生成文本token图研究对象识别过程；理论推导并实证验证位置表示的几何结构；提出无需指令的token压缩算法和RoPE缩放技术。

Result: 发现对象识别在模型中经历从属性识别到语义消歧的两阶段过程；揭示了位置表示的潜在几何结构；提出的token压缩算法提升了推理效率，RoPE缩放增强了空间推理能力。

Conclusion: 通过对VLM视觉处理机制的解构分析，不仅加深了对其内部工作原理的理解，也为未来更高效、更强推理能力的VLM架构设计提供了清晰原则。
...
TL;DR: 本文提出一种无需视觉编码器的文本到文本检索方法，利用VLLM生成的图像描述替代原始图像，在减少模态差距、提升组成性和隐私保护的同时，实现了优于或媲美传统多模态模型的零样本检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有的对比训练视觉-语言模型（如CLIP）存在语言理解浅层化、模态差距大、训练成本高和隐私问题，因此需要探索更高效、隐私友好的检索范式。

Method: 提出一种纯文本单编码器检索框架，使用大语言模型生成的结构化图像描述将图像内容转化为文本，从而实现从文本到文本的跨模态检索，并在少量GPU资源上进行校准优化。

Result: 该方法在多个检索和组成性基准测试上达到最先进的零样本性能，即使仅使用0.3B参数的模型也能超越传统多模态模型，同时显著降低计算成本并提升隐私保护。

Conclusion: 视觉编码器在检索任务中并非必需，通过文本化图像表示可有效缩小模态差距，提升模型组成性和实用性，为高效、安全的视觉-语言检索提供了新方向。
...
TL;DR: 本文研究了对比视觉-语言模型在理解长而密集的标题时的组成性能力，发现组成性训练与长标题理解之间存在双向促进关系，但性能增益受数据质量和模型设计影响。高质量的长标题数据可同时提升两种能力。


<details>
  <summary>Details</summary>
Motivation: 尽管对比视觉-语言模型在绑定视觉和文本信息方面取得进展，但在理解长且密集的标题方面仍存在挑战。作者假设组成性（即对象-属性绑定和对象间关系推理）是关键因素。

Method: 通过训练和评估一系列针对组成性和长标题理解能力的模型，分析二者之间的相互作用。实验考察不同数据结构、参数更新策略和模型设计的影响。

Result: 发现组成性训练能提升长标题检索性能，而长标题训练也能增强组成性理解；但若使用结构不良的数据或限制参数更新，则无法泛化；冻结位置嵌入等保持对齐的策略无助于组成性理解。

Conclusion: 组成性理解和长标题理解是相互关联的能力，可通过高质量、密集且有 grounding 的描述联合学习，为提升VLM的泛化能力提供实践指导。
...
TL;DR: 提出了一种结合合成RGB图像、少量真实标注和基于GAN的跨模态对齐的框架，以提升热成像中植物分割的准确性，在复杂田间环境中显著提高了分割性能。
...
Method: 使用1,128张合成图像训练模型，并引入最多5张真实手动分割图像，结合CycleGAN-turbo实现RGB到热成像的跨模态转换，进行语义分割。

Result: 相比全真实数据基线，杂草类最大相对提升22%，植物类提升17%。

Conclusion: 结合合成数据、少量真实标注和生成模型的跨域翻译可显著提升多模态影像在复杂环境下的分割性能。
...
TL;DR: 提出并公开了HyKid数据集，包含48名儿童脑积水患者的高分辨率3D MRI和手动分割的脑组织及脉络丛标注，结合临床报告提取结构化信息，发现脉络丛体积与脑脊液总量具有强相关性，可作为脑积水评估的潜在生物标志物。


<details>
  <summary>Details</summary>
Motivation: 儿童脑积水评估困难，且缺乏公开的、专家标注的神经影像数据集，尤其是包含脉络丛分割的数据，限制了相关研究的发展。

Method: 收集48名儿科脑积水患者的低分辨率MRI，通过切片到体积（slice-to-volume）算法重建为1mm各向同性高分辨率3D MRI；由经验丰富的神经科医生手动标注白质、灰质、侧脑室、外部脑脊液和脉络丛；使用检索增强生成（RAG）框架从临床放射报告中提取结构化数据；分析脉络丛体积与脑脊液总量的关系，并构建预测模型。

Result: 成功构建了高质量的HyKid数据集，提供了精确的手动分割标注和结构化临床信息；发现脉络丛体积与总脑脊液体积之间存在强相关性；基于该特征的预测模型在脑积水评估中表现出色（AUC = 0.87）。

Conclusion: HyKid数据集为儿童脑积水的神经影像研究提供了宝贵的公开资源，揭示了脉络丛在脑积水评估中的重要作用，支持未来算法开发和生物标志物探索。
...
TL;DR: 提出了一种多尺度特征交互网络（MsFIN），用于从行车记录仪视频中进行早期事故预测，通过多尺度特征聚合、时序特征处理和多尺度特征后融合，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决行车记录仪视角下事故预测中的两个关键挑战：交通参与者之间的特征级交互建模（常因遮挡）以及复杂异步多时态行为线索的捕捉。

Method: 设计了多尺度模块以提取短、中、长期时间尺度的场景表征，并利用Transformer架构增强特征交互；通过时序特征处理捕捉在因果约束下的场景与对象特征演变；最后进行多尺度特征后融合生成综合风险表示。

Result: 在DAD和DADA数据集上的实验表明，MsFIN在预测准确性和预警及时性方面显著优于单尺度特征提取的最先进模型，消融实验验证了各模块的有效性。

Conclusion: MsFIN通过多尺度特征融合与上下文交互建模，有效提升了行车记录仪视频中的早期事故预测性能。
...
TL;DR: 本文提出一种基于持续学习的面部伪造检测方法，采用开发式混合专家（MoE）架构与LoRA模块，通过正交梯度防止灾难性遗忘，在新伪造类型不断出现的情况下实现高效自适应检测。


<details>
  <summary>Details</summary>
Motivation: 由于伪造人脸技术快速演进，传统检测模型难以跟上新类型，且受限于数据和计算资源，无法持续学习新伪造类型而不遗忘旧知识。

Method: 提出Developmental Mixture of Experts架构，使用LoRA作为专家模块，分为Real-LoRA学习真实人脸特征，多个Fake-LoRA分别学习不同伪造类型；引入正交梯度约束，确保新任务学习方向与已有子空间正交，避免干扰和遗忘。

Result: 在数据集和伪造类型增量协议下的实验表明，该方法能有效适应新伪造类型，同时保持对已学伪造类型的检测性能，优于现有持续学习和伪造检测方法。

Conclusion: 将面部伪造检测建模为持续学习问题，结合LoRA-based MoE架构与正交梯度策略，实现了高效、可扩展且抗遗忘的伪造检测框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有模型在多模态理解与生成上存在局限，如仅支持简单任务或低分辨率生成，缺乏统一框架将理解能力用于优化生成过程。

Method: 提出Lavida-O模型，采用弹性混合Transformer架构、通用文本条件化和分层采样等技术，实现理解与生成的统一建模，并引入自省机制优化生成过程。

Result: 在RefCOCO物体定位、GenEval文本到图像生成和ImgEdit图像编辑等多个基准上达到SOTA，优于Qwen2.5-VL和FluxKontext-dev等模型，并显著提升推理速度。

Conclusion: Lavida-O是首个能利用自身理解能力改进生成结果的统一多模态扩散模型，兼具高性能与高效推理，推动了多模态生成模型的发展。
...
TL;DR: 本文提出了基于概念的视频相似性估计任务（ConViS）和相应的基准数据集ConViS-Bench，旨在通过语义概念对视频进行细粒度、可解释的相似性评估，并推动语言驱动的视频理解研究。


<details>
  <summary>Details</summary>
Motivation: 现有视频相似性模型多依赖全局相似性评分，难以捕捉人类从多个语义层面判断视频相似性的能力；因此需要一种能按不同语义概念进行细粒度比较的方法。

Method: 提出ConViS任务，通过预定义的语义概念集对视频对进行逐概念的相似性评分，并构建包含细粒度标注的ConViS-Bench基准数据集，用于评估模型在概念级视频相似性判断上的表现。

Result: 评测了多种前沿模型，发现它们在不同概念上的表现存在显著差异，表明某些概念更具挑战性；ConViS-Bench能有效反映模型与人类判断的一致性。

Conclusion: ConViS及其基准为语言驱动的视频理解提供了新方向，促进了更接近人类推理方式的视频相似性建模。
...
TL;DR: 本文提出了一种基于对抗优化的VQ-GAN框架，通过密集运动符号化来压缩时空热图，有效保留人体运动的细粒度轨迹，显著优于基线方法，并揭示了2D和3D运动表示在码本大小上的本质差异。


<details>
  <summary>Details</summary>
Motivation: 由于人体运动具有高维性和内在冗余性，如何高效压缩和表示连续人体运动是一个核心挑战，现有方法存在重构伪影和时序不稳定问题。

Method: 提出一种结合密集运动符号化与对抗优化的VQ-GAN框架，利用对抗训练消除运动模糊和时序错位等重建伪影，提升运动表示质量。

Result: 在CMU Panoptic数据集上，SSIM指标超过dVAE基线9.31%，时序不稳定性降低37.1%；发现2D运动可用128个符号紧凑表示，而3D运动需要1024个符号的码本才能准确重建。

Conclusion: 所提方法在运动压缩与表示方面性能优越，具备实际部署潜力，适用于多种人体运动分析应用。
...
<details>
  <summary>Details</summary>
Motivation: 传统放射组学方法常忽略病灶内复杂的空间异质性，难以准确区分恶性肿瘤与混淆性病变，亟需更可靠的影像学生物标志物。

Method: GrRAiL首先基于体素级放射组学特征对病灶子区域进行聚类，然后构建加权图并计算图论指标以量化簇间空间关联，从而表征病灶内异质性。

Result: 在947例患者中验证，GrRAiL在胶质母细胞瘤复发识别、脑转移瘤及胰腺IPMN风险分层任务中表现优异，交叉验证和测试准确率分别为89%/78%、84%/74%、84%/75%，较现有方法提升超10%。

Conclusion: GrRAiL能有效捕获病灶内高阶空间异质性信息，显著提高多种肿瘤场景下良恶性病变的鉴别能力，具有良好的临床可行性和应用前景。
...
TL;DR: 本文提出CLOPS，首个仅依赖自我中心视觉感知环境并导航的人类化身模型，通过解耦低层次运动技能与高层次视觉到运动的控制学习，实现了类人运动行为。


<details>
  <summary>Details</summary>
Motivation: 现有动作生成方法忽视了人类感知与运动间的内在依赖，使用非人感知方式，难以生成真正类人的行为。作者认为类人行为需要类人感知。

Method: 采用两阶段方法：首先在大规模动作捕捉数据上训练运动先验模型；然后使用Q-learning训练策略网络，将自我中心视觉输入映射为对运动先验的高层控制指令。

Result: 实验证明，仅凭自我中心视觉即可使化身表现出类人运动特征，如根据视野中的障碍物调整行走路径进行避障。

Conclusion: 赋予化身类人传感器（尤其是自我中心视觉）有望实现更自然、类人的人类行为模拟。
...
TL;DR: 本文提出了OverLayScore指标和OverLayBench基准，用于评估布局到图像生成中重叠边界框的复杂性，并提出CreatiLayout-AM模型以改善在复杂重叠情况下的生成性能。


<details>
  <summary>Details</summary>
Motivation: 现有布局到图像生成方法在处理边界框显著重叠的情况下表现不佳，且当前基准偏向简单案例，缺乏对复杂重叠场景的有效评估。

Method: 引入OverLayScore量化重叠复杂度，构建OverLayBench新基准，并基于去遮挡掩码数据集微调CreatiLayout-AM模型。

Result: 分析显示现有方法在高OverLayScore情况下性能下降，新基准覆盖更复杂的重叠场景，CreatiLayout-AM在复杂重叠条件下表现出改进。

Conclusion: OverLayScore和OverLayBench为评估复杂重叠布局提供了更全面的标准，CreatiLayout-AM为解决该问题提供了初步有效方案，推动了现实场景下布局到图像生成的发展。
...
TL;DR: 本文提出VolSplat，一种基于体素对齐高斯分布的新型前馈3D高斯点阵化方法，克服了传统像素对齐方法在多视角一致性、密度分布偏差和对输入视图依赖性方面的局限性，在RealEstate10K和ScanNet上实现了最先进的新视角合成性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于像素对齐的3D高斯点阵化方法存在对输入视图数量依赖性强、视图偏差密度分布以及因遮挡或低纹理导致的对齐误差等问题，限制了重建质量与多视角一致性，因此需要一种更鲁棒、一致且可扩展的替代范式。

Method: VolSplat通过预测一个3D体素网格，并在该网格上直接生成体素对齐的高斯分布，取代传统的像素对齐方式；利用3D体素特征避免了不稳定的2D特征匹配，实现更可靠的多视角几何一致性，并支持根据场景复杂度自适应控制高斯密度。

Result: 在RealEstate10K和ScanNet等基准上，VolSplat取得了当前最优的新视角合成性能，生成的高斯点云更逼真、视角间更一致，具有更好的几何准确性和渲染质量。

Conclusion: VolSplat建立了一种更可扩展、更鲁棒的前馈式3D重建框架，通过体素对齐机制克服了像素对齐的固有缺陷，为高斯点阵化及多视角重建领域的后续研究提供了新的方向。
...
TL;DR: 提出了一种名为CAR-Flow的条件感知重参数化方法，用于扩散和流模型中的条件生成建模，通过调整源和目标分布的位置，缩短模型需学习的概率路径，从而加速训练并提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散和流模型在条件生成中需要同时学习质量传输和条件注入，对模型要求较高。为减轻模型负担，希望将条件信息融入源或目标分布中，以简化学习过程。

Method: 提出Condition-Aware Reparameterization for Flow Matching（CAR-Flow），通过一个轻量级的可学习偏移来调整源分布、目标分布或两者，使模型在更短的概率路径上进行学习，提升效率和效果。

Result: 在低维合成数据上验证了CAR-Flow对概率路径的可视化和量化影响；在ImageNet-256上的实验显示，将CAR-Flow应用于SiT-XL/2模型后，FID从2.07降至1.68，且仅增加不到0.6%的额外参数。

Conclusion: CAR-Flow通过引入条件感知的分布重参数化，有效降低了流匹配模型的学习难度，提升了生成质量与训练效率，是一种高效且轻量的改进方法。
...
TL;DR: 本文提出了一种具有动态提示调度机制的统一多任务学习框架，通过提示池和任务感知调度策略提升模型在多任务和跨域场景下的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在多任务和跨域设置下泛化能力不足的问题，克服传统固定提示模板方法（如SPoT）的局限性。

Method: 引入提示池和任务感知的调度策略，动态组合和对齐不同任务的提示；利用任务嵌入和门控机制精细控制提示信号，并通过联合多任务学习目标自动优化调度权重。

Result: 实验表明该方法在语言理解与知识推理任务上显著提升性能，有效缓解任务干扰和负迁移，在不同提示温度和任务数量下均表现出良好的稳定性和可迁移性。

Conclusion: 所提出的动态提示调度机制在统一多任务建模和跨域适应中具有良好的适用性和有效性。
...
TL;DR: GAUSS是一个评估大语言模型数学能力的基准，涵盖十二个核心技能维度，分为知识理解、问题解决与沟通、元技能与创造力三大领域，通过细粒度任务构建可解释的能力画像。
...
Method: 将数学问题按认知技能分类，设计隔离特定能力的任务，构建多维、结构化的评估框架。

Result: 成功生成GPT-5-thinking和o4-mini-high的技能画像，揭示了二者在不同技能维度上的优劣差异。

Conclusion: GAUSS能有效评估并可视化模型的数学能力结构，支持更精准的能力分析与模型改进。
...
TL;DR: 本文提出了一种基于Rubin因果模型的事件因果识别方法，通过生成“合成控制”来模拟干预效果，从而更可靠地识别文本中的事件因果关系。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖语言模式和多跳推理，容易因非正式因果用法和虚假图推理导致错误的因果判断。因此需要一种更鲁棒的方法来准确识别事件间的因果关系。

Method: 采用Rubin因果模型，将前一个事件视为处理（treatment），后一个事件视为结果（outcome）。通过在文本中寻找或构造与主角经历相似但接受不同处理的‘双胞胎’个体，利用文本嵌入合成与逆向技术生成合成控制组，以估计干预对结果的影响。

Result: 该方法在COPES-hard因果基准上表现出优于包括GPT-4在内的现有方法的性能，能够更稳健地识别事件因果关系。

Conclusion: 基于合成控制和Rubin因果模型的方法为事件因果识别提供了新的有效途径，显著提升了因果推断的准确性与可靠性。
...
TL;DR: 提出了一种名为ZERA的新框架，通过系统化、低开销的方式联合优化系统和用户提示，显著提升大模型在多种任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 现有自动提示优化方法通常只关注用户提示，依赖非结构化反馈，且需要大量样本和长迭代周期，导致成本高且鲁棒性差。

Method: ZERA利用八个可泛化的评分标准及自动推断权重对提示进行打分，并基于结构化批评进行提示修订，实现少样本、短周期的快速收敛。

Result: 在五个大语言模型和九个多样化数据集上验证了ZERA的有效性，涵盖推理、摘要和代码生成任务，结果一致优于强基线方法。消融研究进一步证明各组件对提示构建的贡献。

Conclusion: ZERA通过结构化、联合优化系统与用户提示，实现了高效、稳定的提示优化，具有广泛的应用前景。
...
TL;DR: 本文研究了外部信息对具有逐步推理能力的大语言模型（LLM）推理过程的因果影响，提出了SciAux数据集来测试模型在不同信息条件下的鲁棒性。结果表明，虽然有用信息能提升准确性，但误导性信息会通过“思考模式”放大错误，导致性能急剧下降，凸显出赋予模型批判性评估信息能力的重要性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在复杂领域中的应用依赖于其推理能力，但在现实场景中常被提供可能无关或误导的外部信息，因此需要研究这些信息如何影响模型的推理过程。

Method: 构建了一个名为SciAux的新数据集（源自ScienceQA），用于系统性测试模型在面对有用、无关和误导性信息时的推理表现，并分析其逐步思考过程中的行为变化。

Result: 发现模型的“思考模式”是一把双刃剑：有用上下文可提高准确率，但误导性信息会导致性能灾难性下降，且这种错误会被推理过程进一步放大。

Conclusion: 仅仅让模型‘思考’是不够的，关键在于赋予其批判性评估所依赖信息的能力，以增强推理的鲁棒性。
...
TL;DR: 提出一种基于多智能体框架的检索增强生成方法，通过决策者和知识选择器两个轻量级代理，结合LLM-as-a-Judge的细粒度过程监督，提升RAG系统的准确性与可解释性。


<details>
  <summary>Details</summary>
Motivation: 传统RAG中检索器与生成器独立运行，导致交互不充分、检索结果冗余或无关，生成器难以充分利用证据，因此需要更紧密的协同机制。

Method: 构建一个过程监督的多智能体框架，引入决策代理判断是否继续检索或生成答案，知识选择代理筛选最相关文档；使用LLM作为评判器对每一步操作提供过程奖励，并采用树形展开策略与PPO算法进行端到端训练。

Result: 在单跳和多跳问答任务上优于标准RAG基线，具有更高的准确率、更稳定的收敛性和更强的推理可解释性。

Conclusion: 该框架实现了检索与生成的高效协同，无需修改原有组件，具备模块化和即插即用特性，适合实际应用部署。
...
TL;DR: 提出了一种新的多模态对话情绪识别与预测架构ERFC，用于提升客服场景中的客户体验。


<details>
  <summary>Details</summary>
Motivation: 在客服等场景中，准确预测对话中未来语句的情绪有助于改善客户情绪和体验。

Method: 提出ERFC架构，融合多模态信息、情绪属性、上下文及说话人间的语句依赖关系进行情绪识别与预测。

Result: 在IEMOCAP数据集上的实验验证了ERFC的有效性和可行性。

Conclusion: ERFC能有效预测对话中的未来情绪，具有在客服等重视客户满意度的应用中带来显著商业价值的潜力。
...
TL;DR: 本研究评估了八种开源大语言模型在检测反犹太主义内容方面的能力，提出了一种新的引导式思维链（Guided-CoT）提示方法，显著提升了模型性能，并分析了模型推理中的语义偏差与异常行为。


<details>
  <summary>Details</summary>
Motivation: 自动化检测仇恨内容具有重要意义，但需应对社交媒体内容的快速演变。现有模型依赖持续训练，而利用大语言模型结合上下文策略定义进行即时适应是一种有前景的替代方案。

Method: 采用八种开源大语言模型，通过多种提示技术（包括新设计的Guided-CoT）进行 antisemitism 检测任务，使用上下文中的政策定义作为指导，并评估不同解码配置、模型规模和推理能力下的表现。同时引入新指标量化模型生成理由中的语义偏离。

Result: Guided-CoT 显著提升所有模型的性能，且不受模型大小或解码方式影响；Llama 3.1 70B 超过微调版 GPT-3.5；发现模型间存在显著的可解释性和可靠性差异，并观察到一些悖论式行为。

Conclusion: Guided-CoT 是一种有效且通用的提示方法，能增强大语言模型在仇恨内容检测中的表现；不同模型在语义一致性和推理可靠性上存在差异，未来需更关注可解释性与稳定性评估。
...
### [132] [Are Smaller Open-Weight LLMs Closing the Gap to Proprietary Models for Biomedical Question Answering?](https://arxiv.org/abs/2509.18843)
*Damian Stachura,Joanna Konieczna,Artur Nowak*

Main category: cs.CL

TL;DR: 开源权重的大型语言模型（如DeepSeek-V3）在生物医学问答任务中表现优异，通过检索增强、上下文学习和集成方法，小规模开源模型在某些情况下可超越GPT-4和Claude等闭源模型。


<details>
  <summary>Details</summary>
Motivation: 探究小型开源权重语言模型是否能在生物医学问答任务中有效替代大型闭源模型。

Method: 采用多种技术提升问答性能，包括基于嵌入距离检索相关文本片段、上下文学习、结构化输出，并对部分提交使用集成方法融合多个模型的输出。

Result: 开源权重模型表现与闭源模型相当，在某些情况下甚至更优，尤其是在应用集成策略时。

Conclusion: 开源权重语言模型在生物医学问答任务中具有竞争力，集成方法可进一步提升其性能，具备替代闭源模型的潜力。
...
TL;DR: 本文提出了一种名为Prefix-to-Tree (P2T) 的方法，用于改进大语言模型在推理任务中的强化学习训练。基于P2T，作者提出了TEMPO算法，该算法无需批评网络即可实现精确的token级信用分配，在数学和医学问答等可验证奖励的任务中优于PPO和GRPO。


<details>
  <summary>Details</summary>
Motivation: 强化学习在提升大语言模型推理能力方面具有潜力，但由于长序列上的稀疏延迟奖励，token级别的信用分配成为瓶颈。现有方法如PPO复杂且易过拟合，GRPO则忽略分支结构并平均分配奖励，因此需要更优的解决方案。

Method: 提出Prefix-to-Tree (P2T) 方法，将多个响应构造成前缀树，并通过聚合后代结果计算非参数化的前缀值V(s)；在此基础上设计TEMPO算法，利用树结构引入分支门控的时间差分修正，在分支token处提供精确的信用分配，而无需学习价值网络或额外标注。

Result: 在Qwen3-1.7B/4B模型上，TEMPO在MATH、MedQA等分布内任务以及GSM-HARD、AMC23、MedMCQA、MMLU-Medical等分布外任务上均优于PPO和GRPO，达到更高的验证准确率，且训练时间相当。

Conclusion: TEMPO是一种简单有效的无批评器强化学习算法，通过前缀树结构实现了精确的token级信用分配，在可验证奖励的推理任务中表现出优越性能，兼具高效性与泛化能力。
...
TL;DR: 本文探索了将大语言模型（LLM）作为知识图谱（KG）推理路径的奖励模型，以提升其在医疗诊断中的可靠性和可解释性。实验评估了不同任务设定和训练范式，发现尽管特定的奖励优化方法在路径判断上表现良好，但向下游任务的迁移能力仍然有限。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在诊断推理中缺乏可靠且基于知识的推断能力。知识图谱（如UMLS）提供了结构化的医学知识，但现有方法多通过检索增强或微调将其内容插入提示词中，未能实现结构化推理。因此，需要一种新的范式来提升LLM在临床推理中的可信度和有效性。

Method: 提出将LLM视为KG推理路径的奖励模型，训练其判断给定患者输入下某条推理路径是否能导向正确诊断。系统评估了五种任务设定和八种训练范式，并测试该能力在诊断总结和医学问答等下游任务中的泛化性能。使用三个开源的指令调优LLM进行实验。

Result: 特定的奖励优化和蒸馏方法在知识路径判断任务上表现出色，表明LLM可以有效学习对推理路径的评估能力。然而，这种能力在迁移到下游诊断任务时表现较弱，显示出当前方法在泛化上的局限性。

Conclusion: 这是首次系统评估“奖励模型风格”在临床知识图谱上的推理方法。研究揭示了基于结构化奖励监督对生成式AI医疗系统的潜力与挑战，为未来提升诊断推理的可靠性提供了新方向。
...
### [135] [Pathways of Thoughts: Multi-Directional Thinking for Long-form Personalized Question Answering](https://arxiv.org/abs/2509.19094)
...
TL;DR: 提出了一种名为“思维路径”（Pathways of Thoughts, PoT）的推理阶段方法，用于在不进行任务特定微调的情况下实现个性化问答系统的优化，通过建模大语言模型的迭代决策过程，动态选择推理、修订、个性化和澄清等认知操作，生成多样化候选响应并根据用户偏好加权聚合，最终提升个性化问答性能。


<details>
  <summary>Details</summary>
Motivation: 个性化问答系统面临从长且嘈杂的上下文中推断用户偏好的挑战，同时需生成准确、合适且符合用户背景知识的回答，现有方法在此方面探索不足。

Method: 将大语言模型的推理建模为包含推理、修订、个人化和澄清等认知操作的迭代决策过程，动态探索多条推理路径生成多样化的候选答案，并基于推断出的用户偏好对这些候选进行聚合与重加权，以生成最终个性化回答。

Result: 在LaMP-QA个性化问答基准上的实验表明，PoT持续优于多个强基线方法，相对性能提升最高达13.1%；人工评估显示，66%的情况下标注者更偏好PoT生成的结果，仅15%的情况为平局。

Conclusion: PoT是一种无需微调、适用于任何大语言模型的通用个性化推理框架，能够通过多路径推理和用户偏好建模显著提升个性化问答的效果和用户体验。
...
TL;DR: 本文提出了一种名为SubSpec的即插即用方法，用于加速参数卸载，实现无损且无需训练的大语言模型推理。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在内存受限设备上部署困难，现有压缩和卸载方法存在质量下降或推理速度慢的问题。

Method: SubSpec通过从卸载的目标LLM部分生成低比特量化的替代层来构建高度对齐的草稿模型，并共享GPU驻留层和KV-Cache以减少内存开销并增强对齐。

Result: SubSpec在Qwen2.5 7B上实现了9.1倍的加速，在Qwen2.5 32B上平均实现了12.5倍的加速。

Conclusion: SubSpec是一种有效的、无需训练的加速参数卸载方法，能够在保持模型质量的同时显著提升推理速度。
...
TL;DR: 提出了一种名为Speech Vecalign的并行语音文档对齐方法，无需文本转录即可实现语音段嵌入的单调对齐，在对齐长度和鲁棒性上优于现有方法，并在更少数据下达到或超过了SpeechMatrix模型的性能。


<details>
  <summary>Details</summary>
Motivation: 为了在没有文本转录的情况下实现高质量的平行语音文档对齐，并提升对齐结果的长度与鲁棒性，以支持低资源条件下的语音到语音翻译。

Method: 提出Speech Vecalign方法，通过单调对齐语音段嵌入来实现语音-语音对齐，不依赖文本转录，并在VoxPopuli的3000小时英德语音数据上进行应用和验证。

Result: 在3000小时未标注的英德语音数据上获得了约1000小时高质量对齐；相比Global Mining方法，在En-to-De和De-to-En翻译任务上分别提升了0.37和0.18 ASR-BLEU；且仅用1/8的数据量即达到或超过SpeechMatrix模型性能。

Conclusion: Speech Vecalign是一种高效、鲁棒的无监督语音对齐方法，能够在较少数据下生成高质量对齐结果，显著提升语音到语音翻译性能。
...
TL;DR: 提出一种基于LLM辅助的实时语音说话人日志校正系统，结合用户口头反馈，在流式ASR和日志基础上提升准确性。


<details>
  <summary>Details</summary>
Motivation: 传统语音处理系统缺乏用户反馈机制，难以纠正说话人归属错误，影响准确性。引入人类参与的闭环系统有望提升性能。

Method: 系统结合流式ASR与说话人日志，利用LLM生成摘要供用户快速理解，并接收用户的简短语音反馈进行实时修正；采用SWM技术拆分被误合并的多说话人段落，并通过在线说话人注册减少未来错误。

Result: 在AMI测试集上的LLM仿真显示，系统使说话人错误率（DER）降低9.92%，说话人混淆错误减少44.23%；并验证了不同设置下校正效果的变化。

Conclusion: 该LLM辅助的交互式校正框架能有效提升说话人日志准确性，具备实际应用潜力。
...
TL;DR: 本文提出了NormGenesis，一个跨文化的对话生成与标注框架，通过引入违反-解决（V2R）对话类型来建模社会规范的动态交互，并结合示例驱动的迭代优化方法提升多语言对话的语用一致性。


<details>
  <summary>Details</summary>
Motivation: 为了使对话系统不仅能生成连贯回应，还能符合不同文化背景下的社会规范，需构建能够捕捉社会规范动态演变的多语言、多文化对话数据。

Method: 提出Violation-to-Resolution (V2R) 对话类型，模拟规范违反后的识别与修复过程；采用基于示例的迭代精炼方法，在对话合成早期对低资源语言进行优化；在英、中、韩三种语言中构建包含10,800个多轮对话的数据集，并进行细粒度标注。

Result: 人类和大模型评估表明，NormGenesis在对话自然性、精炼质量及泛化能力上优于现有数据集；使用V2R增强数据训练的模型在伦理敏感场景中展现出更强的语用理解与应对能力。

Conclusion: NormGenesis为文化自适应对话建模建立了新基准，提供了一种可扩展的、面向多元语言与文化的规范感知对话生成方法。
...
TL;DR: 本文评估了大语言模型（LLM）生成富含文化表达的波斯文学文本的能力，构建了一个涵盖20个不同主题的用户生成波斯文学数据集，并采用改编的托兰斯创造性思维测试从原创性、流畅性、灵活性和 elaboration 四个维度评估模型输出。通过使用大语言模型作为自动评分裁判，并验证其与人类评分的一致性，研究还分析了模型对明喻、隐喻、夸张和对照四种核心文学手法的理解与运用能力，揭示了LLM在波斯文学生成中的优势与局限。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要集中于英语文学生成，缺乏对非英语文学传统（如波斯语）中LLM创造力的系统评估，且缺少标准化的创造力评价方法。

Method: 构建包含20个主题的用户生成波斯文学数据集；基于托兰斯创造性思维测试设计四维创造力评估框架（原創性、流暢性、靈活性、精緻性）；采用大语言模型作为自动评分器，并以组内相关系数验证其与人类评分的一致性；分析模型对四种文学修辞手法（明喻、隐喻、夸张、对照）的掌握情况。

Result: LLM作为评分器与人类判断表现出强一致性，验证了自动化评估的可靠性；模型在生成具有文化相关性的波斯文学文本方面展现一定能力，但在运用特定文学修辞手法上存在局限。

Conclusion: 大语言模型具备生成创造性波斯文学文本的潜力，但需进一步优化以更好理解和运用文化特定表达及文学修辞；本研究为跨文化文学生成提供了可复现的评估框架。
...
TL;DR: 本研究提出了一种基于语言模型和对话对齐（CA）分数的自动化方法，用于大规模测量医患对话中的共享决策（SDM），并通过深度学习和微调BERT模型验证了其与SDM结果的相关性。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏可大规模自动测量共享决策（SDM）的方法，因此需要开发一种可扩展、自动化的工具来评估患者-医生对话中的SDM水平。

Method: 使用157段患者-医生对话的转录文本，共42,559句话，通过上下文-响应对和负采样训练深度学习模型和微调BERT模型（NSP任务），计算四种CA分数，并采用随机效应模型分析CA分数与SDM指标（DCS和OPTION12）的关系。

Result: 微调后的BERTbase模型在recall@1上表现最佳（0.640）。使用无stylebook策略的DL模型生成的AbsMax和Max CA分数与OPTION12显著相关；BERTbase生成的Max CA分数与DCS显著相关。模型规模不影响CA分数与SDM的关联。

Conclusion: 该研究成功开发了一种可解释、可扩展的自动化方法，利用CA分数量化SDM，具有在大规模场景下评估SDM干预措施的潜力。
...
TL;DR: CogniLoad是一个基于认知负荷理论的新型合成基准，用于精确分析大语言模型在长上下文推理中的表现，通过独立调节任务难度、干扰比和任务长度，揭示了任务长度是主要限制因素，并发现了模型对内在复杂性和干扰比的不同容忍度。


<details>
  <summary>Details</summary>
Motivation: 现有长上下文推理基准混淆了任务复杂性、干扰干扰和任务长度等关键因素，难以进行精确的失败分析。

Method: 基于认知负荷理论（CLT）设计CogniLoad，生成具有可调参数的自然语言逻辑谜题，分别控制内在负荷（d）、外在负荷（ρ）和相关负荷（N）。

Result: 在22个最先进的推理LLM上评估发现，任务长度是性能的主要制约因素，不同模型对内在复杂性敏感度不同，且对干扰比呈现U型响应。

Conclusion: CogniLoad通过系统性、因子化控制认知负荷维度，提供了一个可复现、可扩展且诊断性强的工具，有助于剖析LLM推理局限并指导未来模型开发。
...
TL;DR: LAWCAT是一种线性注意力框架，通过从预训练Transformer中迁移能力，实现高效、长上下文的模型部署，显著减少对长序列训练数据和计算资源的依赖。


<details>
  <summary>Details</summary>
Motivation: Transformer的二次计算复杂度在长序列场景下成为瓶颈，而现有的线性注意力模型训练成本仍高，因此需要一种能高效利用已有预训练模型的方法。

Method: 提出LAWCAT框架，结合因果Conv1D层增强局部依赖建模，并采用归一化门控线性注意力提升不同上下文长度下的泛化能力，通过蒸馏预训练模型实现高效迁移。

Result: 在仅使用1K长度序列蒸馏Mistral-7B时，LAWCAT在22K token下仍保持超过90%的passkey检索准确率；Llama3.2-1B的LAWCAT版本在多个长文本任务上表现优异，且预训练token消耗不足0.1%。此外，在超过8K序列时prefill速度优于FlashAttention-2。

Conclusion: LAWCAT为构建高性能、长上下文的线性注意力模型提供了高效路径，适合边缘部署，大幅降低资源需求。
...
TL;DR: 本文对基于大语言模型（LLM）的图推理方法在文本型应用中的能力进行了系统性评估，涵盖多种交互模式、数据集领域、图结构特性等变量，并揭示了不同方法在图数据上的表现与依赖关系。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型被广泛应用于文本丰富的图学习任务，但其与图数据交互的能力缺乏系统理解，本文旨在填补这一空白。

Method: 通过大规模、受控的实验评估，比较了提示（prompting）、工具使用和代码生成三种LLM-图交互模式，在多个数据集领域、结构类型（同质/异质图）、特征类型（短/长文本）及模型配置下进行分析，并通过特征截断、边删除和标签移除来量化对不同类型输入的依赖。

Result: 1) 作为代码生成器的LLM整体性能最强，尤其在长文本或高度图上优势明显；2) 所有交互策略在异质图上均有效，挑战了LLM方法在低同质性下失效的假设；3) 代码生成能灵活调整对结构、特征或标签的依赖，利用最有信息量的输入。

Conclusion: 研究全面揭示了当前LLM-图交互模式的优势与局限，为未来方法设计提供了实用且可操作的指导原则。
...
TL;DR: 提出一种基于ByT5模型的方法，通过字节级多语言Transformer和规则驱动的音步转换，在保持语义连贯性的同时，实现阿拉伯诗歌节奏的自动对齐。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯古典诗歌严格遵循特定节奏模式，但人工创作耗时且需深厚语言功底，因此需要自动化方法辅助创作。

Method: 采用ByT5模型，结合基于规则的图素到音步（grapheme-to-beat）转换方法提取完全带音标的阿拉伯语文本节奏；使用条件去噪目标进行微调，使模型重建被掩码的词语以匹配目标节奏；采用课程学习策略，先在通用阿拉伯语数据集上预训练，再在诗歌数据集上微调，并探索从英语到阿拉伯语的跨语言迁移。

Result: 实验结果表明，该模型在多种阿拉伯诗律上实现了高节奏对齐率，同时保持了生成短语的语义连贯性，尤其在课程学习和跨语言迁移设置下表现更优。

Conclusion: 所提出的方法能有效生成符合特定节奏的阿拉伯诗句片段，具备应用于辅助创作古典阿拉伯诗歌的共创意工具的潜力。
...
TL;DR: 提出一种基于文本内部结构的轻量级框架，用于检测原始和经过简单改写的AI生成文本，通过注意力机制和对比学习提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有词级别检测器易受改写和提示攻击，存在模型偏差且对修改后的文本性能下降，需要更鲁棒的检测方法。

Method: 利用预训练语言模型提取句子嵌入，通过注意力机制建模其关系，采用对比学习减少生成偏差，并引入因果图与反事实方法分离结构特征与主题偏差。

Result: 在两个自建数据集（摘要对比和修订生活FAQ）上验证了方法的有效性，对原始和改写文本均表现出良好检测性能。

Conclusion: 基于文本结构的检测方法具有更强的鲁棒性和泛化能力，能有效应对词级别变化和主题偏差，适用于轻量级部署。
...
TL;DR: 本文提出了一种名为CCQA的新型推理方法，基于循环一致性，在小语言模型（SLMs）上实现了优于现有最先进方法的表现，尤其在数学和常识推理任务中表现突出。


<details>
  <summary>Details</summary>
Motivation: 现有的推理时策略在大语言模型上有效，但在小语言模型上效果不佳，因此需要一种适用于小模型的有效推理方法。

Method: 提出CCQA方法，通过从每个推理路径和答案生成问题，利用与原始问题的相似度进行评估，并选择相似度最高的候选作为最终答案；使用轻量级Flan-T5模型辅助问题生成。

Result: 实验结果显示，CCQA在八个模型上的数学和常识推理基准测试中 consistently 超过现有SOTA方法，显著提升小模型的推理性能。

Conclusion: CCQA为小语言模型提供了一种高效且实用的推理新基线。
...
TL;DR: 提出一种基于词频统计的先验数据过滤方法，作为困惑度过滤的高效替代方案，在20个下游任务上表现优异且速度提升超1000倍。


<details>
  <summary>Details</summary>
Motivation: 困惑度（PPL）-based数据过滤虽有效，但计算成本高且在噪声或分布外样本上不可靠，需更高效稳定的方法。

Method: 利用语料库级别的词频统计估计token先验概率，通过token先验的均值和标准差进行文档过滤，无需模型推理。

Result: 该方法在20个下游基准测试中取得最高平均性能，时间成本降低1000倍以上，并适用于代码、数学等符号语言及多语言语料。

Conclusion: 基于先验的过滤方法是一种简单、快速、有效的数据筛选方案，可广泛应用于大规模语言模型的数据预处理。
...
TL;DR: 提出TsqLoRA，一种结合数据质量感知采样与敏感度感知低秩适应的高效微调方法，在减少计算开销的同时保持或提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有参数高效微调方法忽视了不同模型层的敏感性差异及训练数据的重要性，导致效率和性能受限。

Method: TsqLoRA包含两个核心组件：基于数据质量的选择机制，筛选最具信息量的训练样本；动态秩分配模块，根据各层对参数更新的敏感度调整其低秩适配的秩大小。

Result: 实验结果表明，TsqLoRA在多种NLP任务上优于现有参数高效微调方法，提升了微调效率和模型性能。

Conclusion: TsqLoRA通过联合优化数据选择与层敏感度感知的低秩适配，有效平衡了微调的效率与效果，适用于资源受限场景。
...
TL;DR: 本文提出了UniECG，首个能够同时进行基于证据的心电图（ECG）解释和文本条件心电图生成的统一模型，通过解耦的两阶段训练方法实现ECG-to-Text和Text-to-ECG功能。


<details>
  <summary>Details</summary>
Motivation: 现有的统一模型如GPT-5在处理心电信号时无法准确理解或生成ECG信号，也无法提供可靠的医学诊断，因此需要专门针对ECG任务设计统一模型。

Method: 采用解耦的两阶段训练策略：第一阶段训练模型掌握基于证据的ECG解释能力（ECG-to-Text），第二阶段通过潜在空间对齐注入文本到心电图的生成能力（Text-to-ECG）。

Result: UniECG能够根据用户输入自主选择解释或生成心电图，在多个ECG相关任务上表现出强大能力，显著扩展了当前ECG模型的功能边界。

Conclusion: UniECG是首个支持双向ECG-Text任务的统一模型，为未来智能心脏疾病诊断和个性化ECG数据生成提供了新思路和技术基础。
...
TL;DR: 本研究通过Planorama平台发现，用户偏好和模型对计划的评估并不能准确预测哪些计划真正有助于用户完成复杂任务，表面特征（如简洁性）影响偏好但不决定实际帮助性，因此提出需要基于真实用户交互反馈来对齐大语言模型。


<details>
  <summary>Details</summary>
Motivation: 现有对齐方法（如RLHF和Chatbot Arena）依赖用户偏好作为帮助性的代理指标，但假设未被验证。本文旨在检验用户偏好是否真正反映计划的实际帮助性，并揭示当前对齐方式的潜在偏差。

Method: 设计Planorama实验平台，让126名用户在300个多步骤问题中执行LLM生成的计划，收集4388次执行和5584次比较数据；分析用户成功与否与计划偏好的关系，并训练代理和奖励模型模拟人类偏好与表现。

Result: 1) 用户偏好、模型评分和代理成功率均不能准确预测计划的实际帮助性；2) 用户使用自己偏好或不偏好的计划时成功率相似，说明偏好非个性化有效因素；3) 简洁性和问题相似性等表面特征强烈影响偏好，但无法预测实际帮助性。

Conclusion: 仅依赖用户偏好进行LLM对齐可能导致与实际帮助性的错位，应转向基于真实用户交互结果的反馈机制，以构建真正有用的规划助手。
...
TL;DR: 本文提出了CAPE-KG，一种一致性感知的参数保持型知识编辑框架，用于多跳问答任务，通过确保知识图谱构建、更新和检索的一致性，有效提升了编辑后的推理准确性。


<details>
  <summary>Details</summary>
Motivation: 现有基于知识图谱的参数保持型知识编辑方法在多跳问答中常出现不一致问题，导致知识污染和推理不稳定，影响可靠性。

Method: 提出CAPE-KG框架，在知识图谱的构建、更新和检索过程中引入一致性机制，确保各阶段与多跳问答任务需求保持一致。

Result: 在MQuAKE基准上的实验表明，CAPE-KG显著提升了多跳问答中参数保持型知识编辑的准确性。

Conclusion: 通过显式建模一致性，CAPE-KG有效增强了PPKE在多跳推理场景下的可靠性和性能。
...
TL;DR: 提出了一种基于共形预测的框架，用于分析LLM作为评估工具时的不确定性，提供具有覆盖率保证的预测区间，并引入中点评分作为低偏差替代方案。


<details>
  <summary>Details</summary>
Motivation: LLM-as-a-judge在自然语言生成评估中广泛应用，但其评估结果的不确定性尚未充分探索，影响可靠性与实际部署。

Method: 采用共形预测构建连续预测区间，并设计了适用于离散评分任务的序数边界调整方法，同时提出以区间中点作为低偏差评分。

Result: 实验表明该方法能提供有效的预测区间，具备覆盖率保证；中点评分和重提示策略有助于提升评估质量。

Conclusion: 所提框架有效量化了LLM评估中的不确定性，提升了评估的可靠性和可解释性，为实际应用提供了更稳健的评判机制。
...
TL;DR: MemOrb是一种轻量级、即插即用的记忆增强模块，通过结构化反思提升大语言模型代理在客户服务中的长期可靠性和一致性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理在跨会话中容易遗忘、重复错误且缺乏持续自我改进机制，导致在动态环境中稳定性不足。

Method: 提出MemOrb，一个无需微调的言语强化记忆层，将多轮交互提炼为策略性反思并存储于共享记忆库中，用于指导后续决策。

Result: 实验显示，MemOrb显著提升了任务成功率和一致性表现，多轮任务成功率最高提升63个百分点，并在多次试验中表现出更稳定的性能。

Conclusion: 结构化反思是增强冻结LLM代理在客户服务场景下长期可靠性与一致性的有效机制。
...
TL;DR: LOTUSDIS是一个公开的泰国语会议语料库，包含114小时的真实远场对话，用于推动远场语音识别研究。该数据集通过多种麦克风设备在不同距离下录制，包含自然重叠语音和混响噪声，提供了标准划分和可复现的基线系统。实验表明，使用该数据集微调显著提升了Whisper模型在远场场景下的鲁棒性，大幅降低词错误率。


<details>
  <summary>Details</summary>
Motivation: 现有的预训练ASR模型在处理泰国语远场语音时性能显著下降，主要由于训练数据与真实远场条件（如距离、混响、噪声）不匹配。因此，需要一个具有距离多样性的高质量真实对话语料库来提升远场ASR系统的鲁棒性。

Method: 收集了114小时的自发性三人会议对话，每次持续15-20分钟，使用九个独立单声道设备（六种麦克风类型）在0.12米到10米的不同距离同时录音，保留真实的声学特性。提供标准数据划分，并构建基于Whisper的基线系统，在零样本和微调条件下进行评估。

Result: 基准测试显示，现成模型在远距离录音上性能严重下降，验证了预训练数据与远场语音之间的不匹配。在LOTUSDIS上微调后，整体WER从64.3降至38.3，远场WER从81.6降至49.5，尤其在最远麦克风上增益最大。

Conclusion: 多样化的录音距离对于构建鲁棒的远场ASR系统至关重要。LOTUSDIS为泰国语远场语音识别提供了宝贵的资源，有助于推动该领域的可复现研究。
...
TL;DR: 本文提出了一种名为DyGRASP的新方法，用于处理动态文本属性图（DyTAGs），结合大语言模型和时序图神经网络，有效捕捉节点的近期与全局语义动态，并在多个基准任务上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注静态文本属性图，难以有效建模动态图中的近期与全局语义演化，且在处理大量时序文本时效率低下。

Method: 设计了以节点为中心的隐式推理与滑动窗口机制捕捉近期语义；通过定制提示和类RNN链结构进行显式推理以捕获全局语义；并通过更新与融合层整合时序语义与图结构信息。

Result: 在DyTAG基准数据集上实验表明，DyGRASP在目标节点检索任务中Hit@10指标最高提升34%，并展现出对不同时序GNN和大语言模型的良好泛化能力。

Conclusion: DyGRASP有效解决了动态文本属性图中近期-全局语义建模的挑战，实现了高效且准确的图推理。
...
TL;DR: 研究探讨了多语言子词分词器中词汇重叠对跨语言迁移的影响，发现词汇重叠有助于建立跨语言语义关系，并提升在XNLI和XQuAD等任务上的迁移性能。


<details>
  <summary>Details</summary>
Motivation: 解决先前研究中关于词汇重叠是否促进跨语言迁移的争议，控制词汇频率和分词粒度等混杂因素。

Method: 通过系统性地调整双语自回归模型的词汇重叠程度，在多个语言对上进行训练，并分析隐藏表示中的语义相似性。

Result: 具有词汇重叠的模型在捕捉跨语言语义关系方面表现更强，并且在XNLI和XQuAD任务上优于无重叠模型，性能随重叠增加而提升。

Conclusion: 词汇重叠有利于跨语言迁移，共享词汇仍是多语言分词器的有效设计选择。
...
TL;DR: 长上下文监督微调（SFT）出乎意料地提升大语言模型在短上下文任务上的表现，研究揭示了注意力机制与前馈网络的独立增益及知识偏好偏差，混合训练可有效缓解该偏差。


<details>
  <summary>Details</summary>
Motivation: 尽管长上下文预训练常导致短任务性能下降，但长上下文SFT对短任务的影响尚不明确，本文旨在系统探究SFT数据长度对模型行为的影响。

Method: 通过解耦多头注意力（MHA）和前馈网络（FFN）分析其各自在长上下文SFT中的作用，并研究二者交互中表现出的知识偏好偏差，提出并验证混合训练策略。

Result: 发现长上下文SFT能提升短上下文任务性能；MHA和FFN均从中受益；识别出长上下文SFT偏向上下文知识、短上下文SFT偏向参数化知识的偏差；混合训练可缓解此偏差。

Conclusion: 长上下文SFT对短上下文任务有益，且存在知识偏好机制，推荐采用混合长度数据进行SFT以优化大模型微调效果。
...
TL;DR: 提出一种基于10-K文件的无监督方法，利用自然语言处理技术提取企业间风险关系，通过时间与词汇模式微调模型，生成可解释的风险关联分数。


<details>
  <summary>Details</summary>
Motivation: 传统依赖专家判断的手动分析方法主观性强、耗时且难以扩展，无法有效识别企业间复杂的风险传导关系。

Method: 利用10-K文件中的时间与词汇模式，采用无监督微调方法训练领域特定的金融编码器，捕捉隐含和抽象的风险关联，并引入量化风险关系评分机制。

Result: 实验表明，该方法在多个评估场景下均优于强基线模型，能够更准确地识别企业间风险关系。

Conclusion: 所提方法能有效从文本中提取可解释的企业间风险关联，为投资组合管理和投资策略提供可靠的量化支持。
...
TL;DR: 本文提出了AECBench，一个面向建筑、工程和施工（AEC）领域的大型语言模型（LLM）综合评估基准，涵盖五个认知层次的23项任务，并构建了由工程师编制、专家评审的4800题数据集，结合“以LLM为裁判”的评估方法，评估结果显示现有LLM在知识记忆和理解层面表现良好，但在推理、计算和专业文档生成等高阶任务上性能显著下降，表明其在安全关键型工程应用中仍存在可靠性问题。


<details>
  <summary>Details</summary>
Motivation: 由于AEC领域具有高度专业化和安全性要求，而当前LLMs在此类场景下的鲁棒性和可靠性尚未得到系统评估，因此需要建立专门的基准来衡量其实际能力与局限。

Method: 提出AECBench评估框架，包含五个认知层级（知识记忆、理解、推理、计算、应用）共23项源自真实AEC实践的任务；构建4800道题目的多格式数据集，主要由工程师编写并经两轮专家评审；引入基于专家评分标准的LLM-as-a-Judge方法，用于自动评估长文本输出。

Result: 对九个主流LLM的评测显示，模型在知识记忆和理解任务上表现较好，但在表格信息解读、复杂推理与计算、以及专业文档生成等高级认知任务上性能明显下降，暴露出其在AEC领域应用中的关键缺陷。

Conclusion: 现有LLMs在AEC这类安全关键领域的高阶认知任务中仍不够可靠，AECBench为未来提升LLMs在工程领域的稳健性和实用性提供了评估基础和研究方向。
...
TL;DR: 本研究使用模型差异分析方法，比较了Gemma-2-9b-it与SimPO增强版本之间的能力差异，发现SimPO在安全性、多语言能力和指令遵循方面显著提升，同时减少了自我引用和幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 随着微调成为提升大语言模型性能的主要范式，传统基准测试难以解释模型间性能差异的原因，因此需要更细粒度的分析方法来理解微调过程中模型的变化。

Method: 采用模型差异分析（model diffing）和crosscoders技术，识别并分类两个模型之间的潜在表征差异。

Result: SimPO增强了安全性（+32.8%）、多语言能力（+43.8%）和指令遵循（+151.7%），同时减少了自我引用（-44.1%）和幻觉管理（-68.5%）的倾向。

Conclusion: 模型差异分析能提供超越排行榜指标的细粒度洞察，将性能差距归因于具体的机制性能力，为比较大语言模型提供了透明且有针对性的框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于提示的无监督方法通常采用单阶段推理和统一提示，难以充分发挥大语言模型在复杂关键短语提取任务中的潜力，尤其在处理不同长度文本时表现受限。

Method: 提出MAPEX框架，结合基于大语言模型的多智能体协作，包含专家招募、候选提取、主题引导、知识增强和后处理模块，并采用双路径策略：短文本使用知识驱动提取，长文本使用主题引导提取。

Result: 在六个基准数据集和三种不同大语言模型上实验表明，MAPEX在F1@5指标上平均比最先进的无监督方法提升2.44%，比标准大语言模型基线提升4.01%。

Conclusion: MAPEX通过多智能体协作和自适应策略有效提升了关键短语提取的性能，展现出良好的通用性和泛化能力。
...
TL;DR: 本文研究了多特征融合在AI文本检测中的效果，提出了一种名为MHFD的多层级特征检测方法，结合语义、句法和统计特征。实验表明，尽管理论上多特征应有优势，但实际上性能提升有限（仅0.4-2.6%），计算开销却显著增加（4.2倍），说明现代神经模型已能高效捕捉检测信号。


<details>
  <summary>Details</summary>
Motivation: 探究多特征方法是否能在现代大语言模型生成文本的背景下，显著提升AI文本检测性能，并验证其计算代价是否值得。

Method: 提出MHFD框架，融合DeBERTa语义分析、句法解析和统计概率特征，通过自适应融合策略实现多层级特征整合。

Result: 在多个基准数据集上，MHFD在领域内检测准确率达89.7%，跨领域保持84.2%，相比现有方法提升0.4-2.6%，但计算开销增加4.2倍。

Conclusion: 多特征融合对现代AI文本检测带来的性能增益有限，且代价高昂，表明当前神经模型已能有效捕获大多数检测所需信号，无需复杂特征工程。
...
TL;DR: 提出DivEye，一种基于意外性的可解释AI生成文本检测框架，通过捕捉文本中不可预测性的波动，在多个基准上优于现有零样本检测器，并提供关于为何标记文本的可解释性见解。


<details>
  <summary>Details</summary>
Motivation: 现有的AI生成文本检测方法依赖于词元级别的似然性或不透明的黑箱分类器，难以应对高质量生成内容且缺乏可解释性。

Method: DivEye利用基于意外性（surprisal）的统计特征，捕捉人类文本与LLM生成文本在词汇和结构不可预测性上的差异，提取可解释的特征用于检测。

Result: DivEye在多个基准上比现有零样本检测器性能最高提升33.2%，作为辅助信号可将现有检测器性能提升18.7%，且对改写和对抗攻击具有鲁棒性，跨模型和领域泛化能力强。

Conclusion: DivEye不仅在检测性能上优于现有方法，还提供了关于检测决策的可解释性，揭示了节奏性不可预测性是检测LLM生成文本的一个强有力且未被充分探索的信号。
...
TL;DR: JEDI是一种基于编码器的架构，无需生成模型即可联合执行抽取式原子事实分解和可解释推理，在自然语言推断任务中表现出良好的准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖资源消耗大的生成式大语言模型进行原子事实分解，限制了效率和可扩展性。

Method: 提出JEDI，一种仅使用编码器的架构，并构建大规模合成理由语料库用于训练，实现抽取式分解与推理的联合学习。

Result: 实验表明，JEDI在分布内任务上达到竞争性准确性，在分布外和对抗场景下显著提升鲁棒性，优于仅依赖抽取式理由监督的模型。

Conclusion: 仅使用编码器架构和合成理由即可实现NLI中的可解释性和鲁棒泛化能力。
...
TL;DR: 提出使用动态时间规整（DTW）对齐语音和文本嵌入，有效缩小端到端语音翻译中的模态差距，提升对齐精度且在低资源场景下表现更优。


<details>
  <summary>Details</summary>
Motivation: 语音与文本模态间的表示差异导致模态鸿沟问题，现有对齐方法受限于语言覆盖和对齐准确性。

Method: 采用动态时间规整（DTW）在训练过程中对齐语音和文本嵌入，无需依赖外部对齐工具。

Result: 相比先前方法，实现了更精确的对齐，训练速度更快，在6个语言方向中5个低资源设置下表现更优，端到端语音翻译性能相当。

Conclusion: DTW是一种高效、准确且适用于低资源语言的语音-文本对齐方法，有助于缩小模态差距。
...
TL;DR: 本文首次系统研究了测试时扩展（TTS）在机器翻译中的应用，发现其在高资源语言中能有效提升翻译质量，小模型配合高N值可匹敌大模型性能，但在固定计算预算下，大模型更高效，低资源情况下TTS可能因评估指标盲区导致性能下降。


<details>
  <summary>Details</summary>
Motivation: 尽管扩大模型参数是提升NLP系统性能的常用策略，但计算成本高昂。测试时扩展（TTS）提供了一种替代方案，即在推理阶段投入更多计算资源。然而，TTS在机器翻译中的效果尚未被系统研究。

Method: 采用简单的best-of-N框架，在WMT24基准上进行实验，涵盖六种高资源和一种低资源语言对、五种模型规模（3B-72B）以及不同的TTS计算预算（N最大为1024）。

Result: 实验结果表明：a) 对于高资源语言，TTS能根据多种神经机器翻译评估指标提升翻译质量，并得到人工评测验证；b) 小模型配合较大的N值可以匹配或超过单次运行的大模型性能，但计算成本更高；c) 在固定计算预算下，大模型通常更高效，而在低资源情况下，TTS可能因评估指标盲区导致质量下降。

Conclusion: TTS是一种有效的机器翻译性能提升手段，尤其适用于高资源语言场景，但在低资源情况下需谨慎使用，并注意评估指标的局限性；同时，在计算资源受限时，更大模型更具效率优势。
...
TL;DR: 本研究通过分析CLiC-it会议过去十年的论文，追踪意大利计算语言学和自然语言处理领域的研究趋势，构建了CLiC-it语料库，并对作者信息和论文内容进行了综合分析，揭示了该领域的发展动态。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的兴起，自然语言处理领域迅速发展，需要系统性地了解意大利学术界的研究演变和趋势。

Method: 收集并构建了2014至2024年十届CLiC-it会议的论文元数据和内容语料库，进行元数据分析（如作者背景、性别、机构）和主题内容分析。

Result: 揭示了意大利CL/NLP社区在研究重点、作者结构和合作模式方面的变化趋势，例如从词汇语义资源向语言建模和多模态的转变。

Conclusion: CLiC-it语料库为意大利及国际研究社区提供了有价值的洞察，有助于指导未来的研究方向和决策。
...
TL;DR: 本文通过使用NLTK库分析不同语料库中句子的重复情况，检验了语言学中“大多数语句都是全新的”这一观点，发现虽然独特句子常占多数，但重复句子在各类语料中均占有不可忽视的比例。


<details>
  <summary>Details</summary>
Motivation: 检验语言学中关于大多数语言表达都是全新组合的普遍主张，利用现代大型语料库进行实证研究。

Method: 使用NLTK Python库对多种类型的语料库进行解析，统计各语料库中完全相同句子字符串的出现次数。

Result: 结果显示，在大多数语料库中，完全独特的句子虽常占多数，但重复句子的比例因体裁而异，且在所有语料库中都占有一定比例，不可忽略。

Conclusion: 语言中新颖句子虽占主导，但重复表达在不同语体中普遍存在，挑战了‘几乎所有句子都是全新’的绝对化观点。
...
TL;DR: 本文介绍了首个用于吉尔吉斯语的命名实体识别数据集KyrgyzNER，包含1,499篇新闻文章和27类命名实体，并评估了多种模型在该数据集上的表现，发现多语言预训练模型（如多语言RoBERTa）在资源有限的语言处理中具有潜力。


<details>
  <summary>Details</summary>
Motivation: 为资源匮乏的吉尔吉斯语构建首个高质量的手动标注命名实体识别数据集，以促进其自然语言处理研究。

Method: 从24.KG新闻门户收集1,499篇新闻文章，进行人工标注，共包含39,075个实体提及，涵盖27个命名实体类别；采用条件随机场和多语言Transformer模型（如多语言RoBERTa）进行实验评估。

Result: 所有模型在稀有实体类别上均表现不佳，但多语言RoBERTa等模型在精确率和召回率之间达到了较好的平衡，其他多语言模型也表现出相近性能。

Conclusion: 多语言预训练模型在低资源语言的命名实体识别任务中具有应用潜力，未来可探索更细粒度的标注方案以进一步提升吉尔吉斯语处理能力。
...
TL;DR: 提出一种结合LLM引导的多维度编码与动态聚类的上下文感知分层分类法生成框架，显著提升了分类的连贯性、细粒度和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有自动构建科学文献分类体系的方法（如无监督聚类或直接提示大模型）往往缺乏连贯性和细粒度，难以有效组织快速增长的科研文献。

Method: 利用大语言模型识别每篇论文的关键方面（如方法、数据集、评估等），生成面向特定方面的摘要，并对每个方面进行编码和动态聚类，逐步构建出层次化的分类结构。

Result: 在包含156个专家构建分类体系、涵盖11.6k篇论文的新基准上验证了方法的有效性，实验表明该方法在分类连贯性、细粒度和可解释性方面显著优于先前方法，达到SOTA性能。

Conclusion: 所提出的上下文感知、多维度融合的分类生成框架能更有效地组织科研文献，为构建高质量科学知识体系提供了新思路。
...
TL;DR: 提出了一种名为“anecdoctoring”的新红队方法，用于跨语言和文化自动生成对抗性提示，以应对生成式AI滥用导致的虚假信息风险。


<details>
  <summary>Details</summary>
Motivation: 现有的红队评估数据集大多以美国和英语为中心，缺乏跨文化和多语言的鲁棒性，难以应对全球范围内生成式AI的滥用风险。

Method: 从三个语言（英语、西班牙语、印地语）和两个地区（美国、印度）的事实核查网站收集错误信息声明，将其聚类为广泛叙事，并用知识图谱表征这些聚类，进而增强攻击者LLM以生成对抗性提示。

Result: 该方法相比少样本提示具有更高的攻击成功率，并提供更好的可解释性，验证了其在多语言环境下的有效性。

Conclusion: 需要基于真实世界对抗性滥用、具备全球可扩展性的虚假信息缓解措施，且应涵盖多样化的语言与文化背景。
...
<details>
  <summary>Details</summary>
Motivation: 缺乏对AI生成低质量文本（即“slop”）的明确定义和衡量方法，阻碍了对其有效评估和改进。

Method: 结合自然语言处理、写作和哲学领域专家的访谈构建slop的分类体系，并通过跨度级别标注分析二元判断与潜在文本特征的关系。

Result: 提出了一个可解释的slop评估框架，发现二元slop判断虽具一定主观性，但与文本的 coherence 和 relevance 等维度存在相关性。

Conclusion: 该框架可用于检测和偏好任务中评估AI生成文本质量，有助于深入理解影响文本质量判断的语言和风格因素。
...
TL;DR: 本文首次提出了一种可扩展的强化学习方法来训练连续思维链（CoT），无需从离散思维链中蒸馏，使用“软”令牌和嵌入噪声促进探索，在数学推理任务中表现出更高的多样性，并在保持基础模型预测能力方面表现更优。


<details>
  <summary>Details</summary>
Motivation: 连续令牌相比离散令牌具有更强的表达能力和推理效率，但现有方法受限于训练困难和计算成本，难以实现长序列连续CoT的有效学习。

Method: 采用强化学习框架，引入软令牌（token混合）和输入嵌入噪声以增强探索性，直接端到端训练连续CoT，避免依赖离散CoT蒸馏。

Result: 在Llama和Qwen等8B规模模型上验证，连续CoT训练在pass@1上与离散CoT相当，在pass@32上更优，显示出更高推理路径多样性；最佳策略为训练用连续、推理用离散；且在域外任务上更好保留基础模型性能。

Conclusion: 连续CoT通过强化学习可高效训练，支持长序列推理，提升多样性并温和调整基础模型，具备实际部署潜力。
...
TL;DR: 本文提出了Online Process Reward Learning (OPRL)，一种用于代理强化学习的通用时序信用分配方法，通过隐式过程奖励模型将轨迹偏好转化为步骤级奖励，结合结果奖励实现高效稳定的策略更新。


<details>
  <summary>Details</summary>
Motivation: 稀疏且有时不可验证的奖励使得长视野代理学习中的时序信用分配极具挑战，现有过程监督方法存在标注偏差、奖励博弈或高方差等问题。

Method: OPRL交替优化隐式过程奖励模型（PRM）和策略，利用基于轨迹的DPO目标生成步骤奖励，并结合步骤级与episode级优势函数进行策略更新，无需额外rollout或显式步骤标签。

Result: 在WebShop、VisualSokoban和SOTOPIA三个基准上，OPRL优于前沿大模型和强RL基线，实现更高的样本效率、更低训练方差和更优探索能力。

Conclusion: OPRL提供了一种稳定、高效且无需精细标注的信用分配机制，理论保证其奖励一致性与梯度有界性，显著提升代理在复杂交互环境中的学习性能。
...
TL;DR: 本文提出了一种轻量级、模型无关的解码框架SafeCoDe，用于提升多模态大语言模型在上下文感知安全决策方面的能力，通过对比解码和全局感知的标记调制策略，有效平衡了过敏感与欠敏感问题。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型在处理安全决策时难以平衡对良性查询的过度拒绝（过敏感）和对视觉相关风险的漏检（欠敏感），缺乏上下文感知的安全对齐机制。

Method: 提出Safety-aware Contrastive Decoding（SafeCoDe），包含两个阶段：1）对比解码机制，通过真实图像与高斯噪声图像对比突出对视觉上下文敏感的标记；2）全局感知标记调制策略，结合场景级推理与标记级调整，动态调整生成行为以适应安全判断。

Result: 在多种MLLM架构和安全基准上的实验表明，SafeCoDe在降低过敏感和欠敏感问题的同时，提升了上下文相关的拒绝行为，并保持了模型的有用性。

Conclusion: SafeCoDe是一种有效的、即插即用的安全增强方法，能够在不牺牲帮助性的情况下提升多模态大语言模型的上下文感知安全决策能力。
...
TL;DR: 本研究比较了多种预训练注意力模型在电子健康记录（EHR）信息提取任务中的表现，使用n2c2挑战赛的CMED数据集进行药物提取、医疗事件检测和多维上下文分类。结果显示，在临床数据上预训练的模型更擅长检测药物及相关事件，而通用领域预训练的Bert Base在上下文分类任务中表现最佳。


<details>
  <summary>Details</summary>
Motivation: 为了提升从非结构化临床文本中提取药物事件相关信息的准确性，需要评估不同预训练注意力模型在真实EHR场景下的性能差异，以指导模型选择与优化。

Method: 采用Bert Base、BioBert、两种Bio+Clinical Bert变体、RoBerta和Clinical Longformer等预训练模型，在CMED数据集上进行微调，完成药物提取、医疗事件检测和多维度上下文分类任务。同时设计了EHR文本处理方法以适配模型输入，并基于CMED评估部分构建医学术语，使用召回率、精确率和F1分数进行性能评估。

Result: 在药物和医疗事件检测任务中，基于临床语料预训练的模型表现更优；而在药物事件上下文分类任务中，通用语料预训练的Bert Base模型表现最好。

Conclusion: 模型在特定任务上的表现与其预训练数据密切相关：临床领域预训练有助于实体与事件识别，而通用领域预训练可能更适合复杂语义的上下文理解。因此，应根据具体任务需求选择合适的预训练模型。
...
TL;DR: CompLLM是一种面向实际部署的软上下文压缩技术，通过将长上下文分段独立压缩，实现线性复杂度、可扩展性和计算重用，显著提升大模型在长上下文场景下的推理效率。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在处理长上下文时面临自注意力机制的平方复杂度问题，现有压缩方法因整体压缩导致复杂度高且无法重用计算，限制了实际应用。

Method: 提出CompLLM，将输入上下文划分为多个段并独立压缩，支持缓存和复用已压缩段，降低压缩和推理的计算开销。

Result: 在高压缩率下，CompLLM在长上下文场景中最高可将首令牌时间（TTFT）加速4倍，KV缓存减少50%，性能与未压缩上下文相当甚至更优。

Conclusion: CompLLM通过分段压缩设计，在效率、可扩展性和重用性方面表现优异，适合实际部署，有效解决长上下文处理的挑战。
...
TL;DR: 本文提出了Reinforcement Learning on Pre-Training data (RLPT)，一种新的大语言模型训练扩展范式，利用预训练数据中的下一个文本片段预测任务作为奖励信号，通过强化学习提升模型推理能力，无需依赖人工标注反馈。


<details>
  <summary>Details</summary>
Motivation: 由于高质量文本数据增长有限，而计算资源呈指数增长，传统的大语言模型扩展方法受到限制，因此需要一种不依赖人工标注、能有效利用已有预训练数据的新扩展方式。

Method: 提出RLPT方法，采用下一文本片段推理目标，将预训练数据转化为强化学习的奖励信号，使策略模型能自主探索上下文路径，并在无监督情况下通过强化学习优化自身推理能力。

Result: 在多个通用和数学推理基准（如MMLU、GPQA-Diamond、AIME等）上验证了RLPT的有效性，在Qwen3-4B-Base模型上实现了最高达8.1分的绝对提升，且表现出良好的可扩展性。

Conclusion: RLPT为大语言模型提供了一种可扩展、无需人工反馈的强化学习训练范式，显著提升了模型的推理能力，并为后续方法（如RLVR）提供了坚实基础。
...
TL;DR: 提出一种通过编码原型描述来从大语言模型中提取概念空间的新方法，并通过微调使原型嵌入与概念空间维度对齐，实验证明该方法非常有效。
...
Method: 通过将特征（如甜度）对应原型（如非常甜的食物）的描述进行嵌入编码，并微调LLM以对齐原型嵌入与概念空间维度。

Result: 实验分析表明该方法在提取概念空间方面高度有效。

Conclusion: 该策略为从大语言模型中提取具有认知意义的概念空间提供了一种可行且高效的方法，有助于推动可解释AI的发展。
...
TL;DR: 本文提出了一个名为SloPalSpeech的大规模斯洛伐克语ASR数据集，包含2806小时的议会录音，并通过精细的处理流程生成高质量的音频-文本对。利用该数据集微调OpenAI Whisper系列模型，在标准斯洛伐克语基准上显著降低了词错误率（WER），部分小型模型性能接近大型模型。作者公开发布了数据集、转录文本和微调后的模型，以促进低资源语言语音识别的研究。


<details>
  <summary>Details</summary>
Motivation: 由于训练数据稀缺，斯洛伐克语等低资源语言的自动语音识别（ASR）发展受限。为解决这一问题，需要构建大规模、高质量的语音数据集以提升模型性能。

Method: 构建了一个鲁棒的处理流程，用于对长时录音进行对齐和分段，生成适合训练的30秒音频-文本对；使用SloPalSpeech数据集对多个OpenAI Whisper模型（small到large-v3-turbo）进行微调，并在Common Voice和FLEURS等标准基准上评估性能。

Result: 微调后的Whisper-small模型在标准基准上的词错误率（WER）最多降低了70%，性能接近未微调的Whisper-large-v3模型；所有模型均表现出显著的WER下降。

Conclusion: SloPalSpeech是一个高质量、大规模的斯洛伐克语ASR数据集，能有效提升低资源语言ASR模型的性能；公开发布的数据和模型有助于推动相关领域的研究。
...
TL;DR: 本文介绍了首个用于学术研究的沃洛夫语意图分类数据集WolBanking77，旨在解决低资源语言在意图分类中的研究空白。该数据集包含9791个银行领域的文本句子和超过4小时的语音数据，实验在多种文本和语音模型上进行，结果表现良好。


<details>
  <summary>Details</summary>
Motivation: 现有意图分类研究主要集中于高资源语言，忽视了低资源语言地区（如塞内加尔）的需求，其中沃洛夫语使用广泛但读写率低，缺乏相关数据集支持研究。

Method: 构建并发布WolBanking77数据集，涵盖文本和语音数据，并在多种自然语言处理和自动语音识别基线模型上进行实验，评估F1分数和词错误率等指标。

Result: 在WolBanking77数据集上，各类基线模型表现出 promising 的性能，验证了数据集的有效性，并提供了详细的数据分析和模型比较结果。

Conclusion: WolBanking77为低资源语言特别是沃洛夫语的意图分类研究提供了重要基础，未来将持续维护更新数据集并开源代码以促进相关领域发展。
...
TL;DR: DRISHTIKON 是一个专注于印度文化的多模态、多语言基准，涵盖15种语言和64,000多个文本-图像对，用于评估生成式AI对印度多元文化理解的能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准多为全球性或通用性，缺乏对特定文化（尤其是印度丰富地域文化）的深入覆盖，因此需要一个专门评估AI文化理解能力的基准。

Method: 构建包含文本-图像对的多模态数据集，覆盖印度所有邦和联邦属地，涉及节日、服饰、饮食、艺术和历史遗产等主题，并在零样本和思维链设置下评估多种视觉-语言模型。

Result: 评估揭示了当前模型在处理基于文化的多模态输入时存在显著局限，尤其是在低资源语言和记录较少的传统方面表现不佳。

Conclusion: DRISHTIKON 填补了包容性AI研究的关键空白，为发展具有文化感知能力的多模态语言技术提供了有力测试平台。
...
### [184] [MMCD: Multi-Modal Collaborative Decision-Making for Connected Autonomy with Knowledge Distillation](https://arxiv.org/abs/2509.18198)
*Rui Liu,Zikang Wang,Peng Gao,Yu Shen,Pratap Tokekar,Ming Lin*

Main category: cs.AI

TL;DR: 本文提出了一种用于连接自主系统的多模态协同决策框架MMCD，通过跨模态知识蒸馏提升在传感器缺失情况下的决策鲁棒性，在地面及空地协同驾驶任务中显著提高了驾驶安全性。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常假设训练和测试时所有数据模态和连接车辆均可用，但在实际中由于传感器故障或车辆连接缺失，这一假设不成立，导致决策性能下降。

Method: 提出MMCD框架，融合主车与协作车辆的多模态感知数据（RGB图像和LiDAR点云），并采用基于教师-学生模型的跨模态知识蒸馏方法，使学生模型能在部分模态缺失时仍保持良好性能。

Result: 在地面自动驾驶和空地协同任务实验中，该方法相比现有最佳基线将驾驶安全性提升最高达20.7%，有效增强了对潜在事故的检测与安全决策能力。

Conclusion: MMCD框架通过多模态协同感知与知识蒸馏，实现了在模态缺失场景下鲁棒的协同决策，具有较强的实用性和推广价值。
...
### [185] [Change in Quantitative Bipolar Argumentation: Sufficient, Necessary, and Counterfactual Explanations](https://arxiv.org/abs/2509.18215)
*Timotheus Kampik,Kristijonas Čyras,José Ruiz Alarcón*

Main category: cs.AI

TL;DR: 本文提出了一种形式化方法，用于解释定量双极论证框架（QBAF）中推理变化的原因，通过追踪论点强度顺序的变化（称为强度不一致性），并识别其根本原因作为解释。


<details>
  <summary>Details</summary>
Motivation: 在动态更新的QBAF中，结论可能随强度关系变化而改变，需要一种可解释的方法来追踪和理解这些变化的原因。

Method: 提出基于语义建立的论点强度偏序关系中的“强度不一致性”概念，定义充分、必要和反事实解释，并设计启发式算法以高效搜索此类解释。

Result: 证明了强度不一致性解释的存在性当且仅当更新导致强度不一致，并实现了相应的解释搜索算法。

Conclusion: 该方法为QBAF中的动态推理变化提供了可计算、可解释的追踪机制，增强了论证系统的透明性和可信度。
...
### [186] [A Cost-Benefit Analysis of On-Premise Large Language Model Deployment: Breaking Even with Commercial LLM Services](https://arxiv.org/abs/2509.18101)
*Guanzhong Pan,Haibo Wang*

Main category: cs.AI

TL;DR: 本文提出了一种成本效益分析框架，帮助组织评估本地部署开源大语言模型相较于商业云服务的经济可行性。


<details>
  <summary>Details</summary>
Motivation: 由于数据隐私、供应商锁定和长期运营成本等问题，组织对本地部署开源大语言模型的兴趣增加，需要决策支持工具。

Method: 通过分析Qwen、Llama、Mistral等开源模型的硬件需求、运维开销和性能基准，将其本地部署总成本与主流云服务商的订阅费用进行比较。

Result: 得出了基于使用强度和性能需求的盈亏平衡点估计，明确了本地部署在何种条件下更具经济优势。

Conclusion: 该框架为组织制定大语言模型部署策略提供了实用的量化依据。
...
### [187] [SPADE: A Large Language Model Framework for Soil Moisture Pattern Recognition and Anomaly Detection in Precision Agriculture](https://arxiv.org/abs/2509.18123)
*Yeonju Lee,Rui Qi Chen,Joseph Oboamah,Po Nien Su,Wei-zhen Liang,Yeyin Shi,Lu Gan,Yongsheng Chen,Xin Qiao,Jing Li*

Main category: cs.AI

TL;DR: 本研究提出SPADE框架，利用大语言模型（如ChatGPT-4.1）对土壤湿度时间序列数据进行零样本的灌溉模式与异常检测，无需任务特定标注或微调，通过将数据转为文本并设计领域提示模板，实现高精度、可解释的分析。


<details>
  <summary>Details</summary>
Motivation: 现有土壤湿度分析方法依赖阈值规则或需要大量数据的机器学习模型，适应性和可解释性差，难以满足精准农业需求。

Method: 将土壤湿度时间序列转化为文本表示，结合领域知识设计提示模板，利用ChatGPT-4.1进行零样本推理，检测灌溉事件、估算灌溉量、识别并分类异常，并生成结构化报告。

Result: 在真实农场数据上实验表明，SPADE在异常检测中召回率和F1分数优于现有方法，能准确分类异常类型；在灌溉事件检测中也表现出高精确率和召回率，且生成的报告具有良好的可解释性。

Conclusion: SPADE展示了大语言模型在精准农业中的潜力，能够融合定性知识与数据驱动推理，提供可操作的洞察，提升土壤湿度监测与灌溉调度的准确性与适应性。
...
### [188] [Position Paper: Integrating Explainability and Uncertainty Estimation in Medical AI](https://arxiv.org/abs/2509.18132)
*Xiuyi Fan*

Main category: cs.AI

TL;DR: 本文提出了可解释的不确定性估计（XUE），将可解释性与不确定性量化相结合，以提升医疗AI系统的可信度和可用性。


<details>
  <summary>Details</summary>
Motivation: 当前医疗AI系统无法有效量化和传达不确定性，且现有的可解释性方法与不确定性估计脱节，限制了AI在临床实践中的应用。

Method: 通过系统映射医学不确定性与AI不确定性概念，提出XUE框架，并探讨多模态不确定性量化、模型无关的可视化技术及不确定性感知决策支持系统等技术方向。

Result: 明确了实现XUE的关键挑战和技术路径，并提出了指导原则，以促进XUE的有效实现。

Conclusion: XUE有助于弥合可解释性与不确定性之间的鸿沟，推动符合临床复杂需求的可信医疗AI系统的发展。
...
### [189] [HSGM: Hierarchical Segment-Graph Memory for Scalable Long-Text Semantics](https://arxiv.org/abs/2509.18168)
*Dong Liu,Yanxuan Yu*

Main category: cs.AI

TL;DR: 提出了一种分层段-图记忆（HSGM）框架，用于解决长文档语义解析中的计算和内存瓶颈，实现高效、准确的语义建模。


<details>
  <summary>Details</summary>
Motivation: 长文档的语义解析面临因成对组合和内存需求二次增长带来的挑战，现有方法难以扩展到超长文本。

Method: 将输入分解为多个有意义的段，每段构建局部语义图，并提取紧凑的摘要节点形成全局图记忆；支持增量更新和分层查询处理，通过摘要节点检索相关段并在其局部图中进行细粒度推理。

Result: 理论复杂度从O(N^2)降至O(Nk + (N/k)^2)；在三个基准任务上实现了2-4倍推理加速、超过60%的峰值内存降低，并保持≥95%的基线准确率。

Conclusion: HSGM有效解决了长文档语义解析的可扩展性问题，为资源受限和实时NLP应用提供了可行方案。
...
### [190] [Foam-Agent: An End-to-End Composable Multi-Agent Framework for Automating CFD Simulation in OpenFOAM](https://arxiv.org/abs/2509.18178)
*Ling Yue,Nithin Somasekharan,Tingwen Zhang,Yadi Cao,Shaowu Pan*

Main category: cs.AI

TL;DR: Foam-Agent是一个多智能体框架，通过自然语言提示实现OpenFOAM计算流体动力学（CFD）全流程自动化，显著降低使用门槛。


<details>
  <summary>Details</summary>
Motivation: CFD在工程中至关重要，但其复杂的手动设置和陡峭的学习曲线限制了广泛应用。现有自动化工具无法覆盖完整流程，且缺乏灵活性与高精度配置生成能力。

Method: 提出Foam-Agent，采用多智能体架构，集成Meshing Agent进行几何建模与网格处理，支持Gmsh生成新几何及外部网格文件导入；通过Model Context Protocol（MCP）实现可组合的服务架构，支持与其他智能系统（如Claude-code）集成；利用分层多索引RAG和依赖感知的生成机制确保配置准确性，并自动生成HPC提交脚本和ParaView可视化。

Result: 在110个模拟任务的基准测试中，Foam-Agent使用Claude 3.5 Sonnet实现了88.2%的成功率，显著优于MetaOpenFOAM的55.5%。

Conclusion: Foam-Agent首次实现了从自然语言到完整CFD仿真的端到端自动化，具备高成功率、灵活集成能力和精确配置生成，有效降低了CFD使用的专业门槛，展示了专用多智能体系统在科学计算普及化中的潜力。
...
### [191] [Large Language Models and Operations Research: A Structured Survey](https://arxiv.org/abs/2509.18180)
*Yang Wang,Kai Li*

Main category: cs.AI

TL;DR: 本文综述了大语言模型（LLMs）在运筹学（OR）中的应用进展，将其方法归纳为自动建模、辅助优化和直接求解三类，探讨了现有挑战如语义到结构映射不稳定、研究碎片化、泛化能力有限及评估体系不足，并展望了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 传统运筹学方法在处理大规模、动态、多约束问题时存在局限，依赖专家建模和手动调参，难以应对复杂系统决策需求。大语言模型具备语义理解、结构化生成和推理控制能力，有望弥补这些不足。

Method: 本文采用综述方式，将LLMs在OR中的应用分为三类：自动建模（自然语言转数学模型或代码）、辅助优化（生成启发式算法或进化算法）和直接求解（端到端优化）。同时梳理了评估基准和领域应用。

Result: 总结了当前LLMs在OR中集成的主要方法与应用场景，识别出语义映射不稳定、研究分散、泛化能力弱和评估体系不完善等关键开放问题。

Conclusion: LLMs为运筹学提供了新的技术路径，未来需加强跨领域融合、提升模型稳定性与可解释性，并建立统一的评估框架以推动该方向发展。
...
### [192] [Synthesizing Attitudes, Predicting Actions (SAPA): Behavioral Theory-Guided LLMs for Ridesourcing Mode Choice Modeling](https://arxiv.org/abs/2509.18181)
*Mustafa Sameen,Xiaojian Zhang,Xilei Zhao*

Main category: cs.AI

TL;DR: 本文提出了一种名为SAPA的分层框架，利用大语言模型（LLM）合成基于理论的潜在态度，以提高网约车出行选择预测的准确性。该方法结合生成的旅行者画像、倾向性得分和潜变量评分，在大规模调查数据上显著优于现有模型，PR-AUC提升高达75.9%。


<details>
  <summary>Details</summary>
Motivation: 现有网约车选择模型因忽略关键心理因素且面临严重类别不平衡问题，导致预测精度有限。因此，需要一种能更好捕捉个体决策机制并应对稀有事件挑战的新方法。

Method: 提出SAPA框架：首先使用大语言模型从原始出行调查数据中生成定性旅行者画像；然后基于人口统计和行为特征训练倾向性得分模型，并融入画像信息；接着由LLM为理论驱动的潜变量（如时间与成本敏感度）打分；最后，综合倾向性得分、潜变量及其交互项以及可观测行程特征，通过分类器预测出行模式选择。

Result: 在大规模多年度出行调查数据上的实验表明，SAPA在测试集上的PR-AUC指标相比最先进基线模型最高提升了75.9%，显著提高了对网约车选择的预测性能。

Conclusion: SAPA框架有效提升了网约车出行选择的预测准确率，解决了传统模型在心理因素建模和类别不平衡方面的局限，提供了一个可迁移至其他领域的强大方法论工具。
...
### [193] [An Outcome-Based Educational Recommender System](https://arxiv.org/abs/2509.18186)
*Nursultan Askarbekuly,Timur Fayzrakhmanov,Sladjan Babarogić,Ivan Luković*

Main category: cs.AI

TL;DR: OBER是一个基于学习成果的教育推荐系统框架，通过将学习目标和评估项目嵌入数据结构，能够直接评估推荐算法对学习掌握程度的影响。


<details>
  <summary>Details</summary>
Motivation: 现有教育推荐系统多以点击或评分为优化目标，难以衡量其实际教学效果，因此需要一个能直接关联学习成果的评估框架。

Method: OBER采用极简的实体关系模型、基于日志的掌握度计算公式和插件式架构，集成到非正式学习领域的e-learning系统中，并通过为期两周的随机分组实验，对比固定专家路径、协同过滤（CF）和基于知识（KB）过滤三种推荐方法。

Result: 实验结果显示协同过滤在用户留存上表现最好，但固定专家路径在学习掌握度上最高；OBER能从同一日志中统一生成业务、相关性和学习成效指标，无需额外测试开销。

Conclusion: OBER提供了一种方法无关、可扩展的框架，使教育推荐系统能够直接优化学习成果，平衡相关性、参与度与掌握度之间的权衡。
...
### [194] [From latent factors to language: a user study on LLM-generated explanations for an inherently interpretable matrix-based recommender system](https://arxiv.org/abs/2509.18980)
*Maxime Manderlier,Fabian Lecron,Olivier Vu Thanh,Nicolas Gillis*

Main category: cs.AI

TL;DR: 该研究探讨了大型语言模型（LLM）能否从一个数学上可解释的推荐模型生成有效的用户导向解释，该模型基于带约束的矩阵分解，并通过用户研究评估了解释在透明度、有效性、说服力、信任和满意度等方面的质量。


<details>
  <summary>Details</summary>
Motivation: 为了克服传统可解释AI中自动评估指标无法准确反映用户实际需求和感知的问题，研究旨在通过用户中心的方法评估LLM生成的自然语言解释的有效性。

Method: 采用基于约束矩阵分解的可解释推荐模型，其中用户类型被显式表示且预测评分与观测评分同尺度；通过精心设计的LLM提示将模型结构转化为自然语言解释，并在326名参与者中进行用户研究，比较多种解释策略在五个维度上的表现。

Result: 所有类型的解释总体上都获得良好反馈，不同策略间存在中等程度的统计差异；用户评论提供了超出量化结果的补充见解，揭示了对不同类型解释的具体反应。

Conclusion: 基于可解释模型并结合精心设计提示的LLM能够生成高质量、用户认可的推荐解释，验证了用户中心评估方法的重要性及多策略生成解释的可行性。
...
TL;DR: 本文提出了Neural DNA（nDNA）作为AI基础模型的语义基因型表征，通过潜层几何的三个维度——谱曲率、热力学长度和信念向量场——捕捉模型的内在认知身份，开创了‘神经基因组学’新领域。


<details>
  <summary>Details</summary>
Motivation: 随着AI模型能力增强，仅靠行为基准无法揭示其内在认知结构，需要一种能刻画模型‘认知身份’的本质性表征方法。

Method: 提出Neural DNA（nDNA），基于潜层几何的三个原则性维度：谱曲率（概念流动的曲率）、热力学长度（表示转换的语义代价）、信念向量场（语义扭转场），构建模型的稳定、坐标无关的指纹。

Result: nDNA能够追踪模型在预训练、微调、对齐、剪枝、蒸馏和合并过程中的谱系，量化继承关系，检测因数据或目标变化导致的认知漂移，并揭示文化印记、架构漂移等‘神经疤痕’。

Conclusion: nDNA为理解AI模型的内在认知提供了新的几何视角，开启了‘神经基因组学’的研究方向，使模型被视为具有可追溯内在认知的数字语义有机体。
...
TL;DR: 本文提出了相似性场理论（Similarity Field Theory），通过定义实体间的相似性场及其演化，形式化了动态系统中相似关系的保持与变换，并在此基础上提出了一种生成式智能定义，可用于刻画和构建智能系统。


<details>
  <summary>Details</summary>
Motivation: 为了理解动态系统中可理解性的结构基础，特别是相似性关系如何持续并演化，从而为智能系统提供统一的数学描述框架。

Method: 引入相似性场S: U×U→[0,1]，定义概念K的纤维F_α(K)，并通过序列Z_p=(X_p, S^(p))描述系统演化；提出生成算子G，并定义其关于概念K的智能性。

Result: 证明了两个定理：(i) 不对称性阻碍相互包含；(ii) 稳定性需要锚点坐标或最终限制在某个水平集内；并将该框架应用于大语言模型和社会认知分析。

Conclusion: 相似性场理论为理解和构建智能系统提供了基础语言，能够形式化智能的生成过程，并用于解释复杂系统如大语言模型的行为。
...
TL;DR: 提出了一种名为VL-RiskFormer的分层堆叠视觉-语言多模态Transformer模型，用于个体健康风险预测，在MIMIC-IV数据集上表现出色。


<details>
  <summary>Details</summary>
Motivation: 应对慢性病负担上升和临床数据多模态、异构化的挑战，亟需统一的AI框架进行主动健康风险预测。

Method: 基于双流架构，引入跨模态对比预训练、自适应时间间隔位置编码的时间融合模块、疾病本体图适配器，并在顶层嵌入大语言模型推理头。

Result: 在MIMIC-IV纵向队列上，VL-RiskFormer平均AUROC达到0.90，预期校准误差为2.7%。

Conclusion: VL-RiskFormer有效整合多源异构临床数据，具备高精度和良好校准性的健康风险预测能力，具有临床应用潜力。
...
TL;DR: 本文提出了一种名为ChefMind的混合架构，结合了CoE、知识图谱、RAG和大语言模型，用于解决个性化食谱推荐中的模糊用户意图、语义准确性和细节覆盖问题，在多个指标上优于基线模型。


<details>
  <summary>Details</summary>
Motivation: 个性化食谱推荐在处理模糊用户意图、确保语义准确性和充分的细节覆盖方面存在挑战。

Method: 提出ChefMind，结合Chain of Exploration（CoE）、知识图谱（KG）、检索增强生成（RAG）和大语言模型（LLM），其中CoE将模糊查询转化为结构化条件，KG提供语义推理，RAG补充烹饪上下文细节，LLM整合输出生成连贯推荐。

Result: 在Xiachufang数据集和人工标注查询上评估，ChefMind在准确性、相关性、完整性和清晰度方面表现优异，平均得分8.7，显著高于消融模型的6.4-6.7分，并将未处理查询减少至1.6%。

Conclusion: ChefMind能有效应对模糊用户需求，提升推荐质量，在多组件协同下实现了鲁棒且可解释的个性化食谱推荐。
...
TL;DR: 提出了一种基于N-Plus-1多智能体架构的GPT代理系统，用于机械工程问题的初步分析，通过集成多个独立解并利用Condorcet陪审团定理提高求解可靠性，强调透明性和教学价值。


<details>
  <summary>Details</summary>
Motivation: 由于GPT在解决工程问题时存在不稳定性（成功率仅85%），直接使用“开箱即用”的GPT在教育和工程实践中不可靠，因此需要一种更可靠、可解释的解决方案生成机制。

Method: 构建一个N-Plus-1 GPT代理系统：首先由N个独立的Agent Solve生成N个问题解，然后由Agent Compare对这些解进行汇总、比较，并推荐最终方案；结合Condorcet陪审团定理分析正确解占优的概率。

Result: 该方法能以高概率确保推荐解是正确的（当单次求解成功率>1/2且N足够大时）；Agent Compare还能融合不同数学模型或求解路径的次要解，提升解的多样性与鲁棒性；与商业多智能体模型Grok Heavy相比，在设计和性能上相似，但更注重透明性和教学意义。

Conclusion: N-Plus-1 GPT代理系统显著提升了生成式AI在机械工程问题求解中的可靠性与可解释性，适合应用于教育场景和初步工程分析。
...
TL;DR: 提出了一种轻量级分层强化学习框架ComputerAgent，用于桌面应用控制，在减少模型大小和推理时间的同时，取得了与大型多模态语言模型相当甚至更好的性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于多模态大语言模型的方法在桌面控制任务中存在推理延迟高、样本效率低、难以在设备上部署等问题，需要更高效、实用的解决方案。

Method: 构建了一个双层选项架构（manager和subpolicy），采用三模态状态编码器（截图、任务ID、数值状态），引入元动作与早停机制，并使用紧凑的视觉主干网络和小型策略网络以实现高效的端侧推理。

Result: 在135个真实桌面任务中，简单任务成功率达92.1%，困难任务达58.8%，性能媲美2000亿参数的MLLM基线，模型参数仅1500万，推理时间减半。

Conclusion: 分层强化学习为计算机控制提供了一种高效、可扩展且适用于端侧部署的替代方案，显著优于大型语言模型方法。
...
TL;DR: 当前医疗AI基准测试存在局限性，高分并不反映真实世界的表现，模型依赖捷径学习和应试技巧，而非真正的医学理解。


<details>
  <summary>Details</summary>
Motivation: 揭示大型AI模型在医疗基准测试中表现优异的背后存在的脆弱性和错误推理问题，警示仅依赖排行榜分数可能误导对模型实际医疗应用能力的判断。

Method: 对六个主流AI模型在六个常用医疗基准上进行压力测试，并结合临床医生指导的评分标准评估模型表现，分析其推理过程和稳定性。

Result: 发现模型在输入缺失、提示微小变化时表现不稳定，常通过捷径学习猜测答案，且生成看似合理但存在缺陷的推理；不同基准测量内容差异大但被等同对待，掩盖了失败模式。

Conclusion: 医疗AI的可靠性不能仅凭基准分数衡量，必须强调鲁棒性、合理推理及与真实医疗需求的一致性，以建立可信的临床AI系统。
...
TL;DR: 本文研究了在计算资源受限的情况下，通过限制推理长度和模型量化两种策略来降低推理语言模型的计算开销，并探讨其对模型安全性的影响。


<details>
  <summary>Details</summary>
Motivation: 随着测试时计算量增加以提升推理性能，计算成本显著上升，因此需要探索在保证性能的同时降低计算消耗的方法。

Method: 采用基于长度控制策略优化（LCPO）的强化学习方法进行模型微调，并应用模型量化技术，在用户定义的计算约束下最大化生成思维链（CoT）序列。

Result: 两种计算约束策略均能在一定程度上减少计算需求，但会影响模型的安全性表现，揭示了计算效率与模型安全性之间的权衡。

Conclusion: 合理施加计算约束可在控制成本的同时维持一定性能，但需注意对模型安全性的潜在负面影响。
...
TL;DR: 本文提出了“哥德尔测试”，用于评估大语言模型是否能为简单但未解决的数学猜想生成正确证明，并通过GPT-5在五个组合优化问题上的表现进行验证，结果显示其在较易问题上表现良好，偶有原创性突破，但在跨论文综合推理方面仍有局限。


<details>
  <summary>Details</summary>
Motivation: 评估当前大语言模型在高级数学领域中提出并证明新猜想的能力，探索其在创造性数学推理中的潜力与边界。

Method: 提出“哥德尔测试”框架，选取五个来自组合优化的未解猜想，向模型提供来源论文但隐藏作者提出的猜想，系统评估GPT-5生成证明的正确性与推理过程。

Result: GPT-5在三个较易问题上几乎完全正确；在问题2中甚至推翻原猜想并给出新的有效近似保证；问题4因需结合两篇论文结果而失败；问题5中提出正确算法但分析失败，显示证明难度超出预期。

Conclusion: 尽管样本较小，结果表明GPT-5在常规推理上有显著进展，偶尔展现原创性，但在需要跨文献综合推理的任务中仍存在明显局限，可能是迈向通过哥德尔测试的早期步骤。
...
TL;DR: 本文介绍了首个用于《协调关税表》（HTS）编码分类的基准数据集，并提出了一种基于LLaMA-3.3-70B的微调模型Atlas，在准确性和成本上均显著优于现有大模型，推动HTS分类成为机器学习社区的新基准任务。


<details>
  <summary>Details</summary>
Motivation: 准确的HTS编码分类对全球贸易至关重要，但目前在机器学习领域关注不足，且错误分类会导致货物运输中断，因此亟需高效、低成本且保障数据隐私的自动化解决方案。

Method: 基于美国海关裁定在线搜索系统（CROSS）构建HTS分类基准数据集，并对Atlas模型（基于LLaMA-3.3-70B）进行微调，评估其在10位和6位HTS编码上的分类性能，并与GPT-5-Thinking和Gemini-2.5-Pro-Thinking等领先大模型进行对比。

Result: Atlas模型在10位HTS编码分类中达到40%完全正确率，6位编码中达57.5%，分别比GPT-5-Thinking高15个百分点，比Gemini-2.5-Pro-Thinking高27.5个百分点；同时成本更低，可自托管以保障数据隐私。

Conclusion: Atlas为HTS分类建立了强有力的基线，但该任务仍具挑战性（仅40%十位准确率），作者通过公开数据集和模型，呼吁学界关注并推动在检索、推理与对齐方面的后续研究。
...
TL;DR: IFEval-FC 是一个评估大语言模型在函数调用中精确遵循格式指令能力的新基准，发现即使是 GPT-5 和 Claude 4.1 Opus 等最先进模型也常无法遵守基本格式规则。


<details>
  <summary>Details</summary>
Motivation: 现有函数调用基准未测试模型对参数描述中嵌入格式指令的遵循情况，而实际AI代理应用中精确格式遵循至关重要。

Method: 受 IFEval 启发，IFEval-FC 将可验证的格式要求直接编码到 JSON schema 描述中，构建包含750个测试用例的数据集，并采用完全算法化的评估方式。

Result: 实验表明，包括 GPT-5 和 Claude 4.1 Opus 在内的最先进的专有模型经常无法遵循基本的格式规则，如引号或日期格式要求。

Conclusion: 大语言模型在精确遵循函数调用中的格式指令方面存在明显不足，这限制了其在现实世界代理系统中的可靠性，需引起重视并改进。
...
TL;DR: 提出了一种名为Memory-QA的新任务，旨在基于先前存储的多模态记忆回答视觉内容的回忆问题，并提出了Pensieve系统以应对相关挑战，在新构建的基准上表现出优于现有方法的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的问答系统在处理基于历史多模态记忆的回忆问题时存在不足，缺乏对时间、位置信息的有效利用以及多记忆整合能力，因此需要专门的任务和系统来解决这些现实世界中的挑战。

Method: 提出了Pensieve系统，包含记忆特定增强、时空感知的多信号检索模块，以及多记忆问答微调策略；同时构建了一个多模态基准来评估Memory-QA任务。

Result: 在自建的多模态基准测试中，Pensieve相比现有最先进方法在问答准确率上最高提升了14%。

Conclusion: Pensieve有效解决了Memory-QA任务中的关键挑战，验证了利用时间、位置和多记忆融合对于提升回忆型视觉问答性能的重要性。
...
TL;DR: 提出了一种名为FERA的击剑裁判助手，结合姿态识别与规则推理实现花剑比赛中的自动判罚辅助。


<details>
  <summary>Details</summary>
Motivation: 解决击剑比赛中主观判罚、人为错误、偏见及训练环境中裁判资源有限的问题。

Method: 从视频中提取2D关节位置，归一化后计算101维运动学特征，使用Transformer进行多标签动作和剑法分类，并结合编码了优先权规则的语言模型进行判罚决策与解释生成。

Result: 在有限标注数据下，5折交叉验证平均macro-F1得分为0.549，优于TCN、BiLSTM和普通Transformer等基线模型。

Conclusion: FERA展示了AI在花剑裁判辅助中的潜力，虽尚未达到部署水平，但为裁判辅助和击剑教学等AI应用提供了新方向。
...
TL;DR: 本文提出了一种名为LLMZ+的新方法，通过上下文相关的提示白名单机制，增强基于大语言模型的智能体的安全性，有效抵御常见的越狱攻击，同时不影响合法业务通信。


<details>
  <summary>Details</summary>
Motivation: 由于代理型AI具有对数据和API工具的特权访问，并依赖非确定性行为，因此相较于传统软件面临更大的安全风险，需要更有效的防护机制。

Method: 提出LLMZ+方法，采用基于上下文的提示白名单机制，仅允许符合预定义使用场景和操作边界的输入与LLM交互，从而替代传统的恶意意图检测方法。

Result: 实验表明，LLMZ+在防止常见越狱提示方面表现出强韧性，且在实验设置中可将误报率和漏报率均降至0，同时不干扰合法用户通信。

Conclusion: LLMZ+通过简化安全框架并提升长期抗攻击能力，为代理型大语言模型提供了一种高效、低资源消耗的信息安全解决方案。
...
TL;DR: 提出一种新方法，通过分解问题生成方程并结合外部符号求解器与二次估算验证机制，显著提升大模型在数学应用题上的表现。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在解决数学应用题时存在困难，因其需要较强的推理和数学能力。现有方法虽有所改进，但仍需进一步提升准确性和泛化能力。

Method: 首先引导大模型从问题分解中生成方程，使用外部符号求解器得出答案；然后让模型再次估算答案以进行验证，若不一致则进行迭代修正。

Result: 在多个数值和代数数学应用题数据集上达到新的SOTA，平均比之前最佳结果提升近2%；并在三角函数类题目上取得良好表现。同时发布了两个新数据集SVAMPClean和Trig300。

Conclusion: 该方法有效提升了大模型解决数学应用题的准确性与鲁棒性，验证机制和迭代修正策略对提高性能至关重要。
...
TL;DR: 提出了一种结合气候灾害数据与经济主体进化学习的地理空间代理模型，用于评估直接和级联气候风险。


<details>
  <summary>Details</summary>
Motivation: 为了更准确地评估气候变化对异质性经济系统的复杂影响，特别是考虑空间异质性灾害与适应性经济行为之间的交互。

Method: 结合Mesa空间建模与CLIMADA气候影响评估，构建基于代理的模型，引入基于适应度选择和变异的进化学习机制，模拟企业在预算分配、定价、工资和风险适应方面的策略演化。

Result: 在RCP8.5情景下，企业通过进化适应可在数十年后恢复至无灾害基准生产水平；未直接受灾企业也因供应链中断受到影响，世纪末商品平均价格上涨5.6%。

Conclusion: 该开源框架能有效量化直接和间接气候风险，为金融机构和企业提供评估成本效益高的适应策略的工具。
...
TL;DR: 提出TERAG框架，通过引入个性化PageRank降低图构建中的LLM token消耗，在保持80%以上准确率的同时，仅使用3%-11%的输出token。


<details>
  <summary>Details</summary>
Motivation: 现有基于图的RAG系统在图构建过程中忽略LLM token使用的高成本，限制了其大规模应用。

Method: 受HippoRAG启发，TERAG在检索阶段引入个性化PageRank（PPR），以更低的token消耗构建信息丰富的图。

Result: TERAG在多个指标上达到主流图基RAG方法80%以上的准确性，同时输出token消耗仅为3%-11%。

Conclusion: TERAG是一种简单而有效的低开销图基RAG框架，显著降低了token成本，具备良好的实际应用潜力。
...
TL;DR: 本文探讨了在航空系统中应用机器学习（ML）时的安全合规问题，提出了机器学习模型描述（MLMD）的概念，并强调语义保持的重要性，以确保模型在目标环境中的准确复制和安全运行。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习在航空系统中的应用日益增多，必须确保其安全性并符合相关适航指南。目前EASA和EUROCAE/SAE正在制定相关规范，但缺乏对模型与其描述之间差异的明确定义。

Method: 提出机器学习模型描述（MLMD）的概念，明确区分ML模型与其形式化描述，并引入语义保持的概念以确保模型实现的准确性。通过多个工业用例验证该方法的有效性。

Result: 成功构建并比较了多个目标模型，验证了MLMD和语义保持在实际应用中的可行性与重要性，有助于提升ML系统在航空领域的可认证性。

Conclusion: MLMD和语义保持是确保航空领域ML系统安全性和合规性的关键要素，为未来适航认证提供了理论基础和实践路径。
...
TL;DR: 本文系统回顾了大语言模型（LLM）在医疗领域的最新研究进展，分析了其训练技术、应用场景、优势与局限，并创新性地对医疗LLM按训练方法进行分类，提出未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在各行业的广泛应用，医疗领域成为其最具潜力的应用场景之一，亟需系统梳理其发展现状与挑战，推动医疗LLM的进一步发展。

Method: 通过系统性文献综述，总结大语言模型在医疗领域的训练技术、适应方法和应用实践，并基于训练方式将医疗LLM分为三类，评估方法分为两类。

Result: 明确了医疗LLM的当前发展状态，识别出关键挑战，并提出了应对策略和未来研究方向。

Conclusion: 发展医疗大语言模型具有必要性，本文为后续研究提供了清晰的指导框架和理论基础。
...
TL;DR: 本文提出了自主数据代理（DataAgents），其结合大语言模型推理与任务分解、动作推理和工具调用，能够自动处理复杂数据任务，实现从数据到知识的范式转变。


<details>
  <summary>Details</summary>
Motivation: 随着数据规模和复杂性的增长，传统数据处理方式难以高效扩展，且数据结构常不适应AI需求。如何通过密集的数据操作将更多知识嵌入数据中成为一个关键问题。

Method: 提出DataAgents框架，整合LLM的推理能力，支持任务分解、动作规划、工具调用，并生成可执行代码以完成多样化的数据操作任务。

Result: DataAgents能自主完成数据收集、集成、预处理、转换、增强、修复和检索等任务，显著提升数据处理效率与智能化水平。

Conclusion: DataAgents代表了向自主化数据到知识系统的范式转变，未来需在工作流优化、开放基准、隐私保护和可信机制方面协同推进。
...
TL;DR: 提出“经验扩展”框架，通过大语言模型在部署后与环境自主交互并共享经验，持续提升性能，突破人类生成数据的局限。


<details>
  <summary>Details</summary>
Motivation: 随着人类生成文本数据耗尽，传统通过扩大模型规模和训练数据的方法趋于饱和，需要新的扩展路径以持续提升大语言模型性能。

Method: 提出经验扩展框架，使大语言模型在部署后通过与环境自主交互积累经验，将原始交互提炼为可复用的知识，并定期优化存储内容以保持相关性和效率。

Result: 在模拟真实场景中验证表明，该框架能提升准确性、长期维持性能，并在面对新任务、重复查询和知识过载时保持性能增益。

Conclusion: 结构化的部署后学习可通过经验扩展突破大语言模型的能力边界，为持续智能进步提供可扩展的路径。
...
TL;DR: 本文介绍了Agent Directory Service (ADS)，一个用于发现AI代理能力、元数据和来源的分布式目录，基于Open Agentic Schema Framework和Kademlia DHT实现高效、可验证的多维发现。


<details>
  <summary>Details</summary>
Motivation: 为解决异构多代理系统中代理能力发现与互操作性不足的问题，提供一种标准化、可扩展且安全的发现机制。

Method: 采用内容寻址存储、分层分类法和加密签名，通过两层映射在Kademlia-based DHT上实现解耦的能力索引，并集成OCI/ORAS和Sigstore进行分发与来源验证。

Result: 实现了支持多模态代理（如LLM提示代理、MCP服务器等）的可扩展、高效且安全的发现服务，具备良好的性能与互操作性。

Conclusion: ADS为多代理系统提供了可验证、去中心化的发现基础设施，推动了开放代理生态系统的互操作性与标准化发展。
...
TL;DR: 本文提出了LLMCHECKER，一种基于模型检测的方法，用于验证大语言模型（LLM）文本生成过程中概率计算树逻辑（PCTL）属性的一致性。


<details>
  <summary>Details</summary>
Motivation: 由于LLM在生成文本时通常只选择有限且不固定的token，缺乏对生成过程的严格形式化验证方法，因此需要一种能够量化并验证其行为一致性的技术。

Method: 提出α-k-受限文本生成模型，限制每步仅考虑累积概率不超过α的top-k个token，并结合模型检测技术对PCTL属性进行形式化验证，支持多种文本量化方式（如质量与偏见评估）。

Result: 在Llama、Gemma、Mistral、Genstruct和BERT等多个LLM上成功应用该方法，实现了对PCTL属性的有效验证，证明了该方法的适用性和有效性。

Conclusion: LLMCHECKER是首个将PCTL-based模型检测应用于LLM文本生成过程一致性的验证框架，为LLM的行为分析提供了新的形式化工具。
...
TL;DR: 提出了一种基于开源大语言模型的模块化框架，用于ICD-10-CM编码预测，通过合理选择基础模型、去冗余数据采样和结构化输入设计，提升了自动医学编码的准确性和可部署性。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在ICD编码自动化中面临基础模型选择困难、输入上下文设计不合理和训练数据冗余等问题，导致效果受限，亟需系统性解决方案。

Method: 采用LLM-as-judge评估协议结合Plackett-Luce聚合方法进行模型选择；引入基于嵌入相似性的度量和去冗余采样策略处理数据重复；利用结构化出院记录评估上下文影响，并比较通用与特定章节建模范式。

Result: 所选基础模型在微调后于内外部评估中 consistently 优于基线模型；纳入更多临床文本章节能持续提升预测性能。

Conclusion: 该框架通过有依据的模型选择、高效的数据精炼和上下文感知提示，为自动化医学编码提供了可扩展、可直接部署的实用解决方案。
...
TL;DR: 本文提出了一种新的强化学习策略MAPO，通过动态重加权优势函数来解决GRPO中的优势反转和镜像问题，提升基础模型在推理任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有GRPO方法在优势分配上存在优势反转和优势镜像问题，导致不同样本间的优势评估不合理。

Method: 提出Mixed Advantage Policy Optimization (MAPO)，引入轨迹确定性概念，使用优势百分偏差，并对不同确定性的样本动态重加权优势函数。

Result: 实验表明，MAPO在多种基准上优于现有最先进方法，消融研究验证了各优势变体的有效性。

Conclusion: MAPO通过自适应配置优势函数，有效解决了优势分配问题，提升了推理任务的性能。
...
TL;DR: 本文提出了ProfileBench，一个来自真实视频平台的工业级用户画像基准，并提出Conf-Profile框架，通过置信度驱动的两阶段推理方法实现无需真实标签的可靠用户画像，显著提升了大模型在用户画像任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 现有用户画像研究缺乏全面的基准，且难以获取大规模真实标签，用户数据异构噪声多，影响大模型可靠性。

Method: 提出Conf-Profile框架：第一阶段利用大模型生成带置信度提示的高质量合成标签；第二阶段通过置信度加权投票和校准提升准确性，并将多结果、理由和置信度蒸馏至轻量模型；进一步采用置信度引导的无监督强化学习增强推理能力。

Result: 实验表明，Conf-Profile通过两阶段训练显著提升性能，在Qwen3-8B上F1分数提高13.97。

Conclusion: Conf-Profile实现了无需真实标签的可靠用户画像，结合置信度机制有效应对噪声数据挑战，在工业场景下具有应用潜力。
...
TL;DR: 本文提出了一个关于大语言模型（LLM）记忆的统一操作定义和四部分分类法，构建了涵盖参数化、上下文、外部及程序/情景记忆的评估框架，并提出DMM Gov系统以实现可审计的记忆更新与遗忘机制，最终形成可复现、可比较且可治理的研究与部署坐标系。


<details>
  <summary>Details</summary>
Motivation: 现有研究中对LLM记忆的定义和评估缺乏统一标准，导致不同设置间的比较失真，亟需一个系统性框架来整合记忆机制、评估与治理。

Method: 提出LLM记忆的操作定义和四维分类（参数化、 contextual, 外部, 程序/ episodic），设计记忆四元组（位置、持久性、读写路径、可控性），建立三层实验设置协议和分层评估体系，并引入DMM Gov实现记忆治理闭环。

Result: 构建了一个集成记忆机制分析、多层级评估、时间维度治理与不确定性报告的完整框架，在相同数据和时间线上实现能力与信息可用性的解耦评估，并提出四项可验证命题。

Conclusion: 该框架为LLM记忆研究提供了可复现、可比较和可治理的坐标系统，有助于推动记忆机制的标准化评估与安全可控部署。
...
TL;DR: LongCat-Flash-Thinking是一个5600亿参数的开源混合专家（MoE）推理模型，通过精心设计的训练流程实现最先进的复杂推理能力，包括长链式思维数据冷启动和大规模强化学习，并采用领域并行训练策略与DORA异步系统提升效率，显著降低令牌消耗，现已公开发布以推动推理系统和智能体AI研究。


<details>
  <summary>Details</summary>
Motivation: 为了提升大规模语言模型在复杂推理任务中的性能和效率，尤其是在正式推理、代码生成和智能体行为等多领域表现，同时解决传统同步强化学习训练速度慢的问题，需要一种高效且可扩展的训练方法。

Method: 首先采用长链式思维（CoT）数据进行冷启动训练，提升模型的基础推理能力；然后提出领域并行训练方案，将不同领域（如STEM、代码、智能体）的优化解耦，分别训练专家模型后再融合为一个接近帕累托最优的统一模型；整个过程基于自主研发的DORA系统，支持异步rollout的大规模强化学习，在数万个加速器上实现三倍以上训练加速。

Result: LongCat-Flash-Thinking在多个复杂推理任务中达到开源模型中的最先进水平；在AIME-25上将平均令牌消耗从19,653减少到6,965，降幅达64.5%，且未牺牲任务准确性；模型具备卓越的智能体推理效率。

Conclusion: LongCat-Flash-Thinking通过创新的冷启动策略、领域并行训练和高效的DORA强化学习框架，实现了高性能与高效率的平衡，是当前最先进的开源大规模推理模型之一，其发布有望推动推理系统和智能体AI的进一步发展。
...
TL;DR: 本文系统研究了视觉语言模型（VLMs）中的视觉空间推理（VSR），提出了空间智能的三个能力层级，并构建了包含近20个开源数据集的SIBench基准。实验表明，当前VLM在感知任务上表现良好，但在理解与规划任务上仍存在显著不足。


<details>
  <summary>Details</summary>
Motivation: 尽管VLM在视觉-语言任务中取得进展，但在实现人类水平的视觉空间推理方面仍面临挑战，尤其是在三维空间表征与推理方面。需要系统性评估和基准来推动该领域发展。

Method: 提出空间智能的三层能力分类（基础感知、空间理解、空间规划），并构建SIBench基准，涵盖23种任务设置下的近20个开源数据集；对现有VLM进行系统性评测。

Result: 当前最先进的VLM在基础感知任务上表现尚可，但在数值估计、多视角推理、时序动态和空间想象等理解和规划任务上表现不佳，暴露出感知与推理之间的显著差距。

Conclusion: 实现真正的空间智能仍面临重大挑战，本文提供了系统的分析框架和综合基准SIBench，为未来VSR研究指明方向。
...
TL;DR: 本文提出了一种名为DEAL的新框架，结合低秩适应（LoRA）与连续微调策略，有效缓解了传统微调中的灾难性遗忘和数据效率低下的问题，在15个数据集上表现出优越的任务准确性和资源效率。


<details>
  <summary>Details</summary>
Motivation: 传统微调方法存在灾难性遗忘和数据效率不足的问题，限制了其在实际应用中的效果，因此需要一种更高效、可持续的微调方法。

Method: 提出DEAL框架，结合低秩适应（LoRA）与连续微调策略，引入知识保留和自适应参数更新模块，以提升微调过程中的性能和效率。

Result: 在15个不同数据集上的实验表明，DEAL在任务准确性和资源效率方面均显著优于基线方法。

Conclusion: DEAL能够有效提升大语言模型的持续适应能力，在保持隐私保护效率的同时增强任务性能和资源利用率。
...
TL;DR: 本文首次全面综述了基于大语言模型（LLM）代理中的幻觉问题，提出了一种新的分类法，识别出在不同阶段出现的代理幻觉类型，并深入分析了导致这些幻觉的十八个触发原因。同时，文章总结了现有研究中缓解和检测幻觉的方法，并指出了未来研究的潜在方向。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM-based agents在多个领域展现出巨大潜力，但其存在的幻觉问题可能导致任务执行错误，影响系统可靠性。因此，亟需系统性地梳理和理解该问题以推动更可靠智能系统的发展。

Method: 通过仔细分析代理的完整工作流程，提出了一个新的分类体系，并对18个导致幻觉的根本原因进行了深入探讨；同时系统回顾了大量现有研究，归纳了幻觉检测与缓解方法。

Result: 建立了首个关于LLM-based agents中幻觉问题的综合调查，提出了新分类法和成因分析框架，总结了当前的应对策略。

Conclusion: 该综述有助于推动对LLM-based agents中幻觉问题的理解，为构建更稳健、可靠的代理系统提供了理论基础和未来研究方向。
...
TL;DR: 本文比较了四种剩余时间预测方法在航空物流出库流程中的应用，发现深度学习模型精度最高，但传统提升方法等浅层模型也能达到相近精度且计算资源需求更少。


<details>
  <summary>Details</summary>
Motivation: 为了提高物流仓储流程中对进行中流程执行的剩余时间预测准确性，并评估不同预测方法在真实场景下的性能与资源消耗平衡。

Method: 基于一家航空物流公司提供的包含169,523条轨迹的真实事件日志，对比了四种剩余时间预测方法，包括深度学习模型和传统提升技术等浅层方法。

Result: 深度学习模型预测精度最高，但传统提升等浅层方法表现也具竞争力，且所需计算资源显著更少。

Conclusion: 在实际应用场景中，尽管深度学习模型精度最优，但浅层方法因其高效性和较低资源消耗仍具有重要应用价值。
...
TL;DR: 本文提出了基于玩家感知的地标、纪念碑和信标概念，用于解决程序化生成内容自动分解与评估中的难题，旨在连接人文与技术游戏研究，提升PCG的计算评估能力。


<details>
  <summary>Details</summary>
Motivation: 算法在评估程序化生成内容时难以找到与人类体验一致的度量标准，尤其是在复合人工物方面。现有方法缺乏从玩家中心视角出发的可分解性概念。

Method: 结合游戏研究和游戏AI的研究成果，提出‘地标’、‘纪念碑’和‘信标’三个嵌套概念，基于可感知性、唤起性和行动召唤性进行定义，并探讨其在当前研究与工业技术下的可检测性与可评估性。

Result: 这些概念具有跨类型通用性，可在混合主动PCG和组合式PCG中实现自动化识别与评估，为PCG的自动分解及其显著子成分的评价提供了可行路径。

Conclusion: 该方法有助于桥接人文学科与技术游戏研究，推动更符合人类体验的PCG自动化评估发展。
...
TL;DR: 本文提出了一种新的因果表示学习框架，利用可观测的源变量作为辅助变量，通过保持体积的编码器识别潜在变量，扩展了现有方法的边界。


<details>
  <summary>Details</summary>
Motivation: 现有的因果表示学习方法通常假设辅助变量是外部的，限制了其应用范围。本文旨在通过引入可观测的源变量作为辅助变量来克服这一限制，以促进潜在因素的识别。

Method: 提出一个新框架，将可观测的源变量作为有效的条件变量，并使用保持体积的编码器来识别整个潜在变量，最多只能进行子空间变换和排列。当存在多个已知的辅助变量时，提供一种变量选择方案来最大化潜在因素的可恢复性。

Result: 实验结果表明，该框架在合成图数据和图像数据上有效，能够识别出潜在变量并提高潜在因素的可恢复性。

Conclusion: 本文提出的框架成功地利用了可观测的源变量作为辅助变量，增强了因果表示学习中潜在变量的识别能力，为未来的研究提供了新的方向。
...
TL;DR: 本文提出了一种名为CoPiC的新方法，通过生成多样化的高层规划程序并利用领域自适应批评器选择最符合长期奖励的计划，显著提高了大型语言模型在顺序决策任务中的规划效果，同时大幅降低了查询成本。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的规划方法依赖频繁查询和短期环境反馈进行迭代优化，导致查询成本高且难以生成与长期奖励一致的计划。

Method: CoPiC使用大语言模型生成多个高层规划程序来产生和迭代改进候选计划，并引入一个训练好的领域自适应批评器评估这些候选计划，选择最优者执行。

Result: 在ALFWorld、NetHack和StarCraft II Unit Building三个任务上的实验表明，CoPiC相比AdaPlanner和Reflexion等先进基线方法，平均提升了23.33%的成功率，并减少了91.27%的查询成本。

Conclusion: CoPiC通过结合高层规划程序和领域自适应批评器，在降低查询开销的同时实现了更优的长期规划性能，为LLM代理的高效决策提供了新思路。
...
TL;DR: 本文提出了一种名为AgentInit的多智能体系统初始化方法，通过引入自然语言到格式的机制和基于帕累托原则的平衡团队选择策略，优化智能体团队结构，提升系统效率与协作性能。


<details>
  <summary>Details</summary>
Motivation: 现有MAS初始化方法未能充分考虑后续阶段中智能体之间的协作需求，缺乏对团队构成有效性的系统设计。

Method: 在智能体生成过程中引入多轮交互与反思机制，结合自然语言到格式（NL2F）机制确保输出一致性，并采用基于帕累托最优的团队选择策略，综合考虑团队多样性与任务相关性。

Result: 实验表明，AgentInit在多种框架和任务下均优于当前最先进的初始化方法和预定义策略，性能最高提升1.2倍和1.6倍，显著降低token消耗，且具备良好的迁移能力。

Conclusion: AgentInit是一种高效、可迁移且适应性强的MAS初始化方法，能有效提升多智能体系统的整体性能与协作效率。
...
TL;DR: 本研究探讨了在阿拉伯世界中常识推理的跨文化迁移，利用13个阿拉伯国家的文化基础数据集，评估了几种轻量级对齐方法，发现少量文化特异性示例即可显著提升其他文化背景下的模型表现，并揭示了跨文化常识迁移的潜力。


<details>
  <summary>Details</summary>
Motivation: 大语言模型常带有西方中心主义偏见，限制其在多元文化背景下的应用；现有研究较少探索一种文化中的对齐效果能否迁移到其他文化中，尤其是在具有语言和历史相似性但存在本地文化差异的地区。

Method: 使用覆盖13个阿拉伯国家的文化嵌入式常识推理数据集，评估了上下文学习、基于示范的强化（DITTO）等轻量级对齐方法，并与监督微调和直接偏好优化等基线方法进行比较。

Result: 仅用一个国家的12个文化特异性示例，即可使多语言模型在其他国家的性能平均提升10%；此外，来自印度尼西亚和美国的跨文化示范在多项选择题推理任务中可达到甚至超过本地文化对齐的效果。

Conclusion: 研究表明高效的跨文化对齐是可行的，为适应低资源文化环境下的大语言模型提供了一条有前景的路径。
...
TL;DR: 本文提出使用图像聚焦的机器学习模型来评估Ulam螺旋中特定区域素数分布的规律性，发现靠近5亿附近的区域比2500万以下区域更易学习，表明高数值区域存在更明显的有序模式。


<details>
  <summary>Details</summary>
Motivation: 探索素数在Ulam螺旋中不同区域的规律性差异，并验证机器学习是否能作为数论研究的新工具。

Method: 使用图像化的机器学习模型，训练并比较在Ulam螺旋不同数值区域提取的块数据上的分类性能（精度、召回率等指标）。

Result: 模型在较高数值区域（约5亿附近）的准确率优于低数值区域（低于2500万），且精确率和召回率分析显示模型在不同区域采用不同的分类策略：低数区侧重识别素数模式，高数区侧重排除合数。

Conclusion: 机器学习可作为数论的实验工具，揭示素数分布随数量级增加而趋于规律化的趋势，支持数论中关于高阶素数分布噪声减弱、平均性质主导的猜想，并有望用于密码学中的强弱素数分析。
...
TL;DR: 本文提出了一种基于Wasserstein距离的联邦学习框架，用于在保护隐私的前提下评估和选择异构数据源，通过分布式方法近似计算Wasserstein距离，并结合神经缩放律预测模型性能，实验证明该方法能有效识别高性能的数据组合，推动可信的联邦学习模型市场发展。


<details>
  <summary>Details</summary>
Motivation: 在联邦学习环境中，如何有效评估和选择来自异构数据源的数据是构建可信数据市场的主要挑战。现有方法难以兼顾隐私保护与模型性能预测，缺乏对数据异质性与聚合算法兼容性的量化分析。

Method: 提出一种基于Wasserstein距离的估计器，采用分布式方式近似计算无需访问原始数据的Wasserstein距离，并结合神经缩放律外推模型在未见数据组合上的性能，从而实现高效且隐私安全的数据估值与选择。

Result: 在标签偏斜、错误标注和无标签等多种场景下的实验表明，该方法能稳定识别出高表现力的数据组合，显著优于现有数据选择策略，且无需完整训练即可准确预测模型性能。

Conclusion: 该框架为联邦学习中的数据估值提供了可靠解决方案，增强了模型市场的可追溯性和可信度，推动了隐私保护下数据共享与交易的发展。
...
TL;DR: 本文研究了在随机需求下连续时间库存动态的学习问题，比较了完全学习的Neural ODE与保留物理结构的Universal Differential Equation（UDE）在预测牛鞭效应中的表现。结果表明，在轻尾或自相关需求下，结构先验（UDE）显著提升预测性能；而在重尾需求下，灵活性更强的NODE表现更优。


<details>
  <summary>Details</summary>
Motivation: 探讨在不同需求环境下，结构先验是否有助于或阻碍对牛鞭效应的预测，解决结构化建模与数据驱动方法之间的权衡问题。

Method: 提出BULL-ODE框架，对比Neural ODE（完全数据驱动）与UDE（融合守恒律和‘order-up-to’策略的物理信息模型），在三种需求场景（AR(1)、i.i.d.高斯、重尾对数正态）下单层供应链中进行多步预测实验。

Result: 在AR(1)和高斯需求下，UDE显著优于NODE（如AR(1)下库存RMSE从4.92降至0.26）；在重尾需求下，NODE更具优势。随着训练数据减少，NODE出现相位漂移，UDE稳定但对极端值反应不足。

Conclusion: 当噪声轻尾或时间相关时应保留结构；当极端事件主导时应放松结构。该结论不仅适用于库存控制，也为科学与工程中的混合建模提供了指导原则。
...
TL;DR: 提出一种基于模型的迁移学习方法，利用神经网络代理模型实现桥梁间的知识转移，以支持大规模桥梁网络的可扩展监测。


<details>
  <summary>Details</summary>
Motivation: 解决大规模桥梁网络中结构健康监测的数据扩展性和效率问题，通过在相似结构间迁移知识来提升监测能力。

Method: 采用神经网络构建代理模型，并将其从一座桥梁迁移到另一座具有相似特征的桥梁；结合贝叶斯推断框架，利用监测数据中的模态特征进行持续损伤评估。

Result: 在两个实际桥梁数据上验证了方法的有效性，迁移后的模型对损伤位置、严重程度和范围均表现出高敏感性。

Conclusion: 该方法实现了跨结构的知识迁移，增强了实时监测能力，为桥梁网络层面的智能监测和韧性提升提供了可行方案。
...
TL;DR: 提出了一种名为AdaMixT的新架构，用于多变量时间序列预测，通过自适应加权融合多尺度专家模型，显著提升了预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖预定义的单尺度patch或缺乏有效的多尺度特征融合机制，限制了对时间序列复杂模式的捕捉能力。

Method: 引入多种patch，并结合通用预训练模型（GPM）和领域特定模型（DSM）进行多尺度特征提取，采用门控网络动态分配不同专家模型的权重，实现自适应的多尺度融合。

Result: 在包括Weather、Traffic、Electricity、ILI和四个ETT数据集在内的八个基准上进行了实验，结果表明AdaMixT在真实场景中具有显著的有效性。

Conclusion: AdaMixT通过自适应加权融合多尺度专家模型，有效提升了多变量时间序列预测的性能和泛化能力。
...
TL;DR: EASE是一个开源、模块化的框架，利用大语言模型进行迭代算法生成，集成了生成、测试、分析和评估的反馈循环，支持多LLM协同工作，简化了提示设计和模型管理的复杂性。


<details>
  <summary>Details</summary>
Motivation: 为了简化使用大语言模型生成算法解决方案的复杂性，并提供一个可重复、透明且可扩展的平台，使研究人员和实践者能够协作设计算法。

Method: EASE通过将生成、测试、分析和评估集成到一个反馈循环中，利用多个大语言模型分别担任生成器、分析器和评估器等互补角色，实现算法的迭代优化。

Result: EASE提供了一个开放、模块化、可控制错误处理与质量评估的框架，支持跨领域的算法协同设计，提升了生成过程的透明度和可扩展性。

Conclusion: EASE为利用大语言模型进行算法生成提供了一个高效、灵活且易于使用的平台，推动了人机协同算法设计的发展。
...
TL;DR: 本文提出了一种基于AIS数据的机器学习管道，用于在海峡尺度上对移动船舶进行类型分类，仅使用轨迹级特征即实现了92.15%的准确率。


<details>
  <summary>Details</summary>
Motivation: 准确识别船舶类型对海上安全监管和打击非法、未报告和无管制（IUU）活动至关重要，而现有方法依赖人工或额外传感器，成本高且难以扩展。

Method: 基于八天AIS历史数据，通过数据清洗、轨迹分割和特征工程提取31个轨迹级特征（包括运动学、时空、地理和船型特征），采用按MMSI分组的五折交叉验证训练树模型，重点使用随机森林结合SMOTE采样。

Result: 在五个船舶类别（货船、油轮、客船、高速船、渔船）上，随机森林模型达到92.15%准确率，宏F1得分为93.27%，ROC-AUC高达0.9897；特征重要性分析显示桥位比和最大航速最具判别力，主要误分类发生在货船与油轮之间。

Conclusion: 仅利用AIS轨迹提取的轻量特征即可实现海峡区域船舶类型的高精度实时分类，具备实际应用价值，未来可通过DBSCAN轨迹分割和梯度提升集成进一步提升性能。
...
### [239] [Individualized non-uniform quantization for vector search](https://arxiv.org/abs/2509.18471)
*Mariano Tepper,Ted Willke*

Main category: cs.LG

TL;DR: 本文提出了一种名为NVQ的非均匀向量量化方法，用于高效压缩高维嵌入向量，在保持高保真度的同时显著提升检索精度和效率。


<details>
  <summary>Details</summary>
Motivation: 由于高维嵌入向量体积大，导致内存/存储开销高，现有向量搜索技术面临性能和成本挑战。

Method: 提出NVQ，利用新颖的、计算高效的非线性函数构建非均匀向量量化器，并为每个索引向量单独学习量化器。

Result: 实验结果表明，NVQ在最小计算开销下，相比现有最先进方法具有更高的准确性。

Conclusion: NVQ是一种在高保真场景下兼具计算和空间效率的高效向量压缩技术，适用于大规模语义相似性搜索。
...
### [240] [Localized PCA-Net Neural Operators for Scalable Solution Reconstruction of Elliptic PDEs](https://arxiv.org/abs/2509.18110)
*Mrigank Dhingra,Romit Maulik,Adil Rasheed,Omer San*

Main category: cs.LG

TL;DR: 提出了一种基于补丁的PCA-Net框架，通过将高维解场分解为小块并分别进行PCA降维，显著降低了神经算子学习中的计算复杂度，同时保持高精度，处理速度比全局PCA快3.7到4倍。


<details>
  <summary>Details</summary>
Motivation: 传统PCA在高维PDE解场中计算开销大，限制了数据驱动方法的效率。

Method: 采用局部补丁划分策略，设计两种patch-based PCA方法：local-to-global和local-to-local，并结合重叠补丁+平滑滤波或CNN两步精修机制优化性能。

Result: patch-based PCA显著降低计算成本，重建精度高，端到端处理时间减少3.7至4倍。

Conclusion: patch-based PCA-Net在保证精度的同时大幅提升计算效率，是PDE系统中高效算子学习的有前景方法。
...
### [241] [Prompt Optimization Meets Subspace Representation Learning for Few-shot Out-of-Distribution Detection](https://arxiv.org/abs/2509.18111)
*Faizul Rakib Sayem,Shahana Ibrahim*

Main category: cs.LG

TL;DR: 提出一种基于上下文优化（CoOp）的框架，结合子空间表示学习与提示调优，提升开放世界中AI系统对分布外输入的检测能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于提示学习的OOD检测方法仅依赖softmax概率，忽略了视觉-语言模型中丰富特征嵌入的判别潜力。

Method: 通过将ID特征投影到由提示向量张成的子空间，并将无关特征投影到正交零空间，结合端到端的学习准则进行训练。

Result: 在真实世界数据集上的实验表明，该方法在保持高ID分类精度的同时，显著提升了OOD检测性能。

Conclusion: 所提出的框架有效利用了VLM的特征表示能力，实现了更可靠的开放世界OOD检测。
...
### [242] [Large language models surpass domain-specific architectures for antepartum electronic fetal monitoring analysis](https://arxiv.org/abs/2509.18112)
*Sheng Wong,Ravi Shankar,Beth Albert,Gabriel Davis Jones*

Main category: cs.LG

TL;DR: 本研究首次全面比较了最先进的AI方法在产前电子胎儿监护（CTG）分析中的表现，发现微调后的大语言模型（LLMs）优于基础模型和领域专用架构，为临床CTG解读提供了新途径。


<details>
  <summary>Details</summary>
Motivation: 尽管大模型在医疗领域表现出色，但其在电子胎儿监护（CTG）分析中的潜力尚未充分探索。由于胎心率模式复杂且依赖主观判读，亟需更可靠的自动化分析方法。

Method: 系统比较了时间序列基础模型、大语言模型与专用CTG模型，在超过500例真实临床CTG记录上进行评估，涵盖不同时长的多范式建模性能。

Result: 微调后的大语言模型在产前CTG分析中表现最优，显著优于其他模型，展现出更高的诊断准确性和稳定性。

Conclusion: 大语言模型为自动化CTG解读提供了有前景的解决方案，研究结果为未来围产期护理中AI技术的发展奠定了基础。
...
### [243] [A Study of Skews, Imbalances, and Pathological Conditions in LLM Inference Deployment on GPU Clusters detectable from DPU](https://arxiv.org/abs/2509.18114)
*Javed I. Khan an Henry Uwabor Moye*

Main category: cs.LG

TL;DR: 该研究探讨了在基于Transformer的大语言模型中，自回归推理阶段由于GPU分片间负载不平衡导致的效率问题，并提出利用BlueField-3 DPU辅助框架实时检测和缓解多节点张量并行推理中的负载不均。


<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在解码阶段因GPU间负载不平衡导致的吞吐量下降和延迟增加问题，提升推理效率。

Method: 通过将监控任务卸载到DPU，分析GPU遥测数据和节点间通信模式，实现对负载不平衡的实时检测与反馈。

Result: 系统能够有效识别多GPU执行中的负载倾斜和异常情况，评估其对性能的影响，并验证DPU网络在跟踪和缓解这些问题上的可行性。

Conclusion: DPU辅助框架有望显著改善大规模语言模型在多节点张量并行推理中的负载均衡与整体性能。
...
### [244] [Towards Scalable and Structured Spatiotemporal Forecasting](https://arxiv.org/abs/2509.18115)
*Hongyi Chen,Xiucheng Li,Xinyang Chen,Jing Li,Kehai Chen,Liqiang Nie*

Main category: cs.LG

TL;DR: 提出了一种新的空间平衡注意力模块，用于时空预测，通过子图内和子图间注意力机制平衡局部与全局空间相关性，并构建多尺度模型，在真实数据集上实现了最高7.7%的性能提升且运行成本低。


<details>
  <summary>Details</summary>
Motivation: 在时空预测中，需要平衡空间邻近性和全局相关性，传统方法难以同时有效捕捉局部和全局空间依赖。

Method: 提出空间平衡注意力模块，将空间图划分为子图，使用子图内注意力学习局部相关性，通过子图聚合和子图间注意力捕获全局相关性，并构建多尺度时空预测模型。

Result: 在中到大规模真实时空数据集上验证了模型的有效性和效率，相比基线方法性能最高提升7.7%，且运行成本低。

Conclusion: 所提方法能有效建模结构化空间相关性，具有良好的可扩展性和实用性，适用于高效时空预测任务。
...
### [245] [Amortized Latent Steering: Low-Cost Alternative to Test-Time Optimization](https://arxiv.org/abs/2509.18116)
*Nathan Egbuna,Saatvik Gaur,Sunishchal Dev,Ashwinee Panda,Maheep Chaudhary*

Main category: cs.LG

TL;DR: 提出Amortized Latent Steering (ALS)，将测试时优化的迭代过程压缩为一次离线计算，在保持甚至超越迭代方法性能的同时，实现2-5倍的速度提升，显著改善效率与准确率的权衡。


<details>
  <summary>Details</summary>
Motivation: 现有的测试时优化方法（如迭代 refinement 和多步验证）因每次查询需要大量计算而难以规模化；隐空间优化方法（如LatentSeek）虽更直接，但仍需昂贵的每查询优化循环。因此需要一种高效、低成本的替代方案。

Method: ALS通过计算成功与失败生成样本在隐状态上的均值差异，得到一个校准方向向量，并在推理时以恒定代价应用该向量，引导模型隐表示回到“成功流形”附近，从而避免每查询迭代优化。

Result: 在GSM8K和MATH-500基准上，ALS相比迭代方法实现2-5倍加速，性能匹配或超过贪婪CoT和Self-Consistency基线，效率-准确率权衡最高提升101%。

Conclusion: 隐空间测试时优化的大部分收益可通过离线计算捕获，ALS使复杂推理技术在生产环境中部署成为可能。
...
### [246] [Robust and continuous machine learning of usage habits to adapt digital interfaces to user needs](https://arxiv.org/abs/2509.18117)
*Eric Petit,Denis Chêne*

Main category: cs.LG

TL;DR: 提出一种基于贝叶斯统计的机器学习方法，用于动态适应不同用户和使用策略的数字界面设计。


<details>
  <summary>Details</summary>
Motivation: 为了提升用户体验，使系统能够根据个体用户的浏览习惯而非群体偏好进行自适应调整。

Method: 采用贝叶斯统计建模用户行为，结合在线增量学习，实现对用户任务的持续学习与先验知识的保留，并生成带有使用统计信息的导航图模型。

Result: 仿真结果表明该方法在静态和非静态环境中均能有效进行可靠预测，即使数据量少或环境变化也能保持性能。

Conclusion: 该研究为开发能帮助用户更高效导航和操作界面的自适应系统提供了可行路径。
...
### [247] [Decentor-V: Lightweight ML Training on Low-Power RISC-V Edge Devices](https://arxiv.org/abs/2509.18118)
*Marcelo Ribeiro,Diogo Costa,Gonçalo Moreira,Sandro Pinto,Tiago Gomes*

Main category: cs.LG

TL;DR: 本文提出了一种针对RISC-V架构微控制器的8位量化轻量级随机梯度下降（L-SGD）算法，实现了高效、低内存的设备端训练，显著提升了训练速度并减少了内存使用，同时保持了模型精度。


<details>
  <summary>Details</summary>
Motivation: 由于大多数物联网设备缺乏GPU或专用加速器，设备端训练通常不可行，依赖云端训练会带来隐私和连接性问题。联邦学习虽提供了解决方案，但需要高效的优化算法。RISC-V架构尚缺乏对设备端训练的良好支持，因此需要适配和优化。

Method: 将适用于Arm Cortex-M的轻量级SGD（L-SGD）扩展到RISC-V架构，并引入8位量化版本以应对RISC-V MCU缺乏浮点单元（FPU）带来的性能瓶颈。在Arm和RISC-V平台上使用32位浮点数进行评估，并对比8位量化后的性能提升。

Result: 8位量化的L-SGD在RISC-V上实现了近4倍的内存占用减少和2.2倍的训练速度提升，且精度损失可忽略不计。

Conclusion: 该工作为RISC-V架构的MCU实现了高效的设备端机器学习训练，推动了在资源受限设备上部署联邦学习的可行性，具有良好的隐私保护和离线运行优势。
...
### [248] [MobileRL: Online Agentic Reinforcement Learning for Mobile GUI Agents](https://arxiv.org/abs/2509.18119)
*Yifan Xu,Xiao Liu,Xinghan Liu,Jiaqi Fu,Hanchen Zhang,Bohao Jing,Shudan Zhang,Yuting Wang,Wenyi Zhao,Yuxiao Dong*

Main category: cs.LG

TL;DR: 本文提出了一种名为MOBILERL的在线代理强化学习框架，用于提升移动设备上图形用户界面（GUI）代理的性能，其核心算法ADAGRPO通过难度自适应正向回放、失败课程筛选和最短路径奖励调整策略，显著提高了训练稳定性与样本效率，并在多个基准上实现了最先进的成功率。


<details>
  <summary>Details</summary>
Motivation: 由于任务难度分布呈重尾特性以及大规模环境采样效率低下，使用强化学习开发高效的移动GUI代理仍然具有挑战性。

Method: 提出了MOBILERL框架及其核心算法ADAGRPO，包含难度自适应正向回放、失败课程过滤和基于任务长度的最短路径奖励重塑策略，以提升强化学习训练效果。

Result: 在AndroidWorld和AndroidLab两个基准上，基于Qwen2.5-VL-7B-Instruct和GLM-4.1V-9B-Base构建的MOBILERL-9B模型分别取得了75.8%和46.8%的成功率，达到SOTA水平。

Conclusion: MOBILERL框架有效提升了移动GUI代理在多样化应用和任务中的性能，具备高样本效率和强泛化能力，已应用于AutoGLM产品并开源。
...
### [249] [A Coopetitive-Compatible Data Generation Framework for Cross-silo Federated Learning](https://arxiv.org/abs/2509.18120)
*Thanh Linh Nguyen,Quoc-Viet Pham*

Main category: cs.LG

TL;DR: 本文提出了CoCoGen，一个结合生成式AI和博弈论的框架，用于在统计异质和经济竞争环境下优化跨组织联邦学习中的协作行为，提升社会福利。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注统计异质性，忽略了组织间经济竞争对参与意愿的影响，且二者共同作用对系统整体效益的影响尚不明确。

Method: 利用生成式AI生成数据，结合势博弈理论建模每轮训练为加权势博弈，通过效用和性能公式刻画竞争与异质性，并设计最大化社会福利的数据生成策略。

Result: 在Fashion-MNIST上的实验表明，不同异质性和竞争水平影响组织行为，且CoCoGen在提升社会福利方面 consistently 优于基线方法。

Conclusion: CoCoGen有效应对了跨组织联邦学习中竞争与异质性的双重挑战，通过博弈建模和生成式AI实现了更优的协作学习效果。
...
### [250] [Prediction of Coffee Ratings Based On Influential Attributes Using SelectKBest and Optimal Hyperparameters](https://arxiv.org/abs/2509.18124)
*Edmund Agyemang,Lawrence Agbota,Vincent Agbenyeavu,Peggy Akabuah,Bismark Bimpong,Christopher Attafuah*

Main category: cs.LG

TL;DR: 本研究利用监督机器学习算法，结合用户评论中的文本和数值特征预测咖啡评分，发现集成方法和多层感知机表现最佳。


<details>
  <summary>Details</summary>
Motivation: 为了提升咖啡品质评估的客观性和效率，探索基于用户评论数据的自动化评分预测方法。

Method: 采用TF-IDF进行文本特征提取，使用SelectKBest进行特征选择，并训练六种机器学习模型（决策树、K近邻、多层感知机、随机森林、额外树和XGBoost），通过超参数优化和F1-score、G-mean、AUC等指标评估模型性能。

Result: 集成方法（如随机森林、额外树、XGBoost）和多层感知机在各项指标上显著优于决策树和K近邻等简单分类器，表明特征选择与超参数调优对模型性能至关重要。

Conclusion: 该研究验证了机器学习在感官产品评价中的有效性，为传统专家品鉴提供了可重复、数据驱动的补充方法。
...
### [251] [NurseSchedRL: Attention-Guided Reinforcement Learning for Nurse-Patient Assignment](https://arxiv.org/abs/2509.18125)
*Harsha Koduri*

Main category: cs.LG

TL;DR: 提出了一种基于强化学习的护士-患者分配框架NurseSchedRL，通过结合结构化状态编码、约束动作掩码和注意力机制，在满足现实约束的同时动态适应环境变化，显著提升了调度效率、技能匹配度并降低了护士疲劳。


<details>
  <summary>Details</summary>
Motivation: 传统优化和启发式排班方法难以应对医疗系统中护士资源分配的多约束、动态环境问题，包括技能异质性、患者病情严重程度、护士疲劳和护理连续性等挑战。

Method: 提出NurseSchedRL框架，采用近端策略优化（PPO）算法，引入可行性掩码以确保满足实际约束，并结合注意力机制对技能、疲劳和地理情境进行建模，实现动态调整。

Result: 在真实数据模拟中，NurseSchedRL相比基线启发式方法和无约束强化学习方法，表现出更高的调度效率、更好的技能-需求匹配以及更低的护士疲劳水平。

Conclusion: 强化学习在复杂、高风险的医疗人力资源管理决策支持中具有显著应用潜力。
...
### [252] [Anomaly Detection in Electric Vehicle Charging Stations Using Federated Learning](https://arxiv.org/abs/2509.18126)
*Bishal K C,Amr Hilal,Pawan Thapa*

Main category: cs.LG

TL;DR: 本文研究了联邦学习（FL）在电动汽车充电站（EVCS）异常检测中的应用，评估了其在系统和数据异构环境下的性能，发现FedAvgM在非独立同分布数据和系统异构情况下表现优于FedAvg，具有更好的收敛性和检测精度。


<details>
  <summary>Details</summary>
Motivation: 现有的基于联邦学习的入侵检测系统（IDS）评估忽略了实际应用中的系统异构和非独立同分布（non-IID）数据等挑战，而电动汽车充电站作为关键物联网设施亟需兼顾隐私保护与安全检测的有效方案。

Method: 采用FedAvg和FedAvgM两种主流优化方法，在模拟EVCS的物联网环境中进行实验，评估其在IID和非IID数据分布以及系统异构条件下的异常检测性能。

Result: 在IID条件下，FedAvg的表现优于集中式模型；但在非IID和系统异构环境下性能下降，而FedAvgM在这些情况下始终优于FedAvg，表现出更强的鲁棒性、更高的检测准确率和更好的收敛性。

Conclusion: 联邦学习能够在不显著损失性能的前提下应对物联网环境中EVCS的异构性挑战，其中FedAvgM是实现隐私保护且鲁棒的安全方案的有力候选方法。
...
### [253] [Safe-SAIL: Towards a Fine-grained Safety Landscape of Large Language Models via Sparse Autoencoder Interpretation Framework](https://arxiv.org/abs/2509.18127)
*Jiaqi Weng,Han Zheng,Hanyu Zhang,Qinqin He,Jialing Tao,Hui Xue,Zhixuan Chu,Xiting Wang*

Main category: cs.LG

TL;DR: 本文提出了Safe-SAIL框架，用于解释大语言模型中稀疏自编码器（SAE）的安全相关特征，以提升对安全关键行为的机制性理解，并发布包含SAE检查点和人类可读神经元解释的工具包。


<details>
  <summary>Details</summary>
Motivation: 现有LLM安全研究多局限于输出评估或特定任务，难以应对广泛且未定义的风险；而现有SAE应用缺乏对细粒度安全概念的解释能力，无法有效识别毒性生成等高风险行为。

Method: 提出Safe-SAIL框架，系统性识别具备最佳概念特异性可解释性的SAE，解释安全相关神经元，并引入高效策略以扩展解释规模。

Result: 实现了对LLM中安全相关特征的细粒度解释，提升了对高风险行为的识别能力，并开发了可扩展的解释方法。

Conclusion: Safe-SAIL有助于深化对LLM安全机制的理解，所发布的工具包将支持未来LLM安全风险的实证研究。
...
### [254] [Accounting for Uncertainty in Machine Learning Surrogates: A Gauss-Hermite Quadrature Approach to Reliability Analysis](https://arxiv.org/abs/2509.18128)
*Amirreza Tootchi,Xiaoping Du*

Main category: cs.LG

TL;DR: 提出了一种基于Gauss-Hermite求积的解耦方法，用于在物理可靠性分析中同时处理由模型近似误差引起的认知不确定性和输入参数的随机不确定性。


<details>
  <summary>Details</summary>
Motivation: 机器学习代理模型被广泛用于替代昂贵的物理模型，但其引入的认知不确定性与输入的随机不确定性耦合，影响可靠性预测的准确性。

Method: 采用Gauss-Hermite求积法，结合一次和二次可靠性方法（First and Second Order Reliability Methods）评估在随机不确定性下的条件失效概率，并在整个认知不确定性空间上进行积分以实现不确定性解耦。

Result: 三个算例表明，该方法在保持计算效率的同时，比忽略模型不确定性的传统方法提供了更可靠的预测结果。

Conclusion: 所提出的Gauss-Hermite求积方法能有效解耦认知与随机不确定性，提升基于机器学习代理模型的可靠性分析的准确性与可信度。
...
### [255] [Research on Metro Transportation Flow Prediction Based on the STL-GRU Combined Model](https://arxiv.org/abs/2509.18130)
*Zijie Zhou,Huichen Ma*

Main category: cs.LG

TL;DR: 本文提出了一种结合STL分解和门控循环单元（GRU）的地铁换乘客流预测模型，通过Keras构建GRU网络，利用图深度优先搜索识别出行路径，并采用STL分解与3σ原则处理时间序列数据，有效提升了工作日、周五及休息日的预测精度。


<details>
  <summary>Details</summary>
Motivation: 为了提高地铁系统内部换乘客流预测的准确性，支持智能化运营决策，弥补现有预测模型在精度和稳定性上的不足。

Method: 提出STL-GRU组合模型：首先使用图的深度优先搜索算法从刷卡数据中识别乘客路径并构建换乘客流时间序列；接着用STL方法将时间序列分解为趋势、周期和残差分量，并基于3σ原则清洗残差中的异常值；最后利用Keras搭建GRU网络进行预测。

Result: 在实际地铁站点数据上的实验表明，相较于LSTM、GRU以及STL-LSTM模型，所提出的STL-GRU模型在工作日（除周五）、周五和休息日的预测MAPE分别至少降低了2.3、1.36和6.42个百分点，显著提升了预测精度。

Conclusion: STL-GRU模型能更有效地捕捉换乘客流的时间序列特征，具有更高的预测准确性和鲁棒性，为地铁运营优化提供了可靠的技术支持。
...
### [256] [Two ways to knowledge?](https://arxiv.org/abs/2509.18131)
*Jean-Michel Tucny,Abhisek Ganguly,Santosh Ansumali,Sauro Succi*

Main category: cs.LG

TL;DR: Transformer模型在解决物理问题时表现出权重矩阵的随机性，暗示机器学习与科学方法可能是两种不同的知识获取路径。


<details>
  <summary>Details</summary>
Motivation: 探讨机器学习模型（特别是Transformer）在物理应用中权重矩阵的随机性及其对可解释性的影响。

Method: 分析两个典型物理应用中Transformer模型的权重矩阵，并将其操作类比于广义路径积分技术。

Result: 发现权重矩阵呈现随机特征，缺乏与物理问题结构的直接对应关系；类比路径积分虽能部分解释该现象，但未能解决可解释性的根本矛盾。

Conclusion: 机器学习可能提供有效的预测工具，但缺乏科学洞察力，强调了‘知识获取’与‘真正理解’之间的区别。
...
### [257] [Self-Evolving LLMs via Continual Instruction Tuning](https://arxiv.org/abs/2509.18133)
*Le Huang,Jiazheng Kang,Cheng Hou,Zhe Zhao,Zhenxiang Yan,Chuan Shi,Ting Bai*

Main category: cs.LG

TL;DR: 提出MoE-CL，一种参数高效的对抗性混合专家框架，用于工业级大语言模型的持续指令微调，有效缓解灾难性遗忘并支持知识迁移。


<details>
  <summary>Details</summary>
Motivation: 现有持续学习方法在处理新任务时容易遗忘旧任务知识，难以满足工业场景中大模型持续演进的需求。

Method: 采用双专家设计：每个任务配备独立的LoRA专家以保留特定知识，同时引入共享LoRA专家促进跨任务迁移；结合任务感知判别器（GAN机制）过滤无关信息，通过对抗学习提升共享专家的泛化能力。

Result: 在MTL5和腾讯自研Tencent3基准上表现优越，在腾讯视频内容合规审核的A/B测试中降低人工审核成本15.3%。

Conclusion: MoE-CL在大规模工业部署中具有实用价值，能有效平衡知识保持与跨任务泛化，支持模型自我进化。
...
### [258] [A Weighted Gradient Tracking Privacy-Preserving Method for Distributed Optimization](https://arxiv.org/abs/2509.18134)
*Furan Xie,Bing Liu,Li Chai*

Main category: cs.LG

TL;DR: 本文研究了分布式优化中的隐私保护问题，揭示了梯度追踪技术存在的隐私泄露风险，并提出了一种带衰减权重因子的加权梯度追踪算法以消除该风险。算法在时变异构步长下仍能收敛至最优解，仿真验证了其在分布式估计和卷积神经网络训练中的有效性。


<details>
  <summary>Details</summary>
Motivation: 在分布式优化中，梯度追踪技术虽提升了收敛速度，但存在隐私泄露风险。本文旨在揭示这一风险并提出既能保护隐私又保持收敛性的新算法。

Method: 提出一种加权梯度追踪的隐私保护算法，通过引入衰减权重因子防止隐私泄露，并在时变异构步长条件下分析其收敛性。

Result: 理论证明所提算法在温和假设下可精确收敛至最优解，数值实验在分布式估计和CNN训练任务中验证了算法的有效性。

Conclusion: 所提出的加权梯度追踪算法成功消除了传统方法的隐私泄露风险，同时保持了良好的收敛性能，适用于需要隐私保护的分布式优化场景。
...
### [259] [SDGF: Fusing Static and Multi-Scale Dynamic Correlations for Multivariate Time Series Forecasting](https://arxiv.org/abs/2509.18135)
*Shaoxun Wang,Xingjun Zhang,Qianyang Li,Jiawei Cao,Zhendong Tan*

Main category: cs.LG

TL;DR: 提出了一种新的静态-动态图融合网络（SDGF），通过双路径图结构学习方法捕捉多尺度时间序列间的相关性，结合静态图和动态图的优势，并利用注意力门控模块融合信息，实验证明了模型的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在建模多变量时间序列的多尺度依赖关系方面存在局限，难以捕捉其复杂且动态变化的相关性。

Method: 采用基于先验知识的静态图锚定长期稳定依赖，结合多级小波分解提取多尺度特征构建自适应动态图，并设计注意力门控模块融合双图信息，使用多核空洞卷积网络深化时序模式理解。

Result: 在多个真实世界基准数据集上的实验表明，该模型在多变量时间序列预测任务中显著优于现有方法。

Conclusion: SDGF能有效捕捉多尺度、动态演化的跨序列相关性，为多变量时间序列预测提供了新的解决方案。
...
### [260] [From Parameters to Performance: A Data-Driven Study on LLM Structure and Development](https://arxiv.org/abs/2509.18136)
*Suqing Wang,Zuchao Li,Luohe Shi,Bo Du,Hai Zhao,Yun Li,Qianren Wang*

Main category: cs.LG

TL;DR: 本文提出一个大规模数据集，涵盖多种开源大语言模型结构及其在多个基准上的性能，通过数据挖掘分析模型结构配置与性能之间的关系，并结合机制可解释性技术验证结果，旨在为未来大模型的优化提供数据驱动的指导。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在各领域取得显著进展，但关于结构配置如何影响性能的系统性、数据驱动研究仍然缺乏，因此需要构建数据集并进行系统分析。

Method: 构建了一个包含多种开源大语言模型结构及其多基准性能的大规模数据集，结合历史发展回顾与趋势预测，采用数据挖掘方法分析不同结构选择对性能的影响，并使用机制可解释性技术进行验证。

Result: 揭示了不同结构配置与模型性能之间的定量关系，发现某些结构特征在特定任务中具有显著影响，并通过机制可解释性技术进一步支持这些发现。

Conclusion: 该研究为理解大语言模型结构与性能的关系提供了数据驱动的实证基础，有助于指导未来模型的针对性设计与优化。
...
### [261] [LoRALib: A Standardized Benchmark for Evaluating LoRA-MoE Methods](https://arxiv.org/abs/2509.18137)
*Shaoheng Wang,Yao Lu,Yuqi Li,Yaxin Gao,Jiaqi Nie,Shanqing Yu,Yingli Tian,Qi Xuan*

Main category: cs.LG

TL;DR: 本文提出了一个统一的LoRA基准LoRALib，标准化了40个下游任务的数据集、超参数和评估方法，并基于此对多种LoRA-MoE方法进行了大规模实验，发现LoRAMoE表现最佳，且选择与目标任务相关的LoRA可进一步提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有的LoRA-MoE方法在模型、数据集、超参数和评估方式上缺乏统一标准，导致难以进行公平比较，因此需要建立统一基准以促进研究发展。

Method: 构建了一个包含40个下游任务、17种模型架构和680个LoRA模块的统一基准LoRALib，并使用OpenCompass工具对三种典型的LoRA-MoE方法及不同LoRA选择机制进行大规模评估。

Result: 实验表明LoRAMoE性能最优，且优先选择与目标任务相关的LoRA模块能进一步提升MoE系统的性能。

Conclusion: 统一的基准LoRALib有助于推动LoRA-MoE方法的公平比较与研究进展，未来的工作可在此基础上优化专家选择机制和跨任务泛化能力。
...
### [262] [Rank-Induced PL Mirror Descent: A Rank-Faithful Second-Order Algorithm for Sleeping Experts](https://arxiv.org/abs/2509.18138)
*Tiantian Zhang*

Main category: cs.LG

TL;DR: 提出了一种名为Rank-Induced Plackett-Luce Mirror Descent (RIPLM)的新算法，首次在睡眠专家设置中同时实现了rank-faithful和方差自适应。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理专家身份时未能保持与rank基准的结构等价性，且缺乏对分布变化的适应性。

Method: 利用rank基准与分布基准之间的结构等价性，直接在rank-induced Plackett-Luce参数化空间中进行更新，确保每轮策略分布始终属于rank-induced分布类。

Result: RIPLM是首个在睡眠专家设置中同时满足rank-faithful和方差自适应的算法，理论上保证了与rank基准的等价性。

Conclusion: RIPLM通过新的参数化方式，在保持结构一致性的同时实现了更好的自适应性能，为相关问题提供了新的解决思路。
...
### [263] [Comparative Analysis of FOLD-SE vs. FOLD-R++ in Binary Classification and XGBoost in Multi-Category Classification](https://arxiv.org/abs/2509.18139)
*Akshay Murthy,Shawn Sebastian,Manil Shangle,Huaduo Wang,Sopam Dasgupta,Gopal Gupta*

Main category: cs.LG

TL;DR: 本研究比较了规则-based分类器FOLD-SE与FOLD-R++和XGBoost在二分类和多类别分类中的性能，发现FOLD-SE在保持高可解释性的同时，在准确性和效率方面表现优异，尤其在多类别任务中优于XGBoost。


<details>
  <summary>Details</summary>
Motivation: 为了平衡机器学习模型的准确性、效率与可解释性，解决传统模型（如神经网络）在可解释性方面的不足，研究者希望评估新兴规则-based方法FOLD-SE在不同分类任务中的表现。

Method: 通过在多个数据集上对比FOLD-SE、FOLD-R++和XGBoost的分类性能，使用准确率、F1分数和处理时间作为主要评价指标，分别进行二分类和多类别分类实验。

Result: FOLD-SE在二分类中优于FOLD-R++，规则更简洁，仅轻微损失准确率和处理效率；在多类别分类中，FOLD-SE比XGBoost更精确且高效，并生成可理解的规则集。

Conclusion: FOLD-SE在兼顾高可解释性的同时，表现出优越的分类性能，表明规则-based方法有望成为黑箱模型的可行替代方案。
...
### [264] [A Machine Learning Framework for Pathway-Driven Therapeutic Target Discovery in Metabolic Disorders](https://arxiv.org/abs/2509.18140)
*Iram Wajahat,Amritpal Singh,Fazel Keshtkar,Syed Ahmad Chan Bukhari*

Main category: cs.LG

TL;DR: 本研究提出了一种结合机器学习与基因无关的通路映射的新框架，用于识别2型糖尿病高风险个体并发现潜在治疗靶点，基于Pima印第安人数据集实现了78.43%的预测准确率，并提出了针对高风险人群的新型治疗策略。


<details>
  <summary>Details</summary>
Motivation: 针对代谢性疾病（尤其是2型糖尿病）在全球范围内的重大健康负担，特别是在遗传易感人群（如Pima印第安人）中更为显著的问题，亟需可解释、可扩展的精准医学方法来实现早期检测和靶向干预。

Method: 采用逻辑回归和t检验分析Pima印第安人数据集以识别T2DM关键预测因子，并结合主成分分析（PCA）构建机器学习预测模型；同时开发了一种基因无关的通路映射策略，将预测因子与胰岛素信号、AMPK和PPAR等关键信号通路关联，进而通过通路富集分析提出并验证潜在治疗策略。

Result: 模型整体准确率达78.43%，成功识别出与T2DM相关的关键预测因子及其涉及的生物学通路，提出了包括双效GLP-1/GIP受体激动剂、AMPK激活剂、SIRT1调节剂和植物化学物在内的潜在治疗方案，并通过通路富集分析进行了验证。

Conclusion: 该框架将预测性分析与生物学机制洞察相结合，为代谢性疾病的早期检测和精准干预提供了可解释且可扩展的解决方案，推动了面向高风险人群的精准医学发展。
...
### [265] [KM-GPT: An Automated Pipeline for Reconstructing Individual Patient Data from Kaplan-Meier Plots](https://arxiv.org/abs/2509.18141)
*Yao Zhao,Haoyue Sun,Yantian Ding,Yanxun Xu*

Main category: cs.LG

TL;DR: KM-GPT是首个完全自动化的AI驱动管道，可从Kaplan-Meier图中高精度、鲁棒且可重复地重建个体患者数据（IPD），无需人工干预，显著提升临床研究中的证据合成效率。


<details>
  <summary>Details</summary>
Motivation: 现有从Kaplan-Meier图重建个体患者数据的方法依赖手动数字化，易出错且难以扩展，限制了证据合成的效率和准确性。

Method: KM-GPT结合先进的图像预处理、基于GPT-5的多模态推理和迭代重建算法，构建混合推理架构，并通过用户友好的网页界面和集成AI助手实现无需编程的操作。

Result: 在合成和真实世界数据集上的评估表明，KM-GPT在IPD重建准确性方面表现优异；应用于胃癌免疫治疗试验的荟萃分析中，成功支持了基于生物标志物的亚组分析。

Conclusion: KM-GPT通过自动化和可扩展的解决方案，将传统的手动流程转变为高效、可重复的数据重建过程，推动临床研究向更精准的循证决策迈进。
...
### [266] [AdaSTI: Conditional Diffusion Models with Adaptive Dependency Modeling for Spatio-Temporal Imputation](https://arxiv.org/abs/2509.18144)
*Yubo Yang,Yichen Zhu,Bo Jiang*

Main category: cs.LG

TL;DR: 本文提出了一种基于条件扩散模型的新型时空数据插补方法AdaSTI，通过双向S4模型和噪声感知网络有效提取并利用多步扩散过程中的动态时空依赖关系，在真实数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在时空数据插补中存在误差累积问题，且忽略了不同扩散步骤下时空依赖关系在噪声数据中的变化性，需要更鲁棒的依赖建模机制。

Method: 提出AdaSTI框架，包含用于预插补的BiS4PI双向S4网络、提取条件信息的时空条件化网络（STC），以及带门控注意力机制的噪声感知时空网络（NAST），以捕捉扩散过程中变化的时空依赖。

Result: 在三个真实世界数据集上进行了广泛实验，AdaSTI在所有设置下均优于现有方法，插补误差最多降低46.4%。

Conclusion: AdaSTI通过自适应建模扩散过程中的时空依赖关系，显著提升了时空数据插补的准确性，验证了考虑噪声变化性和依赖动态性的重要性。
...
### [267] [Early Prediction of Multi-Label Care Escalation Triggers in the Intensive Care Unit Using Electronic Health Records](https://arxiv.org/abs/2509.18145)
*Syed Ahmad Chan Bukhari,Amritpal Singh,Shifath Hossain,Iram Wajahat*

Main category: cs.LG

TL;DR: 本研究提出了一种多标签分类框架，利用ICU前24小时的数据预测多种临床恶化迹象（如呼吸衰竭、血流动力学不稳定等），在MIMIC-IV数据集上使用XGBoost模型取得了良好的性能，具有临床可解释性和早期预警潜力。


<details>
  <summary>Details</summary>
Motivation: 传统早期预警系统（如SOFA、MEWS）局限于单一结局指标，难以捕捉重症患者复杂的多维度生理恶化过程，因此需要一种能同时预测多种临床恶化事件的新型预警框架。

Method: 基于MIMIC-IV数据库，定义24至72小时内的护理升级触发事件（CETs）为标签，从前24小时提取生命体征、实验室指标和人口统计学特征，采用多标签分类方法训练多个模型（包括XGBoost），评估其在各标签上的预测性能。

Result: 在85,242次ICU住院数据中，XGBoost模型表现最佳，对呼吸、血流动力学、肾脏和神经功能恶化的F1分数分别为0.66、0.72、0.76和0.62，显著优于基线模型；特征分析显示呼吸频率、血压和肌酐等临床相关参数最具预测价值。

Conclusion: 该多标签分类框架能够有效利用早期ICU数据实现对多种临床恶化事件的可解释性预测，具备作为早期预警系统的应用潜力，且无需复杂的时间序列或自然语言处理技术。
...
### [268] [ConceptFlow: Hierarchical and Fine-grained Concept-Based Explanation for Convolutional Neural Networks](https://arxiv.org/abs/2509.18147)
*Xinyu Mu,Hui Dou,Furao Shen,Jian Zhao*

Main category: cs.LG

TL;DR: ConceptFlow 是一种新的概念级可解释性框架，通过追踪卷积神经网络中概念在层间的出现与演化，揭示模型的内部推理路径。


<details>
  <summary>Details</summary>
Motivation: 现有方法大多忽视了单个滤波器的语义角色以及概念在层间动态传播的过程，难以准确反映模型的内部推理机制。

Method: 提出 ConceptFlow 框架，包含两个核心组件：概念注意力（将每个滤波器与高层语义概念关联）和概念通路（通过概念转移矩阵量化概念在滤波器间的传播与转换）。

Result: 实验结果表明，ConceptFlow 能生成语义上有意义的模型推理洞察，验证了其在解释决策行为方面的有效性。

Conclusion: ConceptFlow 通过建模分层概念通路，提供了对 CNN 内部逻辑的深入理解，支持生成更忠实且符合人类认知的解释。
...
### [269] [Sparse Training Scheme for Multimodal LLM](https://arxiv.org/abs/2509.18150)
*Kean Shi,Liang Chen,Haozhe Zhao,Baobao Chang*

Main category: cs.LG

TL;DR: 提出了一种基于稀疏表示的训练高效框架STS，包含视觉令牌压缩器和层动态跳过器，适用于多种MLLM架构。


<details>
  <summary>Details</summary>
Motivation: MLLMs在多模态数据下训练效率低，输入序列长且层间计算利用率低。

Method: 提出Sparse Training Scheme（STS），包括Visual Token Compressor压缩视觉令牌，以及Layer Dynamic Skipper动态跳过不必要的语言模型层。

Result: 在多个基准上验证了该方法的有效性和高效性，显著提升了训练效率。

Conclusion: STS框架能广泛适用于不同MLLM架构，有效降低训练开销，提升计算资源利用率。
...
### [270] [HyperNAS: Enhancing Architecture Representation for NAS Predictor via Hypernetwork](https://arxiv.org/abs/2509.18151)
*Jindi Lv,Yuhao Zhou,Yuxin Tian,Qing Ye,Wentao Feng,Jiancheng Lv*

Main category: cs.LG

TL;DR: 本文提出了一种名为HyperNAS的新型神经预测器范式，通过全局编码方案和共享超网络来增强架构表示学习，在少量样本场景下显著提升了神经架构搜索的效率和性能。


<details>
  <summary>Details</summary>
Motivation: 现有的神经预测器由于难以捕捉不同架构之间的复杂关系，导致泛化能力差，限制了神经架构搜索的效率。

Method: 提出HyperNAS，包含全局编码方案以捕获宏观结构信息，以及共享超网络以增强跨架构模式的学习，并设计动态自适应多任务损失以提高训练稳定性。

Result: 在五个代表性搜索空间（包括ViT）上的实验表明，HyperNAS在少样本情况下优于现有方法，例如在CIFAR-10上达到97.60%的top-1准确率，在ImageNet上达到82.4%，且所需样本至少减少5倍。

Conclusion: HyperNAS通过改进架构表示学习和训练稳定性，在多种搜索空间中实现了最先进的性能，尤其适用于样本稀缺的场景。
...
### [271] [WLFM: A Well-Logs Foundation Model for Multi-Task and Cross-Well Geological Interpretation](https://arxiv.org/abs/2509.18152)
*Zhenyu Qi,Qing Yu,Jichen Wang,Yun-Bo Zhao,Zerui Li,Wenjun Lv*

Main category: cs.LG

TL;DR: 提出了一种用于测井解释的基础模型WLFM，通过三阶段自监督预训练和多任务微调，在孔隙度估计和岩性分类上优于现有方法，并展现出地层感知能力和可重用的地质词汇。


<details>
  <summary>Details</summary>
Motivation: 测井解释面临工具响应异质性、信号噪声和标签稀缺的挑战，需要一种可扩展且可迁移的通用模型来提升地下表征能力。

Method: WLFM包含三个阶段：将测井曲线分块 tokenize 为地质token，采用掩码token建模和地层感知对比学习进行自监督预训练，以及少样本多任务微调。

Result: WLFM在孔隙度估计中达到0.0041 MSE，岩性分类准确率达74.13%；微调后进一步提升至0.0038 MSE和78.10%准确率；模型展现出地层感知、地质词汇学习和曲线重建能力，但在浅层和超深层存在系统性偏差。

Conclusion: WLFM是一种可扩展、可解释且可迁移的地质AI基础模型，为日志、地震和文本数据的多模态融合提供了基础。
...
### [272] [A deep reinforcement learning platform for antibiotic discovery](https://arxiv.org/abs/2509.18153)
...
TL;DR: 提出了一种名为ApexAmphion的深度学习框架，结合大规模蛋白语言模型与强化学习，用于从头设计具有广谱抗菌活性的肽类抗生素，实验验证显示高命中率和强效性。


<details>
  <summary>Details</summary>
Motivation: 由于抗菌素耐药性（AMR）日益严重，预计到2050年每年将导致多达1000万人死亡，迫切需要开发新型抗生素。

Method: 采用一个64亿参数的蛋白质语言模型，先在精选的肽序列数据上进行微调，再通过近端策略优化（PPO）结合最小抑菌浓度（MIC）预测分类器和可微分理化目标组成的复合奖励函数进行强化学习优化。

Result: 体外测试的100种设计肽全部表现出低MIC值（部分达到纳摩尔级别），99%具有广谱抗菌活性，且主要通过强效靶向细胞质膜杀灭细菌。

Conclusion: 该方法通过统一生成、评分和多目标优化于单一深度强化学习流程中，能够快速生成多样且高效的候选抗生素，为肽类抗生素开发提供了可扩展的平台，并可在数小时内实现对活性和可开发性的迭代优化。
...
### [273] [MiniCPM-V 4.5: Cooking Efficient MLLMs via Architecture, Data, and Training Recipe](https://arxiv.org/abs/2509.18154)
*Tianyu Yu,Zefan Wang,Chongyi Wang,Fuwei Huang,Wenshuo Ma,Zhihui He,Tianchi Cai,Weize Chen,Yuxiang Huang,Yuanqian Zhao,Bokai Xu,Junbo Cui,Yingjing Xu,Liqing Ruan,Luoyuan Zhang,Hanyu Liu,Jingkun Tang,Hongyuan Liu,Qining Guo,Wenhao Hu,Bingxiang He,Jie Zhou,Jie Cai,Ji Qi,Zonghao Guo,Chi Chen,Guoyang Zeng,Yuxuan Li,Ganqu Cui,Ning Ding,Xu Han,Yuan Yao,Zhiyuan Liu,Maosong Sun*

Main category: cs.LG

TL;DR: MiniCPM-V 4.5是一个8B参数的多模态大语言模型，通过改进架构、数据策略和训练方法，在高效性和性能上均优于现有主流模型。


<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型在训练和推理效率方面的瓶颈，提升其可扩展性和可用性。

Method: 提出统一的3D-Resampler架构用于图像和视频的紧凑编码，采用统一学习范式处理文档知识和文本识别，并设计混合强化学习策略以支持长短推理模式。

Result: 在OpenCompass评测中超越GPT-4o-latest等闭源模型及Qwen2.5-VL 72B等大型开源模型；在VideoMME基准上，仅用Qwen2.5-VL 7B的46.7%显存和8.7%推理时间即达到SOTA性能。

Conclusion: MiniCPM-V 4.5在显著降低资源消耗的同时实现了卓越的多模态理解能力，为高效MLLM提供了有效范例。
...
### [274] [Developing Training Procedures for Piecewise-linear Spline Activation Functions in Neural Networks](https://arxiv.org/abs/2509.18161)
*William H Patty*

Main category: cs.LG

TL;DR: 本文提出并比较了9种训练方法，用于探索具有参数化线性B样条激活函数的神经网络中的双重优化动态。实验结果显示，在前馈神经网络（FNN）中模型误差率最高降低94%，在卷积神经网络（CNN）中降低51%，相较于传统的ReLU模型有显著提升，但代价是增加了开发和训练复杂性以及模型延迟。


<details>
  <summary>Details</summary>
Motivation: 传统神经网络通常使用经验验证的静态激活函数（如ReLU、tanh或sigmoid），通过优化激活函数的形状，可以为神经元分配更优的激活函数，从而提高模型的参数效率和准确性。

Method: 采用参数化的线性B样条作为可学习的激活函数，结合9种不同的训练方法进行双重优化（即同时优化网络权重和激活函数形状），在FNN和CNN上进行实验验证。

Result: 在FNN中实现了最高94%的模型误差下降，在CNN中实现了51%的误差下降，显著优于传统ReLU模型。

Conclusion: 通过优化激活函数形状可以显著提升神经网络性能，尽管带来了更高的训练复杂性和推理延迟，但在精度和参数效率方面具有明显优势。
...
### [275] [A Simple and Reproducible Hybrid Solver for a Truck-Drone VRP with Recharge](https://arxiv.org/abs/2509.18162)
*Meraryslan Meraliyev,Cemil Turan,Shirali Kadyrov*

Main category: cs.LG

TL;DR: 提出一种结合ALNS和强化学习的混合求解器，用于优化卡车-无人机协同的末端配送路径，在显式电池管理下实现更短的完成时间。


<details>
  <summary>Details</summary>
Motivation: 在考虑无人机续航限制和充电要求的前提下，优化卡车与无人机协同配送的最后一公里路径规划，提升效率并减少总完成时间。

Method: 将基于ALNS的卡车路径（结合2/3-opt和Or-opt）与小型指针/注意力机制的强化学习策略相结合，调度无人机飞行任务；通过硬可行性掩码确保续航和充电约束，并使用精确的时间线模拟器计算真实完成时间。

Result: 在N=50、E=0.7、R=0.1的欧几里得实例上，平均完成时间为5.203±0.093，优于ALNS的5.349±0.038（提升2.73%），接近NN的5.208±0.124（仅差0.10%）；且在所有种子实验中均不逊于ALNS，三分之二情况下优于NN。分析显示该方法有效平衡了卡车等待与行驶时间。

Conclusion: 所提出的混合RL方法能高效处理带电池管理的无人机-卡车协同配送问题，在性能上优于传统启发式方法，接近神经网络方法，具备良好的实用性和可复现性。
...
### [276] [DSFT: Inspiring Diffusion Large Language Models to Comprehend Mathematical and Logical Patterns](https://arxiv.org/abs/2509.18164)
*Ranfei Chen,Ming Chen*

Main category: cs.LG

TL;DR: 提出了一种名为DSFT的扩散SFT策略，通过调整掩码策略和损失函数，提升扩散大语言模型在数学和逻辑任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有训练方法在数学和逻辑任务上缺乏对数值敏感性和顺序敏感性的充分理解，难以有效提升dLLMs在这类任务上的性能。

Method: 提出DSFT方法，改进掩码策略和损失函数，以引导模型学习数学和逻辑模式，并可与预训练、强化学习等方法结合。

Result: 在LLaDA和Dream系列模型上验证，小规模数据下数学问题提升5-10%，逻辑问题提升约2%。

Conclusion: DSFT是一种简单有效的训练策略，能显著提升dLLMs在数学和逻辑任务上的表现，且易于与其他方法集成，具有广泛适用性。
...
### [277] [MobiGPT: A Foundation Model for Mobile Wireless Networks](https://arxiv.org/abs/2509.18166)
*Xiaoqian Qi,Haoye Chai,Yong Li*

Main category: cs.LG

TL;DR: 本文提出了一种用于移动数据预测的基础模型MobiGPT，能够统一预测基站流量、用户应用使用和信道质量三种数据类型，通过软提示学习和时间掩码机制，在多种预测任务中表现出优异的准确性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的移动数据预测方法针对不同类型的数据采用定制化模型，导致在大规模异构网络中复杂性和部署成本增加，缺乏通用性和可扩展性。

Method: 设计了一个统一结构的基础模型MobiGPT，引入软提示学习以适应不同数据类型的特征，并采用时间掩码机制支持短期预测、长期预测和分布生成三种任务。

Result: 在超过10万样本的真实数据集上评估显示，MobiGPT相比现有模型在预测精度上分别提升了27.37%、20.08%和7.27%，并在未见场景中展现出超过21.51%的零样本/少样本性能提升。

Conclusion: MobiGPT作为一个通用基础模型，在多类型移动数据预测中表现出强大的泛化能力和迁移能力，显著降低了模型部署的复杂性，适用于未来大规模异构移动网络。
...
### [278] [PiMoE: Token-Level Routing for Integrating High-Precision Computation and Reasoning](https://arxiv.org/abs/2509.18169)
*Hengbo Xiao,Jingyuan Fan,Xin Tong,Jingzhao Zhang,Chao Lu,Guannan He*

Main category: cs.LG

TL;DR: 提出PiMoE架构，将计算与推理能力内生集成于神经网络中，通过token级路由实现高效、可解释、可扩展的多专家协同。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型难以内在支持高精度数值计算，主流多智能体方法存在通信开销大、效率低、扩展性差等问题。

Method: 设计物理隔离的混合专家系统（PiMoE），分别训练文本到计算模块、计算专家和路由器，在推理时通过路由器在token级别动态调度计算与推理任务。

Result: 在两类推理-计算任务上，PiMoE相比LLM微调和多智能体方法，在准确率、响应延迟、token使用量和GPU能耗方面均有显著提升。

Conclusion: PiMoE提供了一种高效、可解释且可扩展的新一代科学与工业智能系统的架构范式。
...
### [279] [FedIA: A Plug-and-Play Importance-Aware Gradient Pruning Aggregation Method for Domain-Robust Federated Graph Learning on Node Classification](https://arxiv.org/abs/2509.18171)
*Zhanting Zhou,KaHou Tam,Zeqin Wu,Pengzhao Sun,Jinbo Wang,Fengli Zhang*

Main category: cs.LG

TL;DR: 本文提出了一种名为FedIA的联邦图学习框架，通过“先投影后聚合”的策略解决领域偏移下的表示不兼容问题，利用服务器端的坐标筛选和轻量级正则化动量权重，在不增加上行流量和服务器内存的情况下，实现了更稳定、高效的模型收敛与更高准确率。


<details>
  <summary>Details</summary>
Motivation: 在存在领域偏移的联邦图学习中，客户端模型容易产生不兼容的表示，导致传统聚合方法不稳定且无效，其根本原因在于梯度信号中存在大量由领域特异性方差主导的噪声维度。

Method: 提出FedIA框架，采用“先投影”策略：第一阶段在服务器端使用top-ρ掩码保留最信息丰富的约5%梯度坐标；第二阶段引入轻量级的影响正则化动量权重以抑制异常客户端。该方法在聚合前对客户端更新进行去噪处理。

Result: 在同构（Twitch Gamers）和异构（Wikipedia）图数据上，FedIA相比九个强基线方法表现出更平稳、更稳定的收敛过程和更高的最终准确率，同时保持了最优的收敛速率O(σ²/√T)，且不增加上行通信开销和显著的服务器内存消耗。

Conclusion: FedIA通过聚焦于梯度信号的去噪而非调整聚合权重，有效应对了领域偏移带来的挑战，提供了一种高效、实用且可部署的联邦图学习解决方案。
...
### [280] [SBVR: Summation of BitVector Representation for Efficient LLM Quantization](https://arxiv.org/abs/2509.18172)
*Wonjun Bang,Jongseok Park,Hongseung Yu,Kyungmin Bin,Kyunghan Lee*

Main category: cs.LG

TL;DR: 本文提出了一种名为SBVR的新型大语言模型量化方法，通过非均匀表示点和定制CUDA内核实现高效、准确且硬件友好的快速推理。


<details>
  <summary>Details</summary>
Motivation: 现有PTQ方法在处理大语言模型权重的高斯样分布时存在精度或推理速度问题，需要一种兼顾压缩效率与硬件性能的量化方案。

Method: SBVR将权重映射到符合实际分布的非均匀表示点，并设计专用CUDA内核以在不需解压的情况下直接进行矩阵运算。

Result: 在4位量化下，SBVR相比FP16模型实现了2.21x-3.04x的端到端生成速度提升，并在困惑度和准确率上达到SOTA水平。

Conclusion: SBVR是一种高效、准确且硬件友好的大语言模型后训练量化方法，显著提升了量化模型的推理速度与性能。
...
### [281] [TurnBack: A Geospatial Route Cognition Benchmark for Large Language Models through Reverse Route](https://arxiv.org/abs/2509.18173)
...
TL;DR: 本文提出一个大规模基准来评估大语言模型（LLM）在地理空间路径认知方面的能力，构建了包含36000条路线的数据集，并提出了PathBuilder工具以实现自然语言与导航路径之间的双向转换，同时设计了新的评估框架和指标。实验表明，当前LLM在路径反向任务中表现不佳，普遍存在无法返回起点、生成路线不准确且置信度过高的问题。


<details>
  <summary>Details</summary>
Motivation: 现有研究对大语言模型在地理空间认知方面的能力探索不足，缺乏可量化的评估指标、高质量的评测数据集以及清晰的研究体系，限制了该领域的发展。

Method: 构建了一个包含全球12个大城市的36000条路线的大规模评估数据集；提出PathBuilder工具，实现自然语言指令与导航路径之间的相互转换；设计新的评估框架和量化指标，对11种最先进大语言模型在路径反向任务上的表现进行系统评估。

Result: 实验结果显示大多数大语言模型在路径反向任务中表现较差：生成的反向路径大多无法回到起点，且与最优路径差异较大；同时存在路线生成鲁棒性低、错误答案置信度高的问题。

Conclusion: 当前大语言模型在地理空间路径认知方面存在显著局限，特别是在路径反向等复杂任务上表现不佳，亟需提升其空间推理能力与对地理信息的理解精度。
...
### [282] [Conversational Orientation Reasoning: Egocentric-to-Allocentric Navigation with Multimodal Chain-of-Thought](https://arxiv.org/abs/2509.18200)
*Yu Ti Huang*

Main category: cs.LG

TL;DR: 提出多模态思维链（MCoT）框架，用于解决对话系统中以自我为中心的空间表达向绝对方向的转换问题，在真实环境投影的繁体中文导航场景中实现了接近完美的定位准确率。


<details>
  <summary>Details</summary>
Motivation: 在GPS信号弱且缺乏详细地图的室内或复杂设施中，实现从以自我为中心（如“我的右边”）到绝对方向（N/E/S/W）的空间理解对对话代理至关重要；现有方法在多模态空间推理方面探索不足，尤其在非英语和ASR转录场景下。

Method: 提出Conversational Orientation Reasoning (COR)基准，并设计多模态思维链（MCoT）框架，结合ASR语音与地标坐标，通过三步结构化推理：提取空间关系、映射绝对方向、推断用户朝向；采用课程学习策略在Taiwan-LLM-13B-v2.0-Chat模型上逐步训练。

Result: MCoT在干净转录文本上达到100%朝向识别准确率，在ASR转录下仍达98.1%，显著优于单模态和非结构化基线；在ASR错误、多语言混用、跨领域、语言变异和指代模糊等噪声条件下表现出强鲁棒性。

Conclusion: 结构化的多模态思维链推理能有效提升资源受限场景下具身导航系统的可解释性与效率，为非英语、真实语音输入的对话导航提供了可行路径。
...
### [283] [Variational Task Vector Composition](https://arxiv.org/abs/2509.18208)
*Boyuan Zhang,Yingjun Du,Xiantong Zhen,Ling Shao*

Main category: cs.LG

TL;DR: 提出变分任务向量组合方法，通过贝叶斯框架和Spike-and-Slab先验实现样本特定的稀疏、高效任务向量融合，显著提升模型泛化与可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有任务向量组合方法在任务层面操作，缺乏对样本特定差异和结构冗余的考虑，导致组合效率低且不够灵活。

Method: 将组合系数视为隐变量，在贝叶斯框架下进行推断；引入Spike-and-Slab先验促进稀疏性，并设计门控采样机制，基于不确定性和重要性过滤系数，实现可控后验。

Result: 实验表明该方法在多个数据集上 consistently 优于现有方法，能更稳定地选择最可靠和信息丰富的任务向量成分。

Conclusion: 所提变分任务向量组合框架通过样本级稀疏建模和门控采样，提高了组合效率、稳定性与可解释性，为任务向量融合建立了新标准。
...
### [284] [MolPILE - large-scale, diverse dataset for molecular representation learning](https://arxiv.org/abs/2509.18353)
*Jakub Adamczyk,Jakub Poziemski,Franciszek Job,Mateusz Król,Maciej Makowski*

Main category: cs.LG

TL;DR: 本文提出了MolPILE，一个大规模、多样化且经过严格筛选的小分子化合物数据集，包含2.22亿个化合物，旨在解决化学信息学中现有预训练数据集的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的小分子数据集在规模、多样性和质量上存在不足，限制了分子表示学习的效果，亟需一个类似ImageNet的标准化数据集来推动基础模型的发展。

Method: 通过整合6个大规模数据库，并采用自动化数据清洗流程，构建了包含2.22亿个化合物的MolPILE数据集，并对现有预训练数据集进行了系统分析，重新训练并评估了现有模型在该数据集上的表现。

Result: MolPILE在规模和多样性上显著优于现有数据集，重新训练的模型在其上表现出更强的泛化能力，验证了高质量大规模数据集对分子表示学习的重要性。

Conclusion: MolPILE为分子化学领域提供了一个标准化的大规模预训练资源，有望成为该领域的基准数据集，推动分子表示学习和相关应用的发展。
...
### [285] [FastMTP: Accelerating LLM Inference with Enhanced Multi-Token Prediction](https://arxiv.org/abs/2509.18362)
*Yuxuan Cai,Xiaozhuan Liang,Xinghua Wang,Jin Ma,Haijin Liang,Jinwen Luo,Xinyu Zuo,Lisheng Duan,Yuyang Yin,Xi Chen*

Main category: cs.LG

TL;DR: 本文提出FastMTP，一种通过改进多令牌预测（MTP）训练方式以提升大语言模型推理速度的方法，实现了平均2.03倍的加速，且保持输出质量无损。


<details>
  <summary>Details</summary>
Motivation: 自回归生成的串行特性导致大语言模型推理吞吐量受限，尽管MTP在训练中表现出优势，但其在推理加速方面的潜力尚未充分挖掘。

Method: 通过在自蒸馏数据上微调具有位置共享权重的单一MTP头，使MTP训练与推理模式对齐，并引入语言感知的动态词汇压缩以降低计算开销。

Result: 在七个基准测试中，FastMTP相比标准单令牌预测平均实现2.03倍加速，性能优于普通MTP达82%，且接受率高、计算成本低。

Conclusion: FastMTP是一种轻量、高效且易于部署的LLM推理加速方案，显著提升了多步草稿生成质量与整体推理效率。
...
### [286] [Multi-Worker Selection based Distributed Swarm Learning for Edge IoT with Non-i.i.d. Data](https://arxiv.org/abs/2509.18367)
*Zhuoyu Yao,Yue Wang,Songyang Zhang,Yingshu Li,Zhipeng Cai,Zhi Tian*

Main category: cs.LG

TL;DR: 本文提出了一种新的多工作节点选择算法M-DSL，以应对分布式 swarm 学习中非独立同分布数据带来的挑战，并引入了新的非i.i.d.度量指标来评估数据异质性对模型训练的影响。


<details>
  <summary>Details</summary>
Motivation: 现有分布式swarm学习在处理非独立同分布数据时性能下降，且缺乏关于数据异质性如何影响模型准确性的理论指导。

Method: 提出M-DSL算法，引入非i.i.d.度量指标，通过衡量局部数据集间的统计差异来优化多工作节点的选择。

Result: 理论分析证明了M-DSL的收敛性，实验结果表明其在不同异构数据集上优于基准方法，提升了性能和网络智能。

Conclusion: M-DSL有效应对了数据异质性问题，提高了分布式swarm学习的训练效果和稳定性。
...
### [287] [GnnXemplar: Exemplars to Explanations - Natural Language Rules for Global GNN Interpretability](https://arxiv.org/abs/2509.18376)
*Burouj Armgaan,Eshan Jain,Harsh Pandey,Mahesh Chandran,Sayan Ranu*

Main category: cs.LG

TL;DR: 提出了一种名为GnnXemplar的新型全局解释方法，用于图神经网络（GNN）的节点分类任务，通过在嵌入空间中选择代表性节点并结合大语言模型生成自然语言规则，实现了高保真、可扩展且易于理解的全局解释。


<details>
  <summary>Details</summary>
Motivation: 现有的GNN全局解释方法在大规模真实场景中表现不佳，难以处理高维属性和复杂的结构-属性交互，且依赖于子图重复的假设，限制了其适用性。

Method: 受认知科学中的样例理论启发，GnnXemplar将样例选择建模为反向k近邻覆盖最大化问题，并采用贪心算法进行高效近似；利用大语言模型通过自 refine 提示策略从样例邻居中提取可读性强的自然语言规则。

Result: 在多个基准数据集上的实验表明，GnnXemplar在解释保真度、可扩展性和人类可理解性方面显著优于现有方法，并通过包含60名参与者的用户研究验证了其优越性。

Conclusion: GnnXemplar为GNN提供了有效且可解释的全局解释框架，尤其适用于大规模复杂图数据，推动了可信GNN的发展与实际应用。
...
### [288] [Graph Enhanced Trajectory Anomaly Detection](https://arxiv.org/abs/2509.18386)
*Jonathan Kabala Mbuya,Dieter Pfoser,Antonios Anastasopoulos*

Main category: cs.LG

TL;DR: 提出了一种基于图增强的轨迹异常检测框架GETAD，通过融合道路网络拓扑、语义信息和历史出行模式，利用图注意力网络和Transformer解码器实现更精准的异常检测。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅将轨迹视为采样点序列，忽略底层移动网络（如道路网络）的结构约束和连接信息，导致对复杂环境下的异常检测能力不足。

Method: GETAD框架结合图注意力网络学习道路感知的嵌入表示，并引入基于图的位置编码；使用Transformer解码器建模序列移动行为，采用多目标损失函数（自回归预测与有监督链路预测）训练模型，并设计了置信加权负对数似然（CW NLL）作为异常评分函数。

Result: 在真实和合成数据集上的实验表明，GETAD在检测道路受限环境中的细微异常方面显著优于现有方法，尤其在复杂城市交通场景中表现突出。

Conclusion: 融合图结构和上下文语义信息能有效提升轨迹异常检测的精度和鲁棒性，GETAD为道路约束环境下的轨迹分析提供了新的有效解决方案。
...
### [289] [Towards Provable Emergence of In-Context Reinforcement Learning](https://arxiv.org/abs/2509.18389)
*Jiuqi Wang,Rohan Chandra,Shangtong Zhang*

Main category: cs.LG

TL;DR: 本文研究了为何强化学习预训练算法能够生成支持上下文内强化学习（ICRL）的网络参数，提出并验证了一个假设：具备ICRL能力的参数是预训练损失的最小值之一。


<details>
  <summary>Details</summary>
Motivation: 尽管许多ICRL工作使用标准强化学习算法进行预训练，但尚不清楚为何这些预训练能产生支持在新任务中无需参数更新即可学习的模型。本文旨在探究这一现象背后的理论原因。

Method: 通过一个案例研究，分析Transformer在策略评估预训练中的表现，并证明其预训练损失的一个全局最小值可以实现上下文内的时序差分学习。

Result: 证明了在特定条件下，Transformer预训练损失的一个全局最小解能够支持in-context时序差分学习，为ICRL现象提供了初步理论支持。

Conclusion: 支持了“ICRL能力源于预训练损失最小化解”的假设，表明标准预训练过程可能隐式地学习到了可在新任务中通过上下文适应的机制。
...
### [290] [Development of Deep Learning Optimizers: Approaches, Concepts, and Update Rules](https://arxiv.org/abs/2509.18396)
*Doğay Altınel*

Main category: cs.LG

TL;DR: 本文综述了深度学习中各类优化器的发展，从随机梯度下降到最新的如Momentum、AdamW、Sophia和Muon等，详细分析了它们的更新规则、特点、技术贡献及默认超参数设置，并探讨了当前深度学习优化中的开放性挑战。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型的学习效果高度依赖于所使用的优化器，随着优化器种类的迅速发展，有必要系统梳理其演进脉络与特性，为研究者提供全面参考。

Method: 本文按时间顺序对主流深度学习优化器进行逐一回顾，详细阐述每个优化器的更新规则、核心概念、变量含义、技术手段及其在优化过程中的作用，并总结其默认超参数配置。

Result: 系统地整理了从SGD到最新优化器（如AdamW、Sophia、Muon）的发展历程，明确了各优化器的独特机制和技术贡献，并归纳了当前深度学习优化领域面临的主要挑战。

Conclusion: 该综述为理解现有深度学习优化器提供了全面资源，有助于指导实践中的优化器选择，并为未来优化算法的研究指明潜在方向。
...
### [291] [Explicit Path CGR: Maintaining Sequence Fidelity in Geometric Representations](https://arxiv.org/abs/2509.18408)
*Sarwan Ali*

Main category: cs.LG

TL;DR: 提出了一种新的信息保持型混沌游戏表示法（R-CGR），通过显式路径编码和有理数精度控制实现生物序列的完全恢复，解决了传统CGR方法中序列信息丢失的问题。


<details>
  <summary>Details</summary>
Motivation: 传统CGR方法在几何映射过程中会丢失序列信息，限制了其在需要精确序列恢复的应用中的使用。

Method: 引入了反向CGR（R-CGR）方法，结合显式路径编码与有理数算术精度控制，存储每一步的位置和字符信息，从而实现完整的序列重建。

Result: 在生物序列分类任务中表现出与传统基于序列的方法相当的性能，同时生成适合深度学习的特征丰富图像，并提供可解释的几何可视化。

Conclusion: R-CGR在保持完全序列信息的同时实现了高性能分类，为需要准确性和序列可恢复性的可解释生物信息学分析开辟了新途径。
...
### [292] [Diffusion Policies with Offline and Inverse Reinforcement Learning for Promoting Physical Activity in Older Adults Using Wearable Sensors](https://arxiv.org/abs/2509.18433)
*Chang Liu,Ladda Thiamwong,Yanjie Fu,Rui Xie*

Main category: cs.LG

TL;DR: 本文提出了一种名为KANDI的新方法，结合Kolmogorov-Arnold网络和扩散策略，用于离线逆向强化学习，以解决在老年人防跌倒项目中促进身体活动的挑战。


<details>
  <summary>Details</summary>
Motivation: 在医疗健康领域应用离线强化学习面临奖励定义困难和从专家行为中准确推断奖励函数的难题，尤其是在复杂环境中。

Method: 利用Kolmogorov-Arnold网络进行灵活的函数逼近来估计奖励函数，并采用基于扩散的策略在Actor-Critic框架内提供生成式动作优化方法。

Result: KANDI在D4RL基准测试中优于现有最先进方法，并在PEER临床试验数据上验证了其有效性。

Conclusion: KANDI能够有效应对医疗应用中离线强化学习的关键挑战，为促进身体活动的干预策略提供了有效的解决方案。
...
### [293] [MeshODENet: A Graph-Informed Neural Ordinary Differential Equation Neural Network for Simulating Mesh-Based Physical Systems](https://arxiv.org/abs/2509.18445)
*Kangzheng Liu,Leixin Ma*

Main category: cs.LG

TL;DR: 本文提出了一种结合图神经网络（GNN）和神经常微分方程（Neural ODE）的通用框架MeshODENet，用于加速复杂结构系统的数据驱动建模，在长期预测中表现出更高的精度、稳定性及计算效率。


<details>
  <summary>Details</summary>
Motivation: 传统数值求解器在处理多查询任务时计算成本高，而标准GNN在长期预测中存在误差累积和不稳定问题，因此需要一种更高效稳定的替代方法。

Method: 提出MeshODENet框架，将GNN的空间推理能力与Neural ODE的连续时间建模相结合，实现对非线性大变形结构力学问题的高效仿真。

Result: 在多个结构力学问题上验证了该方法的有效性，相比基线模型显著提升了长期预测的准确性和稳定性，并比传统求解器大幅加快了计算速度。

Conclusion: MeshODENet为复杂物理系统的建模提供了一种强大且可推广的数据驱动代理模型方案，具有良好的应用前景。
...
### [294] [Fast Linear Solvers via AI-Tuned Markov Chain Monte Carlo-based Matrix Inversion](https://arxiv.org/abs/2509.18452)
...
TL;DR: 提出了一种AI驱动的框架，利用图神经网络和贝叶斯优化自动推荐MCMC预条件参数，显著减少Krylov迭代求解大型稀疏线性系统的迭代次数。


<details>
  <summary>Details</summary>
Motivation: 传统MCMC预条件子的效果依赖于随矩阵变化的参数，手动或网格搜索成本高且效率低，需要一种自动化方法来高效选择最优参数。

Method: 使用图神经网络作为代理模型预测预条件效果，结合贝叶斯获取函数选择最有可能减少迭代次数的参数组合。

Result: 在未见过的病态系统上，仅用传统方法50%的搜索预算即达到更优预条件效果，迭代次数减少约10%。

Conclusion: 该AI驱动框架能高效适配不同矩阵，为大规模系统中应用MCMC预条件子提供了可行路径。
...
### [295] [GluMind: Multimodal Parallel Attention and Knowledge Retention for Robust Cross-Population Blood Glucose Forecasting](https://arxiv.org/abs/2509.18457)
...
TL;DR: 本文提出了一种基于Transformer的多模态框架GluMind，用于持续且长期的血糖预测，通过交叉注意力和多尺度注意力机制提升预测精度，并结合知识保留技术缓解灾难性遗忘，在AIREADI数据集上表现出优于现有模型的性能。


<details>
  <summary>Details</summary>
Motivation: 为了实现长期、连续的血糖预测，并解决多源生理与行为信号采样率不一致及模型遗忘问题。

Method: 设计了包含交叉注意力和多尺度注意力的Transformer架构，并引入知识保留模块以增强模型对历史知识的记忆能力。

Result: 在AIREADI数据集上，GluMind相较于现有模型在RMSE和MAE上分别提升了约15%和9%，展现出优异的稳定性和适应性。

Conclusion: GluMind是一种高效的持续血糖预测框架，其多模态融合与知识保留机制显著提升了长期预测性能。
...
### [296] [Probabilistic Geometric Principal Component Analysis with application to neural data](https://arxiv.org/abs/2509.18469)
*Han-Lin Hsieh,Maryam M. Shanechi*

Main category: cs.LG

TL;DR: 本文提出了Probabilistic Geometric Principal Component Analysis (PGPCA)，一种能够显式融入非线性流形信息的新型概率降维方法，推广了PPCA以更好地建模分布在非线性几何结构上的高维数据。


<details>
  <summary>Details</summary>
Motivation: 传统的PPCA及其扩展方法基于线性模型且局限于欧几里得空间，难以描述神经科学等领域的非线性流形数据分布，因此需要一种能结合非线性几何结构的更强大降维方法。

Method: 提出PGPCA模型，通过拟合数据的非线性流形并构建几何坐标系来捕捉数据偏离流形和噪声的特征；推导了一种数据驱动的EM算法用于学习模型参数。

Result: 在模拟和脑数据分析中，PGPCA能有效建模多种给定流形周围的数据显示出优于PPCA的性能，并可检验几何坐标系是否比欧几里得坐标系更优地描述数据。

Conclusion: PGPCA通过融合非线性流形几何结构，增强了对含噪声且分布于非线性流形附近的高维数据的降维与分布建模能力，为神经科学等领域提供了更有效的分析工具。
...
### [297] [Discrete-time diffusion-like models for speech synthesis](https://arxiv.org/abs/2509.18470)
*Xiaozhou Tan,Minghui Zhao,Mattias Cross,Anton Ragni*

Main category: cs.LG

TL;DR: 本文探讨了离散时间扩散过程在语音生成中的应用，提出几种新型噪声处理方法，实验表明其在训练和推理效率及一致性上优于连续时间扩散模型。


<details>
  <summary>Details</summary>
Motivation: 克服连续时间扩散模型在训练和推理条件间的不匹配问题，并提升效率。

Method: 探索并提出了多种离散时间扩散过程变体，包括加性高斯噪声、乘性高斯噪声、模糊噪声及混合噪声。

Result: 实验结果显示所提离散时间过程在主观和客观语音质量上与连续模型相当，且训练和推理更高效、一致。

Conclusion: 离散时间扩散过程是语音生成中高效且一致的替代方案，具有实际应用潜力。
...
TL;DR: SimpleFold是首个基于流匹配的蛋白质折叠模型，仅使用通用Transformer模块，在标准基准上实现了与最先进模型相当的性能，同时在部署和推理效率方面表现更优。


<details>
  <summary>Details</summary>
Motivation: 探索在不依赖复杂领域特定架构的情况下，是否可以通过通用架构和生成式方法构建高效的蛋白质折叠模型。

Method: 采用标准Transformer块结合自适应层，通过生成式流匹配目标并加入结构项进行训练，模型规模扩展至30亿参数，并在约900万蛋白质结构数据上训练。

Result: SimpleFold-3B在标准折叠基准上达到与现有最先进模型相当的性能，在集成预测任务中表现优异，且在消费级硬件上具有高效的部署和推理能力。

Conclusion: SimpleFold证明了无需复杂的领域特定设计也能构建高性能蛋白质折叠模型，挑战了现有范式，为未来研究提供了新的架构方向。
...
TL;DR: 提出一种基于Kolmogorov Arnold Networks（KANs）并结合物理信息损失函数的新方法，用于高精度、低数据需求的量子动力学响应预测。


<details>
  <summary>Details</summary>
Motivation: 传统数值方法在高维希尔伯特空间中计算成本高昂，现有神经网络模型需要大量训练数据且存在物理不可解释的伪振荡，难以准确预测量子系统的时变演化。

Method: 采用Kolmogorov Arnold Networks（KANs）并引入物理信息损失函数以强制满足Ehrenfest定理，同时设计了嵌入时间因果性的Chain of KANs架构。

Result: 该方法仅需200个训练样本（为TCN的5.4%）即达到更高预测精度，并显著减少伪振荡，提升物理一致性。

Conclusion: 物理信息驱动的KANs在量子动力学预测中优于传统黑箱模型，在大幅降低数据需求的同时保持数学严谨性和物理可解释性。
...
TL;DR: 提出使用混合数据集来增强反洗钱模型的效用，结合合成数据与公开的真实世界特征，在保护隐私的同时提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 由于隐私和保密问题，缺乏可用于训练反洗钱模型的真实数据。

Method: 采用混合数据集方法，将模拟生成的数据与公开可用的真实世界特征相结合，用于训练图神经网络等AML模型。

Result: 混合数据集不仅保护了数据隐私，还显著提升了模型的实用性与性能。

Conclusion: 混合数据集为金融机构提供了一条切实可行的路径，以在不牺牲隐私的前提下增强反洗钱系统。
...
TL;DR: 本文提出了APRIL（Active Partial Rollouts in Reinforcement Learning），一种提升大模型强化学习训练效率的方法，通过动态管理 rollout 生成过程减少GPU空闲时间，显著提高吞吐量和收敛速度。


<details>
  <summary>Details</summary>
Motivation: 在大规模语言模型的强化学习训练中，rollout生成占用了超过90%的运行时间，且由于响应长度的长尾分布，部分长响应拖慢整体批次，导致GPU利用率低下。现有框架难以有效应对这一瓶颈。

Method: 提出APRIL方法，在rollout阶段超额分配请求，一旦达到目标响应数量即终止，并将未完成的rollout留到后续步骤继续执行，从而避免浪费计算资源。该方法不依赖特定框架或硬件，兼容多种RL算法和GPU平台。

Result: 实验表明，APRIL在GRPO、DAPO、GSPO等常用RL算法上最高提升44%的rollout吞吐量，加速模型收敛，并在任务最终准确率上最高提升8%。已在slime RL框架中集成，支持NVIDIA和AMD GPU。

Conclusion: APRIL有效缓解了强化学习训练中的长尾延迟问题，兼顾系统效率与算法性能，为大规模语言模型的高效RL训练提供了可扩展的解决方案，并推动RL系统层面的进一步优化。
...
TL;DR: 本文提出了一种模型无关的微调方法RCCR，通过直接惩罚DNA序列及其反向互补序列预测之间的差异，提升模型在反向互补对称性上的鲁棒性，同时保持或提高任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有的DNA语言模型在处理序列及其反向互补序列时常常产生不一致的预测，缺乏生物学上应有的对称性，影响了模型的可靠性。

Method: 提出Reverse-Complement Consistency Regularization (RCCR)，在微调阶段加入对序列与其反向互补序列预测结果之间差异的正则化惩罚，使模型学习到RC对称性。

Result: 在多种DNA模型（Nucleotide Transformer、HyenaDNA、DNABERT-2）和多项基因组任务上验证，RCCR显著减少了预测翻转和错误，提升了RC一致性，同时不牺牲甚至提高了任务准确率。

Conclusion: RCCR是一种简单、通用且有效的方法，通过引入关键的生物学先验知识，实现了单一模型在多种生物任务中的内在鲁棒性和计算效率。
...
TL;DR: 本文提出了一种名为Symphony-MoE的新框架，通过融合多个不同预训练模型的专家来构建高性能的混合专家模型，解决了传统上仅复制单一密集模型导致专家多样性不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的MoE模型通常通过复制单个预训练密集模型的前馈网络层作为专家，限制了专家之间的多样性。本文旨在利用多个结构相同但训练数据或目标不同的预训练模型作为专家来源，提升模型性能和泛化能力。

Method: 提出Symphony-MoE，采用两阶段方法：第一阶段通过层感知融合策略构建共享主干，并使用基于激活的功能对齐缓解专家间的参数错位；第二阶段仅训练一个轻量级的路由器来协调整个架构。

Result: 实验表明，该方法能有效整合来自异构源的专家，在多领域任务和分布外泛化上显著优于基线方法。

Conclusion: Symphony-MoE为构建多样化且高性能的MoE模型提供了有效途径，无需从头训练，同时提升了模型的适应性和鲁棒性。
...
TL;DR: 本文提出了一种通过可训练的逆温度和偏置在Sigmoid损失下进行对比预训练的新方法，解释了SigLIP模型成功的原因，并引入了(m, b_rel)-Constellations这一新概念来理论分析表示对齐与模态差距。


<details>
  <summary>Details</summary>
Motivation: 为了更好地理解对比预训练中温度和偏置的作用，以及为何像SigLIP这样的模型能在检索任务中表现出色，同时存在模态差距问题。

Method: 通过理论分析Sigmoid损失函数下的可训练逆温度和偏置，定义并研究了一类新的组合结构——(m, b_rel)-Constellations，用于描述表示空间中的配置。

Result: 理论上证明了温度和偏置可以使损失趋近于零的一类丰富配置；解释了SigLIP的成功及其模态差距现象；提出了带显式相对偏置的Sigmoid损失重参数化方法，在合成数据实验中改善了训练动态。

Conclusion: 该工作为对比学习中的温度和偏置提供了理论支持，揭示了高质量表示所需的维度条件，并提出了改进训练的方法。
...
TL;DR: 本文首次全面综述了可解释图神经网络（XGNNs）在痴呆症研究中的应用，涵盖阿尔茨海默病、帕金森病、轻度认知障碍及多疾病诊断，提出针对痴呆任务的可解释性方法分类体系，并讨论模型在临床应用中的挑战与未来方向。


<details>
  <summary>Details</summary>
Motivation: 痴呆症具有高度异质性，导致诊断和分型困难；现有图神经网络在鲁棒性、数据稀缺性和可解释性方面的局限限制了其临床应用。

Method: 对用于痴呆研究的可解释图神经网络（XGNNs）进行系统性综述，提出针对该领域任务的可解释性方法分类体系，并比较现有模型在临床场景中的表现。

Result: 总结了XGNNs在多种痴呆相关疾病中的应用进展，识别出关键挑战，包括泛化能力不足、未充分探索的研究领域以及大语言模型在早期检测中的整合问题。

Conclusion: 该综述旨在推动XGNNs在痴呆研究中向更可信、具临床意义且可扩展的方向发展，为未来研究提供指导。
...
TL;DR: 提出了一种名为Interaction Topological Transformer（ITT）的统一数据高效框架，用于多尺度预测多孔材料的吸附、传输和稳定性性能。


<details>
  <summary>Details</summary>
Motivation: 由于结构-性能关系的多尺度特性以及标注数据稀疏且分布不均，现有方法难以在不同多孔材料间实现泛化预测。

Method: ITT通过引入新的交互拓扑来捕捉从结构、元素、原子到成对元素等多个层次的材料信息，并利用内置的Transformer架构整合多尺度特征；采用两阶段训练策略：在60万个未标记结构上进行自监督预训练，再进行监督微调。

Result: ITT在吸附、传输和稳定性属性预测上达到了最先进的精度，并表现出良好的可迁移性。

Conclusion: ITT为化学和结构多样性的多孔材料提供了可扩展的学习引导发现路径。
...
TL;DR: 本文提出了一种新的时间序列生成扩散模型DS-Diffusion，通过风格引导核和分层去噪机制减少生成数据与真实数据之间的分布偏差，并提升模型的可解释性和适应性，无需重新训练即可适应特定条件。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列生成扩散模型需要为特定条件重新训练，存在生成数据与真实数据之间的分布偏差，且推理过程不可解释。

Method: 提出DS-Diffusion模型，采用基于风格引导核的扩散框架和基于时间信息的分层去噪机制（THD），避免重新训练，减少分布偏差，并增强生成样本的可解释性。

Result: 在多个公开数据集上验证，相比ImagenTime等先进模型，预测得分和判别得分分别下降5.56%和61.55%，分布偏差更小，推理过程更可解释，且无需重训练提升了灵活性。

Conclusion: DS-Diffusion有效解决了现有扩散模型在时间序列生成中的重训练需求、分布偏差和不可解释性问题，具有更强的适应性和实用性。
...
TL;DR: 提出“行动前反思”（REBACT）方法，通过在每步行动前引入反思机制提升大语言模型在交互决策任务中的表现，显著提高成功率并具备良好的计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在交互决策中易出现错误累积且缺乏有效的自我纠正机制，需要一种能及时发现并修正错误的方法。

Method: 在模型采取行动前增加一个反思步骤，利用环境反馈进行即时错误修正，从而优化决策路径。该方法在ALFWorld、WebShop和TextCraft三个环境中验证。

Result: 在WebShop上成功率提升至61%（+24%），ALFWorld达98.51%（+6.72%），TextCraft达99.5%（+0.5%），且仅需少量修改步骤，计算高效。

Conclusion: REBACT通过引入前置反思机制有效缓解了错误累积问题，提升了LLM在交互式决策任务中的性能和适应性。
...
TL;DR: 提出Flow Marching算法和生成式PDE基础模型，结合神经算子学习与流匹配，提升长期预测稳定性与生成灵活性，通过P2VAE和FMT实现高效大规模预训练。


<details>
  <summary>Details</summary>
Motivation: 现有PDE基础模型多为确定性Transformer架构，缺乏科学与工程应用所需的生成灵活性，且存在长期推演误差累积问题。

Method: 提出Flow Marching算法，联合采样噪声水平与物理时间步，学习统一速度场；设计P2VAE压缩物理状态至潜在空间，FMT结合扩散强制与潜在时间金字塔提升计算效率。

Result: 在约250万条跨12类PDE的轨迹上完成预训练；相比全序列视频扩散模型效率提升达15倍；在未见Kolmogorov湍流任务中展现Few-shot适应能力、更优长期稳定性及不确定性分层生成结果。

Conclusion: 生成式PDE基础模型能有效支持不确定感知的集合预测与长期稳定推演，对现实世界科学与工程应用具有重要意义。
...
TL;DR: 本文提出了一种名为HyperAdapt的参数高效微调方法，通过行列缩放实现高秩更新，显著减少了可训练参数数量，在多个基准任务上性能媲美全微调和其他先进PEFT方法。


<details>
  <summary>Details</summary>
Motivation: 适应基础模型到特定应用通常需要微调，但传统微调计算和内存开销大，现有PEFT方法仍有改进空间以进一步减少可训练参数。

Method: HyperAdapt通过对预训练权重矩阵应用行和列方向的对角矩阵缩放来实现适应，从而在仅需n+m个可训练参数的情况下为n×m矩阵引入高秩更新，并理论分析了其更新秩的上界。

Result: 在GLUE、算术推理和常识推理等基准上，使用高达14B参数的模型实验表明，HyperAdapt在可训练参数少几个数量级的情况下，性能达到或接近全微调和最先进的PEFT方法。

Conclusion: HyperAdapt是一种高效的参数微调方法，能够在显著降低参数量的同时保持优异性能，适用于大规模模型的定制化适配。
...
TL;DR: 提出了一种针对高维矩阵集合的子空间聚类新框架（SCoS），通过构建基于输入矩阵的三阶张量的块项分解（BTD），联合估计聚类成员和部分共享子空间，在高噪声下表现出优越的准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统子空间聚类方法通常处理向量化数据，难以有效利用矩阵结构信息；而现实中的数据（如高光谱图像）天然以矩阵形式存在，需直接建模其列空间进行聚类。

Method: 将每个样本建模为矩阵，构造由输入矩阵组成的三阶张量，并采用块项分解（BTD）方法进行分解，从而实现对样本所属子空间的聚类，同时引入与GCCA和传统子空间聚类的理论联系与区别分析。

Result: 在真实高光谱图像数据上验证了该方法优于现有子空间聚类技术，尤其在高噪声和干扰环境下具有更高的聚类精度和鲁棒性，并给出了该模型的首个可识别性理论结果。

Conclusion: 所提出的SCoS框架能更有效地捕捉高维数据中的结构性信息，适用于超越向量表示的复杂应用场景，展示了在高维、结构化数据聚类中的潜力。
...
TL;DR: 该研究利用图机器学习推动理性农药设计，创建了最大的蜜蜂毒性数据集ApisTox，并评估多种ML模型在农药毒性预测中的表现，发现药物化学中有效的模型在农业化学品中泛化能力有限，强调需开发领域专用模型和基准。


<details>
  <summary>Details</summary>
Motivation: 受药物发现中计算机方法的启发，为加速开发更安全、环保的农药，需解决现有机器学习模型在农药生态毒性预测中泛化能力不足的问题。

Method: 构建了最大的蜜蜂毒性数据集ApisTox，系统评估了包括分子指纹、图核、图神经网络（GNN）和预训练Transformer在内的多种分子图分类机器学习模型。

Result: 实验表明，在药物化学中表现良好的模型在农药毒性预测任务中常表现不佳，说明当前模型难以迁移到农业化学品领域，凸显了领域差异性和对专用模型的需求。

Conclusion: 农药发现需要针对其独特挑战设计专门的机器学习模型和综合基准测试体系，未来工作将聚焦于此以提升模型性能与实用性。
...
TL;DR: 提出广义双模拟度量（GBSM），用于多MDP场景下的状态相似性计算，具备严格数学性质并提供 tighter 理论界和更优的样本复杂度。


<details>
  <summary>Details</summary>
Motivation: 现有双模拟度量（BSM）难以应用于多MDP场景且缺乏严格的数学性质分析，限制了其在策略迁移等任务中的理论发展。

Method: 形式化定义GBSM，证明其对称性、跨MDP三角不等式及状态空间距离界三个基本性质，并基于这些性质推导策略迁移、状态聚合和采样估计的理论边界。

Result: GBSM提供了比标准BSM更紧的理论界和闭式样本复杂度，数值实验验证了其在多MDP场景中的有效性。

Conclusion: GBSM为多MDP间的状态相似性度量提供了坚实的理论基础，推动了策略迁移等相关任务的理论与应用进展。
...
TL;DR: 提出一种结合强化学习（RL）与大语言模型（LLM）的电商支付反欺诈新方法，利用LLM优化RL奖励函数，提升检测准确率并展现零样本能力。


<details>
  <summary>Details</summary>
Motivation: 传统强化学习在支付反欺诈中依赖人工设计复杂的奖励函数，成本高且难以适应多变场景，需要自动化、智能化的方法来提升奖励函数的设计效率与效果。

Method: 将交易风险建模为多阶段马尔可夫决策过程（MDP），利用大语言模型（LLM）迭代优化强化学习的奖励函数，通过其推理和代码生成能力自动改进奖励机制。

Result: 在真实数据上的实验表明，该框架在长期评估中显著提升了欺诈检测的准确性、鲁棒性和适应性，并展现出良好的零样本迁移能力。

Conclusion: LLM能够有效增强工业级强化学习应用中的奖励函数设计，所提出的方法为电商支付反欺诈提供了更具潜力的解决方案。
...
TL;DR: 提出了一种新的卷积神经网络架构——周期性CNN，其在卷积层中引入了周期性边界条件，并证明了其可逼近d维输入空间中依赖d-1个线性变量的脊函数，而在低维情况下无法实现，从而精确刻画了其表达能力。


<details>
  <summary>Details</summary>
Motivation: 扩展CNN的逼近理论基础，探索具有周期性边界条件的CNN在高维脊状结构数据上的表达能力。

Method: 提出周期性CNN架构，并通过严格的逼近定理分析其表达能力，理论证明其在高维脊函数逼近中的优势。

Result: 周期性CNN能够逼近d维空间中依赖d-1个变量的脊函数，而传统CNN在低维（d-2或更少）脊函数上无法实现该逼近，形成表达力的显著差异。

Conclusion: 周期性CNN在处理具有高内在维度脊状结构的数据（如环绕域图像、物理信息学习和材料科学）时具有优越的逼近能力和实际应用潜力。
...
TL;DR: 提出MOMEMTO，一种基于补丁记忆模块的时间序列基础模型，用于缓解重建型深度模型在异常检测中的过泛化问题，通过多域训练策略实现跨数据集联合微调，在23个基准数据集上表现出优于基线方法的性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于重建的深度模型在时间序列异常检测中容易过泛化，难以有效识别新出现的异常；同时，引入记忆结构的方法训练成本高，且未与时间序列基础模型有效结合。

Method: 设计了一个基于补丁的记忆模块，初始化为预训练编码器的潜在表示，组织成补丁级单元，并通过注意力机制更新；采用多域训练策略，使单个模型可在多个数据集上联合微调，提升对正常模式的记忆能力并抑制异常重构。

Result: 在23个单变量基准数据集上评估显示，MOMEMTO在AUC和VUS指标上优于基线方法，尤其在少样本学习场景下显著提升了其骨干时间序列基础模型的性能。

Conclusion: MOMEMTO有效缓解了重建模型的过泛化问题，通过多域记忆机制实现了高效、可扩展的时间序列异常检测，具备良好的实际应用潜力。
...
TL;DR: 本文研究了对角线性网络的训练轨迹与Lasso正则化路径之间的关系，发现训练时间起到了逆正则化参数的作用。


<details>
  <summary>Details</summary>
Motivation: 对角线性网络的隐式正则化可以被严格分析，但其训练轨迹与经典正则化方法（如Lasso）的关系尚不明确。

Method: 通过理论分析和数值模拟，建立对角线性网络训练过程与Lasso正则化路径之间的联系，并在单调性假设下证明精确对应，一般情况下给出近似结果。

Result: 证明了在单调性假设下，对角线性网络的训练轨迹与Lasso正则化路径完全一致；在一般情况下存在近似关系。同时通过模拟验证了该结论。

Conclusion: 对角线性网络的训练过程可视为一种动态正则化机制，训练时间等效于Lasso中的逆正则化参数。
...
TL;DR: 提出了一种基于集成概率机器学习的诊断框架，用于量化和自动化预测不确定性，从而提升数据驱动的一致性故障诊断性能。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络在故障诊断中广泛应用，但在评估预测置信度方面存在困难，尤其在对误报敏感的一致性诊断中问题突出。

Method: 采用集成概率机器学习方法构建诊断框架，通过量化预测不确定性来改进诊断性能。

Result: 在多个案例研究中进行消融和对比实验，结果显示该方法在多种诊断指标上均有稳定提升。

Conclusion: 所提出的框架能有效提升数据驱动一致性诊断中的不确定性评估与整体诊断性能。
...
TL;DR: 提出一种基于预训练扩散模型的轻量级通用数据同化方法，结合粒子滤波算法，无需额外训练，以GenCast为例展示了其在全局集合天气预报中的应用。


<details>
  <summary>Details</summary>
Motivation: 为了在无需额外训练的情况下，利用预训练扩散模型提高数据同化在气象、海洋学等领域的估计精度。

Method: 基于粒子滤波算法，将预训练的扩散模型用于数据同化，提出一种轻量且通用的方法。

Result: 该方法在GenCast模型上成功实现了全球集合天气预报的数据同化，表现出良好的适用性和性能。

Conclusion: 所提方法是一种高效、通用的数据同化框架，可直接应用于已有的扩散模型，具有广泛的应用前景。
...
TL;DR: 本文提出了LoRD和B-LoRD两种图聚类方法，通过仅放松正交约束并引入块对角正则化，在保持双重随机性和低秩性的同时提升聚类性能，并设计了全局收敛的优化算法。


<details>
  <summary>Details</summary>
Motivation: 现有图聚类方法过度松弛低秩、非负、双重随机和正交约束以保证数值可行性，可能限制聚类效果。本文旨在减少不必要的松弛，保留更多原始约束以提升性能。

Method: 提出LoRD模型，仅放松核k均值中的正交约束；理论证明在双重随机约束下正交性与块对角性等价，据此引入块对角正则化得到B-LoRD；将非凸的双重随机约束转化为含类别概率参数的线性凸约束，并利用梯度Lipschitz连续性设计投影梯度下降算法实现全局收敛优化。

Result: 所提LoRD和B-LoRD在多个数据集上显著优于传统图聚类方法，验证了保留更多原始约束的有效性；提出的优化算法具有良好的收敛性和稳定性。

Conclusion: 适度松弛约束并引入块对角正则化可有效提升图聚类性能，所提方法在理论与实验上均表现出优越性。
...
TL;DR: 提出了一种新的神经网络扩展方法SWE和SVoD，有效抑制新神经元的不活跃问题，并在多个数据集上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 扩展神经网络以增加容量时，新增神经元常因无法适应已训练网络而变得不活跃，导致容量增长无效。

Method: 提出Shared-Weights Extender (SWE)通过共享权重耦合新旧神经元，促进新神经元融合；同时提出Steepest Voting Distributor (SVoD)，基于梯度分配新神经元到深层网络各层。

Result: 在四个数据集上的实验表明，该方法能有效抑制新神经元的不活跃现象，在性能上优于其他扩展方法和基线模型。

Conclusion: SWE和SVoD能有效提升神经网络扩展过程中的新神经元利用率和整体性能，为动态网络扩展提供了可行方案。
...
TL;DR: 本文提出了NGRPO算法，通过优势校准和非对称裁剪机制，解决GRPO在处理同质错误响应时梯度消失的问题，显著提升了大模型在数学推理任务中的表现。


<details>
  <summary>Details</summary>
Motivation: GRPO在所有响应都错误或都正确时无法学习，尤其是全错组导致优势函数为零、梯度消失，损失重要学习信号。

Method: 提出NGRPO，包含两个关键机制：1）优势校准，引入虚拟最高奖励样本以调整组内奖励的均值和方差，使同质错误样本的优势非零；2）非对称裁剪，放松正样本更新幅度，严格限制负样本，稳定探索压力。

Result: 在Qwen2.5-Math-7B上实验表明，NGRPO在MATH500、AMC23和AIME2025等数学基准上显著优于PPO、GRPO、DAPO和PSR-NSR等基线方法。

Conclusion: NGRPO能有效利用同质错误响应作为学习信号，实现更稳定且显著的数学推理能力提升。
...
TL;DR: 该论文首次分析了异配性（heterophily）在图级别学习中的影响，提出基于motif的任务需要多频动态，而非频率主导机制，理论与实验证明频率自适应模型更优。


<details>
  <summary>Details</summary>
Motivation: 尽管异配性在节点级任务中已被广泛研究，但其在图级别任务中的影响尚不清楚，本文旨在填补这一空白。

Method: 提出图级别标签方案的分类体系，聚焦于局部结构标记中的motif-based任务，采用基于能量的梯度流分析揭示motif检测需要混合频率动态。

Result: 理论表明motif目标与全局频率主导不一致，实验在可控异配性的合成数据集和真实分子属性预测数据上验证了频率自适应模型优于频率主导模型。

Conclusion: 图级别学习中的异配性需要不同于节点级任务的架构设计，频率自适应机制更有利于motif检测等任务。
...
TL;DR: 本文提出了一种在联邦学习中解耦后门任务与主任务的新型攻击方法，通过动态优化触发器在min-max框架内实现更强且更持久的后门攻击。


<details>
  <summary>Details</summary>
Motivation: 现有后门攻击方法通常使用固定模式或对抗性扰动作为触发器，导致主任务与后门任务紧密耦合，易被诚实更新稀释且难以抵御联邦学习中的防御机制。因此需要一种更具鲁棒性和持久性的攻击方式。

Method: 提出一种基于min-max框架的动态触发器优化方法：内层最大化中毒样本与正常样本的性能差距，外层将自适应触发器注入本地模型，从而实现后门任务与主任务的解耦。

Result: 在计算机视觉和自然语言处理任务上评估了该方法，并与六种现有攻击方法在六种防御算法下进行比较，结果表明该方法具有优越的攻击性能和良好的兼容性。

Conclusion: 所提方法能有效提升后门攻击的鲁棒性和持久性，且易于集成到现有攻击技术中，对联邦学习安全防御提出了更大挑战。
...
TL;DR: 提出GNARL框架，将神经算法推理问题重构为马尔可夫决策过程，利用模仿和强化学习解决NAR在组合优化、解构造和专家算法缺失下的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有神经算法推理（NAR）方法在解的合法性、多解推理、NP难问题和缺乏专家算法时存在局限。

Method: 将算法轨迹学习重构为马尔可夫决策过程，提出GNARL框架，结合强化学习与图神经网络，适用于多种图结构问题。

Result: 在CLRS-30多个任务上实现高图准确率，性能优于或匹配现有NAR方法，尤其在NP难问题和无专家策略情况下仍有效。

Conclusion: GNARL通过引入强化学习范式，显著提升了神经算法推理的通用性、灵活性和性能，拓展了其适用范围。
...
TL;DR: 本文提出使用可信网络（CN）作为平衡贝叶斯网络（BN）隐私与效用的新方法，通过掩蔽BN结构抵御追踪攻击，在不引入噪声的情况下实现有效推理并保护隐私。


<details>
  <summary>Details</summary>
Motivation: 随着隐私问题日益严重，公开发布的模型需要保护训练数据中的敏感信息。现有方法通过在参数中添加噪声来防御追踪攻击，但会显著降低模型效用。因此，亟需一种既能保障隐私又不牺牲推理准确性的新方法。

Method: 将追踪攻击的概念扩展到可信网络（CN），利用CN对BN进行掩蔽，避免直接暴露学习到的结构和参数；识别出必须隐藏的关键学习信息以防止攻击者恢复原始BN，并通过调节CN超参数控制隐私与效用的权衡。

Result: 实验表明，CN能在不引入噪声的前提下有效降低追踪攻击的成功率，同时保持有意义的推理能力；通过调整超参数可灵活调控隐私保护水平与模型性能之间的平衡。

Conclusion: 可信网络为构建兼顾隐私性和实用性的概率图模型提供了一种有原则、可行且有效的解决方案，优于传统的加噪方法。
...
TL;DR: 提出了一种用于流数据挖掘的异构在线集成方法HEROS，通过在资源约束下选择性训练模型子集，平衡预测性能与计算成本，实现绿色在线学习。


<details>
  <summary>Details</summary>
Motivation: 现有集成方法过于关注预测性能，忽视了模型的计算开销，缺乏对可持续性的考虑，难以满足绿色在线学习的需求。

Method: HEROS从具有不同超参数的模型池中选择子集进行训练，引入马尔可夫决策过程建模性能与资源消耗的权衡，并设计了多种选择策略，特别是新型的ζ-策略，专注于以更低成本训练近优模型。

Result: 理论证明ζ-策略在较少资源下达到近似最优性能；在11个基准数据集上的实验表明，该方法在保持高精度的同时显著降低资源消耗，部分情况下优于现有方法。

Conclusion: HEROS及其ζ-策略有效平衡了流数据学习中的性能与资源消耗，为可持续的在线学习提供了新的解决方案，是绿色机器学习领域的重要进展。
...
TL;DR: 本文建立了异步更新下Polyak-Ruppert平均Q学习的中心极限定理，提出了一个非渐近性的中心极限定理，并推导出功能型中心极限定理。


<details>
  <summary>Details</summary>
Motivation: 为了更好地理解异步更新下Polyak-Ruppert平均Q-learning的统计性质，特别是在有限样本情况下的收敛行为。

Method: 通过分析Wasserstein距离中的收敛速率，结合迭代次数、状态-动作空间大小、折扣因子和探索质量等因素，提出非渐近性中心极限定理；并证明部分和过程弱收敛于布朗运动的功能型中心极限定理。

Result: 得到了显式反映多种因素影响的非渐近中心极限定理，以及部分和过程收敛到布朗运动的功能型中心极限定理。

Conclusion: 该研究为异步Q-learning算法的统计推断提供了理论基础，揭示了其在实际应用中的收敛特性。
...
TL;DR: 本文提出了一种基于光电设备自然信号衰减特性的新型脉冲神经网络计算范式Otters，实现了高能效的时间到首次脉冲（TTFS）计算，并结合量化网络到SNN的转换算法，在GLUE基准上达到最先进精度，同时能量效率提升1.77倍。


<details>
  <summary>Details</summary>
Motivation: 传统SNN在TTFS编码下虽具高稀疏性潜力，但推理过程中需计算时间衰减函数并与权重相乘，导致能耗高，限制了其能效优势的发挥。

Method: 利用光电设备（如氧化铟）的自然物理衰减特性直接实现TTFS所需的时间函数，将器件模拟输出视为权重与时间衰减的融合乘积，从而避免昂贵的数字运算；提出一种新型量化神经网络转SNN算法，以支持复杂模型（如Transformer）的高效训练与部署。

Result: 所提出的Otters架构在七个GLUE基准数据集上实现了最先进的准确率，并在22nm工艺的能量分析下，相较于先前领先的SNN提升了1.77倍的能量效率。

Conclusion: 通过硬件-软件协同设计，将器件物理特性直接转化为计算原语，建立了一种全新的高能效SNN计算范式。
...
TL;DR: Simulation-Grounded Neural Networks (SGNNs) 是一种基于机制模拟生成的合成数据进行训练的预测模型，在真实标签有限或不可观测的情况下表现出色。本文提出了SGNN的理论基础，证明其等价于在模拟先验下的摊销贝叶斯推断，并能收敛到贝叶斯最优预测器。同时推导了模型误设下的泛化界，证明SGNN可学习传统方法无法捕捉的不可观测科学量，并实现基于模拟机制的可解释性。实验验证了理论预测，SGNN在参数恢复、鲁棒性和模型选择任务中显著优于经典方法。


<details>
  <summary>Details</summary>
Motivation: 在许多科学领域中，真实世界的数据标签稀缺或无法观测，限制了传统监督学习的应用。尽管SGNN在实践中表现优异，但缺乏理论支持，尤其是关于其为何有效、能否逼近最优解以及是否能超越经验方法的能力尚不明确。因此，需要建立SGNN的理论基础以解释其有效性并拓展其在科学发现中的应用。

Method: 作者从贝叶斯推断的角度出发，将SGNN建模为在机制模拟先验下的摊销推断过程。通过理论分析证明SGNN收敛到贝叶斯最优预测器，并推导在模型误设情况下的泛化误差界。提出一种新的机制可解释性方法，通过归因预测到生成数据的模拟机制来提供科学合理的解释。并通过数值实验验证理论结果，包括参数恢复、模型误设鲁棒性及与AIC等经典方法的比较。

Result: 理论层面，SGNN被证明实现了摊销贝叶斯推断，收敛至贝叶斯最优预测器，并能在模型误设下保持良好泛化；SGNN能够学习传统经验方法无法识别的不可观测科学变量；此外，SGNN支持基于模拟机制的后验一致的可解释性。实验结果显示SGNN能准确恢复潜变量参数，在模拟与现实存在差异时仍具鲁棒性，并在模型选择任务中错误率仅为AIC的一半。

Conclusion: SGNN不仅是一种有效的数据驱动工具，更是一个有原则的科学预测框架。其理论基础揭示了其在数据稀缺场景下的优越性，兼具高性能、强解释性和对不可观测量的学习能力，为科学机器学习提供了可靠且可解释的新范式。
...
TL;DR: 提出了一种名为CR-Net的跨层低秩残差网络，通过利用层间激活残差的低秩特性，在减少参数和内存消耗的同时提升大语言模型预训练效率。


<details>
  <summary>Details</summary>
Motivation: 现有低秩方法在模型性能、计算开销和激活内存节省方面存在不足，需改进以实现高效且高性能的LLM预训练。

Method: 设计双路径架构，结合前一层输出及其低秩差异重构层激活，并开发专用的激活重计算策略。

Result: 在60M到7B参数规模的模型上实验表明，CR-Net在计算资源和内存需求更低的情况下， consistently 优于当前最先进的低秩框架。

Conclusion: CR-Net有效解决了低秩方法的关键缺陷，实现了高效、高性能的大语言模型预训练。
...
TL;DR: 本文综述了从无标签数据中进行表示学习的最新理论进展，探讨了当前深度学习模型（如视觉基础模型）在自监督和掩码自编码器等方法下学到的表示难以用经典理论解释的问题，并强调需结合统计学与优化理论工具来理解其成功原因。


<details>
  <summary>Details</summary>
Motivation: 现有的深度学习模型在无监督表示学习方面取得了巨大成功，但其学习到的表示缺乏理论解释，难以理解为何能在多种预测任务中表现良好或展现出涌现行为。

Method: 结合统计学和优化领域的数学工具，对基于无标签数据的表示学习进行理论分析，综述近期理论进展并介绍作者在此方向上的贡献。

Result: 提供了无标签数据表示学习的理论框架，推动了对自监督和掩码自编码器等方法的理解。

Conclusion: 通过融合经典统计理论与现代深度学习实践，能够更好地刻画和解释现代表示学习模型的成功机制。
...
TL;DR: 提出了一种完全可学习的神经奖励机（FLNRM），能端到端地学习符号接地函数和自动机，无需先验知识，结合深度强化学习在非马尔可夫任务中优于RNN方法。


<details>
  <summary>Details</summary>
Motivation: 现有非马尔可夫强化学习方法依赖预定义的符号接地函数或任务先验知识，限制了其通用性和实用性。

Method: 提出Fully Learnable Neural Reward Machines（FLNRM），通过端到端训练同时学习符号接地函数和自动机结构，并将其与深度强化学习框架结合。

Result: 实验表明，FLNRM在非马尔可夫任务中优于基于RNN的方法，且因自动机的有限性和紧凑性具有更高可解释性。

Conclusion: FLNRM无需先验知识即可处理复杂时序任务，兼具DRL的易用性和符号系统的可解释性，是处理非马尔可夫RL任务的有效方案。
...
TL;DR: 本文提出OmniBridge，一个统一且模块化的多模态框架，支持视觉-语言理解、生成和检索任务，通过轻量级双向潜在对齐模块和两阶段解耦训练策略，在复用预训练大语言模型的同时实现跨模态对齐，在多个基准上达到领先性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常孤立处理多模态任务或需从头训练大模型，导致计算成本高且跨模态泛化能力有限。

Method: 采用语言中心化设计，复用预训练大语言模型，引入轻量级双向潜在对齐模块，并提出两阶段解耦训练策略：第一阶段为监督微调与潜在空间对齐，第二阶段为语义引导的扩散训练，通过可学习查询嵌入对齐跨模态潜在空间。

Result: 在多个理解、生成和检索基准上实验表明，OmniBridge在三项任务中均取得具有竞争力或最先进的性能，验证了潜在空间对齐在统一多模态建模中的有效性。

Conclusion: OmniBridge通过模块化设计和有效的训练策略，实现了高效、统一的多模态建模，能够在共享表示空间下良好支持多种任务，具备良好的通用性和实用性。
...
TL;DR: 提出一种基于Transformer的GAN模型，用于生成高质量的欺诈交易样本，有效解决信用卡欺诈检测中的类别不平衡问题。


<details>
  <summary>Details</summary>
Motivation: 传统过采样方法（如SMOTE）生成的样本过于简单，难以捕捉复杂欺诈模式；现有生成模型在高维依赖建模上仍存在不足。

Method: 结合生成对抗网络（GAN）与Transformer编码器模块，利用GAN的对抗训练机制和Transformer的自注意力机制学习特征间复杂交互，生成更真实的欺诈交易数据。

Result: 在公开信用卡欺诈数据集上测试，相比SMOTE、CTGAN和TVAE等方法，该方法在Recall、F1-score和AUC指标上均有显著提升。

Conclusion: 所提出的Transformer-GAN混合方法能有效生成多样且高质量的少数类样本，显著提升欺诈检测性能，优于传统及现有生成式重采样策略。
...
TL;DR: 提出了一种基于潜在扩散模型的黑盒对抗攻击框架JAD，利用CNN和ViT的注意力图进行联合注意力蒸馏，生成具有高跨架构迁移性和查询效率的对抗样本。


<details>
  <summary>Details</summary>
Motivation: 现有黑盒对抗攻击方法依赖特定网络结构或需要大量查询，导致跨架构迁移性差和查询成本高。

Method: 提出JAD框架，通过潜在扩散模型结合从CNN和ViT中提取的注意力图，聚焦于跨架构共同敏感的图像区域，生成更易迁移的对抗样本。

Result: 实验表明，JAD在攻击泛化性、生成效率和跨架构迁移性方面优于现有方法。

Conclusion: JAD是一种架构无关、高效且具有强迁移性的黑盒对抗攻击新范式。
...
TL;DR: 本文研究了三种无需反向传播（BP）的训练方法（Forward-Forward、Cascaded-Forward 和 Mono-Forward），发现Mono-Forward在准确率、能耗和训练时间上均优于传统BP方法，为高效可持续深度学习提供了新方向。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络对计算和能源的高需求主要源于反向传播算法，这制约了AI的可持续发展，因此亟需探索不依赖BP的高效训练方法。

Method: 在MLP和CNN架构上实现并比较FF、CaFo和MF算法，使用Optuna优化超参数，并采用早停机制确保公平对比；通过NVML和CodeCarbon进行硬件级能效与碳排放评估。

Result: MF不仅在分类准确率上超越BP，还减少最多41%的能耗和34%的训练时间，且收敛到更优的验证损失最小值；硬件分析揭示其计算效率高，但并非所有BP-free方法都更省内存。

Conclusion: MF是一种兼具高性能与可持续性的BP替代方案，本文为未来节能深度学习提供了数据驱动的发展路径。
...
TL;DR: 提出Diffusion Bridge Variational Inference (DBVI)，通过可学习的、数据依赖的初始分布改进深度高斯过程中的变分推断，提升推理效率和后验质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法DDVI使用固定的无条件初始分布，与复杂的真实后验差距大，导致推理路径低效、收敛慢。

Method: 引入可学习的、数据依赖的初始分布，通过摊销神经网络参数化，并基于ELBO梯度逐步优化；利用诱导输入实现可扩展摊销，并结合Doob-桥扩散过程重新解释先验。

Result: 在回归、分类和图像重建任务中，DBVI在预测精度、收敛速度和后验质量上均优于DDVI及其他变分基线方法。

Conclusion: DBVI通过改进反向扩散的初始化机制，有效缩小了后验差距，提升了深度高斯过程中变分推断的效率与性能。
...
TL;DR: 提出了一种受Axelrod文化传播模型启发的新型图神经网络架构AxelGNN，通过相似性门控的概率交互、特征级复制机制和全局极化保持，有效解决了特征过平滑、异质关系建模和特征向量刚性处理等问题，在多种图结构和同质-异质特性下表现出优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有图神经网络存在特征过平滑、难以处理异质关系以及特征向量整体处理缺乏灵活性等局限性，需要一种更灵活、统一且能适应不同图特性的模型。

Method: 基于Axelrod文化传播模型，设计了相似性门控的概率交互机制，实现基于节点相似性的自适应聚合或分离；引入特征级（trait-level）复制机制进行细粒度特征聚合；并通过维持全局极化来保留节点的独特性。

Result: 在节点分类和影响力估计任务上，AxelGNN在多种具有不同同质-异质特性的图数据集上均优于或媲美当前最先进的GNN方法。

Conclusion: AxelGNN通过统一框架有效应对了GNN中的过平滑、异质性和特征刚性问题，具备处理同质与异质图的双稳态动态特性，展现出强大且通用的图表示学习能力。
...
TL;DR: 研究了迁移学习场景下的非上下文多臂赌博机问题，提出KL-UCB-Transfer算法，在源与目标分布接近时显著优于无先验基线。


<details>
  <summary>Details</summary>
Motivation: 将经典多臂赌博机扩展到迁移学习框架，利用源域的先验样本提升目标域决策性能。

Method: 基于KL-UCB设计新策略KL-UCB-Transfer，结合源分布样本和已知距离约束进行决策。

Result: 推导出包含迁移参数的新渐近下界，并证明KL-UCB-Transfer在高斯情形下可达到该下界。

Conclusion: 所提方法能有效利用迁移信息降低累积遗憾，理论与实验均验证其优越性。
...
TL;DR: 本论文探讨了深度学习模型在安全关键应用中的鲁棒性问题，提出了对抗样本、领域泛化和大语言模型越狱攻击与防御的新算法和技术。


<details>
  <summary>Details</summary>
Motivation: 确保深度学习模型在安全关键应用中的决策不受对抗性攻击影响，提升模型在不同分布数据上的泛化能力和对恶意提示的抵抗能力。

Method: 引入新的技术成果、训练范式和认证算法来应对对抗样本；提出新算法实现跨领域泛化；设计针对大语言模型的新型攻击与防御方法。

Result: 在计算机视觉、医学图像分析、分子识别和图像分类中实现了最先进的鲁棒性和泛化性能，并在大语言模型的安全防护方面取得前沿进展。

Conclusion: 所提出的算法有效提升了深度学习模型在多种场景下的鲁棒性和安全性，为构建可靠的AI系统提供了重要技术支持。
...
TL;DR: 本文提出了DRO-REBEL，一种基于分布鲁棒优化的离线强化学习框架，通过Wasserstein、KL和χ²模糊集改进现有RLHF方法的过优化问题，在理论和实验上均展现出更强的鲁棒性和收敛性能。


<details>
  <summary>Details</summary>
Motivation: 现有离线RLHF方法存在过优化问题，导致模型在训练中偏离人类偏好行为，本文旨在解决这一问题。

Method: 引入DRO-REBEL框架，使用Fenchel对偶将鲁棒更新转化为相对奖励回归，并结合三种模糊集（Wasserstein、KL、χ²），推导出无需PPO式裁剪或辅助网络的高效算法。

Result: 在标准假设下建立了O(n^{-1/4})估计界并通过局部Rademacher复杂度分析达到O(n^{-1/2})最优速率；实验表明该方法在多种任务和设置下具有强鲁棒性，尤其χ²-REBEL表现优异。

Conclusion: DRO-REBEL统一了多种分布鲁棒方法，理论分析填补了Wasserstein-DPO和KL-DPO的空白，实证验证了其在不同数据规模和模型大小下的最优认知性能与鲁棒性平衡。
...
TL;DR: CARGO是一种可扩展的多标签因果发现方法，用于处理稀疏、高维事件序列，通过两个预训练因果Transformer模型并行推断每条序列的因果图，并通过自适应频率融合聚合，有效实现大规模概率推理。


<details>
  <summary>Details</summary>
Motivation: 在医疗、车辆诊断等领域，理解事件序列中的因果关系（如症状导致疾病）是一个关键但尚未解决的挑战，尤其当数据具有高维稀疏性和大量不平衡标签时。

Method: 提出CARGO方法，使用两个针对事件序列的预训练因果Transformer作为基础模型，分两阶段：首先并行推断每条序列的一次性因果图，然后通过自适应频率融合聚合局部图以重建标签的全局马尔可夫边界。

Result: 在包含超过29,100种事件类型和474个不平衡标签的真实汽车故障预测数据集上验证了CARGO的有效性，展示了其结构化推理能力。

Conclusion: CARGO能够在高维稀疏事件序列中高效发现多标签因果关系，避免全数据集条件独立性检验的计算难题，适用于现实世界复杂场景。
...
TL;DR: 本文提出了FedFiTS，一种结合适应度驱动的客户端选择与分时聚合的信任与公平感知联邦学习框架，在医疗等敏感领域中实现了更高的准确性、鲁棒性和收敛效率。


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习在非独立同分布数据、客户端不可靠和对抗攻击下表现不佳，尤其在医疗等敏感领域面临隐私与安全挑战。

Method: 提出三阶段策略：自由训练、自然选择和分时团队参与，结合动态评分、自适应阈值和基于队列的调度机制，并进行理论收敛性与通信复杂度分析。

Result: 在医学影像、视觉基准和农业数据集上，FedFiTS在准确率、达到目标精度的时间及抗投毒攻击能力方面均优于FedAvg、FedRand和FedPow。

Conclusion: FedFiTS通过信任感知聚合与公平客户端选择，提升了联邦学习的可扩展性与安全性，适用于现实世界的跨领域部署。
...
TL;DR: 本文提出了两种向量（标准差向量和聚类向量）来分析大语言模型权重的分布与相关性特征，发现它们能有效区分不同模型并揭示同族模型间的相似性，且LoRA微调后标准差向量受数据集影响，而聚类向量保持与预训练模型高度一致。


<details>
  <summary>Details</summary>
Motivation: 为了深入理解大语言模型在架构和参数上的特性，研究者希望找到能够有效描述模型权重特征的方法，以分析不同模型之间的关联与差异。

Method: 提出标准差向量和聚类向量两种表示方法：前者基于投影矩阵的标准差归一化，假设权重服从正态分布；后者通过SVD提取奇异值并使用K-Means聚类，将同类矩阵的聚类结果组合成向量。

Result: 两种向量均能有效区分不同模型，并清晰展现同家族模型间的相似性；LoRA微调实验表明，标准差向量受训练数据集影响，而聚类向量在微调后仍保持与预训练模型高度一致的相关性结构。

Conclusion: 标准差向量反映模型权重的分布特性并受训练数据影响，聚类向量则刻画权重间的内在相关性且具有稳定性，两者结合可为模型分析提供有效的特征表示工具。
...
TL;DR: 本文提出了PipelineRL，一种通过并发异步数据生成和模型训练来提升大语言模型强化学习效率的方法，实现了更高的硬件利用率和更新鲜的训练数据，相比传统方法学习速度提升约2倍。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型强化学习方法在扩展时面临硬件利用率低和产生过时（off-policy）数据的问题，影响训练效果。

Method: 提出PipelineRL，采用并发异步的数据生成与模型训练，并引入飞行中权重更新（in-flight weight updates），使生成引擎在生成过程中能及时接收最新模型权重，减少中断。

Result: 在128块H100 GPU上实验表明，PipelineRL在保持高on-policy数据质量的同时，学习速度比传统RL基线快约2倍。

Conclusion: PipelineRL有效平衡了硬件效率与数据新鲜度，显著提升了大语言模型强化学习的可扩展性，并提供了可扩展、模块化的开源实现。
...
TL;DR: 本文提出了GSTM-HMU，一种生成式时空框架，用于建模人类移动的语义和时间复杂性，在多个真实数据集上显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以充分捕捉人类移动中的语义信息和长期生活习惯，缺乏可解释性和个性化建模能力。

Method: 提出GSTM-HMU框架，包含四个核心组件：时空概念编码器（STCE）、认知轨迹记忆（CTM）、生活方式概念库（LCB）和任务导向的生成头，联合建模位置、语义、时间和用户偏好。

Result: 在Gowalla、WeePlace等四个数据集上，于下一地点预测、轨迹-用户识别和时间估计三个任务中均取得显著性能提升。

Conclusion: GSTM-HMU能有效提取复杂移动数据中的语义规律，生成式建模为可解释、个性化和泛化的人类移动智能提供了有力支持。
...
TL;DR: 本研究探讨了激活函数在强化学习中神经网络训练动态及灾难性遗忘中的作用，提出了一类新的激活函数——大象激活函数，其能同时产生稀疏输出和稀疏梯度，有效缓解灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 目前对导致灾难性遗忘的神经网络架构特性理解不足，尤其是激活函数的作用尚不明确，本文旨在填补这一空白。

Method: 通过分析激活函数的梯度稀疏性及其对训练动态的影响，设计了一类新的具有稀疏输出和稀疏梯度的大象激活函数，并将其应用于基于值的强化学习算法中进行验证。

Result: 实验表明，使用大象激活函数可显著提升神经网络对灾难性遗忘的抵抗能力，在多个任务中实现了更高的样本效率和内存效率。

Conclusion: 激活函数的梯度稀疏性是减少灾难性遗忘的关键因素之一，引入大象激活函数为构建更高效、稳定的强化学习系统提供了新思路。
...
TL;DR: 本文提出了功能缩放定律（FSL），用于刻画大语言模型预训练过程中学习率调度对群体风险演化的影响，通过新型的内禀时间视角和随机微分方程建模，理论解释了多种经验性训练实践的有效性，并在0.1B到1B参数规模的模型上验证了FSL的实际应用价值。


<details>
  <summary>Details</summary>
Motivation: 现有缩放定律研究多关注最终损失，忽视训练过程中的损失动态及学习率调度的影响，本文旨在填补这一空白。

Method: 基于教师-学生核回归框架，采用在线随机梯度下降训练，结合内禀时间视角与随机微分方程（SDE）对SGD进行建模，提出功能缩放定律（FSL）。

Result: FSL通过卷积型泛函项显式捕捉学习率调度的影响；理论分析支持高容量模型更高效、学习率衰减提升效率、WSD类调度优于直接衰减等经验实践；实验验证FSL可有效拟合、预测和优化大模型预训练损失曲线。

Conclusion: FSL框架深化了对大语言模型预训练动态的理解，为大规模模型训练提供了理论支持与优化洞见。
...
TL;DR: 提出一种基于训练数据中“弱鲁棒”样本的模型鲁棒性验证方法，通过局部鲁棒性分析识别易受扰动影响的样本，用于早期发现模型漏洞并指导性能提升。


<details>
  <summary>Details</summary>
Motivation: 传统鲁棒性评估依赖扰动测试集，难以在训练早期发现模型脆弱性；而模型对常见扰动（如对抗攻击和常见噪声）敏感，影响可靠性。

Method: 在训练集中通过局部鲁棒性分析提取最易受扰动影响的“弱鲁棒”样本，将其作为敏感指标用于模型鲁棒性评估，并据此进行针对性优化。

Result: 在CIFAR-10、CIFAR-100和ImageNet上的实验表明，该方法能有效识别模型弱点，并显著提升模型在对抗攻击和常见噪声下的鲁棒性。

Conclusion: 利用训练集中的弱鲁棒样本进行鲁棒性验证是一种有效的早期诊断手段，有助于提升深度模型在扰动环境下的可靠性。
...
TL;DR: PPG-Distill是一种知识蒸馏框架，通过多层级蒸馏提升小型模型在可穿戴设备上的PPG信号分析效率，在心率估计和房颤检测任务中显著提升性能，同时实现更快的推理速度和更低的内存占用。


<details>
  <summary>Details</summary>
Motivation: 由于大型PPG基础模型难以部署在资源受限的可穿戴设备上，因此需要一种高效的知识迁移方法来提升轻量级模型的性能。

Method: 提出PPG-Distill框架，结合预测层、特征层和块级蒸馏，引入形态蒸馏保留局部波形模式，以及节律蒸馏捕捉块间时序结构。

Result: 在心率估计和房颤检测任务上，学生模型性能最高提升21.8%，推理速度快7倍，内存使用减少19倍。

Conclusion: PPG-Distill有效实现了PPG模型的轻量化，支持在可穿戴设备上高效进行生理信号分析。
...
TL;DR: FedFusion是一种联邦迁移学习框架，通过统一领域自适应和精简标注，在异构特征空间、非独立同分布数据和标签稀缺的客户端中实现高效联邦学习。


<details>
  <summary>Details</summary>
Motivation: 现实场景中的联邦学习面临特征空间异构、数据非独立同分布和标签稀缺等挑战，现有方法难以兼顾个性化、领域适应性和标注效率。

Method: 提出FedFusion框架，包含多样性/聚类感知编码器（DivEn系列），利用标记的教师客户端通过置信度过滤的伪标签和领域自适应迁移指导学习者客户端；采用相似性加权分类器耦合（可选聚类平均）保持全局一致性；结合自监督/半监督预训练与选择性微调的精简标注流程以减少标注需求。

Result: 在表格和图像基准数据集上，FedFusion在IID、非IID和标签稀缺情况下均优于最先进的基线方法，表现出更高的准确性、鲁棒性和公平性，同时通信和计算开销相当。

Conclusion: 将个性化、领域适应性和标签效率相结合是应对现实世界约束下鲁棒联邦学习的有效策略。
...
TL;DR: 本文研究了文本到视频生成模型的延迟和能耗，提出了一个计算受限的分析模型，并在多个模型上验证了其预测准确性。


<details>
  <summary>Details</summary>
Motivation: 由于文本到视频生成系统计算成本高且能耗不明确，需要对其性能和能效进行系统性研究。

Method: 开发了一个计算受限的分析模型，用于预测不同空间分辨率、时间长度和去噪步数下的扩展规律，并通过细粒度实验验证模型预测结果。

Result: 实验表明延迟和能耗随空间和时间维度呈二次增长，随去噪步数线性增长；在六种不同T2V模型上的分析提供了运行时间和能耗对比。

Conclusion: 研究为设计和部署更可持续的生成视频系统提供了基准参考和实用见解。
...
TL;DR: 本文研究了在能源转型背景下，结合物理约束的混合机器学习模型在电力系统潮流模拟中的应用，通过自定义基准工具LIPS对不同模型进行评估，涵盖准确性、物理一致性、工业适用性和分布外泛化能力四个方面。


<details>
  <summary>Details</summary>
Motivation: 随着可再生能源和跨境电力交换的增加，电网面临更大的不确定性和运行风险，传统物理求解器难以满足近实时需求，需要快速且符合物理规律的机器学习替代方案。

Method: 采用消融研究方法，比较了将物理约束作为正则化项或无监督损失的不同策略，并测试了从多层感知机到基于图网络的多种模型架构，利用自研基准平台LIPS进行系统评估。

Result: 结果显示，融入物理知识显著影响模型在准确性、物理合规性、工业准备度和分布外泛化方面的表现，不同混合策略各有优劣。

Conclusion: 物理信息的嵌入能提升模型性能，但需权衡复杂性与实用性，图神经网络结合直接物理方程优化展现出潜力，为工业级应用提供了可行路径。
...
TL;DR: 本文通过稳定性分析研究了分布式对抗训练的泛化性能，提出了在凸损失下的扩散策略中，泛化误差随对抗扰动强度和训练步数增长的理论界，并通过实验验证了结果。


<details>
  <summary>Details</summary>
Motivation: 对抗训练虽然提高了模型鲁棒性，但常出现鲁棒过拟合和泛化差距扩大问题，且其在去中心化网络中的泛化性质尚未被探索。

Method: 采用基于算法稳定性的方法，对基于扩散策略的对抗训练在凸损失下的泛化误差进行理论分析，并推导出相关误差界。

Result: 得出泛化误差随对抗扰动强度和训练步数增加而增大的理论界，并在逻辑回归任务上通过数值实验验证了该结论。

Conclusion: 对抗训练在去中心化设置下同样面临泛化误差随训练过程累积的问题，稳定性分析为理解其泛化行为提供了理论依据。
...
TL;DR: 该研究发现，与“越长越好”的观点相反，过长的思维链（CoT）和频繁回顾反而会降低大型推理模型的准确性。通过引入基于图结构的分析方法，提出“失败步骤比例（FSF）”作为更优的评估指标，发现有效CoT的关键在于“更少失败”，并验证了结构感知的测试时优化策略优于盲目延长CoT。


<details>
  <summary>Details</summary>
Motivation: 尽管大型推理模型在长思维链上消耗大量计算资源，但什么样的思维链才是真正有效的仍不明确。现有研究对延长CoT和增加回顾是否有益存在矛盾结论，因此需要系统性评估以揭示有效推理过程的本质特征。

Method: 在十个大型推理模型上对数学与科学推理任务进行系统评估，提出基于图结构的CoT表示方法，并引入‘失败步骤比例（FSF）’作为衡量指标；通过两种干预方式——按指标排序候选CoT以提升pass@1准确率，以及编辑去除失败分支来验证因果效应。

Result: 发现单纯延长CoT和增加回顾与更低的准确性相关；FSF指标在预测答案正确性方面显著优于长度和回顾比率；基于FSF的CoT排序带来最大的性能增益，而删除失败分支能显著提高模型准确率。

Conclusion: 有效的思维链并非越长越好或越多回顾越好，而是应减少失败推理分支。研究支持采用结构感知的测试时优化策略，强调质量优于长度，为高效推理提供了新的设计方向。
...
### [357] [Application Management in C-ITS: Orchestrating Demand-Driven Deployments and Reconfigurations](https://arxiv.org/abs/2509.18793)
*Lukas Zanger,Bastian Lampe,Lennart Reiher,Lutz Eckstein*

Main category: cs.RO

TL;DR: 本文提出了一种基于云原生技术（特别是Kubernetes）的需求驱动型应用管理方法，用于应对大规模协同智能交通系统（C-ITS）中动态环境下的资源高效利用问题。


<details>
  <summary>Details</summary>
Motivation: 随着车辆自动化和互联程度的提高，C-ITS对高效、动态的应用管理提出了更高要求，传统方法难以满足动态需求和资源优化的挑战。

Method: 采用Kubernetes和ROS 2构建应用管理框架，支持微服务的按需部署、重构、更新、升级和扩展，并通过动态协调机制响应来自C-ITS各实体的需求变化。

Result: 该框架能够在C-ITS的集体环境感知用例中有效运行，减少计算资源消耗和网络流量，并支持外部服务（如集体环境模型）的按需配置。

Conclusion: 所提出的框架能够有效支持C-ITS中复杂、动态的应用管理需求，提升系统效率与灵活性，且原型代码已开源。
...
### [358] [Proactive-reactive detection and mitigation of intermittent faults in robot swarms](https://arxiv.org/abs/2509.19246)
*Sinan Oğuz,Emanuele Garone,Marco Dorigo,Mary Katherine Heinrich*

Main category: cs.RO

TL;DR: 提出了一种针对机器人集群中间歇性故障的主动-被动检测与缓解策略，基于自组织备份层和多层网络中的分布式共识，在具有持久网络结构的集群中实现高效故障检测与恢复。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注永久性故障，而忽略了更难检测的间歇性故障；随着自组织神经系统（SoNS）方法实现了稳定的网络结构，为检测间歇性故障提供了新可能。

Method: 采用主动-被动结合策略：主动构建动态备份路径以适应拓扑变化；被动使用一次性似然比检验在多层网络中比较信息路径，实现早期故障检测，并通过自组织方式临时重路由通信。

Result: 在编队控制中验证了该方法对位置数据间歇性故障的有效性，能够防止故障干扰编队收敛，且具有高检测准确率和低误报率。

Conclusion: 所提方法有效解决了机器人集群中间歇性故障的检测与缓解问题，提升了系统在动态环境下的可靠性和鲁棒性。
...
### [359] [PEEK: Guiding and Minimal Image Representations for Zero-Shot Generalization of Robot Manipulation Policies](https://arxiv.org/abs/2509.18282)
*Jesse Zhang,Marius Memmel,Kevin Kim,Dieter Fox,Jesse Thomason,Fabio Ramos,Erdem Bıyık,Abhishek Gupta,Anqi Li*

Main category: cs.RO

TL;DR: PEEK是一种将视觉语言模型（VLM）与机器人操作策略解耦的框架，通过VLM预测端效应器路径和任务相关掩码，提升策略的零样本泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有机器人操作策略难以泛化，因需同时学习关注位置、动作选择与执行；而高阶语义推理可由VLM承担，策略应专注于动作执行。

Method: 提出PEEK框架，微调VLM以生成基于点的中间表示（包括端效应器路径和任务相关掩码），并开发自动化标注流水线，在20多个机器人数据集上生成训练数据。该表示可直接叠加于机器人观测，具策略无关性与跨架构可迁移性。

Result: 在真实世界实验中，PEEK显著提升零样本泛化性能：纯仿真训练的3D策略实现41.4倍性能提升，大型视觉语言动作模型（VLAs）和小型操作策略均获得2-3.5倍增益。

Conclusion: 通过将语义与视觉复杂性交由VLM处理，PEEK为操作策略提供了必要的最小化线索（何处、何事、如何），有效解耦感知与控制，提升了跨环境、跨架构的泛化能力。
...
### [360] [Fine-Tuning Robot Policies While Maintaining User Privacy](https://arxiv.org/abs/2509.18311)
*Benjamin A. Christie,Sagar Parekh,Dylan P. Losey*

Main category: cs.RO

TL;DR: 本文提出了PRoP，一种模型无关的个性化与隐私保护机器人策略框架，通过为每个用户分配唯一密钥来加密策略权重，实现个性化行为的同时防止外部代理窥探用户隐私。


<details>
  <summary>Details</summary>
Motivation: 在机器人个性化过程中，用户微调通用策略会泄露其偏好、习惯等敏感数据，如何在实现个性化的同时保护用户隐私成为一个关键问题。

Method: 提出PRoP框架，为每个用户分配唯一密钥，利用该密钥对机器人网络权重进行数学变换；只有使用正确密钥时机器人才展现个性化行为，否则恢复为基线行为。

Result: PRoP在模仿学习、强化学习和分类任务中展示了广泛适用性，保持原始策略结构和行为，并在实验上优于现有的基于编码器的方法。

Conclusion: PRoP有效实现了机器人策略的个性化与隐私保护平衡，具有实际应用优势和良好的扩展性。
...
### [361] [Haptic Communication in Human-Human and Human-Robot Co-Manipulation](https://arxiv.org/abs/2509.18327)
*Katherine H. Allen,Chris Rogers,Elaine S. Short*

Main category: cs.RO

TL;DR: 该研究通过比较人类-人类与人类-机器人共同操作物体时的运动数据，探讨了触觉通信在协作中的作用，发现人类之间的协作更流畅，并建议未来改进机器人以实现类人触觉信号交互。


<details>
  <summary>Details</summary>
Motivation: 探索在无明确信息传递的情况下，人类如何通过物体运动本身（即触觉通信）进行协作，并评估当前机器人在物理协作任务中模拟这种通信的能力。

Method: 记录人类-人类和人类-机器人共同移动物体时的物体运动（使用低成本IMU），并结合问卷调查与IMU数据分析，比较不同条件下的协作流畅性与准确性。

Result: IMU数据显示人类-人类协作的运动模式更流畅，问卷结果也表明其主观体验显著优于人类-机器人协作。

Conclusion: 人类在共同操作中能有效利用触觉通信，而当前机器人尚无法达到同等水平，需进一步研究使机器人具备收发类人触觉信号的能力。
...
### [362] [The Landform Contextual Mesh: Automatically Fusing Surface and Orbital Terrain for Mars 2020](https://arxiv.org/abs/2509.18330)
*Marsette Vona*

Main category: cs.RO

TL;DR: The Landform contextual mesh integrates 2D/3D rover images and orbital data into interactive 3D terrain models for Mars, used in mission planning and public outreach.


<details>
  <summary>Details</summary>
Motivation: To enhance situational awareness and decision-making for Mars 2020 mission operations by providing rich, integrated terrain visualizations.

Method: Automatically fusing thousands of 2D and 3D rover images with orbital elevation and color data to generate contextual meshes during ground processing.

Result: Interactive 3D terrain visualizations are produced for each rover location, utilized in the ASTTRO tool for scientists and partially released on a public website.

Conclusion: The Landform contextual mesh effectively supports both mission planning and public engagement through automated, high-fidelity 3D terrain modeling.
...
### [363] [Semantic-Aware Particle Filter for Reliable Vineyard Robot Localisation](https://arxiv.org/abs/2509.18342)
*Rajitha de Silva,Jonathan Cox,James R. Heselden,Marija Popovic,Cesar Cadena,Riccardo Polvara*

Main category: cs.RO

TL;DR: 提出一种结合语义信息的粒子滤波方法，利用葡萄树干和支撑杆等物体级特征提升机器人在葡萄园环境中的定位精度。


<details>
  <summary>Details</summary>
Motivation: 传统LiDAR方法在重复性高的葡萄园环境中因感知混淆而定位失败，需提高定位鲁棒性。

Method: 引入语义粒子滤波，将检测到的葡萄树干和支撑杆作为稳定地标，投影至鸟瞰图并与LiDAR数据融合；提出语义墙连接相邻地标以缓解行间混淆，并结合自适应噪声GPS先验维持全局一致性。

Result: 实验表明该方法能在真实葡萄园中保持正确行内定位，从偏离中恢复，并优于AMCL和RTAB-Map等现有方法。

Conclusion: 所提语义增强的定位方法有效解决了葡萄园等结构化农业环境中的感知别名问题，显著提升了定位可靠性与鲁棒性。
...
### [364] [AD-VF: LLM-Automatic Differentiation Enables Fine-Tuning-Free Robot Planning from Formal Methods Feedback](https://arxiv.org/abs/2509.18384)
*Yunhao Yang,Junyuan Hong,Gabriel Jacob Perin,Zhiwen Fan,Li Yin,Zhangyang Wang,Ufuk Topcu*

Main category: cs.RO

TL;DR: 提出LAD-VF，一种无需微调的框架，利用形式化验证反馈进行自动提示工程，显著提升大语言模型在机器人任务中的规范遵从性和成功率。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在物理世界部署中常因幻觉或对齐弱而违反安全与合规约束，传统对齐方法依赖昂贵的人工标注或资源密集型微调。

Method: 引入基于形式化验证的文本损失函数，结合LLM-AutoDiff技术，通过迭代优化提示而非模型参数来实现对齐。

Result: 在机器人导航与操作任务中，规范遵从的成功率从60%提升至90%以上。

Conclusion: LAD-VF提供了一种可扩展、可解释且无需微调的方法，推动实现可信的、经形式化验证的LLM驱动控制系统。
...
### [365] [Assistive Decision-Making for Right of Way Navigation at Uncontrolled Intersections](https://arxiv.org/abs/2509.18407)
*Navya Tiwari,Joseph Vazhaeparampil,Victoria Preston*

Main category: cs.RO

TL;DR: 提出一种基于POMDP的驾驶员辅助框架，用于无信号交叉口的路权推理，并在模拟环境中评估了四种决策方法，结果表明概率规划器在部分可观测条件下显著优于规则基线。


<details>
  <summary>Details</summary>
Motivation: 由于模糊的路权规则、遮挡和不可预测的驾驶行为，无控制交叉口事故频发，现有自动驾驶研究缺乏对人工驾驶车辆的辅助支持系统。

Method: 将右路权推理建模为部分可观测马尔可夫决策过程（POMDP），并在包含随机交通参与者、行人、遮挡和对抗场景的自定义仿真环境中评估了有限状态机（FSM）、QMDP、POMCP和DESPOT四种决策方法。

Result: 概率规划器表现优于确定性FSM，最高实现97.5%的无碰撞导航；其中POMCP更注重安全，DESPOT在效率与运行时间之间取得平衡。

Conclusion: 不确定性感知规划对驾驶辅助至关重要，未来需融合传感器融合与环境感知模块以支持实际交通环境中的实时部署。
...
### [366] [Latent Action Pretraining Through World Modeling](https://arxiv.org/abs/2509.18428)
*Bahey Tharwat,Yara Nasser,Ali Abouzeid,Ian Reid*

Main category: cs.RO

TL;DR: 提出了一种模型无关的自监督预训练框架LAWM，通过世界模型从无标签视频数据中学习潜在动作表示，适用于跨任务、环境和实体的迁移，在LIBERO基准和真实场景中表现优于现有方法，且更高效实用。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉-语言-动作（VLA）模型依赖大规模手动标注数据，模型庞大，难以在实际场景中部署；需要一种更高效、可迁移的自监督预训练方法。

Method: 提出LAWM框架，利用世界模型从无标签视频中学习潜在动作表示，支持从机器人或人类操作视频中提取动作特征，实现模型无关的自监督预训练。

Result: LAWM在LIBERO基准和真实世界设置中优于使用真实动作标签训练的模型及其他类似预训练方法，同时具有更高的效率和实用性。

Conclusion: LAWM是一种高效、实用且可迁移的自监督预训练框架，能够在不依赖标注数据的情况下提升模仿学习模型的性能，适合实际机器人应用。
...
### [367] [PrioriTouch: Adapting to User Contact Preferences for Whole-Arm Physical Human-Robot Interaction](https://arxiv.org/abs/2509.18447)
...
TL;DR: 本文提出了一种名为PrioriTouch的框架，用于在多点接触的物理人机交互中对控制目标进行排序和执行，通过结合学习排序方法与分层操作空间控制，能够在护理等复杂场景中有效平衡不同接触点的力需求，提升安全性与舒适性。


<details>
  <summary>Details</summary>
Motivation: 在物理人机交互中，尤其是护理任务中，机器人需要适应个体在多个身体部位上的不同接触偏好，而这些偏好可能存在冲突，难以同时满足，因此需要一种能够进行优先级排序的方法来处理多点接触中的权衡问题。

Method: 提出PrioriTouch框架，结合学习到排序（learning-to-rank）方法与分层操作空间控制，利用仿真循环 rollout 实现数据高效且安全的探索，并通过用户研究获取个性化舒适阈值以指导优先级排序。

Result: 在模拟和真实实验中验证了PrioriTouch能有效适应用户的接触偏好，在保持任务性能的同时提高安全性和舒适性。

Conclusion: PrioriTouch为多点接触的人机交互提供了一个通用且有效的优先级管理框架，特别适用于存在冲突力需求的护理场景，具有良好的实际应用潜力。
...
### [368] [Learning Geometry-Aware Nonprehensile Pushing and Pulling with Dexterous Hands](https://arxiv.org/abs/2509.18455)
*Yunshuang Li,Yiyang Ling,Gaurav S. Sukhatme,Daniel Seita*

Main category: cs.RO

TL;DR: 本文提出了一种基于几何感知的灵巧手推拉操作方法GD2P，通过接触引导采样和物理仿真筛选生成大量有效手部姿态，并利用条件扩散模型实现非抓握式操作，实验证明该方法在多种灵巧手上具有良好的泛化性和实用性。


<details>
  <summary>Details</summary>
Motivation: 非抓握操作（如推、拉）可处理难以抓取的物体，但其动力学建模困难；现有研究多依赖简单工具，而灵巧手具备更丰富的接触模式和稳定性，因此希望利用灵巧手提升非抓握操作的灵活性与鲁棒性。

Method: 将问题建模为预测接触前的灵巧手姿态：通过接触引导采样生成多样姿态，利用物理仿真过滤无效姿态，并训练一个以物体几何为条件的扩散模型来预测可行姿态；测试时结合运动规划选择并执行动作。

Result: 在Allegro Hand上进行了840次真实实验，结果表明GD2P优于基线方法；并在LEAP Hand上验证了其跨形态适用性；发布了包含130万手部姿态、覆盖2300个物体的数据集和预训练模型。

Conclusion: GD2P为灵巧手的非抓握操作提供了一个可扩展的训练框架，展示了基于几何感知和生成模型的方法在复杂操作任务中的潜力，并推动了相关数据与模型的开放共享。
...
### [369] [A Counterfactual Reasoning Framework for Fault Diagnosis in Robot Perception Systems](https://arxiv.org/abs/2509.18460)
*Haeyoon Han,Mahdi Taheri,Soon-Jo Chung,Fred Y. Hadaegh*

Main category: cs.RO

TL;DR: 提出一种基于反事实推理的感知系统故障检测与隔离（FDI）框架，利用分析冗余和因果推理生成可靠性测试，通过信念更新实现被动FDI，并将主动FDI建模为因果赌博机问题，使用MCTS与UCB寻找最大化有效信息（EI）的控制输入，在机器人探索场景中验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 感知系统的故障常与环境感知上下文相关，且多阶段流水线中的错误会在模块间传播，传统依赖物理冗余的故障检测方法成本高且不灵活，因此需要一种更智能、自适应的故障检测与隔离方法。

Method: 采用反事实推理构建分析冗余，构造受系统状态和故障情景影响的因果型可靠性测试；通过反事实推理生成假设故障下的测试结果，更新故障假设的置信度；提出被动FDI（基于信念更新）和主动FDI（建模为因果赌博机问题），后者使用MCTS结合UCB算法优化控制输入以最大化有效信息（EI）指标。

Result: 在空间机器人视觉导航探索任务中验证了该方法，能够主动调整姿态以提升EI，成功隔离由传感器损坏、动态场景和感知退化引起的故障，显著提高了故障检测与隔离的准确性和效率。

Conclusion: 基于反事实推理的FDI框架能有效利用因果关系和分析冗余进行感知系统故障诊断，主动FDI通过优化信息获取策略提升了故障隔离能力，为自动驾驶等安全关键系统提供了可解释且高效的故障管理方案。
...
### [370] [Robotic Skill Diversification via Active Mutation of Reward Functions in Reinforcement Learning During a Liquid Pouring Task](https://arxiv.org/abs/2509.18463)
*Jannick van Buuren,Roberto Giglio,Loris Roveda,Luka Peternel*

Main category: cs.RO

TL;DR: 该论文提出了一种通过在强化学习的奖励函数中引入高斯噪声来突变权重的方法，以在机器人操作任务中生成多样化的技能，特别是在液体倾倒任务中实现了从基本倾倒到容器边缘清洁、液体混合和浇水等新技能的多样化行为。


<details>
  <summary>Details</summary>
Motivation: 受人类运动控制中成本-收益权衡模型启发，旨在通过有意识地改变奖励函数来增强机器人在执行特定任务时的行为多样性，并探索潜在的意外有用技能。

Method: 设计了一个包含准确性、时间和 effort 三个关键项的奖励函数，并在其权重上添加高斯噪声以实现突变；在NVIDIA Isaac Sim仿真环境中使用Franka Emika Panda机械臂进行实验，采用PPO算法进行训练。

Result: 不同权重配置导致策略显著差异，产生了包括精确倾倒、容器边缘清洁、液体混合和浇水在内的多样化行为，表明突变后的奖励函数能引导出原任务之外的新颖且有用的技能。

Conclusion: 通过有意突变奖励函数可有效促进机器人在单一任务中学习多样化技能，并具备泛化至未来未知任务的潜力，为机器人自主技能扩展提供了新方向。
...
### [371] [RL-augmented Adaptive Model Predictive Control for Bipedal Locomotion over Challenging Terrain](https://arxiv.org/abs/2509.18466)
*Junnosuke Kamohara,Feiyang Wu,Chinmayee Wamorkar,Seth Hutchinson,Ye Zhao*

Main category: cs.RO

TL;DR: 提出了一种用于双足机器人在复杂地形（如楼梯、踏脚石和低摩擦表面）上行走的强化学习增强的模型预测控制（RL-augmented MPC）框架，通过参数化MPC的三个关键组件，实现了比传统MPC和RL更优的适应性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统的MPC在复杂地形中建模困难，而RL虽具鲁棒性但缺乏约束保证且依赖奖励设计；现有结合方法多限于平坦地形或四足机器人，因此需要一种适用于复杂地形双足行走的融合方法。

Method: 将强化学习引入MPC框架，参数化基于单刚体动力学MPC的系统动力学、摆动腿控制器和步频三个关键组件，并在NVIDIA IsaacLab仿真环境中进行训练与验证。

Result: 在多种复杂地形（包括楼梯、踏脚石和低摩擦表面）上的仿真结果表明，该方法相比基线MPC和RL方法展现出显著更强的适应性和鲁棒性。

Conclusion: 所提出的RL增强MPC框架有效结合了MPC的约束满足能力和RL的环境适应能力，为双足机器人在复杂和滑动地形上的稳定行走提供了可行解决方案。
...
### [372] [Spatial Envelope MPC: High Performance Driving without a Reference](https://arxiv.org/abs/2509.18506)
*Siyuan Yu,Congkai Shen,Yufei Xi,James Dallas,Michael Thompson,John Subosits,Hiroshi Yasuda,Tulga Ersal*

Main category: cs.RO

TL;DR: 本文提出了一种基于包络的无参考模型预测控制框架，用于实现自动驾驶车辆在多种高动态场景下的高性能驾驶。


<details>
  <summary>Details</summary>
Motivation: 传统基于参考轨迹的规划与控制方法在车辆动力学极限下表现受限，难以应对复杂动态环境中的安全高效行驶需求。

Method: 设计了一种面向优化控制的高效车辆动力学模型，并提出连续可微的数学形式来精确描述可行驶包络；结合强化学习与优化技术进行包络规划，实现动态可行性和安全性约束的统一集成。

Result: 通过仿真和实车实验验证了该框架在赛车、紧急避障和越野导航等任务中的高性能表现，展现出良好的可扩展性和广泛适用性。

Conclusion: 所提出的无参考包络式MPC框架能够有效提升自动驾驶在高动态极限场景下的性能与安全性，具有实际应用潜力。
...
### [373] [LCMF: Lightweight Cross-Modality Mambaformer for Embodied Robotics VQA](https://arxiv.org/abs/2509.18576)
*Zeyi Kang,Liang He,Yanxin Zhang,Zuheng Ming,Kaixing Zhao*

Main category: cs.RO

TL;DR: 本文提出了一种轻量级的LCMF级联注意力框架，通过在Mamba模块中引入多级跨模态参数共享机制，有效融合异构模态并实现语义互补对齐，在VQA和EQA视频任务中表现出色，同时显著降低计算开销，适用于资源受限的人机交互场景。


<details>
  <summary>Details</summary>
Motivation: 解决多模态语义学习中异构数据融合效率低和资源受限环境下计算开销大的问题。

Method: 提出LCMF级联注意力框架，结合Cross-Attention与选择性参数共享的SSM，在Mamba模块中实现多级跨模态参数共享。

Result: 在VQA任务中准确率达74.29%，EQA视频任务中达到与大型语言模型代理相当的中等性能，FLOPs减少4.35倍，参数量仅为166.51M（图像-文本）和219M（视频-文本）。

Conclusion: LCMF框架在保证高性能的同时显著提升计算效率，为资源受限场景下的多模态决策提供了高效解决方案。
...
### [374] [VLN-Zero: Rapid Exploration and Cache-Enabled Neurosymbolic Vision-Language Planning for Zero-Shot Transfer in Robot Navigation](https://arxiv.org/abs/2509.18592)
*Neel P. Bhatt,Yunhao Yang,Rohan Siva,Pranay Samineni,Daniel Milan,Zhangyang Wang,Ufuk Topcu*

Main category: cs.RO

TL;DR: VLN-Zero是一种无需微调即可在新环境中实现高效视觉-语言导航的两阶段框架，利用视觉-语言模型构建场景图并结合符号推理与缓存机制，显著提升成功率与效率。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言导航方法依赖大量探索或固定策略，难以在未见环境中快速适应和泛化，限制了实际应用的可扩展性。

Method: 提出VLN-Zero，包含两个阶段：探索阶段通过结构化提示引导视觉-语言模型生成紧凑的符号场景图；部署阶段采用神经符号规划器结合环境观测进行推理，并利用缓存机制复用历史轨迹以加速执行。

Result: 在多种环境中，VLN-Zero的平均成功率达现有最先进零样本模型的2倍，优于大多数微调基线，到达目标时间缩短一半，且VLM调用次数减少55%。

Conclusion: VLN-Zero通过结合快速探索、符号推理与缓存执行，在未见环境中实现了高效、鲁棒且可扩展的决策，显著提升了视觉-语言导航的泛化能力与实用性。
...
### [375] [Growing with Your Embodied Agent: A Human-in-the-Loop Lifelong Code Generation Framework for Long-Horizon Manipulation Skills](https://arxiv.org/abs/2509.18597)
*Yuan Meng,Zhenguo Sun,Max Fest,Xukun Li,Zhenshan Bing,Alois Knoll*

Main category: cs.RO

TL;DR: 提出一种人机协同框架，通过外部记忆和检索增强生成技术将修正知识编码为可重用技能，显著提升长视野机器人操作任务的成功率与效率。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的代码生成方法在处理长视野机器人操作任务时存在噪声多、上下文窗口有限、难以泛化等问题，且修正知识存储不当导致灾难性遗忘，缺乏可重用技能。

Method: 设计一个人机闭环框架，利用外部记忆模块存储修正信息，结合检索增强生成（RAG）与提示机制，动态复用可重用技能，实现对复杂长视野任务的有效规划与执行。

Result: 在Ravens、Franka Kitchen、MetaWorld及真实场景中实验表明，该方法成功率达到0.93，比基线最高提升27%，修正轮次效率提高42%，并能稳定完成如‘建房子’这类需规划20多个基本动作的极端长视野任务。

Conclusion: 所提框架有效解决了LLM在机器人操作中因推理能力受限和知识遗忘导致的性能下降问题，实现了高效、鲁棒的长视野任务执行，推动了可学习与复用技能的发展。
...
### [376] [End-to-End Crop Row Navigation via LiDAR-Based Deep Reinforcement Learning](https://arxiv.org/abs/2509.18608)
*Ana Luiza Mineiro,Francisco Affonso,Marcelo Becker*

Main category: cs.RO

TL;DR: 提出了一种基于深度强化学习的端到端导航系统，直接从3D LiDAR数据映射到控制指令，通过体素化降采样减少95.83%输入数据量，在仿真中实现了100%成功导航直行农田。


<details>
  <summary>Details</summary>
Motivation: 解决农业环境中GNSS不可靠、行间杂乱和光照变化导致的导航难题。

Method: 采用基于体素的降采样策略处理3D LiDAR数据，并使用纯仿真训练的深度强化学习策略实现端到端控制命令生成。

Result: 在仿真中对直行种植场景达到100%成功率，随着行曲率增加性能逐渐下降。

Conclusion: 该方法无需标注数据或人工设计接口，具备高效性和实用性，适用于复杂农业环境下的自主导航。
...
### [377] [PIE: Perception and Interaction Enhanced End-to-End Motion Planning for Autonomous Driving](https://arxiv.org/abs/2509.18609)
...
TL;DR: PIE是一个集成感知、推理和意图建模的端到端运动规划框架，通过融合相机和LiDAR数据并引入新型解码器，在无需集成或数据增强的情况下，在NAVSIM基准上超越了现有最先进方法。


<details>
  <summary>Details</summary>
Motivation: 现有的端到端自动驾驶系统在场景理解和决策预测方面仍面临挑战，限制了其大规模部署。

Method: 提出PIE框架，采用双向Mamba融合来减少多模态数据压缩损失，并设计了一个融合Mamba和专家混合模型的推理增强解码器，结合动作-运动交互模块利用周围智能体的状态预测优化自车规划。

Result: 在NAVSIM基准上，PIE取得了88.9的PDM分数和85.6的EPDM分数，优于先前的最先进方法。

Conclusion: PIE能够可靠地生成可行且高质量的自车轨迹，展示了其在复杂驾驶环境中优越的规划能力。
...
### [378] [SINGER: An Onboard Generalist Vision-Language Navigation Policy for Drones](https://arxiv.org/abs/2509.18610)
*Maximilian Adang,JunEn Low,Ola Shorinwa,Mac Schwager*

Main category: cs.RO

TL;DR: SINGER提出了一种基于机载感知与计算的语言引导无人机自主导航方法，通过逼真的语言嵌入仿真器、RRT启发的专家轨迹生成和轻量级端到端视觉运动策略，实现了在开放世界中对未见环境和语言目标的零样本迁移能力。


<details>
  <summary>Details</summary>
Motivation: 开放词汇的自主无人机导航因缺乏大规模演示数据、实时控制需求以及可靠位姿估计模块而仍具挑战性。

Method: SINGER采用三部分：基于高斯溅射的逼真语言嵌入飞行仿真器以减小仿真到现实的差距；RRT启发的多轨迹生成专家提供无碰撞导航示范；训练轻量级端到端视觉运动策略实现闭环实时控制。

Result: 在约70万至100万语言条件视觉运动数据对上训练后，硬件实验显示SINGER相比语义引导基线平均多达成目标23.33%，保持目标在视野内多16.67%，且碰撞减少10%。

Conclusion: SINGER实现了高效的零样本仿真到现实迁移，显著提升了语言引导下无人机在未知环境中的自主导航性能。
...
### [379] [The Case for Negative Data: From Crash Reports to Counterfactuals for Reasonable Driving](https://arxiv.org/abs/2509.18626)
*Jay Patrikar,Apoorva Sharma,Sushant Veer,Boyi Li,Sebastian Scherer,Marco Pavone*

Main category: cs.RO

TL;DR: 本文提出一种基于先例检索的自动驾驶决策系统，通过将事故报告与驾驶日志统一为场景-动作表示，提升决策安全性与校准性。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶系统主要基于无事故数据训练，缺乏接近安全边界时的对比性证据，而真实事故报告虽具价值但难以结构化利用。

Method: 将非结构化的第三人称事故叙述规范化为自我中心语言，并将驾驶日志与事故转换为统一的场景-动作表示用于检索；在决策时检索相关先例，结合反事实推理评估替代动作并做出决策。

Result: 在nuScenes基准上，先例检索使情境偏好动作的召回率从24%提升至53%，且系统校准性显著改善；引入反事实推理后仍保持增益，并在高风险区域决策更精准。

Conclusion: 通过统一事故与日志的语义表示并引入基于先例与反事实的推理机制，可有效提升自动驾驶系统在安全关键场景下的决策能力。
...
### [380] [Generalizable Domain Adaptation for Sim-and-Real Policy Co-Training](https://arxiv.org/abs/2509.18631)
*Shuo Cheng,Liqian Ma,Zhenyang Chen,Ajay Mandlekar,Caelan Garrett,Danfei Xu*

Main category: cs.RO

TL;DR: 提出了一种基于模拟和真实数据协同训练的统一框架，通过学习领域不变且任务相关的特征空间，利用最优传输启发的损失函数，显著提升机器人操作策略在真实场景中的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 行为克隆在机器人操作中具有潜力，但真实世界演示数据获取成本高；纯模拟数据存在仿真与现实之间的域差距，难以有效迁移。

Method: 提出一种模拟与真实协同训练框架，核心是学习领域不变且任务相关的特征空间，并引入受最优传输（OT）启发的损失函数，扩展为非平衡OT以应对模拟数据丰富而真实数据稀少的问题。

Result: 在具有挑战性的操作任务上验证了方法的有效性，相比仅使用少量真实数据的方法，真实环境成功率最高提升了30%，并能泛化到仅在模拟中见过的情景。

Conclusion: 该方法能有效利用大量模拟数据和少量真实演示，提升策略在真实世界中的表现和泛化能力，为仿真到现实的迁移提供了高效解决方案。
...
### [381] [Number Adaptive Formation Flight Planning via Affine Deformable Guidance in Narrow Environments](https://arxiv.org/abs/2509.18636)
*Yuan Zhou,Jialiang Hou,Guangtong Xu,Fei Gao*

Main category: cs.RO

TL;DR: 本文提出了一种基于可变形虚拟结构（DVS）的编队规划方法，用于在无人机数量变化和狭窄环境中保持编队形态。通过PAAS策略实现均匀划分与分配，结合时空轨迹规划与分布式避障，实现了快速恢复编队并适应复杂环境的能力。


<details>
  <summary>Details</summary>
Motivation: 在无人机数量动态变化且环境狭窄的情况下，传统编队控制难以维持形状完整性并保证安全性，因此需要一种具备自适应性和鲁棒性的编队规划方法。

Method: 采用Lloyd算法进行均匀划分（PA），匈牙利算法进行分配（AS），构成PAAS策略；利用基于基元的路径搜索和非线性优化规划DVS的时空轨迹；通过仿射变换实现对不同无人机数量和狭窄环境的适应；各无人机在DVS引导下进行分布式轨迹规划，并集成碰撞规避与动力学约束。

Result: 仿真表明该方法支持最多15%的无人机加入或离开集群，并能快速恢复期望编队；相比前沿方法，具备更快的恢复速度和更强的环境适应能力；真实实验验证了方法的有效性与鲁棒性。

Conclusion: 所提出的基于DVS的编队规划方法能够有效应对无人机数量变化和复杂空间约束，实现了安全、灵活且高效的多无人机编队控制。
...
### [382] [Do You Need Proprioceptive States in Visuomotor Policies?](https://arxiv.org/abs/2509.18644)
...
TL;DR: 本文提出了一种无状态的模仿学习策略（State-free Policy），仅依赖视觉观测进行机器人操作控制，显著提升了空间泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的基于模仿学习的视觉运动策略过度依赖本体感知状态输入，导致训练轨迹过拟合和空间泛化性能差。

Method: 移除本体感知状态输入，仅使用双广角腕部相机提供的视觉观测，在相对末端执行器动作空间中构建无状态策略。

Result: 在真实世界的抓取放置、衬衫折叠和全身操作等任务中，高度泛化成功率从0%提升至85%，水平泛化从6%提升至64%，并在数据效率和跨形态适应性上表现更优。

Conclusion: 无状态策略通过仅依赖视觉输入显著增强了空间泛化能力和实际部署的实用性。
...
### [383] [SPiDR: A Simple Approach for Zero-Shot Safety in Sim-to-Real Transfer](https://arxiv.org/abs/2509.18648)
*Yarden As,Chengrui Qu,Benjamin Unger,Dongho Kang,Max van der Hart,Laixi Shi,Stelian Coros,Adam Wierman,Andreas Krause*

Main category: cs.RO

TL;DR: SPiDR（Sim-to-real via Pessimistic Domain Randomization）是一种具有可证明安全保证的可扩展算法，用于解决模拟到现实迁移中的安全问题。


<details>
  <summary>Details</summary>
Motivation: 强化学习在现实世界应用中的安全性是一个主要关注点，而模拟器与现实之间的差异（sim-to-real gap）带来了额外的安全挑战。现有的鲁棒安全RL方法通常难以与标准训练流程兼容，而常用的域随机化方法又常导致实际中的不安全行为。

Method: 提出SPiDR方法，通过悲观的域随机化将sim-to-real差距的不确定性纳入安全约束中，从而在保持与现有训练流水线高度兼容的同时实现安全迁移。

Result: 在多个模拟基准和两个真实机器人平台上进行了广泛实验，结果表明SPiDR能有效确保跨sim-to-real迁移过程中的安全性，同时保持良好性能。

Conclusion: SPiDR是一种实用且可扩展的安全sim-to-real迁移方法，兼具理论保证和实际有效性，为现实世界中安全强化学习的应用提供了可行路径。
...
### [384] [Distributionally Robust Safe Motion Planning with Contextual Information](https://arxiv.org/abs/2509.18666)
*Kaizer Rahaman,Simran Kumari,Ashish R. Hota*

Main category: cs.RO

TL;DR: 提出一种结合上下文信息的分布鲁棒碰撞避免方法，通过条件核均值嵌入在再生核希尔伯特空间中建模障碍物轨迹的条件分布，并构造分布鲁棒性约束用于运动规划。


<details>
  <summary>Details</summary>
Motivation: 传统碰撞避免方法未充分考虑上下文信息和不确定性，导致在复杂场景下性能不足，因此需要引入分布鲁棒性和上下文感知机制以提升安全性。

Method: 将障碍物未来轨迹关于自车运动的条件分布嵌入再生核希尔伯特空间（RKHS），利用条件核均值嵌入算子构建经验估计，并定义包含所有与该估计在RKHS中距离内的分布的模糊集，进而建立分布鲁棒碰撞避免约束并集成到滚动时域运动规划中。

Result: 仿真结果表明，所提方法在多个挑战性场景中相比不考虑上下文信息或分布鲁棒性的方法能更有效地避免碰撞。

Conclusion: 结合上下文信息与分布鲁棒优化的碰撞避免框架显著提升了自动驾驶决策的安全性与鲁棒性，适用于高动态和不确定环境。
...
### [385] [N2M: Bridging Navigation and Manipulation by Learning Pose Preference from Rollout](https://arxiv.org/abs/2509.18671)
*Kaixin Chai,Hyunjun Lee,Joseph J. Lim*

Main category: cs.RO

TL;DR: 本文提出了一种名为N2M的过渡模块，用于在移动操作中引导机器人到达更适合下游操作的初始姿态，显著提高了任务成功率。


<details>
  <summary>Details</summary>
Motivation: 移动操作中，导航模块仅关注到达任务区域，而忽略了下游操作对初始姿态的偏好，导致任务成功率低。

Method: 提出N2M过渡模块，基于自我中心观测引导机器人调整至适合操作的初始姿态，不依赖全局或历史信息，并具备实时适应环境变化的能力。

Result: 在PnPCounterToCab任务中，任务成功率从3%提升至54%；在Toybox Handover任务中，仅用15个数据样本即可在新环境中实现可靠预测。

Conclusion: N2M具有高数据效率、强泛化能力、广泛适用性，并能显著提升移动操作任务的成功率。
...
### [386] [3D Flow Diffusion Policy: Visuomotor Policy Learning via Generating Flow in 3D Space](https://arxiv.org/abs/2509.18676)
*Sangjun Noh,Dongwoo Nam,Kangmin Kim,Geonhyup Lee,Yeonguk Yu,Raeyoung Kang,Kyoobin Lee*

Main category: cs.RO

TL;DR: 提出3D Flow Diffusion Policy（3D FDP），利用场景级3D光流作为中间表示来捕捉细粒度局部运动线索，显著提升机器人在多样化操作任务中的泛化能力和性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常忽略对精确、接触丰富的操作至关重要的局部运动线索，难以实现跨对象和动态交互的泛化。

Method: 引入3D FDP框架，通过预测采样查询点的时间轨迹，并在统一的扩散架构中以交互感知的3D流为条件生成动作。

Result: 在MetaWorld基准的50个任务上达到SOTA性能，尤其在中高难度任务表现突出；在8个真实机器人任务中优于基线方法，特别是在接触丰富和非抓取操作场景。

Conclusion: 3D光流可作为学习可泛化视觉运动策略的有效结构先验，有助于实现更鲁棒、更通用的机器人操作。
...
### [387] [Query-Centric Diffusion Policy for Generalizable Robotic Assembly](https://arxiv.org/abs/2509.18686)
*Ziyi Xu,Haohong Lin,Shiqi Liu,Ding Zhao*

Main category: cs.RO

TL;DR: 提出了一种基于查询的扩散策略（QDP），通过结合物体、接触点和技能信息的查询，桥接高层规划与低层控制，提升机器人装配任务中的精度和成功率。


<details>
  <summary>Details</summary>
Motivation: 机器人装配任务因部件交互复杂且对接触环境中的噪声敏感而具有挑战性，传统分层策略存在高层指令与底层执行不匹配的问题。

Method: 提出Query-centric Diffusion Policy（QDP），采用以查询为中心的机制，利用包含物体、接触点和技能信息的查询来指导低层策略，并结合点云观测增强鲁棒性。

Result: 在FurnitureBench仿真和真实环境中实验表明，QDP在技能精度和长视野任务成功率上优于基线方法，在插入和拧紧任务中技能成功率提升超过50%。

Conclusion: QDP有效解决了高层规划与低层控制之间的不匹配问题，显著提升了复杂装配任务的性能和鲁棒性。
...
### [388] [Learning Obstacle Avoidance using Double DQN for Quadcopter Navigation](https://arxiv.org/abs/2509.18734)
*Nishant Doshi,Amey Sutvani,Sanket Gujar*

Main category: cs.RO

TL;DR: 本文提出了一种基于强化学习的虚拟四旋翼机器人在模拟城市环境中使用深度相机进行导航的方法，以应对GPS精度下降、狭窄空间和动态障碍物等挑战。


<details>
  <summary>Details</summary>
Motivation: 由于城市环境中GPS精度降低、空间狭窄以及动态移动障碍物的存在，自主飞行器的可靠导航成为一个复杂的问题，需要具备利用机载深度传感器信息避障的能力。

Method: 采用强化学习方法训练配备深度相机的虚拟四旋翼机器人，在模拟城市环境中实现自主导航。

Result: 实现了虚拟四旋翼机器人在复杂城市环境中的有效避障与导航。

Conclusion: 强化学习结合深度相机能够提升自主飞行器在城市环境中的导航可靠性。
...
### [389] [MV-UMI: A Scalable Multi-View Interface for Cross-Embodiment Learning](https://arxiv.org/abs/2509.18757)
*Omar Rayyan,John Abanes,Mahmoud Hafez,Anthony Tzes,Fares Abu-Dakka*

Main category: cs.RO

TL;DR: 本文提出了MV-UMI框架，通过结合第三人称视角和第一人称相机，提升手持式采集设备在机器人操作任务中的场景理解能力，显著提高了跨形态模仿学习的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的手持式数据采集设备依赖于腕部摄像头的第一人称视角，难以捕捉足够的场景上下文，限制了其在复杂操作任务中的应用。

Method: 提出MV-UMI框架，融合第三人称视角与第一人称相机输入，缓解人类示范与机器人执行之间的域偏移，同时保持跨形态数据采集的优势。

Result: 实验结果显示，在需要广泛场景理解的子任务中，性能平均提升约47%，并在三个任务上验证了方法的有效性。

Conclusion: MV-UMI有效扩展了手持式采集系统可学习的操作任务范围，同时保留了其跨机器人形态的通用性优势。
...
### [390] [VGGT-DP: Generalizable Robot Control via Vision Foundation Models](https://arxiv.org/abs/2509.18778)
*Shijia Ge,Yinxin Zhang,Shuzhao Xie,Weixiang Zhang,Mingcai Zhou,Zhi Wang*

Main category: cs.RO

TL;DR: 提出VGGT-DP，一种结合3D感知先验和本体感觉反馈的视觉运动策略框架，在MetaWorld任务中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有视觉模仿学习方法忽视视觉编码器结构与容量，限制空间理解与泛化能力；受生物视觉系统启发，需融合视觉与本体感觉信息以提升控制鲁棒性。

Method: 采用VGGT作为视觉编码器，引入本体感觉引导的视觉学习策略，并设计逐帧令牌复用机制和随机令牌剪枝以降低延迟并增强鲁棒性。

Result: 在MetaWorld任务上显著优于DP、DP3等强基线，尤其在高精度和长视野场景中表现突出。

Conclusion: VGGT-DP通过融合几何先验与本体感觉反馈，有效提升了视觉模仿学习的空间接地能力与闭环控制性能。
...
### [391] [Human-Interpretable Uncertainty Explanations for Point Cloud Registration](https://arxiv.org/abs/2509.18786)
*Johannes A. Gaus,Loris Schneider,Yitian Shi,Jongseok Lee,Rania Rayyes,Rudolph Triebel*

Main category: cs.RO

TL;DR: 提出了一种新的点云配准方法GP-CA，能量化并解释配准中的不确定性，结合主动学习提升效率和准确性，在多个数据集和真实机器人实验中表现出优越性能。


<details>
  <summary>Details</summary>
Motivation: 传统方法如ICP在存在传感器噪声、位姿估计误差和遮挡导致的部分重叠时表现不佳，难以处理不确定性。

Method: 提出高斯过程概念归因（GP-CA），通过建模量化配准不确定性，并将不确定性归因于已知误差源；结合主动学习发现新的不确定性来源。

Result: 在三个公开数据集和真实机器人实验中验证了GP-CA的有效性，相比现有方法具有更快的运行速度、更高的采样效率和精度，且支持失效恢复行为。

Conclusion: GP-CA能有效处理点云配准中的不确定性，具备良好的可解释性和实用性，提升了机器人感知的鲁棒性。
...
### [392] [DexSkin: High-Coverage Conformable Robotic Skin for Learning Contact-Rich Manipulation](https://arxiv.org/abs/2509.18830)
*Suzannah Wistreich,Baiyu Shi,Stephen Tian,Samuel Clarke,Michael Nath,Chengyi Xu,Zhenan Bao,Jiajun Wu*

Main category: cs.RO

TL;DR: 本文提出了一种名为DexSkin的柔软、可贴合的电容式电子皮肤，用于实现灵巧机器人操作中的敏感、局部和可校准触觉感知，并展示了其在学习复杂操作任务中的有效性。


<details>
  <summary>Details</summary>
Motivation: 人类皮肤具有丰富的触觉感知能力，能够在大范围且复杂的表面上定位有意或无意的接触事件。然而，在机器人系统中复制这种能力仍然是一个长期存在的挑战。因此，需要一种能够适应不同几何形状并提供全面触觉反馈的电子皮肤来支持数据驱动的机器人操作学习。

Method: 开发了DexSkin，一种软性、可贴合的电容式电子皮肤，具备高灵敏度、局部感知和可校准特性，并将其应用于平行夹爪的两个手指上以实现全表面触觉覆盖；通过模仿学习框架评估其在物体重定向和橡皮筋包裹等复杂操作任务中的性能，同时验证其跨传感器实例的模型迁移能力和在线强化学习的实际应用。

Result: 实验表明，DexSkin能在整个手指表面提供有效的触觉感知，成功支持多种复杂操作任务的学习，并可在不同传感器实例间进行校准以实现模型转移，适用于真实机器人上的在线强化学习。

Conclusion: DexSkin是一种实用且高效的触觉传感解决方案，适用于现实世界中高接触密度的机器人操作任务学习。
...
### [393] [Bi-VLA: Bilateral Control-Based Imitation Learning via Vision-Language Fusion for Action Generation](https://arxiv.org/abs/2509.18865)
*Masato Kobayashi,Thanpimon Buamanee*

Main category: cs.RO

TL;DR: 提出了一种名为Bi-VLA的新框架，通过视觉-语言融合扩展了双边控制模仿学习，使其能在单一模型中处理多个任务，提升了任务成功率和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统双边控制方法依赖任务特定模型，缺乏通用性，限制了在多任务场景中的应用。

Method: 结合机器人关节角度、速度、扭矩数据与视觉特征及自然语言指令，采用SigLIP和FiLM进行视觉-语言融合，在统一模型中实现多任务模仿学习。

Result: 在真实机器人实验中，Bi-VLA成功解析视觉-语言组合，在两类任务上均提高了任务成功率，优于传统方法。

Conclusion: Bi-VLA克服了传统双边控制仅限单任务的局限，验证了视觉与语言融合对提升机器人操作通用性和灵活性的有效性。
...
### [394] [Lang2Morph: Language-Driven Morphological Design of Robotic Hands](https://arxiv.org/abs/2509.18937)
*Yanyuan Qiao,Kieran Gilday,Yutong Xie,Josie Hughes*

Main category: cs.RO

TL;DR: 本文提出了Lang2Morph，一种基于大语言模型的机器人手部形态设计框架，能够将自然语言任务描述转化为可3D打印的任务特定结构。


<details>
  <summary>Details</summary>
Motivation: 传统机器人手部设计依赖专家经验且手动调参繁琐，自动化方法计算成本高且难以实现灵巧操作，因此需要一种高效、零样本的设计方法。

Method: 提出Lang2Morph框架，利用大语言模型将自然语言任务描述转换为符号结构和OPH兼容参数，包含形态设计与选择优化两个阶段。

Result: 在多种任务上验证了该方法能生成多样化且与任务相关的手部形态，且无需训练即可实现零样本生成。

Conclusion: Lang2Morph是首个基于大语言模型的面向任务的机器人手部设计框架，展示了语言驱动设计在机器人形态生成中的潜力。
...
### [395] [Eva-VLA: Evaluating Vision-Language-Action Models' Robustness Under Real-World Physical Variations](https://arxiv.org/abs/2509.18953)
*Hanqing Liu,Jiahuan Long,Junqi Wu,Jiacheng Hou,Huili Tang,Tingsong Jiang,Weien Zhou,Wen Yao*

Main category: cs.RO

TL;DR: 本文提出了Eva-VLA，首个系统评估视觉-语言-动作（VLA）模型在真实物理变化下鲁棒性的统一框架，通过将离散的物理变化转化为连续优化问题，揭示了现有VLA模型在物体3D变换、光照变化和对抗性补丁下的严重脆弱性。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在真实世界物理变化下的鲁棒性尚未得到充分探索，缺乏系统性评估方法，导致实验室表现与实际部署之间存在巨大差距。

Method: 将物理变化分解为物体3D变换、光照变化和对抗性补丁三个关键领域，并提出一种连续黑盒优化框架，将离散变化转化为参数优化，以高效发现最坏情况场景。

Result: 在多个基准上的实验表明，当前先进的OpenVLA模型在各类物理变化下失败率均超过60%，其中物体3D变换在长视野任务中导致高达97.8%的失败率。

Conclusion: Eva-VLA框架揭示了VLA模型在现实部署中的关键缺陷，为提升其鲁棒性提供了可行路径。
...
### [396] [Towards Robust LiDAR Localization: Deep Learning-based Uncertainty Estimation](https://arxiv.org/abs/2509.18954)
*Minoo Dolatabadi,Fardin Ayar,Ehsan Javanmardi,Manabu Tsukada,Mahdi Javanmardi*

Main category: cs.RO

TL;DR: 提出一种基于深度学习的数据驱动框架，可在无参考地图的情况下预测ICP配准的6自由度误差协方差，提升LiDAR定位与SLAM的精度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: ICP算法在特征稀疏或动态环境中易产生错误位姿估计，且现有不确定性预测方法依赖手工模型或简化假设；现有深度学习方法多需预建地图或仅提供二分类结果，无法准确建模不确定性。

Method: 设计一种数据驱动的深度学习框架，在匹配前直接从单帧LiDAR扫描预测ICP的6-DoF误差协方差，无需依赖预建地图，并将预测的协方差用于Kalman滤波中以实现更优的状态估计。

Result: 在KITTI数据集上验证了方法的有效性，能够准确预测协方差，在有地图定位和SLAM任务中均显著降低定位误差并提升系统鲁棒性。

Conclusion: 该方法实现了无需参考地图的ICP不确定性估计，有效提升了LiDAR-based定位与SLAM系统的性能，具有良好的实用性与集成能力。
...
### [397] [Category-Level Object Shape and Pose Estimation in Less Than a Millisecond](https://arxiv.org/abs/2509.18979)
*Lorenzo Shaikewitz,Tim Nguyen,Luca Carlone*

Main category: cs.RO

TL;DR: 提出了一种快速的局部求解器，用于基于类别级先验的物体形状和姿态估计，通过自洽场迭代高效求解，并提供全局最优性证书。


<details>
  <summary>Details</summary>
Motivation: 物体形状和姿态估计是机器人学中的基础问题，现有方法在速度和全局最优性保证方面存在不足。

Method: 使用学习的前端检测稀疏的类别级语义关键点，采用线性主动形状模型表示未知形状，构建基于最大后验的优化问题，并用四元数表达，通过自洽场迭代求解特征值问题。

Result: 求解器每次迭代约100微秒，具有高效的计算速度和全局最优性验证能力，在合成数据、公开数据集和无人机追踪场景中表现良好。

Conclusion: 该方法实现了快速、可靠的形状与姿态估计，适用于需要实时性能和鲁棒性的机器人应用。
...
### [398] [Pure Vision Language Action (VLA) Models: A Comprehensive Survey](https://arxiv.org/abs/2509.19012)
*Dapeng Zhang,Jin Sun,Chenghui Hu,Xiaoyan Wu,Zhenlong Yuan,Rui Zhou,Fei Shen,Qingguo Zhou*

Main category: cs.RO

TL;DR: 该论文综述了视觉语言动作（VLA）模型的最新进展，提出分类体系，分析不同范式的方法、应用及挑战，展望未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 推动通用机器人发展，将视觉语言模型从被动生成器转变为主动决策与操作的智能体。

Method: 对300多项研究进行系统综述，分类为自回归、扩散、强化学习、混合和专用方法，并分析数据集、基准和仿真平台。

Result: 建立了VLA方法的清晰分类体系，全面总结了现有技术、应用场景与核心策略。

Conclusion: VLA模型是实现通用机器人的关键路径，未来需解决可扩展性、泛化能力与实际部署中的挑战。
...
### [399] [Reduced-Order Model-Guided Reinforcement Learning for Demonstration-Free Humanoid Locomotion](https://arxiv.org/abs/2509.19023)
*Shuai Liu,Meng Cheng Lau*

Main category: cs.RO

TL;DR: 提出了一种名为ROM-GRL的两阶段强化学习框架，用于人形机器人行走，无需动作捕捉数据或复杂奖励设计。


<details>
  <summary>Details</summary>
Motivation: 为了在无人类示范和复杂奖励设计的情况下实现稳定、高效的人形机器人步态控制。

Method: 第一阶段使用4-DOF简化模型通过PPO训练生成节能步态模板；第二阶段利用SAC结合对抗判别器，使全身策略模仿该模板的五维步态特征分布。

Result: 在1m/s和4m/s速度下实验显示，相比纯奖励方法，ROM-GRL实现了更稳定、对称的步态，并显著降低了跟踪误差。

Conclusion: ROM-GRL弥合了仅依赖奖励与基于模仿的运动控制方法之间的差距，实现了无需人类示范的自然、多样的人形机器人行走行为。
...
### [400] [TacEva: A Performance Evaluation Framework For Vision-Based Tactile Sensors](https://arxiv.org/abs/2509.19037)
*Qingzheng Cong,Steven Oh,Wen Fan,Shan Luo,Kaspar Althoefer,Dandan Zhang*

Main category: cs.RO

TL;DR: 本文提出了一种名为TacEva的视觉型触觉传感器（VBTS）综合评估框架，通过定义一系列性能指标和标准化实验流程，实现对不同VBTS设计的定量分析与比较，有助于任务导向的传感器选型与设计优化。


<details>
  <summary>Details</summary>
Motivation: 由于现有视觉型触觉传感器在感知机制、结构尺寸等方面的差异导致性能参差不齐，且缺乏统一的评估标准，使得针对特定任务的优化变得困难。

Method: TacEva框架定义了涵盖典型应用场景的关键性能指标，并为每个指标设计了结构化的实验流程，以确保量化结果的一致性和可重复性，进而对多种不同机制的VBTS进行系统评估。

Result: 该框架成功应用于多种VBTS，能够全面评估各设计性能，并为各个性能维度提供定量指标，验证了其有效性。

Conclusion: TacEva为VBTS提供了标准化的定量评估方法，支持按任务需求预选最优传感器，并为后续设计优化提供性能指导，推动了该领域的规范化发展。
...
### [401] [ManipForce: Force-Guided Policy Learning with Frequency-Aware Representation for Contact-Rich Manipulation](https://arxiv.org/abs/2509.19047)
*Geonhyup Lee,Yeongjin Lee,Kangmin Kim,Seongju Lee,Sangjun Noh,Seunghyeok Back,Kyoobin Lee*

Main category: cs.RO

TL;DR: 提出ManipForce系统和Frequency-Aware Multimodal Transformer（FMT），通过融合高频率力矩与视觉数据提升接触丰富操作任务的模仿学习性能。


<details>
  <summary>Details</summary>
Motivation: 现有模仿学习方法主要依赖纯视觉演示，难以满足接触丰富操作任务中对交互力精确控制的需求。

Method: 设计ManipForce手持系统以采集自然人类演示中的高频力矩（F/T）和RGB数据；提出FMT模型，利用频率和模态感知嵌入编码异步RGB和F/T信号，并通过Transformer扩散策略中的双向交叉注意力进行融合。

Result: 在六个真实世界的接触丰富操作任务上实验表明，基于ManipForce数据训练的FMT平均成功率达83%，显著优于纯RGB基线方法；消融和采样频率分析证实高频频力数据和跨模态融合能提升策略性能，尤其在高精度和稳定接触要求的任务中。

Conclusion: 结合高频率力觉与视觉的多模态模仿学习可有效提升接触丰富操作任务的性能，验证了力-视觉融合对精确控制的重要性。
...
### [402] [SlicerROS2: A Research and Development Module for Image-Guided Robotic Interventions](https://arxiv.org/abs/2509.19076)
...
TL;DR: 本文介绍了SlicerROS2的新设计及其在真实图像引导机器人场景中的四个应用，该模块结合了3D Slicer和ROS，用于医学机器人研究。


<details>
  <summary>Details</summary>
Motivation: 为了提供一种标准的医学机器人研究集成方法，改进之前的版本以实现更高的模块化和功能访问。

Method: 重写和重新设计SlicerROS2模块，增加对低级特性和3D Slicer的Python API的访问，并改进数据传输协议。

Result: 实现了更模块化的系统，支持实时加载和可视化机器人，并展示了四个实际应用场景。

Conclusion: 新设计的SlicerROS2模块为医学机器人研究提供了更强大、灵活的工具，推动了图像引导机器人干预的发展。
...
### [403] [World4RL: Diffusion World Models for Policy Refinement with Reinforcement Learning for Robotic Manipulation](https://arxiv.org/abs/2509.19080)
*Zhennan Jiang,Kai Liu,Yuxin Qin,Shuai Tian,Yupeng Zheng,Mingcai Zhou,Chao Yu,Haoran Li,Dongbin Zhao*

Main category: cs.RO

TL;DR: 本文提出World4RL，一个基于扩散模型的高保真世界模型框架，用于在纯想象环境中端到端优化机器人操作策略，避免真实环境交互，显著提升策略成功率。


<details>
  <summary>Details</summary>
Motivation: 模仿学习受限于专家数据稀缺和覆盖范围窄，强化学习在真实机器人上训练成本高且不安全，在仿真中训练又存在仿真到现实的差距。因此需要一种无需真实环境交互即可高效提升策略性能的方法。

Method: 提出World4RL框架，使用预训练的扩散模型作为世界模型，在冻结的高保真想象环境中进行策略优化；设计了专为机器人操作定制的双热编码动作表示，并采用扩散模型骨干网络提升建模精度，实现端到端的策略 refinement。

Result: 在模拟和真实机器人实验中，World4RL展现出高保真的环境建模能力，策略经过纯想象训练后成功率达到显著提升，优于模仿学习及其他基线方法。

Conclusion: 扩散模型可作为高保真世界模型有效支持机器人操作策略的纯想象优化，World4RL为解决sim-to-real问题和减少真实环境交互提供了新思路。
...
### [404] [FUNCanon: Learning Pose-Aware Action Primitives via Functional Object Canonicalization for Generalizable Robotic Manipulation](https://arxiv.org/abs/2509.19102)
...
TL;DR: 本文提出了FunCanon框架，通过将长周期操作任务分解为基于动作块的序列，并利用功能对象规范化实现跨任务和类别的一般化机器人操作策略。


<details>
  <summary>Details</summary>
Motivation: 传统从演示中学习的机器人策略往往局限于特定任务，缺乏在训练分布之外的泛化能力。因此，需要一种更具组合性和可重用性的方法来提升策略的通用性。

Method: 提出FunCanon框架，将操作任务分解为包含主体、动词和物体的动作块；利用大视觉语言模型的可供性线索进行功能对象规范化，使策略具备位姿感知和类别通用性；在此基础上训练以对象和动作为中心的扩散策略模型FuncDiffuser。

Result: 在模拟和真实世界基准实验中，该方法实现了类别级泛化、跨任务行为复用以及稳健的仿真到现实迁移。

Conclusion: 功能规范化为复杂操作领域的可扩展模仿学习提供了强有力的归纳偏置，有助于提升策略的泛化能力和实用性。
...
### [405] [Spectral Signature Mapping from RGB Imagery for Terrain-Aware Navigation](https://arxiv.org/abs/2509.19105)
*Sarvesh Prajapati,Ananya Trivedi,Nathaniel Hanson,Bruce Maxwell,Taskin Padir*

Main category: cs.RO

TL;DR: 本文提出了RS-Net，一种从RGB图像预测光谱特征的深度神经网络，用于户外机器人导航中的地形分类和摩擦系数估计，仅依赖RGB传感器在测试时获取丰富的材质信息。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用几何或语义标签分类可通行表面，但无法区分视觉相似但材质不同的地形；光谱传感虽能推断材质，但受限于硬件成本和计算开销。

Method: 设计了一种名为RS-Net的深度神经网络，从RGB图像块预测光谱特征，并将预测结果映射为地形标签和摩擦系数，分别用于基于采样的运动规划器（轮式机器人）和基于接触力的模型预测控制（四足机器人）。

Result: RS-Net能够在测试时仅用RGB输入实现准确的地形分类与摩擦估计，成功集成到两种机器人系统中，提升了在复杂户外环境中的导航能力。

Conclusion: 该框架通过训练学习任务相关的物理属性，测试时无需光谱传感器，实现了低成本、高实用性的机器人地形感知。
...
### [406] [BiGraspFormer: End-to-End Bimanual Grasp Transformer](https://arxiv.org/abs/2509.19142)
*Kangmin Kim,Seunghyeok Back,Geonhyup Lee,Sangbeom Lee,Sangjun Noh,Kyoobin Lee*

Main category: cs.RO

TL;DR: 提出BiGraspFormer，一种基于Transformer的端到端框架，通过单臂抓取候选引导生成协调的双臂抓取，有效解决碰撞和力分配不均问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在双臂抓取中存在分阶段处理导致的协调性差、碰撞风险和力分布不平衡问题，缺乏统一的端到端解决方案。

Method: 提出Single-Guided Bimanual（SGB）策略，利用Transformer解码器生成单臂抓取候选，并通过专用注意力机制联合预测双臂姿态和质量分数，实现从点云直接生成协调抓取。

Result: 在仿真和真实场景中均优于现有方法，推理速度小于0.05秒，有效降低12-DoF搜索复杂度并提升协调性。

Conclusion: BiGraspFormer通过统一的端到端架构和SGB策略，实现了高效、协调的双臂抓取，验证了其在性能和实用性上的优势。
...
### [407] [A Multimodal Stochastic Planning Approach for Navigation and Multi-Robot Coordination](https://arxiv.org/abs/2509.19168)
*Mark Gonzales,Ethan Oh,Joseph Moore*

Main category: cs.RO

TL;DR: 提出一种基于多模态策略分布的滚动时域采样规划器，利用交叉熵方法优化策略，提升对局部极小值的鲁棒性并有效探索解空间，适用于多机器人无碰撞规划与全局目标优化。


<details>
  <summary>Details</summary>
Motivation: 传统规划方法易陷入局部极小值且难以在复杂环境中充分探索解空间，尤其在多机器人系统中面临死锁和计算复杂度高的问题。

Method: 采用基于采样的滚动时域规划框架，结合交叉熵方法优化多模态策略分布，使机器人能生成多样化的候选策略，并通过分布式方式实现多机器人协同规划。

Result: 数值仿真表明该方法在陷阱环境和多机器人避障中显著提高成功率；硬件实验证明其具备实时性和实际可行性。

Conclusion: 所提出的多模态规划方法在提升规划鲁棒性和探索效率方面具有优势，可有效扩展至多机器人系统，兼顾性能与计算效率。
...
### [408] [MagiClaw: A Dual-Use, Vision-Based Soft Gripper for Bridging the Human Demonstration to Robotic Deployment Gap](https://arxiv.org/abs/2509.19169)
*Tianyu Wu,Xudong Han,Haoran Sun,Zishang Zhang,Bangchao Huang,Chaoyang Song,Fang Wan*

Main category: cs.RO

TL;DR: 本文提出了MagiClaw，一种可兼作手持工具和机器人末端执行器的双指灵巧手，通过硬件一致性桥接人类示范与机器人操作之间的域差距，并利用多模态传感实现高保真数据采集与通用操作策略学习。


<details>
  <summary>Details</summary>
Motivation: 人类示范与机器人执行之间存在感知和形态上的域差距，限制了操作技能的有效迁移。

Method: 设计了一种名为MagiClaw的双指末端执行器，每个手指集成软体多面体网络（SPN）和摄像头，用于6自由度力和接触形变估计；同时集成iPhone进行外部环境感知（6D位姿、RGB视频、LiDAR深度图），并通过自定义iOS应用同步流式传输多模态数据。

Result: 实现了硬件一致的示范数据采集与策略部署，支持实时遥操作、离线策略学习和混合现实控制，显著降低了高保真接触丰富数据集的采集门槛。

Conclusion: MagiClaw通过统一的系统架构有效缩小了人机操作间的域差距，提升了操作策略的泛化能力与开发效率。
...
### [409] [Imitation-Guided Bimanual Planning for Stable Manipulation under Changing External Forces](https://arxiv.org/abs/2509.19261)
*Kuanqi Cai,Chunfeng Wang,Zeqi Li,Haowen Yao,Weinan Chen,Luis Figueredo,Aude Billard,Arash Ajoudani*

Main category: cs.RO

TL;DR: 提出了一种模仿引导的双手机器人操作框架，通过采样稳定交集策略和分层双阶段运动架构，实现高效、稳定的抓取转换与运动优化。


<details>
  <summary>Details</summary>
Motivation: 现有抓取转换策略难以应对外部力变化和复杂运动约束，缺乏对运动性能的优化，导致在动态环境中稳定性与灵活性不足。

Method: 提出策略采样抓取流形中的稳定交集以实现单手与双手抓取间的无缝切换，并设计分层次双阶段运动架构，结合基于模仿学习的全局路径生成器与基于二次规划的局部规划器，确保实时性、避障与高操作性。

Result: 在一系列高受力任务中验证了该方法，显著提升了抓取转换效率和运动性能。

Conclusion: 所提框架有效解决了动态环境中抓取转换的稳定性与运动优化问题，具有较低计算成本和较高的实用性。
...
### [410] [SOE: Sample-Efficient Robot Policy Self-Improvement via On-Manifold Exploration](https://arxiv.org/abs/2509.19292)
*Yang Jin,Jun Lv,Han Xue,Wendi Chen,Chuan Wen,Cewu Lu*

Main category: cs.RO

TL;DR: 本文提出了Self-Improvement via On-Manifold Exploration (SOE)，一种通过在有效动作流形上进行探索来增强机器人操作策略的框架，提升了探索的安全性、多样性和效率。


<details>
  <summary>Details</summary>
Motivation: 现有探索方法依赖随机扰动，导致行为不稳定且不安全，限制了机器人策略的持续改进能力。

Method: SOE学习任务相关因素的紧凑潜在表示，并将探索限制在有效动作的流形上，可作为插件模块集成到任意策略模型中，支持人类引导探索。

Result: 在仿真和真实环境实验中，SOE相比先前方法实现了更高的任务成功率、更平滑安全的探索过程以及更优的样本效率。

Conclusion: SOE为基于流形的探索提供了原则性方法，有效推动了样本高效下的策略自优化。
...
### [411] [Residual Off-Policy RL for Finetuning Behavior Cloning Policies](https://arxiv.org/abs/2509.19301)
*Lars Ankile,Zhenyu Jiang,Rocky Duan,Guanya Shi,Pieter Abbeel,Anusha Nagabandi*

Main category: cs.RO

TL;DR: 提出了一种结合行为克隆（BC）和强化学习（RL）的残差学习框架，利用BC策略作为基础，通过样本高效的离线RL学习每步的残差修正，仅需稀疏二值奖励信号即可在高自由度系统上提升操作策略，并首次在具有灵巧手的人形机器人上实现了成功的现实世界RL训练。


<details>
  <summary>Details</summary>
Motivation: 行为克隆受限于演示质量与数据收集成本，而直接在真实机器人上进行强化学习又面临样本效率低、安全性差和稀疏奖励等问题，尤其在高自由度系统中更为突出。因此需要一种结合两者优势的方法以实现高效且实用的现实世界RL部署。

Method: 采用残差学习框架，将行为克隆策略作为黑箱基础，在其之上使用样本高效的离线强化学习方法学习每一步的轻量级残差动作修正，仅依赖稀疏的二值奖励信号。

Result: 在仿真和真实世界中均显著提升了高自由度系统的操作性能，特别是在灵巧手人形机器人上首次实现了成功的现实世界RL训练，展示了基于视觉任务中的最先进表现。

Conclusion: 该方法为在真实世界中部署强化学习提供了一个实用路径，有效结合了行为克隆的数据效率和强化学习的持续优化能力，适用于复杂高自由度机器人系统。
...
### [412] [Zero-Shot Visual Deepfake Detection: Can AI Predict and Prevent Fake Content Before It's Created?](https://arxiv.org/abs/2509.18461)
*Ayan Sar,Sampurna Roy,Tanupriya Choudhury,Ajith Abraham*

Main category: cs.GR

TL;DR: 本文探讨了零样本深度伪造检测技术，研究了自监督学习、基于Transformer的分类器、生成模型指纹识别和元学习等方法，并提出了AI驱动的预防策略以应对深度伪造威胁。


<details>
  <summary>Details</summary>
Motivation: 由于GAN和扩散模型推动深度伪造技术快速发展，对数字安全、媒体完整性和公众信任构成严重威胁，亟需无需先验训练即可应对新型伪造内容的检测与预防方法。

Method: 采用自监督学习、Transformer-based零样本分类器、生成模型指纹识别和元学习技术，结合对抗扰动、数字水印、实时监控和区块链验证等预防机制。

Result: 提出了一套综合的零样本检测与预防框架，识别出当前在对抗攻击、可扩展性、伦理问题和评估标准方面的挑战，并指出了未来研究方向。

Conclusion: 需要构建融合零样本学习与预防机制的集成防御体系，并通过人工智能、网络安全与政策制定之间的跨学科合作，建立抵御深度伪造攻击的韧性防线。
...
### [413] [Differentiable Light Transport with Gaussian Surfels via Adapted Radiosity for Efficient Relighting and Geometry Reconstruction](https://arxiv.org/abs/2509.18497)
*Kaiwen Jiang,Jia-Mu Sun,Zilu Li,Dan Wang,Tzu-Mao Li,Ravi Ramamoorthi*

Main category: cs.GR

TL;DR: 本文提出了一种基于高斯面元和球谐函数的可微分光传输框架，能够在辐射场中高效地实现包含全局光照的逆向渲染，支持漫反射和镜面反射材质，并实现了视点无关的快速渲染，在几何重建、新视角合成和重光照任务上优于先前方法。


<details>
  <summary>Details</summary>
Motivation: 现有的辐射场方法（如高斯溅射）在建模材质反射属性和光照条件方面存在不足，导致几何歧义且难以进行重光照。引入物理渲染虽能改善，但包含全局光照的传统方法计算代价过高。因此，需要一种既高效又准确的可微分光传输框架。

Method: 采用高斯面元作为基本单元，受经典辐射度理论启发，在球谐系数空间中构建可微分光传输框架。扩展了经典辐射度方法以支持非二值可见性和半透明图元，设计了新的求解器来高效求解光传输方程，并推导了比自动微分更高效的反向传播算法。

Result: 该方法在推理时实现视点无关渲染，无需在视角变化时重新计算光传输，可在数百FPS下实现包含视依赖反射的全局光照效果。实验表明，该方法在稀疏数据集下（已知或未知光照）在几何重建、新视角合成和重光照方面均优于之前的逆向渲染和数据驱动基线方法。

Conclusion: 本文提出的基于球谐函数的可微分辐射度方法，在保持高效率的同时显著提升了辐射场在材质、光照建模和重光照方面的能力，为高质量逆向渲染提供了一种新的有效解决方案。
...
### [414] [Text Slider: Efficient and Plug-and-Play Continuous Concept Control for Image/Video Synthesis via LoRA Adapters](https://arxiv.org/abs/2509.18831)
*Pin-Yen Chiu,I-Sheng Fang,Jun-Cheng Chen*

Main category: cs.GR

TL;DR: 提出了一种名为Text Slider的轻量级、高效且即插即用的框架，用于在预训练文本编码器中识别低秩方向，实现对图像和视频合成中视觉概念的连续控制，显著提升了训练效率并降低了资源消耗。


<details>
  <summary>Details</summary>
Motivation: 现有概念控制方法需要大量训练时间和GPU内存，且需针对不同扩散模型重新训练，限制了其可扩展性和适应性。

Method: 在预训练文本编码器中识别低秩方向，构建轻量化的Text Slider框架，实现无需重训练的即插即用连续控制，并支持多概念组合与灵活操控。

Result: 相比Concept Slider训练速度快5倍，相比Attribute Control快47倍，GPU内存消耗分别减少近2倍和4倍，同时支持细粒度属性调节并保持输入的空间结构。

Conclusion: Text Slider是一种高效、通用且可扩展的概念控制方法，在图像和视频生成中实现了快速、连续和多概念的文本驱动控制。
...
### [415] [One-shot Embroidery Customization via Contrastive LoRA Modulation](https://arxiv.org/abs/2509.18948)
*Jun Ma,Qian He,Gaofeng He,Huang Chen,Chen Liu,Xiaogang Jin,Huamin Wang*

Main category: cs.GR

TL;DR: 本文提出了一种基于对比学习的框架，利用预训练扩散模型解耦细粒度风格与内容特征，实现仅需单参考图像的精细风格迁移，并在刺绣定制等任务上超越先前方法。


<details>
  <summary>Details</summary>
Motivation: 现有风格迁移方法难以处理如刺绣这类具有复杂纹理和材质特性的细粒度视觉特征，因此需要一种能精确分离并迁移细粒度风格的方法。

Method: 基于图像类比思想，构建目标风格图像对，利用预训练扩散模型的解耦表示进行风格-内容分离；提出两阶段对比LoRA调制技术：第一阶段迭代更新整个LoRA和选定的风格模块以初步分离风格与内容；第二阶段通过自知识蒸馏的对比学习进一步解耦；最后构建仅使用风格模块的推理流程。

Result: 在自建的刺绣定制基准上优于先前方法，同时在艺术风格迁移、草图上色和外观迁移三个领域展现出良好泛化能力。

Conclusion: 所提出的对比学习框架能有效实现单样本细粒度风格迁移，为零售预览、纺织品定制等应用提供了新思路。
...
### [416] [Understand your Users, An Ensemble Learning Framework for Natural Noise Filtering in Recommender Systems](https://arxiv.org/abs/2509.18560)
*Clarita Hawat,Wissam Al Jurdi,Jacques Bou Abdo,Jacques Demerjian,Abdallah Makhoul*

Main category: cs.IR

TL;DR: 本文提出了一种新的框架，用于识别推荐系统中的噪声评分，通过模块化三层设计提高推荐准确性。


<details>
  <summary>Details</summary>
Motivation: 由于用户偏好和行为的多变性，定义噪声成为推荐系统中的一个挑战，需要有效区分外部因素、意外偏好和偶然交互等现象。

Method: 采用模块化框架，包括基于自然噪声算法的项目分类、集成学习模型进行项目精细评估，以及基于签名的噪声识别方法。

Result: 该框架能够有效识别噪声评分，并提供更干净的训练数据集，同时提出量化指标评估意外发现和群体验证效果。

Conclusion: 所提方法有助于提升推荐系统的鲁棒性和准确性，从而改善用户满意度和参与度。
...
### [417] [The Ranking Blind Spot: Decision Hijacking in LLM-based Text Ranking](https://arxiv.org/abs/2509.18575)
*Yaoyao Qian,Yifan Zeng,Yuchao Jiang,Chelsi Jain,Huazheng Wang*

Main category: cs.IR

TL;DR: 本文研究了大语言模型（LLM）在多文档比较任务中的“排序盲点”问题，提出了两种攻击方法：决策目标劫持和决策标准劫持，表明恶意内容提供者可利用该弱点操纵排序结果，且更强的LLM更易受攻击。


<details>
  <summary>Details</summary>
Motivation: 探究LLM在指令遵循能力下的排序决策机制，揭示其在信息检索中可能存在的系统性盲点和安全风险。

Method: 通过决策目标劫持和决策标准劫持两种方式，分析LLM在成对排序和不同排序方案中的脆弱性，并在多种LLM和真实场景中进行实验验证。

Result: 实验证明所提出的攻击方法在多种LLM和排序方案中均有效，且更强的LLM反而更易受攻击，代码已开源。

Conclusion: LLM存在“排序盲点”，可被恶意利用以提升特定文档排名，这对基于LLM的检索系统构成潜在威胁，需引起重视。
...
### [418] [Agentic AutoSurvey: Let LLMs Survey LLMs](https://arxiv.org/abs/2509.18661)
*Yixin Liu,Yonghui Wu,Denghui Zhang,Lichao Sun*

Main category: cs.IR

TL;DR: 提出了一种名为Agentic AutoSurvey的多智能体框架，用于自动生成文献综述，相较于现有方法在综合质量上显著提升。


<details>
  <summary>Details</summary>
Motivation: 科学文献的指数级增长使得研究人员难以在快速发展的领域中整合知识，现有自动综述方法在合成质量和组织结构方面存在局限性。

Method: 设计了一个包含四个专门智能体（论文搜索专家、主题挖掘与聚类、学术综述撰写者和质量评估者）协同工作的多智能体系统，通过专业化分工和协调生成高质量文献综述。

Result: 在COLM 2024六个代表性LLM主题上的实验表明，该方法评分为8.18/10，显著优于AutoSurvey的4.77/10；系统处理了总计847篇论文，覆盖高引用文献（通常≥80%），并在12个维度的评估中展现出更强的组织性、综合性和批判性分析能力。

Conclusion: 多智能体架构为快速发展的科学领域中的自动化文献综述生成提供了实质性进展，具有更高的合成质量与可扩展性。
...
### [419] [Robust Denoising Neural Reranker for Recommender Systems](https://arxiv.org/abs/2509.18736)
*Wenyu Mao,Shuchang Liu,Hailan Yang,Xiaobei Wang,Xiaoyu Yang,Xu Gao,Xiang Li,Lantao Hu,Han Li,Kun Gai,An Zhang,Xiang Wang*

Main category: cs.IR

TL;DR: 本文提出了一种用于多阶段推荐系统的去噪重排序框架DNR，通过理论分析指出直接使用检索器分数的局限性，并将重排序任务视为噪声消除问题。DNR结合对抗训练机制，包含去噪、对抗生成和分布正则化三个目标，实验验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有两阶段推荐系统中，检索器分数在重排序阶段的作用未被充分挖掘，且直接作为输入存在理论缺陷，需更有效地利用这一信息以提升排序性能。

Method: 提出DNR框架，引入对抗式去噪机制：1）设计去噪目标以对齐用户反馈；2）构建对抗性检索分数生成器增强探索；3）加入分布正则项使生成分数分布接近真实分布。

Result: 在三个公开数据集上进行了广泛实验，结果表明DNR在多项指标上优于基线模型，验证了其在噪声过滤和排序优化方面的有效性。

Conclusion: 将重排序视为对检索器分数的去噪过程是有效的，DNR通过联合优化去噪与对抗生成，显著提升了推荐系统的排序精度。
...
### [420] [Single-Branch Network Architectures to Close the Modality Gap in Multimodal Recommendation](https://arxiv.org/abs/2509.18807)
*Christian Ganhör,Marta Moscati,Anna Hausberger,Shah Nawaz,Markus Schedl*

Main category: cs.IR

TL;DR: 本文提出了一种基于单分支神经网络的混合推荐系统，通过权重共享、模态采样和对比损失来缩小模态差距，在模态缺失场景下仍能提供准确推荐。


<details>
  <summary>Details</summary>
Motivation: 在冷启动或某些模态信息缺失的情况下，传统混合推荐系统性能下降，因此需要一种更鲁棒的方法来处理不完整模态输入。

Method: 采用单分支神经网络，结合权重共享机制、模态采样策略和对比损失函数，统一学习多模态信息，并与多分支结构进行对比。

Result: 在三个数据集上进行了广泛实验，使用六项准确性指标和四项超越准确性的指标评估，结果表明单分支网络在模态缺失场景下显著优于多分支方法，且能更好地拉近物品不同模态在嵌入空间中的距离。

Conclusion: 单分支神经网络在温启动和模态缺失场景下均表现优异，有效缩小了模态差距，提升了推荐系统的鲁棒性和性能。
...
### [421] [RELATE: Relation Extraction in Biomedical Abstracts with LLMs and Ontology Constraints](https://arxiv.org/abs/2509.19057)
*Olawumi Olasunkanmi,Mathew Satursky,Hong Yi,Chris Bizon,Harlin Lee,Stanley Ahalt*

Main category: cs.IR

TL;DR: RELATE是一个三阶段管道，用于将大语言模型提取的生物医学关系映射到标准化本体谓词，从而实现知识图谱的增强。


<details>
  <summary>Details</summary>
Motivation: 生物医学知识图谱不完整，而大语言模型提取的关系缺乏标准化，难以整合到知识图谱中。

Method: 提出RELATE框架，包括本体预处理、基于SapBERT的相似性检索和大语言模型重排序，并结合向量搜索与上下文推理。

Result: 在ChemProt基准上达到52%精确匹配和94%准确率@10，在2,400篇HEAL项目摘要中有效拒绝无关关联（0.4%）并识别否定陈述。

Conclusion: RELATE通过结合向量搜索与大语言模型推理，为构建标准化生物医学知识图谱提供了一个可扩展且语义准确的框架。
...
### [422] [A Knowledge Graph and a Tripartite Evaluation Framework Make Retrieval-Augmented Generation Scalable and Transparent](https://arxiv.org/abs/2509.19209)
*Olalekan K. Akindele,Bhupesh Kumar Mishra,Kenneth Y. Wertheim*

Main category: cs.IR

TL;DR: 本文提出了一种结合知识图谱和向量检索的检索增强生成（RAG）聊天机器人，并创新性地设计了一个基于大语言模型的三元评估框架RAG-Eval，用于实时评估查询相关性、事实准确性等多维度质量，并提供用户可理解的置信度评分，提升了高数据量场景下对话系统的可信度与可验证性。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型显著提升了对话AI的能力，但在处理领域特定信息时仍存在事实不一致和准确性不足的问题，尤其是在处理大量工程邮件等高密度数据时，缺乏有效的评估机制来确保生成内容的可靠性。

Method: 采用检索增强生成（RAG）架构，融合知识图谱与向量搜索进行精准信息检索，减少文档分块需求；同时提出RAG-Eval评估框架，利用链式思维大模型并行分析用户查询、检索文档和生成回答，从相关性、事实性、覆盖度、连贯性等多个指标进行综合评分。

Result: 实验表明，RAG-Eval在摘要评估任务中优于BERTScore和G-EVAL，能有效识别事实缺失和查询不匹配问题；系统通过提供1-100%的置信度分数和包含邮件ID、时间戳的元数据，增强了结果透明度和用户可验证性。

Conclusion: 该方法为构建高准确度、可信赖的领域专用聊天机器人提供了可扩展的技术路径，有效平衡了对话流畅性与事实准确性之间的矛盾，适用于高要求的数据密集型应用场景。
